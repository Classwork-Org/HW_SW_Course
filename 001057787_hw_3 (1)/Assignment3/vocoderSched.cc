//////////////////////////////////////////////////////////////////////
// C++ source file generated by SpecC V2.2.1
// Design: vocoderSched
// File:   vocoderSched.cc
// Time:   Tue Nov  5 02:12:30 2019
//////////////////////////////////////////////////////////////////////

// Note: User-defined include files are inlined in this file.

// Note: System-defined include files are inlined in this file.

#include "vocoderSched.h"


unsigned int _IDcnt = 0;
// variable definitions //////////////////////////////////////////////

#line 8 "src/common/F_gamma.sc"
short int F_gamma1[10] = { 

  ((short int)29491),((short int)26542),((short int)23888),((short int)21499),((short int)19349),
  ((short int)17414),((short int)15672),((short int)14105),((short int)12694),((short int)11425) };

short int F_gamma2[10] = { 

  ((short int)19661),((short int)11797),((short int)7078),((short int)4247),((short int)2548),
  ((short int)1529),((short int)917),((short int)550),((short int)330),((short int)198) };



short int F_gamma3[10] = { 

  ((short int)22938),((short int)16057),((short int)11240),((short int)7868),((short int)5508),
  ((short int)3856),((short int)2699),((short int)1889),((short int)1322),((short int)925) };


short int F_gamma4[10] = { 

  ((short int)24576),((short int)18432),((short int)13824),((short int)10368),((short int)7776),
  ((short int)5832),((short int)4374),((short int)3281),((short int)2461),((short int)1846) };

#line 74 "src/common/basic_func.sc"
static const short int log_table[33] = { 

  ((short int)0),((short int)1455),((short int)2866),((short int)4236),((short int)5568),((short int)6863),((short int)8124),((short int)9352),((short int)10549),((short int)11716),
  ((short int)12855),((short int)13967),((short int)15054),((short int)16117),((short int)17156),((short int)18172),((short int)19167),((short int)20142),((short int)21097),((short int)22033),
  ((short int)22951),((short int)23852),((short int)24735),((short int)25603),((short int)26455),((short int)27291),((short int)28113),((short int)28922),((short int)29716),((short int)30497),
  ((short int)31266),((short int)32023),((short int)32767) };

#line 150 "src/common/basic_func.sc"
static const short int pow_table[33] = { 

  ((short int)16384),((short int)16743),((short int)17109),((short int)17484),((short int)17867),((short int)18258),((short int)18658),((short int)19066),((short int)19484),((short int)19911),
  ((short int)20347),((short int)20792),((short int)21247),((short int)21713),((short int)22188),((short int)22674),((short int)23170),((short int)23678),((short int)24196),((short int)24726),
  ((short int)25268),((short int)25821),((short int)26386),((short int)26964),((short int)27554),((short int)28158),((short int)28774),((short int)29405),((short int)30048),((short int)30706),
  ((short int)31379),((short int)32066),((short int)32767) };

#line 219 "src/common/basic_func.sc"
static const short int sqrt_table[49] = { 


  ((short int)32767),((short int)31790),((short int)30894),((short int)30070),((short int)29309),((short int)28602),((short int)27945),((short int)27330),((short int)26755),((short int)26214),
  ((short int)25705),((short int)25225),((short int)24770),((short int)24339),((short int)23930),((short int)23541),((short int)23170),((short int)22817),((short int)22479),((short int)22155),
  ((short int)21845),((short int)21548),((short int)21263),((short int)20988),((short int)20724),((short int)20470),((short int)20225),((short int)19988),((short int)19760),((short int)19539),
  ((short int)19326),((short int)19119),((short int)18919),((short int)18725),((short int)18536),((short int)18354),((short int)18176),((short int)18004),((short int)17837),((short int)17674),
  ((short int)17515),((short int)17361),((short int)17211),((short int)17064),((short int)16921),((short int)16782),((short int)16646),((short int)16514),((short int)16384) };

#line 47 "src/common/basic_op.sc"
bool Overflow = false;
bool Carry = false;

#line 3 "src/lp_analysis/lp_analysis.tab"
static short int window_232_8[240] = { 

  ((short int)2621),((short int)2623),((short int)2627),((short int)2634),((short int)2644),((short int)2656),((short int)2671),((short int)2689),((short int)2710),((short int)2734),
  ((short int)2760),((short int)2789),((short int)2821),((short int)2855),((short int)2893),((short int)2933),((short int)2975),((short int)3021),((short int)3069),((short int)3120),
  ((short int)3173),((short int)3229),((short int)3288),((short int)3350),((short int)3414),((short int)3481),((short int)3550),((short int)3622),((short int)3697),((short int)3774),
  ((short int)3853),((short int)3936),((short int)4021),((short int)4108),((short int)4198),((short int)4290),((short int)4385),((short int)4482),((short int)4582),((short int)4684),
  ((short int)4788),((short int)4895),((short int)5004),((short int)5116),((short int)5230),((short int)5346),((short int)5464),((short int)5585),((short int)5708),((short int)5833),
  ((short int)5960),((short int)6090),((short int)6221),((short int)6355),((short int)6491),((short int)6629),((short int)6769),((short int)6910),((short int)7054),((short int)7200),
  ((short int)7348),((short int)7498),((short int)7649),((short int)7803),((short int)7958),((short int)8115),((short int)8274),((short int)8434),((short int)8597),((short int)8761),
  ((short int)8926),((short int)9093),((short int)9262),((short int)9432),((short int)9604),((short int)9778),((short int)9952),((short int)10129),((short int)10306),((short int)10485),
  ((short int)10665),((short int)10847),((short int)11030),((short int)11214),((short int)11399),((short int)11586),((short int)11773),((short int)11962),((short int)12152),((short int)12342),
  ((short int)12534),((short int)12727),((short int)12920),((short int)13115),((short int)13310),((short int)13506),((short int)13703),((short int)13901),((short int)14099),((short int)14298),
  ((short int)14497),((short int)14698),((short int)14898),((short int)15100),((short int)15301),((short int)15504),((short int)15706),((short int)15909),((short int)16112),((short int)16316),
  ((short int)16520),((short int)16724),((short int)16928),((short int)17132),((short int)17337),((short int)17541),((short int)17746),((short int)17950),((short int)18155),((short int)18359),
  ((short int)18564),((short int)18768),((short int)18972),((short int)19175),((short int)19379),((short int)19582),((short int)19785),((short int)19987),((short int)20189),((short int)20390),
  ((short int)20591),((short int)20792),((short int)20992),((short int)21191),((short int)21390),((short int)21588),((short int)21785),((short int)21981),((short int)22177),((short int)22372),
  ((short int)22566),((short int)22759),((short int)22951),((short int)23143),((short int)23333),((short int)23522),((short int)23710),((short int)23897),((short int)24083),((short int)24268),
  ((short int)24451),((short int)24633),((short int)24814),((short int)24994),((short int)25172),((short int)25349),((short int)25525),((short int)25699),((short int)25871),((short int)26042),
  ((short int)26212),((short int)26380),((short int)26546),((short int)26711),((short int)26874),((short int)27035),((short int)27195),((short int)27353),((short int)27509),((short int)27664),
  ((short int)27816),((short int)27967),((short int)28115),((short int)28262),((short int)28407),((short int)28550),((short int)28691),((short int)28830),((short int)28967),((short int)29102),
  ((short int)29234),((short int)29365),((short int)29493),((short int)29619),((short int)29743),((short int)29865),((short int)29985),((short int)30102),((short int)30217),((short int)30330),
  ((short int)30440),((short int)30548),((short int)30654),((short int)30757),((short int)30858),((short int)30956),((short int)31052),((short int)31146),((short int)31237),((short int)31326),
  ((short int)31412),((short int)31495),((short int)31576),((short int)31655),((short int)31730),((short int)31804),((short int)31874),((short int)31942),((short int)32008),((short int)32071),
  ((short int)32131),((short int)32188),((short int)32243),((short int)32295),((short int)32345),((short int)32392),((short int)32436),((short int)32477),((short int)32516),((short int)32552),
  ((short int)32585),((short int)32615),((short int)32643),((short int)32668),((short int)32690),((short int)32709),((short int)32726),((short int)32740),((short int)32751),((short int)32759),
  ((short int)32765),((short int)32767),((short int)32767),((short int)32097),((short int)30112),((short int)26895),((short int)22576),((short int)17333),((short int)11380),((short int)4962) };




static short int window_160_80[240] = { 

  ((short int)2621),((short int)2624),((short int)2633),((short int)2648),((short int)2668),((short int)2695),((short int)2727),((short int)2765),((short int)2809),((short int)2859),
  ((short int)2915),((short int)2976),((short int)3043),((short int)3116),((short int)3194),((short int)3279),((short int)3368),((short int)3464),((short int)3565),((short int)3671),
  ((short int)3783),((short int)3900),((short int)4023),((short int)4151),((short int)4285),((short int)4423),((short int)4567),((short int)4716),((short int)4870),((short int)5029),
  ((short int)5193),((short int)5362),((short int)5535),((short int)5714),((short int)5897),((short int)6084),((short int)6277),((short int)6473),((short int)6674),((short int)6880),
  ((short int)7089),((short int)7303),((short int)7521),((short int)7742),((short int)7968),((short int)8197),((short int)8430),((short int)8667),((short int)8907),((short int)9151),
  ((short int)9398),((short int)9648),((short int)9902),((short int)10158),((short int)10417),((short int)10680),((short int)10945),((short int)11212),((short int)11482),((short int)11755),
  ((short int)12030),((short int)12307),((short int)12586),((short int)12867),((short int)13150),((short int)13435),((short int)13722),((short int)14010),((short int)14299),((short int)14590),
  ((short int)14882),((short int)15175),((short int)15469),((short int)15764),((short int)16060),((short int)16356),((short int)16653),((short int)16950),((short int)17248),((short int)17546),
  ((short int)17844),((short int)18141),((short int)18439),((short int)18736),((short int)19033),((short int)19330),((short int)19625),((short int)19920),((short int)20214),((short int)20507),
  ((short int)20799),((short int)21090),((short int)21380),((short int)21668),((short int)21954),((short int)22239),((short int)22522),((short int)22803),((short int)23083),((short int)23360),
  ((short int)23635),((short int)23907),((short int)24177),((short int)24445),((short int)24710),((short int)24972),((short int)25231),((short int)25488),((short int)25741),((short int)25991),
  ((short int)26238),((short int)26482),((short int)26722),((short int)26959),((short int)27192),((short int)27422),((short int)27647),((short int)27869),((short int)28087),((short int)28300),
  ((short int)28510),((short int)28715),((short int)28916),((short int)29113),((short int)29305),((short int)29493),((short int)29676),((short int)29854),((short int)30028),((short int)30197),
  ((short int)30361),((short int)30519),((short int)30673),((short int)30822),((short int)30966),((short int)31105),((short int)31238),((short int)31366),((short int)31489),((short int)31606),
  ((short int)31718),((short int)31825),((short int)31926),((short int)32021),((short int)32111),((short int)32195),((short int)32273),((short int)32346),((short int)32413),((short int)32475),
  ((short int)32530),((short int)32580),((short int)32624),((short int)32662),((short int)32695),((short int)32721),((short int)32742),((short int)32756),((short int)32765),((short int)32767),
  ((short int)32767),((short int)32756),((short int)32720),((short int)32661),((short int)32578),((short int)32471),((short int)32341),((short int)32188),((short int)32012),((short int)31813),
  ((short int)31592),((short int)31349),((short int)31084),((short int)30798),((short int)30492),((short int)30165),((short int)29818),((short int)29453),((short int)29068),((short int)28666),
  ((short int)28247),((short int)27810),((short int)27358),((short int)26891),((short int)26408),((short int)25913),((short int)25404),((short int)24883),((short int)24350),((short int)23807),
  ((short int)23255),((short int)22693),((short int)22124),((short int)21548),((short int)20965),((short int)20378),((short int)19786),((short int)19191),((short int)18593),((short int)17994),
  ((short int)17395),((short int)16796),((short int)16199),((short int)15604),((short int)15012),((short int)14424),((short int)13842),((short int)13265),((short int)12696),((short int)12135),
  ((short int)11582),((short int)11039),((short int)10507),((short int)9986),((short int)9477),((short int)8981),((short int)8499),((short int)8031),((short int)7579),((short int)7143),
  ((short int)6723),((short int)6321),((short int)5937),((short int)5571),((short int)5225),((short int)4898),((short int)4591),((short int)4305),((short int)4041),((short int)3798),
  ((short int)3577),((short int)3378),((short int)3202),((short int)3048),((short int)2918),((short int)2812),((short int)2729),((short int)2669),((short int)2633),((short int)2621) };

#line 8 "src/lp_analysis/q_plsf_5.tab"
static const short int table[65] = { 

  ((short int)32767),((short int)32729),((short int)32610),((short int)32413),((short int)32138),((short int)31786),((short int)31357),((short int)30853),
  ((short int)30274),((short int)29622),((short int)28899),((short int)28106),((short int)27246),((short int)26320),((short int)25330),((short int)24279),
  ((short int)23170),((short int)22006),((short int)20788),((short int)19520),((short int)18205),((short int)16846),((short int)15447),((short int)14010),
  ((short int)12540),((short int)11039),((short int)9512),((short int)7962),((short int)6393),((short int)4808),((short int)3212),((short int)1608),
  ((short int)0),((short int)-1608),((short int)-3212),((short int)-4808),((short int)-6393),((short int)-7962),((short int)-9512),((short int)-11039),
  ((short int)-12540),((short int)-14010),((short int)-15447),((short int)-16846),((short int)-18205),((short int)-19520),((short int)-20788),((short int)-22006),
  ((short int)-23170),((short int)-24279),((short int)-25330),((short int)-26320),((short int)-27246),((short int)-28106),((short int)-28899),((short int)-29622),
  ((short int)-30274),((short int)-30853),((short int)-31357),((short int)-31786),((short int)-32138),((short int)-32413),((short int)-32610),((short int)-32729),
  ((short int)-32768) };

#line 25 "src/lp_analysis/q_plsf_5.tab"
static const short int slope[64] = { 

  ((short int)-26887),((short int)-8812),((short int)-5323),((short int)-3813),((short int)-2979),((short int)-2444),((short int)-2081),((short int)-1811),
  ((short int)-1608),((short int)-1450),((short int)-1322),((short int)-1219),((short int)-1132),((short int)-1059),((short int)-998),((short int)-946),
  ((short int)-901),((short int)-861),((short int)-827),((short int)-797),((short int)-772),((short int)-750),((short int)-730),((short int)-713),
  ((short int)-699),((short int)-687),((short int)-677),((short int)-668),((short int)-662),((short int)-657),((short int)-654),((short int)-652),
  ((short int)-652),((short int)-654),((short int)-657),((short int)-662),((short int)-668),((short int)-677),((short int)-687),((short int)-699),
  ((short int)-713),((short int)-730),((short int)-750),((short int)-772),((short int)-797),((short int)-827),((short int)-861),((short int)-901),
  ((short int)-946),((short int)-998),((short int)-1059),((short int)-1132),((short int)-1219),((short int)-1322),((short int)-1450),((short int)-1608),
  ((short int)-1811),((short int)-2081),((short int)-2444),((short int)-2979),((short int)-3813),((short int)-5323),((short int)-8812),((short int)-26887) };

#line 47 "src/lp_analysis/q_plsf_5.tab"
static short int mean_lsf[10] = { 

  ((short int)1384),
  ((short int)2077),
  ((short int)3420),
  ((short int)5108),
  ((short int)6742),
  ((short int)8122),
  ((short int)9863),
  ((short int)11092),
  ((short int)12714),
  ((short int)13701) };

#line 69 "src/lp_analysis/q_plsf_5.tab"
static short int dico1_lsf[512] = { 

  ((short int)-451),((short int)-1065),((short int)-529),((short int)-1305),
  ((short int)-450),((short int)-756),((short int)-497),((short int)-863),
  ((short int)-384),((short int)-619),((short int)-413),((short int)-669),
  ((short int)-317),((short int)-538),((short int)-331),((short int)-556),
  ((short int)-414),((short int)-508),((short int)-424),((short int)-378),
  ((short int)-274),((short int)-324),((short int)-434),((short int)-614),
  ((short int)-226),((short int)-500),((short int)-232),((short int)-514),
  ((short int)-263),((short int)-377),((short int)-298),((short int)-410),
  ((short int)-151),((short int)-710),((short int)-174),((short int)-818),
  ((short int)-149),((short int)-412),((short int)-156),((short int)-429),
  ((short int)-288),((short int)-462),((short int)-186),((short int)-203),
  ((short int)-170),((short int)-302),((short int)-191),((short int)-321),
  ((short int)-131),((short int)-147),((short int)-297),((short int)-395),
  ((short int)-228),((short int)-214),((short int)-245),((short int)-192),
  ((short int)-67),((short int)-316),((short int)-71),((short int)-327),
  ((short int)-104),((short int)-205),((short int)-94),((short int)-183),
  ((short int)-143),((short int)-38),((short int)-193),((short int)-95),
  ((short int)16),((short int)-76),((short int)-124),((short int)-248),
  ((short int)23),((short int)-237),((short int)24),((short int)-244),
  ((short int)18),((short int)-136),((short int)44),((short int)-111),
  ((short int)-33),((short int)-24),((short int)-25),((short int)0),
  ((short int)149),((short int)19),((short int)23),((short int)-143),
  ((short int)158),((short int)-169),((short int)174),((short int)-181),
  ((short int)133),((short int)-55),((short int)165),((short int)-26),
  ((short int)111),((short int)84),((short int)98),((short int)75),
  ((short int)87),((short int)183),((short int)-115),((short int)-11),
  ((short int)-8),((short int)130),((short int)11),((short int)170),
  ((short int)254),((short int)77),((short int)205),((short int)17),
  ((short int)183),((short int)112),((short int)262),((short int)194),
  ((short int)202),((short int)287),((short int)95),((short int)189),
  ((short int)-42),((short int)-105),((short int)234),((short int)179),
  ((short int)39),((short int)186),((short int)163),((short int)345),
  ((short int)332),((short int)199),((short int)299),((short int)161),
  ((short int)-54),((short int)285),((short int)-78),((short int)281),
  ((short int)-133),((short int)141),((short int)-182),((short int)111),
  ((short int)249),((short int)341),((short int)271),((short int)364),
  ((short int)93),((short int)403),((short int)75),((short int)391),
  ((short int)92),((short int)510),((short int)-138),((short int)220),
  ((short int)-185),((short int)-29),((short int)-34),((short int)361),
  ((short int)-115),((short int)320),((short int)3),((short int)554),
  ((short int)99),((short int)286),((short int)218),((short int)591),
  ((short int)-245),((short int)406),((short int)-268),((short int)453),
  ((short int)0),((short int)580),((short int)25),((short int)606),
  ((short int)275),((short int)532),((short int)148),((short int)450),
  ((short int)-73),((short int)739),((short int)-285),((short int)518),
  ((short int)-288),((short int)94),((short int)-203),((short int)674),
  ((short int)-140),((short int)-74),((short int)205),((short int)714),
  ((short int)-114),((short int)299),((short int)176),((short int)923),
  ((short int)182),((short int)557),((short int)240),((short int)705),
  ((short int)-16),((short int)513),((short int)485),((short int)593),
  ((short int)293),((short int)384),((short int)451),((short int)617),
  ((short int)-38),((short int)50),((short int)563),((short int)529),
  ((short int)303),((short int)209),((short int)459),((short int)363),
  ((short int)433),((short int)452),((short int)450),((short int)454),
  ((short int)367),((short int)606),((short int)477),((short int)741),
  ((short int)432),((short int)353),((short int)368),((short int)267),
  ((short int)361),((short int)716),((short int)273),((short int)583),
  ((short int)453),((short int)166),((short int)510),((short int)172),
  ((short int)201),((short int)629),((short int)274),((short int)191),
  ((short int)568),((short int)639),((short int)302),((short int)298),
  ((short int)634),((short int)387),((short int)643),((short int)350),
  ((short int)587),((short int)560),((short int)612),((short int)565),
  ((short int)600),((short int)788),((short int)487),((short int)672),
  ((short int)512),((short int)1015),((short int)321),((short int)333),
  ((short int)357),((short int)854),((short int)-125),((short int)413),
  ((short int)474),((short int)712),((short int)17),((short int)-151),
  ((short int)564),((short int)285),((short int)270),((short int)-241),
  ((short int)971),((short int)889),((short int)489),((short int)220),
  ((short int)510),((short int)896),((short int)549),((short int)924),
  ((short int)327),((short int)825),((short int)290),((short int)911),
  ((short int)540),((short int)1108),((short int)158),((short int)805),
  ((short int)199),((short int)957),((short int)511),((short int)730),
  ((short int)100),((short int)874),((short int)13),((short int)791),
  ((short int)435),((short int)632),((short int)676),((short int)972),
  ((short int)249),((short int)900),((short int)467),((short int)1218),
  ((short int)781),((short int)1074),((short int)585),((short int)785),
  ((short int)-23),((short int)669),((short int)267),((short int)1043),
  ((short int)619),((short int)1084),((short int)615),((short int)1145),
  ((short int)622),((short int)905),((short int)916),((short int)1049),
  ((short int)80),((short int)331),((short int)584),((short int)1075),
  ((short int)89),((short int)639),((short int)988),((short int)961),
  ((short int)770),((short int)720),((short int)798),((short int)699),
  ((short int)492),((short int)447),((short int)899),((short int)627),
  ((short int)271),((short int)1188),((short int)725),((short int)1333),
  ((short int)87),((short int)603),((short int)832),((short int)1603),
  ((short int)616),((short int)1127),((short int)890),((short int)1505),
  ((short int)1000),((short int)1156),((short int)866),((short int)1009),
  ((short int)995),((short int)827),((short int)1149),((short int)858),
  ((short int)817),((short int)1450),((short int)773),((short int)1320),
  ((short int)500),((short int)1389),((short int)312),((short int)1153),
  ((short int)-20),((short int)1084),((short int)64),((short int)1283),
  ((short int)2),((short int)1172),((short int)399),((short int)1869),
  ((short int)514),((short int)1706),((short int)502),((short int)1636),
  ((short int)886),((short int)1522),((short int)416),((short int)600),
  ((short int)1131),((short int)1350),((short int)1275),((short int)1390),
  ((short int)889),((short int)1795),((short int)914),((short int)1766),
  ((short int)227),((short int)1183),((short int)1250),((short int)1826),
  ((short int)505),((short int)1854),((short int)919),((short int)2353),
  ((short int)-199),((short int)431),((short int)152),((short int)1735),
  ((short int)-213),((short int)-28),((short int)392),((short int)1334),
  ((short int)-153),((short int)-52),((short int)978),((short int)1151),
  ((short int)-323),((short int)-400),((short int)813),((short int)1703),
  ((short int)-136),((short int)84),((short int)1449),((short int)2015),
  ((short int)-331),((short int)-143),((short int)-137),((short int)1192),
  ((short int)-256),((short int)534),((short int)-157),((short int)1031),
  ((short int)-307),((short int)-439),((short int)542),((short int)731),
  ((short int)-329),((short int)-420),((short int)-97),((short int)616),
  ((short int)-362),((short int)-168),((short int)-322),((short int)366),
  ((short int)-247),((short int)-110),((short int)-211),((short int)89),
  ((short int)-196),((short int)-309),((short int)20),((short int)59),
  ((short int)-364),((short int)-463),((short int)-286),((short int)89),
  ((short int)-336),((short int)175),((short int)-432),((short int)141),
  ((short int)-379),((short int)-190),((short int)-434),((short int)-196),
  ((short int)-79),((short int)150),((short int)-278),((short int)-227),
  ((short int)-280),((short int)166),((short int)-555),((short int)-422),
  ((short int)-155),((short int)541),((short int)-366),((short int)54),
  ((short int)-29),((short int)-83),((short int)-301),((short int)-774),
  ((short int)186),((short int)628),((short int)-397),((short int)-264),
  ((short int)242),((short int)293),((short int)-197),((short int)-585),
  ((short int)124),((short int)410),((short int)53),((short int)-133),
  ((short int)10),((short int)340),((short int)-570),((short int)-1065),
  ((short int)65),((short int)-446),((short int)68),((short int)-493),
  ((short int)383),((short int)937),((short int)-357),((short int)-711),
  ((short int)-359),((short int)-250),((short int)-677),((short int)-1068),
  ((short int)292),((short int)-26),((short int)363),((short int)6),
  ((short int)607),((short int)1313),((short int)-127),((short int)-10),
  ((short int)1513),((short int)1886),((short int)713),((short int)972),
  ((short int)1469),((short int)2181),((short int)1443),((short int)2016) };


static short int dico2_lsf[1024] = { 

  ((short int)-1631),((short int)-1600),((short int)-1796),((short int)-2290),
  ((short int)-1027),((short int)-1770),((short int)-1100),((short int)-2025),
  ((short int)-1277),((short int)-1388),((short int)-1367),((short int)-1534),
  ((short int)-947),((short int)-1461),((short int)-972),((short int)-1524),
  ((short int)-999),((short int)-1222),((short int)-1020),((short int)-1172),
  ((short int)-815),((short int)-987),((short int)-992),((short int)-1371),
  ((short int)-1216),((short int)-1006),((short int)-1289),((short int)-1094),
  ((short int)-744),((short int)-1268),((short int)-755),((short int)-1293),
  ((short int)-862),((short int)-923),((short int)-905),((short int)-984),
  ((short int)-678),((short int)-1051),((short int)-685),((short int)-1050),
  ((short int)-1087),((short int)-985),((short int)-1062),((short int)-679),
  ((short int)-989),((short int)-641),((short int)-1127),((short int)-976),
  ((short int)-762),((short int)-654),((short int)-890),((short int)-806),
  ((short int)-833),((short int)-1091),((short int)-706),((short int)-629),
  ((short int)-621),((short int)-806),((short int)-640),((short int)-812),
  ((short int)-775),((short int)-634),((short int)-779),((short int)-543),
  ((short int)-996),((short int)-565),((short int)-1075),((short int)-580),
  ((short int)-546),((short int)-611),((short int)-572),((short int)-619),
  ((short int)-760),((short int)-290),((short int)-879),((short int)-526),
  ((short int)-823),((short int)-462),((short int)-795),((short int)-253),
  ((short int)-553),((short int)-415),((short int)-589),((short int)-439),
  ((short int)-533),((short int)-340),((short int)-692),((short int)-935),
  ((short int)-505),((short int)-772),((short int)-702),((short int)-1131),
  ((short int)-263),((short int)-306),((short int)-971),((short int)-483),
  ((short int)-445),((short int)-74),((short int)-555),((short int)-548),
  ((short int)-614),((short int)-129),((short int)-693),((short int)-234),
  ((short int)-396),((short int)-246),((short int)-475),((short int)-250),
  ((short int)-265),((short int)-404),((short int)-376),((short int)-514),
  ((short int)-417),((short int)-510),((short int)-300),((short int)-313),
  ((short int)-334),((short int)-664),((short int)-463),((short int)-814),
  ((short int)-386),((short int)-704),((short int)-337),((short int)-615),
  ((short int)-234),((short int)-201),((short int)-233),((short int)-239),
  ((short int)-167),((short int)-567),((short int)-203),((short int)-619),
  ((short int)-147),((short int)-415),((short int)-115),((short int)-352),
  ((short int)-166),((short int)-750),((short int)-171),((short int)-761),
  ((short int)-270),((short int)-879),((short int)-264),((short int)-903),
  ((short int)-367),((short int)-744),((short int)43),((short int)-475),
  ((short int)14),((short int)-653),((short int)43),((short int)-670),
  ((short int)11),((short int)-448),((short int)-59),((short int)-521),
  ((short int)-126),((short int)-119),((short int)-155),((short int)-613),
  ((short int)-42),((short int)-863),((short int)-27),((short int)-931),
  ((short int)136),((short int)-483),((short int)183),((short int)-468),
  ((short int)55),((short int)-298),((short int)55),((short int)-304),
  ((short int)313),((short int)-609),((short int)313),((short int)-720),
  ((short int)322),((short int)-167),((short int)100),((short int)-541),
  ((short int)-3),((short int)-119),((short int)-111),((short int)-187),
  ((short int)233),((short int)-236),((short int)260),((short int)-234),
  ((short int)26),((short int)-165),((short int)134),((short int)-45),
  ((short int)-40),((short int)-549),((short int)360),((short int)-203),
  ((short int)378),((short int)-388),((short int)450),((short int)-383),
  ((short int)275),((short int)20),((short int)182),((short int)-103),
  ((short int)246),((short int)-111),((short int)431),((short int)37),
  ((short int)462),((short int)-146),((short int)487),((short int)-157),
  ((short int)-284),((short int)-59),((short int)503),((short int)-184),
  ((short int)24),((short int)53),((short int)-3),((short int)54),
  ((short int)122),((short int)259),((short int)333),((short int)66),
  ((short int)484),((short int)104),((short int)436),((short int)68),
  ((short int)195),((short int)116),((short int)190),((short int)206),
  ((short int)269),((short int)-9),((short int)482),((short int)352),
  ((short int)382),((short int)285),((short int)399),((short int)277),
  ((short int)452),((short int)256),((short int)69),((short int)186),
  ((short int)13),((short int)297),((short int)-13),((short int)259),
  ((short int)-95),((short int)30),((short int)56),((short int)394),
  ((short int)196),((short int)425),((short int)205),((short int)456),
  ((short int)281),((short int)577),((short int)15),((short int)191),
  ((short int)375),((short int)290),((short int)407),((short int)576),
  ((short int)-56),((short int)227),((short int)544),((short int)405),
  ((short int)0),((short int)549),((short int)-92),((short int)528),
  ((short int)-229),((short int)351),((short int)-245),((short int)338),
  ((short int)-362),((short int)435),((short int)167),((short int)527),
  ((short int)-75),((short int)302),((short int)91),((short int)824),
  ((short int)129),((short int)599),((short int)496),((short int)679),
  ((short int)186),((short int)749),((short int)153),((short int)737),
  ((short int)-281),((short int)600),((short int)-348),((short int)615),
  ((short int)-236),((short int)769),((short int)41),((short int)881),
  ((short int)38),((short int)890),((short int)-220),((short int)841),
  ((short int)-357),((short int)883),((short int)-393),((short int)903),
  ((short int)-634),((short int)474),((short int)-444),((short int)850),
  ((short int)-175),((short int)678),((short int)-493),((short int)242),
  ((short int)-519),((short int)785),((short int)-714),((short int)582),
  ((short int)-541),((short int)366),((short int)-543),((short int)434),
  ((short int)-597),((short int)500),((short int)-765),((short int)222),
  ((short int)-702),((short int)917),((short int)-743),((short int)962),
  ((short int)-869),((short int)501),((short int)-899),((short int)548),
  ((short int)-379),((short int)200),((short int)-435),((short int)157),
  ((short int)-819),((short int)214),((short int)-861),((short int)157),
  ((short int)-614),((short int)40),((short int)-632),((short int)94),
  ((short int)-883),((short int)-54),((short int)-741),((short int)516),
  ((short int)-501),((short int)298),((short int)-614),((short int)-171),
  ((short int)-870),((short int)-161),((short int)-865),((short int)-23),
  ((short int)-818),((short int)93),((short int)-1015),((short int)-267),
  ((short int)-662),((short int)-359),((short int)-549),((short int)2),
  ((short int)-442),((short int)-121),((short int)-377),((short int)0),
  ((short int)-227),((short int)33),((short int)-414),((short int)-126),
  ((short int)-129),((short int)212),((short int)-934),((short int)34),
  ((short int)-1082),((short int)-282),((short int)-1119),((short int)-268),
  ((short int)-710),((short int)-825),((short int)-420),((short int)-191),
  ((short int)-1076),((short int)-928),((short int)-917),((short int)-93),
  ((short int)-628),((short int)-358),((short int)97),((short int)7),
  ((short int)-206),((short int)-393),((short int)-101),((short int)24),
  ((short int)-203),((short int)38),((short int)-168),((short int)83),
  ((short int)-599),((short int)-423),((short int)-279),((short int)426),
  ((short int)-700),((short int)118),((short int)-75),((short int)206),
  ((short int)-981),((short int)-673),((short int)-680),((short int)417),
  ((short int)-367),((short int)37),((short int)-279),((short int)474),
  ((short int)-129),((short int)-318),((short int)319),((short int)296),
  ((short int)-626),((short int)-39),((short int)343),((short int)602),
  ((short int)-696),((short int)-39),((short int)-303),((short int)940),
  ((short int)104),((short int)233),((short int)-380),((short int)137),
  ((short int)-36),((short int)269),((short int)-75),((short int)-214),
  ((short int)120),((short int)43),((short int)-529),((short int)-477),
  ((short int)459),((short int)164),((short int)-202),((short int)-229),
  ((short int)-49),((short int)-167),((short int)609),((short int)792),
  ((short int)98),((short int)-220),((short int)915),((short int)148),
  ((short int)293),((short int)283),((short int)869),((short int)91),
  ((short int)575),((short int)394),((short int)326),((short int)-78),
  ((short int)717),((short int)67),((short int)365),((short int)-323),
  ((short int)616),((short int)-36),((short int)731),((short int)27),
  ((short int)619),((short int)238),((short int)632),((short int)273),
  ((short int)448),((short int)99),((short int)801),((short int)476),
  ((short int)869),((short int)273),((short int)685),((short int)64),
  ((short int)789),((short int)72),((short int)1021),((short int)217),
  ((short int)793),((short int)459),((short int)734),((short int)360),
  ((short int)646),((short int)480),((short int)360),((short int)322),
  ((short int)429),((short int)464),((short int)638),((short int)430),
  ((short int)756),((short int)363),((short int)1000),((short int)404),
  ((short int)683),((short int)528),((short int)602),((short int)615),
  ((short int)655),((short int)413),((short int)946),((short int)687),
  ((short int)937),((short int)602),((short int)904),((short int)604),
  ((short int)555),((short int)737),((short int)786),((short int)662),
  ((short int)467),((short int)654),((short int)362),((short int)589),
  ((short int)929),((short int)710),((short int)498),((short int)478),
  ((short int)415),((short int)420),((short int)693),((short int)883),
  ((short int)813),((short int)683),((short int)781),((short int)925),
  ((short int)913),((short int)939),((short int)726),((short int)732),
  ((short int)491),((short int)853),((short int)531),((short int)948),
  ((short int)734),((short int)963),((short int)315),((short int)808),
  ((short int)761),((short int)755),((short int)1144),((short int)760),
  ((short int)655),((short int)1076),((short int)826),((short int)1057),
  ((short int)1091),((short int)838),((short int)1003),((short int)808),
  ((short int)1047),((short int)1133),((short int)659),((short int)1101),
  ((short int)992),((short int)1050),((short int)1074),((short int)1075),
  ((short int)971),((short int)694),((short int)1226),((short int)1054),
  ((short int)571),((short int)841),((short int)884),((short int)1404),
  ((short int)1379),((short int)1096),((short int)1080),((short int)861),
  ((short int)1231),((short int)735),((short int)1284),((short int)760),
  ((short int)1272),((short int)991),((short int)1367),((short int)1053),
  ((short int)1257),((short int)700),((short int)1050),((short int)534),
  ((short int)988),((short int)453),((short int)1264),((short int)599),
  ((short int)1140),((short int)679),((short int)1621),((short int)815),
  ((short int)1384),((short int)521),((short int)1317),((short int)393),
  ((short int)1564),((short int)805),((short int)1448),((short int)686),
  ((short int)1068),((short int)648),((short int)875),((short int)307),
  ((short int)1083),((short int)361),((short int)1047),((short int)317),
  ((short int)1417),((short int)964),((short int)675),((short int)571),
  ((short int)1152),((short int)79),((short int)1114),((short int)-47),
  ((short int)1530),((short int)311),((short int)1721),((short int)314),
  ((short int)1166),((short int)689),((short int)514),((short int)-94),
  ((short int)349),((short int)282),((short int)1412),((short int)328),
  ((short int)1025),((short int)487),((short int)-65),((short int)57),
  ((short int)805),((short int)970),((short int)36),((short int)62),
  ((short int)769),((short int)-263),((short int)791),((short int)-346),
  ((short int)637),((short int)699),((short int)-137),((short int)620),
  ((short int)534),((short int)541),((short int)-735),((short int)194),
  ((short int)711),((short int)300),((short int)-268),((short int)-863),
  ((short int)926),((short int)769),((short int)-708),((short int)-428),
  ((short int)506),((short int)174),((short int)-892),((short int)-630),
  ((short int)435),((short int)547),((short int)-1435),((short int)-258),
  ((short int)621),((short int)471),((short int)-1018),((short int)-1368),
  ((short int)-393),((short int)521),((short int)-920),((short int)-686),
  ((short int)-25),((short int)20),((short int)-982),((short int)-1156),
  ((short int)340),((short int)9),((short int)-1558),((short int)-1135),
  ((short int)-352),((short int)48),((short int)-1579),((short int)-402),
  ((short int)-887),((short int)6),((short int)-1156),((short int)-888),
  ((short int)-548),((short int)-352),((short int)-1643),((short int)-1168),
  ((short int)-159),((short int)610),((short int)-2024),((short int)-963),
  ((short int)-225),((short int)193),((short int)-1656),((short int)-1960),
  ((short int)-245),((short int)-493),((short int)-964),((short int)-1680),
  ((short int)-936),((short int)-635),((short int)-1299),((short int)-1744),
  ((short int)-1388),((short int)-604),((short int)-1540),((short int)-835),
  ((short int)-1397),((short int)-135),((short int)-1588),((short int)-290),
  ((short int)-1670),((short int)-712),((short int)-2011),((short int)-1632),
  ((short int)-1663),((short int)-27),((short int)-2258),((short int)-811),
  ((short int)-1157),((short int)184),((short int)-1265),((short int)189),
  ((short int)-1367),((short int)586),((short int)-2011),((short int)201),
  ((short int)-790),((short int)712),((short int)-1210),((short int)3),
  ((short int)-1033),((short int)808),((short int)-1251),((short int)830),
  ((short int)-111),((short int)635),((short int)-1636),((short int)447),
  ((short int)-463),((short int)-949),((short int)-445),((short int)-928),
  ((short int)-504),((short int)-1162),((short int)-501),((short int)-1211),
  ((short int)144),((short int)-351),((short int)-372),((short int)-1052),
  ((short int)-283),((short int)-1059),((short int)-279),((short int)-1123),
  ((short int)-575),((short int)-1438),((short int)-587),((short int)-1614),
  ((short int)-935),((short int)-984),((short int)229),((short int)690),
  ((short int)-921),((short int)-719),((short int)-403),((short int)1362),
  ((short int)-685),((short int)-465),((short int)874),((short int)397),
  ((short int)-509),((short int)-46),((short int)317),((short int)1334),
  ((short int)-485),((short int)456),((short int)813),((short int)439),
  ((short int)-411),((short int)339),((short int)898),((short int)1067),
  ((short int)-425),((short int)46),((short int)1441),((short int)497),
  ((short int)-909),((short int)-800),((short int)1465),((short int)1046),
  ((short int)-254),((short int)-321),((short int)1430),((short int)1165),
  ((short int)68),((short int)350),((short int)1034),((short int)666),
  ((short int)370),((short int)11),((short int)1311),((short int)790),
  ((short int)143),((short int)232),((short int)1041),((short int)1562),
  ((short int)-114),((short int)663),((short int)1616),((short int)1078),
  ((short int)454),((short int)579),((short int)1275),((short int)1040),
  ((short int)-76),((short int)909),((short int)752),((short int)1067),
  ((short int)153),((short int)512),((short int)348),((short int)1214),
  ((short int)614),((short int)385),((short int)1843),((short int)808),
  ((short int)269),((short int)1034),((short int)203),((short int)1086),
  ((short int)652),((short int)1017),((short int)1783),((short int)1130),
  ((short int)429),((short int)1327),((short int)387),((short int)1384),
  ((short int)-49),((short int)1183),((short int)-72),((short int)1215),
  ((short int)-416),((short int)1001),((short int)544),((short int)1749),
  ((short int)-352),((short int)1223),((short int)-502),((short int)1199),
  ((short int)-589),((short int)569),((short int)-227),((short int)1630),
  ((short int)-142),((short int)1578),((short int)-230),((short int)1715),
  ((short int)-714),((short int)1288),((short int)-838),((short int)1398),
  ((short int)1131),((short int)1357),((short int)-208),((short int)1232),
  ((short int)437),((short int)965),((short int)-929),((short int)818),
  ((short int)811),((short int)1410),((short int)859),((short int)1507),
  ((short int)164),((short int)1212),((short int)1387),((short int)1793),
  ((short int)484),((short int)1874),((short int)456),((short int)2063),
  ((short int)996),((short int)1170),((short int)1326),((short int)1402),
  ((short int)1316),((short int)1360),((short int)1135),((short int)1262),
  ((short int)1234),((short int)1618),((short int)1361),((short int)1768),
  ((short int)1421),((short int)1227),((short int)1584),((short int)1347),
  ((short int)854),((short int)672),((short int)1685),((short int)1566),
  ((short int)1139),((short int)1270),((short int)2016),((short int)1825),
  ((short int)1773),((short int)1581),((short int)1532),((short int)1460),
  ((short int)1487),((short int)946),((short int)1659),((short int)1021),
  ((short int)1744),((short int)1212),((short int)1392),((short int)977),
  ((short int)1772),((short int)1161),((short int)1826),((short int)1164),
  ((short int)1718),((short int)1429),((short int)1973),((short int)1591),
  ((short int)1185),((short int)864),((short int)2132),((short int)1061),
  ((short int)1799),((short int)814),((short int)1838),((short int)757),
  ((short int)2104),((short int)1315),((short int)2054),((short int)1258),
  ((short int)2113),((short int)915),((short int)2331),((short int)930),
  ((short int)1467),((short int)1147),((short int)2590),((short int)1439),
  ((short int)2245),((short int)1744),((short int)2090),((short int)1620),
  ((short int)2358),((short int)1454),((short int)2666),((short int)1506),
  ((short int)1876),((short int)1837),((short int)2070),((short int)1975),
  ((short int)1739),((short int)1577),((short int)682),((short int)1289),
  ((short int)1584),((short int)2045),((short int)1454),((short int)2098),
  ((short int)2498),((short int)2004),((short int)2711),((short int)2066),
  ((short int)726),((short int)1588),((short int)2756),((short int)2336),
  ((short int)228),((short int)847),((short int)2456),((short int)1659),
  ((short int)36),((short int)301),((short int)1942),((short int)1957),
  ((short int)-446),((short int)-96),((short int)2154),((short int)1396),
  ((short int)1533),((short int)1101),((short int)14),((short int)608),
  ((short int)-923),((short int)-732),((short int)1383),((short int)1982),
  ((short int)1345),((short int)952),((short int)-680),((short int)321),
  ((short int)1281),((short int)1268),((short int)-1594),((short int)365),
  ((short int)941),((short int)946),((short int)-1737),((short int)-822),
  ((short int)2374),((short int)2787),((short int)1821),((short int)2788) };


static short int dico3_lsf[1024] = { 

  ((short int)-1812),((short int)-2275),((short int)-1879),((short int)-2537),
  ((short int)-1640),((short int)-1848),((short int)-1695),((short int)-2004),
  ((short int)-1220),((short int)-1912),((short int)-1221),((short int)-2106),
  ((short int)-1559),((short int)-1588),((short int)-1573),((short int)-1556),
  ((short int)-1195),((short int)-1615),((short int)-1224),((short int)-1727),
  ((short int)-1359),((short int)-1151),((short int)-1616),((short int)-1948),
  ((short int)-1274),((short int)-1391),((short int)-1305),((short int)-1403),
  ((short int)-1607),((short int)-1179),((short int)-1676),((short int)-1311),
  ((short int)-1443),((short int)-1478),((short int)-1367),((short int)-898),
  ((short int)-1256),((short int)-1059),((short int)-1331),((short int)-1134),
  ((short int)-982),((short int)-1133),((short int)-1149),((short int)-1504),
  ((short int)-1080),((short int)-1308),((short int)-1020),((short int)-1183),
  ((short int)-980),((short int)-1486),((short int)-967),((short int)-1495),
  ((short int)-988),((short int)-922),((short int)-1047),((short int)-1077),
  ((short int)-838),((short int)-1179),((short int)-858),((short int)-1222),
  ((short int)-1131),((short int)-1041),((short int)-1064),((short int)-767),
  ((short int)-872),((short int)-1157),((short int)-701),((short int)-880),
  ((short int)-706),((short int)-906),((short int)-774),((short int)-1016),
  ((short int)-578),((short int)-1080),((short int)-801),((short int)-1478),
  ((short int)-591),((short int)-1111),((short int)-592),((short int)-1146),
  ((short int)-713),((short int)-1388),((short int)-640),((short int)-1376),
  ((short int)-597),((short int)-1059),((short int)-416),((short int)-903),
  ((short int)-686),((short int)-832),((short int)-661),((short int)-708),
  ((short int)-444),((short int)-868),((short int)-490),((short int)-921),
  ((short int)-374),((short int)-776),((short int)-619),((short int)-1170),
  ((short int)-585),((short int)-549),((short int)-769),((short int)-795),
  ((short int)-435),((short int)-659),((short int)-530),((short int)-741),
  ((short int)-498),((short int)-837),((short int)-357),((short int)-597),
  ((short int)-279),((short int)-871),((short int)-243),((short int)-887),
  ((short int)-282),((short int)-665),((short int)-280),((short int)-667),
  ((short int)-165),((short int)-560),((short int)-394),((short int)-903),
  ((short int)-362),((short int)-410),((short int)-448),((short int)-583),
  ((short int)-409),((short int)-574),((short int)-313),((short int)-357),
  ((short int)-637),((short int)-548),((short int)-570),((short int)-436),
  ((short int)-896),((short int)-504),((short int)-382),((short int)-757),
  ((short int)-58),((short int)-481),((short int)-165),((short int)-618),
  ((short int)-191),((short int)-374),((short int)-234),((short int)-382),
  ((short int)-222),((short int)-683),((short int)-25),((short int)-480),
  ((short int)-418),((short int)-359),((short int)-730),((short int)-353),
  ((short int)-324),((short int)-157),((short int)-432),((short int)-322),
  ((short int)-394),((short int)-303),((short int)-284),((short int)-104),
  ((short int)-601),((short int)-289),((short int)-556),((short int)-196),
  ((short int)-588),((short int)-150),((short int)-659),((short int)-608),
  ((short int)-473),((short int)-24),((short int)-68),((short int)-448),
  ((short int)-474),((short int)-8),((short int)-506),((short int)-45),
  ((short int)-748),((short int)-184),((short int)-844),((short int)-252),
  ((short int)-901),((short int)-91),((short int)-584),((short int)-97),
  ((short int)-652),((short int)138),((short int)-764),((short int)-131),
  ((short int)-678),((short int)-12),((short int)-670),((short int)165),
  ((short int)-259),((short int)-3),((short int)-840),((short int)-107),
  ((short int)-909),((short int)37),((short int)-992),((short int)44),
  ((short int)-854),((short int)-415),((short int)-839),((short int)13),
  ((short int)-1001),((short int)-271),((short int)-1026),((short int)-309),
  ((short int)-798),((short int)-478),((short int)-832),((short int)-488),
  ((short int)-943),((short int)168),((short int)-1112),((short int)-387),
  ((short int)-1185),((short int)-101),((short int)-1183),((short int)-40),
  ((short int)-941),((short int)-316),((short int)-1030),((short int)-770),
  ((short int)-1044),((short int)-625),((short int)-1081),((short int)-538),
  ((short int)-1224),((short int)-299),((short int)-1312),((short int)-436),
  ((short int)-1197),((short int)-663),((short int)-1167),((short int)-161),
  ((short int)-1216),((short int)-690),((short int)-1237),((short int)-831),
  ((short int)-1432),((short int)-720),((short int)-1403),((short int)-493),
  ((short int)-898),((short int)-740),((short int)-922),((short int)-801),
  ((short int)-1102),((short int)-402),((short int)-1579),((short int)-964),
  ((short int)-1061),((short int)-638),((short int)-1269),((short int)-1438),
  ((short int)-1499),((short int)-934),((short int)-1502),((short int)-895),
  ((short int)-1598),((short int)-564),((short int)-1723),((short int)-717),
  ((short int)-606),((short int)-597),((short int)-1166),((short int)-1085),
  ((short int)-1369),((short int)-468),((short int)-1946),((short int)-1493),
  ((short int)-1838),((short int)-953),((short int)-1932),((short int)-931),
  ((short int)-1499),((short int)-188),((short int)-1635),((short int)-421),
  ((short int)-1457),((short int)-338),((short int)-1448),((short int)-22),
  ((short int)-1942),((short int)-422),((short int)-2006),((short int)-249),
  ((short int)-496),((short int)-114),((short int)-1910),((short int)-755),
  ((short int)-1289),((short int)174),((short int)-1451),((short int)-109),
  ((short int)-482),((short int)-257),((short int)-1221),((short int)-508),
  ((short int)-1617),((short int)151),((short int)-1694),((short int)208),
  ((short int)-654),((short int)107),((short int)-1651),((short int)29),
  ((short int)-1141),((short int)279),((short int)-1215),((short int)306),
  ((short int)-1228),((short int)-506),((short int)-730),((short int)-175),
  ((short int)-1236),((short int)-101),((short int)-969),((short int)551),
  ((short int)-870),((short int)278),((short int)-823),((short int)315),
  ((short int)-563),((short int)376),((short int)-1051),((short int)228),
  ((short int)-507),((short int)280),((short int)-599),((short int)281),
  ((short int)-758),((short int)253),((short int)-305),((short int)379),
  ((short int)-755),((short int)-134),((short int)-611),((short int)660),
  ((short int)-824),((short int)536),((short int)-817),((short int)646),
  ((short int)-413),((short int)49),((short int)-341),((short int)177),
  ((short int)-453),((short int)526),((short int)-482),((short int)589),
  ((short int)-71),((short int)339),((short int)-657),((short int)264),
  ((short int)-244),((short int)295),((short int)-237),((short int)315),
  ((short int)-387),((short int)569),((short int)-506),((short int)-9),
  ((short int)-377),((short int)14),((short int)-160),((short int)661),
  ((short int)-216),((short int)40),((short int)-308),((short int)-46),
  ((short int)95),((short int)214),((short int)-242),((short int)167),
  ((short int)-86),((short int)192),((short int)-56),((short int)27),
  ((short int)-76),((short int)31),((short int)36),((short int)309),
  ((short int)-106),((short int)-182),((short int)-113),((short int)74),
  ((short int)-441),((short int)-22),((short int)23),((short int)139),
  ((short int)81),((short int)-11),((short int)44),((short int)15),
  ((short int)-87),((short int)-137),((short int)-118),((short int)-207),
  ((short int)-158),((short int)-58),((short int)272),((short int)-92),
  ((short int)-156),((short int)-441),((short int)8),((short int)-136),
  ((short int)128),((short int)-221),((short int)101),((short int)-218),
  ((short int)40),((short int)-197),((short int)-76),((short int)-456),
  ((short int)9),((short int)-445),((short int)33),((short int)-423),
  ((short int)226),((short int)60),((short int)73),((short int)-222),
  ((short int)156),((short int)-399),((short int)280),((short int)-318),
  ((short int)245),((short int)-341),((short int)166),((short int)-499),
  ((short int)339),((short int)-190),((short int)327),((short int)-219),
  ((short int)325),((short int)-137),((short int)-89),((short int)-596),
  ((short int)100),((short int)-627),((short int)144),((short int)-677),
  ((short int)487),((short int)28),((short int)252),((short int)-391),
  ((short int)214),((short int)-41),((short int)282),((short int)-28),
  ((short int)99),((short int)-286),((short int)331),((short int)49),
  ((short int)459),((short int)-388),((short int)565),((short int)-369),
  ((short int)436),((short int)28),((short int)336),((short int)-9),
  ((short int)397),((short int)-167),((short int)618),((short int)34),
  ((short int)596),((short int)-17),((short int)561),((short int)-140),
  ((short int)299),((short int)79),((short int)522),((short int)125),
  ((short int)203),((short int)2),((short int)244),((short int)288),
  ((short int)255),((short int)211),((short int)175),((short int)82),
  ((short int)596),((short int)187),((short int)517),((short int)108),
  ((short int)381),((short int)255),((short int)365),((short int)297),
  ((short int)497),((short int)352),((short int)327),((short int)-82),
  ((short int)25),((short int)210),((short int)371),((short int)245),
  ((short int)261),((short int)3),((short int)545),((short int)449),
  ((short int)140),((short int)294),((short int)44),((short int)295),
  ((short int)212),((short int)347),((short int)244),((short int)494),
  ((short int)331),((short int)528),((short int)201),((short int)307),
  ((short int)349),((short int)411),((short int)613),((short int)284),
  ((short int)614),((short int)413),((short int)464),((short int)322),
  ((short int)624),((short int)397),((short int)97),((short int)200),
  ((short int)-160),((short int)384),((short int)149),((short int)362),
  ((short int)495),((short int)525),((short int)269),((short int)585),
  ((short int)33),((short int)491),((short int)-121),((short int)433),
  ((short int)427),((short int)611),((short int)498),((short int)516),
  ((short int)171),((short int)443),((short int)497),((short int)666),
  ((short int)440),((short int)275),((short int)566),((short int)575),
  ((short int)146),((short int)639),((short int)155),((short int)670),
  ((short int)-33),((short int)173),((short int)212),((short int)696),
  ((short int)-166),((short int)601),((short int)-191),((short int)695),
  ((short int)-489),((short int)503),((short int)175),((short int)742),
  ((short int)214),((short int)476),((short int)372),((short int)1083),
  ((short int)578),((short int)530),((short int)586),((short int)777),
  ((short int)425),((short int)874),((short int)315),((short int)841),
  ((short int)374),((short int)848),((short int)-165),((short int)565),
  ((short int)35),((short int)991),((short int)-39),((short int)1062),
  ((short int)329),((short int)712),((short int)786),((short int)840),
  ((short int)645),((short int)795),((short int)661),((short int)676),
  ((short int)571),((short int)918),((short int)632),((short int)1079),
  ((short int)673),((short int)817),((short int)318),((short int)388),
  ((short int)874),((short int)1012),((short int)564),((short int)848),
  ((short int)880),((short int)620),((short int)557),((short int)479),
  ((short int)671),((short int)453),((short int)692),((short int)468),
  ((short int)840),((short int)642),((short int)844),((short int)645),
  ((short int)506),((short int)428),((short int)897),((short int)567),
  ((short int)837),((short int)387),((short int)962),((short int)499),
  ((short int)691),((short int)561),((short int)939),((short int)926),
  ((short int)783),((short int)296),((short int)790),((short int)268),
  ((short int)1028),((short int)530),((short int)874),((short int)329),
  ((short int)548),((short int)143),((short int)675),((short int)291),
  ((short int)503),((short int)66),((short int)1041),((short int)359),
  ((short int)786),((short int)97),((short int)805),((short int)33),
  ((short int)837),((short int)470),((short int)511),((short int)49),
  ((short int)1092),((short int)327),((short int)1174),((short int)323),
  ((short int)3),((short int)242),((short int)872),((short int)474),
  ((short int)689),((short int)429),((short int)1329),((short int)678),
  ((short int)1042),((short int)620),((short int)1109),((short int)664),
  ((short int)321),((short int)193),((short int)889),((short int)950),
  ((short int)1153),((short int)874),((short int)893),((short int)635),
  ((short int)877),((short int)862),((short int)948),((short int)913),
  ((short int)1293),((short int)665),((short int)1320),((short int)639),
  ((short int)997),((short int)793),((short int)1402),((short int)1030),
  ((short int)1176),((short int)1012),((short int)1110),((short int)959),
  ((short int)1410),((short int)925),((short int)1403),((short int)915),
  ((short int)543),((short int)862),((short int)1116),((short int)1222),
  ((short int)835),((short int)1190),((short int)835),((short int)1190),
  ((short int)959),((short int)1148),((short int)1147),((short int)1376),
  ((short int)1300),((short int)1193),((short int)1415),((short int)1231),
  ((short int)1335),((short int)1341),((short int)746),((short int)1092),
  ((short int)1711),((short int)1283),((short int)1389),((short int)1073),
  ((short int)1334),((short int)1566),((short int)1153),((short int)1475),
  ((short int)1645),((short int)1137),((short int)1825),((short int)1220),
  ((short int)1056),((short int)1382),((short int)1521),((short int)1730),
  ((short int)1632),((short int)1545),((short int)1620),((short int)1542),
  ((short int)855),((short int)1596),((short int)865),((short int)1667),
  ((short int)693),((short int)885),((short int)1716),((short int)1519),
  ((short int)1167),((short int)1296),((short int)2209),((short int)1760),
  ((short int)1952),((short int)1493),((short int)2020),((short int)1482),
  ((short int)1534),((short int)1866),((short int)1694),((short int)2008),
  ((short int)1566),((short int)748),((short int)1761),((short int)825),
  ((short int)294),((short int)1392),((short int)1084),((short int)2058),
  ((short int)621),((short int)1315),((short int)365),((short int)1287),
  ((short int)198),((short int)1028),((short int)488),((short int)1408),
  ((short int)249),((short int)403),((short int)1014),((short int)1561),
  ((short int)324),((short int)363),((short int)1645),((short int)1044),
  ((short int)193),((short int)367),((short int)2034),((short int)1859),
  ((short int)-251),((short int)579),((short int)750),((short int)994),
  ((short int)-243),((short int)30),((short int)1325),((short int)879),
  ((short int)-28),((short int)-169),((short int)624),((short int)917),
  ((short int)-453),((short int)159),((short int)186),((short int)1370),
  ((short int)-614),((short int)6),((short int)537),((short int)392),
  ((short int)-94),((short int)-291),((short int)781),((short int)229),
  ((short int)-128),((short int)-298),((short int)245),((short int)491),
  ((short int)-701),((short int)-648),((short int)972),((short int)789),
  ((short int)-501),((short int)-640),((short int)178),((short int)255),
  ((short int)-365),((short int)-390),((short int)-255),((short int)317),
  ((short int)-958),((short int)-294),((short int)-191),((short int)228),
  ((short int)-775),((short int)-447),((short int)157),((short int)-237),
  ((short int)-657),((short int)-720),((short int)-407),((short int)92),
  ((short int)-117),((short int)-611),((short int)334),((short int)-230),
  ((short int)-679),((short int)-1084),((short int)-144),((short int)-317),
  ((short int)-901),((short int)-861),((short int)-738),((short int)-360),
  ((short int)-85),((short int)-727),((short int)-90),((short int)-787),
  ((short int)100),((short int)-22),((short int)-391),((short int)-263),
  ((short int)-56),((short int)-73),((short int)-337),((short int)-754),
  ((short int)5),((short int)-189),((short int)-706),((short int)-624),
  ((short int)89),((short int)-344),((short int)-135),((short int)-1113),
  ((short int)-353),((short int)-237),((short int)-684),((short int)-1135),
  ((short int)-275),((short int)-1102),((short int)-269),((short int)-1203),
  ((short int)152),((short int)145),((short int)-722),((short int)-1232),
  ((short int)49),((short int)80),((short int)-1248),((short int)-776),
  ((short int)-248),((short int)391),((short int)-732),((short int)-547),
  ((short int)469),((short int)218),((short int)-255),((short int)-864),
  ((short int)69),((short int)366),((short int)-166),((short int)-485),
  ((short int)-688),((short int)191),((short int)-1212),((short int)-1196),
  ((short int)-170),((short int)-169),((short int)-1308),((short int)-1631),
  ((short int)321),((short int)470),((short int)-1419),((short int)-1243),
  ((short int)-64),((short int)272),((short int)-1361),((short int)-248),
  ((short int)492),((short int)565),((short int)-721),((short int)-609),
  ((short int)195),((short int)485),((short int)-573),((short int)-133),
  ((short int)427),((short int)202),((short int)-171),((short int)-118),
  ((short int)199),((short int)575),((short int)2),((short int)-31),
  ((short int)694),((short int)755),((short int)-1366),((short int)-39),
  ((short int)552),((short int)557),((short int)-489),((short int)271),
  ((short int)680),((short int)537),((short int)13),((short int)-453),
  ((short int)855),((short int)954),((short int)-133),((short int)-52),
  ((short int)-81),((short int)738),((short int)-1169),((short int)637),
  ((short int)1055),((short int)1059),((short int)-95),((short int)676),
  ((short int)1259),((short int)1081),((short int)489),((short int)305),
  ((short int)-449),((short int)954),((short int)-534),((short int)996),
  ((short int)-969),((short int)866),((short int)-1058),((short int)1059),
  ((short int)-1294),((short int)618),((short int)-1416),((short int)617),
  ((short int)-458),((short int)1366),((short int)-159),((short int)1821),
  ((short int)-774),((short int)-528),((short int)-14),((short int)1110),
  ((short int)-1202),((short int)-901),((short int)-772),((short int)433),
  ((short int)-1256),((short int)-1255),((short int)-1011),((short int)-302),
  ((short int)-602),((short int)-585),((short int)-759),((short int)-1618),
  ((short int)-760),((short int)-1549),((short int)-840),((short int)-1921),
  ((short int)-816),((short int)-539),((short int)-1769),((short int)-2235),
  ((short int)-227),((short int)-36),((short int)-2034),((short int)-1831),
  ((short int)-2107),((short int)-1126),((short int)-2471),((short int)-1816),
  ((short int)-1470),((short int)252),((short int)-2701),((short int)-415),
  ((short int)-571),((short int)-467),((short int)1509),((short int)1554),
  ((short int)2180),((short int)1975),((short int)2326),((short int)2020) };


static short int dico4_lsf[1024] = { 

  ((short int)-1857),((short int)-1681),((short int)-1857),((short int)-1755),
  ((short int)-2056),((short int)-1150),((short int)-2134),((short int)-1654),
  ((short int)-1619),((short int)-1099),((short int)-1704),((short int)-1131),
  ((short int)-1345),((short int)-1608),((short int)-1359),((short int)-1638),
  ((short int)-1338),((short int)-1293),((short int)-1325),((short int)-1265),
  ((short int)-1664),((short int)-1649),((short int)-1487),((short int)-851),
  ((short int)-1346),((short int)-1832),((short int)-1413),((short int)-2188),
  ((short int)-1282),((short int)-681),((short int)-1785),((short int)-1649),
  ((short int)-966),((short int)-1082),((short int)-1183),((short int)-1676),
  ((short int)-1054),((short int)-1073),((short int)-1142),((short int)-1158),
  ((short int)-1207),((short int)-744),((short int)-1274),((short int)-997),
  ((short int)-934),((short int)-1383),((short int)-927),((short int)-1416),
  ((short int)-1010),((short int)-1305),((short int)-783),((short int)-955),
  ((short int)-1049),((short int)-900),((short int)-993),((short int)-817),
  ((short int)-737),((short int)-823),((short int)-972),((short int)-1189),
  ((short int)-738),((short int)-1094),((short int)-738),((short int)-1154),
  ((short int)-784),((short int)-801),((short int)-810),((short int)-786),
  ((short int)-892),((short int)-520),((short int)-1000),((short int)-818),
  ((short int)-644),((short int)-965),((short int)-577),((short int)-882),
  ((short int)-541),((short int)-694),((short int)-671),((short int)-917),
  ((short int)-595),((short int)-642),((short int)-646),((short int)-615),
  ((short int)-956),((short int)-621),((short int)-925),((short int)-515),
  ((short int)-727),((short int)-483),((short int)-815),((short int)-485),
  ((short int)-840),((short int)-578),((short int)-440),((short int)-713),
  ((short int)-578),((short int)-325),((short int)-657),((short int)-670),
  ((short int)-386),((short int)-570),((short int)-441),((short int)-666),
  ((short int)-514),((short int)-787),((short int)-392),((short int)-529),
  ((short int)-522),((short int)-453),((short int)-487),((short int)-423),
  ((short int)-616),((short int)-585),((short int)-617),((short int)-157),
  ((short int)-662),((short int)-268),((short int)-680),((short int)-348),
  ((short int)-322),((short int)-323),((short int)-632),((short int)-444),
  ((short int)-304),((short int)-430),((short int)-332),((short int)-458),
  ((short int)-277),((short int)-468),((short int)-659),((short int)-793),
  ((short int)-319),((short int)-636),((short int)-227),((short int)-554),
  ((short int)-373),((short int)-347),((short int)-334),((short int)-210),
  ((short int)-456),((short int)-192),((short int)-530),((short int)-242),
  ((short int)-216),((short int)-198),((short int)-366),((short int)-370),
  ((short int)-338),((short int)-161),((short int)-409),((short int)-748),
  ((short int)-107),((short int)-380),((short int)-294),((short int)-643),
  ((short int)-223),((short int)-665),((short int)-234),((short int)-741),
  ((short int)-141),((short int)-496),((short int)-130),((short int)-510),
  ((short int)-139),((short int)-327),((short int)-172),((short int)-305),
  ((short int)-306),((short int)-580),((short int)-164),((short int)-263),
  ((short int)-262),((short int)-172),((short int)-67),((short int)-402),
  ((short int)31),((short int)-366),((short int)-10),((short int)-436),
  ((short int)-86),((short int)-527),((short int)71),((short int)-377),
  ((short int)-22),((short int)-609),((short int)-12),((short int)-678),
  ((short int)-67),((short int)-319),((short int)63),((short int)-191),
  ((short int)35),((short int)-181),((short int)-39),((short int)-242),
  ((short int)126),((short int)-167),((short int)-140),((short int)-544),
  ((short int)155),((short int)-297),((short int)174),((short int)-297),
  ((short int)38),((short int)-8),((short int)117),((short int)-380),
  ((short int)197),((short int)-452),((short int)240),((short int)-522),
  ((short int)223),((short int)-103),((short int)110),((short int)-187),
  ((short int)87),((short int)-155),((short int)169),((short int)-47),
  ((short int)157),((short int)26),((short int)-83),((short int)-100),
  ((short int)128),((short int)80),((short int)209),((short int)-62),
  ((short int)6),((short int)7),((short int)22),((short int)5),
  ((short int)318),((short int)-20),((short int)248),((short int)-45),
  ((short int)-200),((short int)-63),((short int)156),((short int)-69),
  ((short int)250),((short int)-183),((short int)369),((short int)-126),
  ((short int)-113),((short int)-76),((short int)-142),((short int)-122),
  ((short int)-64),((short int)-254),((short int)-31),((short int)35),
  ((short int)-177),((short int)-71),((short int)-7),((short int)171),
  ((short int)93),((short int)27),((short int)108),((short int)212),
  ((short int)-330),((short int)-209),((short int)-123),((short int)-70),
  ((short int)-279),((short int)95),((short int)-96),((short int)20),
  ((short int)-188),((short int)-61),((short int)-314),((short int)87),
  ((short int)-300),((short int)-78),((short int)-354),((short int)-134),
  ((short int)11),((short int)122),((short int)-140),((short int)122),
  ((short int)-275),((short int)152),((short int)-293),((short int)140),
  ((short int)-82),((short int)138),((short int)-321),((short int)-111),
  ((short int)-480),((short int)-156),((short int)-359),((short int)76),
  ((short int)-254),((short int)-40),((short int)-635),((short int)-96),
  ((short int)-522),((short int)79),((short int)-507),((short int)8),
  ((short int)-268),((short int)303),((short int)-539),((short int)68),
  ((short int)-446),((short int)61),((short int)-522),((short int)306),
  ((short int)111),((short int)189),((short int)-435),((short int)122),
  ((short int)-379),((short int)166),((short int)-571),((short int)-398),
  ((short int)-632),((short int)-74),((short int)-747),((short int)-95),
  ((short int)-455),((short int)194),((short int)-952),((short int)83),
  ((short int)-798),((short int)192),((short int)-755),((short int)192),
  ((short int)-781),((short int)-162),((short int)-619),((short int)234),
  ((short int)-663),((short int)-297),((short int)-488),((short int)-109),
  ((short int)-964),((short int)-132),((short int)-838),((short int)-68),
  ((short int)-843),((short int)58),((short int)-1112),((short int)-86),
  ((short int)-805),((short int)-299),((short int)-944),((short int)-253),
  ((short int)-778),((short int)-50),((short int)-965),((short int)-549),
  ((short int)-352),((short int)-98),((short int)-992),((short int)-343),
  ((short int)-1117),((short int)-315),((short int)-1117),((short int)-307),
  ((short int)-1155),((short int)-374),((short int)-637),((short int)-230),
  ((short int)-1166),((short int)-43),((short int)-1299),((short int)-100),
  ((short int)-925),((short int)-393),((short int)-1274),((short int)-600),
  ((short int)-689),((short int)-130),((short int)-1479),((short int)-312),
  ((short int)-1321),((short int)-254),((short int)-1464),((short int)-442),
  ((short int)-1292),((short int)-613),((short int)-1261),((short int)-503),
  ((short int)-1501),((short int)-368),((short int)-1322),((short int)26),
  ((short int)-1432),((short int)-66),((short int)-1743),((short int)-161),
  ((short int)-1644),((short int)-467),((short int)-1760),((short int)-548),
  ((short int)-1393),((short int)-568),((short int)-1556),((short int)-871),
  ((short int)-1495),((short int)-1034),((short int)-1387),((short int)-571),
  ((short int)-1917),((short int)-528),((short int)-1783),((short int)-123),
  ((short int)-1897),((short int)-231),((short int)-2054),((short int)-323),
  ((short int)-2052),((short int)-906),((short int)-1976),((short int)-567),
  ((short int)-1917),((short int)-620),((short int)-2047),((short int)-989),
  ((short int)-1077),((short int)-370),((short int)-2031),((short int)-704),
  ((short int)-2355),((short int)-749),((short int)-2740),((short int)-1089),
  ((short int)-1909),((short int)159),((short int)-2012),((short int)248),
  ((short int)-626),((short int)-123),((short int)-2339),((short int)-962),
  ((short int)-669),((short int)-408),((short int)-1379),((short int)-1174),
  ((short int)-452),((short int)-364),((short int)-1044),((short int)-735),
  ((short int)-132),((short int)183),((short int)-1620),((short int)-752),
  ((short int)-547),((short int)-307),((short int)-777),((short int)-1261),
  ((short int)-98),((short int)41),((short int)-880),((short int)-1091),
  ((short int)-257),((short int)97),((short int)-1602),((short int)-1833),
  ((short int)31),((short int)-26),((short int)-644),((short int)-561),
  ((short int)-180),((short int)-546),((short int)-385),((short int)-1095),
  ((short int)-410),((short int)-802),((short int)-414),((short int)-827),
  ((short int)-457),((short int)-970),((short int)-490),((short int)-1109),
  ((short int)-215),((short int)-916),((short int)-144),((short int)-937),
  ((short int)-493),((short int)-1269),((short int)-517),((short int)-1507),
  ((short int)181),((short int)101),((short int)-332),((short int)-889),
  ((short int)-836),((short int)-937),((short int)-559),((short int)-429),
  ((short int)-629),((short int)-547),((short int)-183),((short int)-337),
  ((short int)-545),((short int)-82),((short int)-250),((short int)-286),
  ((short int)5),((short int)-132),((short int)-348),((short int)-252),
  ((short int)-293),((short int)-472),((short int)-158),((short int)100),
  ((short int)-29),((short int)197),((short int)-236),((short int)-424),
  ((short int)-861),((short int)-213),((short int)-140),((short int)-7),
  ((short int)-427),((short int)-443),((short int)187),((short int)-97),
  ((short int)-684),((short int)-736),((short int)-293),((short int)258),
  ((short int)-368),((short int)-152),((short int)-150),((short int)392),
  ((short int)-609),((short int)175),((short int)-142),((short int)299),
  ((short int)-138),((short int)152),((short int)-119),((short int)329),
  ((short int)-486),((short int)-52),((short int)293),((short int)198),
  ((short int)-183),((short int)117),((short int)175),((short int)331),
  ((short int)-58),((short int)-274),((short int)231),((short int)300),
  ((short int)-288),((short int)330),((short int)-305),((short int)372),
  ((short int)-111),((short int)409),((short int)-9),((short int)423),
  ((short int)83),((short int)256),((short int)67),((short int)367),
  ((short int)-19),((short int)248),((short int)91),((short int)113),
  ((short int)-35),((short int)406),((short int)-191),((short int)154),
  ((short int)238),((short int)296),((short int)5),((short int)197),
  ((short int)141),((short int)221),((short int)313),((short int)198),
  ((short int)211),((short int)421),((short int)244),((short int)334),
  ((short int)88),((short int)426),((short int)-243),((short int)454),
  ((short int)202),((short int)552),((short int)-5),((short int)403),
  ((short int)291),((short int)185),((short int)219),((short int)301),
  ((short int)251),((short int)138),((short int)128),((short int)69),
  ((short int)197),((short int)288),((short int)-140),((short int)-61),
  ((short int)188),((short int)361),((short int)197),((short int)598),
  ((short int)442),((short int)273),((short int)290),((short int)143),
  ((short int)472),((short int)482),((short int)157),((short int)370),
  ((short int)415),((short int)321),((short int)372),((short int)385),
  ((short int)402),((short int)552),((short int)155),((short int)24),
  ((short int)550),((short int)263),((short int)-11),((short int)21),
  ((short int)360),((short int)227),((short int)147),((short int)-254),
  ((short int)424),((short int)97),((short int)366),((short int)-13),
  ((short int)375),((short int)141),((short int)449),((short int)232),
  ((short int)396),((short int)507),((short int)474),((short int)272),
  ((short int)701),((short int)324),((short int)362),((short int)-47),
  ((short int)587),((short int)148),((short int)543),((short int)69),
  ((short int)400),((short int)-51),((short int)561),((short int)59),
  ((short int)220),((short int)-10),((short int)352),((short int)147),
  ((short int)206),((short int)211),((short int)653),((short int)185),
  ((short int)563),((short int)297),((short int)565),((short int)284),
  ((short int)594),((short int)121),((short int)766),((short int)192),
  ((short int)398),((short int)118),((short int)642),((short int)434),
  ((short int)233),((short int)264),((short int)481),((short int)467),
  ((short int)129),((short int)-165),((short int)699),((short int)239),
  ((short int)90),((short int)26),((short int)342),((short int)474),
  ((short int)-55),((short int)27),((short int)388),((short int)94),
  ((short int)-172),((short int)0),((short int)725),((short int)379),
  ((short int)-60),((short int)337),((short int)370),((short int)465),
  ((short int)95),((short int)319),((short int)806),((short int)595),
  ((short int)78),((short int)260),((short int)497),((short int)851),
  ((short int)210),((short int)560),((short int)458),((short int)574),
  ((short int)-464),((short int)202),((short int)497),((short int)625),
  ((short int)-202),((short int)152),((short int)48),((short int)712),
  ((short int)-20),((short int)566),((short int)100),((short int)715),
  ((short int)455),((short int)468),((short int)411),((short int)605),
  ((short int)319),((short int)646),((short int)195),((short int)615),
  ((short int)401),((short int)538),((short int)680),((short int)739),
  ((short int)201),((short int)667),((short int)434),((short int)954),
  ((short int)454),((short int)425),((short int)646),((short int)491),
  ((short int)606),((short int)681),((short int)416),((short int)508),
  ((short int)497),((short int)822),((short int)426),((short int)815),
  ((short int)660),((short int)647),((short int)628),((short int)716),
  ((short int)697),((short int)466),((short int)618),((short int)457),
  ((short int)685),((short int)460),((short int)365),((short int)309),
  ((short int)721),((short int)567),((short int)836),((short int)601),
  ((short int)609),((short int)300),((short int)825),((short int)459),
  ((short int)943),((short int)687),((short int)681),((short int)533),
  ((short int)915),((short int)598),((short int)591),((short int)243),
  ((short int)876),((short int)451),((short int)874),((short int)420),
  ((short int)786),((short int)317),((short int)732),((short int)220),
  ((short int)922),((short int)317),((short int)1108),((short int)367),
  ((short int)531),((short int)466),((short int)1028),((short int)649),
  ((short int)1053),((short int)615),((short int)1034),((short int)553),
  ((short int)829),((short int)602),((short int)1021),((short int)799),
  ((short int)927),((short int)803),((short int)878),((short int)763),
  ((short int)799),((short int)496),((short int)1373),((short int)773),
  ((short int)585),((short int)770),((short int)803),((short int)930),
  ((short int)1099),((short int)793),((short int)1222),((short int)862),
  ((short int)1209),((short int)895),((short int)1025),((short int)727),
  ((short int)772),((short int)845),((short int)1172),((short int)1115),
  ((short int)867),((short int)1021),((short int)830),((short int)1013),
  ((short int)841),((short int)910),((short int)506),((short int)703),
  ((short int)1239),((short int)1077),((short int)620),((short int)819),
  ((short int)1196),((short int)1083),((short int)1155),((short int)1081),
  ((short int)1142),((short int)907),((short int)1547),((short int)1121),
  ((short int)1309),((short int)648),((short int)1343),((short int)612),
  ((short int)1484),((short int)988),((short int)1479),((short int)937),
  ((short int)985),((short int)1328),((short int)955),((short int)1341),
  ((short int)429),((short int)910),((short int)841),((short int)1338),
  ((short int)564),((short int)1179),((short int)412),((short int)1156),
  ((short int)1427),((short int)1320),((short int)1434),((short int)1330),
  ((short int)640),((short int)760),((short int)1726),((short int)1410),
  ((short int)190),((short int)555),((short int)1073),((short int)1005),
  ((short int)426),((short int)257),((short int)839),((short int)980),
  ((short int)235),((short int)231),((short int)1520),((short int)1167),
  ((short int)109),((short int)293),((short int)1014),((short int)1569),
  ((short int)305),((short int)142),((short int)1148),((short int)539),
  ((short int)-291),((short int)-108),((short int)1213),((short int)972),
  ((short int)22),((short int)-216),((short int)667),((short int)828),
  ((short int)-482),((short int)438),((short int)453),((short int)1431),
  ((short int)-581),((short int)-422),((short int)789),((short int)387),
  ((short int)-358),((short int)-454),((short int)174),((short int)780),
  ((short int)-36),((short int)-372),((short int)390),((short int)-134),
  ((short int)-629),((short int)160),((short int)-306),((short int)751),
  ((short int)-1258),((short int)-331),((short int)177),((short int)522),
  ((short int)-248),((short int)574),((short int)-251),((short int)639),
  ((short int)-531),((short int)407),((short int)-596),((short int)394),
  ((short int)-419),((short int)789),((short int)-617),((short int)801),
  ((short int)-986),((short int)399),((short int)-857),((short int)727),
  ((short int)-7),((short int)518),((short int)-703),((short int)310),
  ((short int)-1143),((short int)-24),((short int)-1002),((short int)287),
  ((short int)-960),((short int)363),((short int)-1299),((short int)312),
  ((short int)-1534),((short int)245),((short int)-1557),((short int)305),
  ((short int)28),((short int)153),((short int)-859),((short int)-175),
  ((short int)-33),((short int)332),((short int)-1398),((short int)-154),
  ((short int)212),((short int)410),((short int)-593),((short int)-197),
  ((short int)-1092),((short int)-704),((short int)-904),((short int)-65),
  ((short int)282),((short int)367),((short int)-918),((short int)-686),
  ((short int)345),((short int)93),((short int)-258),((short int)-357),
  ((short int)696),((short int)644),((short int)-693),((short int)-28),
  ((short int)448),((short int)493),((short int)-273),((short int)193),
  ((short int)527),((short int)546),((short int)-243),((short int)-513),
  ((short int)384),((short int)-136),((short int)273),((short int)-353),
  ((short int)512),((short int)-142),((short int)537),((short int)-198),
  ((short int)941),((short int)750),((short int)83),((short int)248),
  ((short int)578),((short int)861),((short int)-56),((short int)592),
  ((short int)842),((short int)44),((short int)892),((short int)24),
  ((short int)33),((short int)890),((short int)-16),((short int)982),
  ((short int)831),((short int)1398),((short int)1535),((short int)1898),
  ((short int)1716),((short int)1376),((short int)1948),((short int)1465) };


static short int dico5_lsf[256] = { 

  ((short int)-1002),((short int)-929),((short int)-1096),((short int)-1203),
  ((short int)-641),((short int)-931),((short int)-604),((short int)-961),
  ((short int)-779),((short int)-673),((short int)-835),((short int)-788),
  ((short int)-416),((short int)-664),((short int)-458),((short int)-766),
  ((short int)-652),((short int)-521),((short int)-662),((short int)-495),
  ((short int)-1023),((short int)-509),((short int)-1023),((short int)-428),
  ((short int)-444),((short int)-552),((short int)-368),((short int)-449),
  ((short int)-479),((short int)-211),((short int)-1054),((short int)-903),
  ((short int)-316),((short int)-249),((short int)-569),((short int)-591),
  ((short int)-569),((short int)-275),((short int)-541),((short int)-191),
  ((short int)-716),((short int)-188),((short int)-842),((short int)-264),
  ((short int)-333),((short int)-248),((short int)-318),((short int)-228),
  ((short int)-275),((short int)1),((short int)-567),((short int)-228),
  ((short int)-115),((short int)-221),((short int)-238),((short int)-374),
  ((short int)-197),((short int)-507),((short int)-222),((short int)-579),
  ((short int)-258),((short int)-432),((short int)-61),((short int)-244),
  ((short int)-345),((short int)2),((short int)-338),((short int)39),
  ((short int)-215),((short int)-169),((short int)-58),((short int)0),
  ((short int)-56),((short int)-6),((short int)-203),((short int)-131),
  ((short int)1),((short int)-186),((short int)-5),((short int)-211),
  ((short int)6),((short int)-380),((short int)11),((short int)-418),
  ((short int)-116),((short int)131),((short int)-134),((short int)113),
  ((short int)89),((short int)-4),((short int)71),((short int)-2),
  ((short int)-19),((short int)-192),((short int)262),((short int)24),
  ((short int)189),((short int)151),((short int)-133),((short int)-109),
  ((short int)186),((short int)-153),((short int)166),((short int)-219),
  ((short int)37),((short int)139),((short int)193),((short int)171),
  ((short int)337),((short int)124),((short int)158),((short int)-61),
  ((short int)141),((short int)226),((short int)-13),((short int)190),
  ((short int)231),((short int)34),((short int)354),((short int)109),
  ((short int)316),((short int)201),((short int)244),((short int)164),
  ((short int)330),((short int)-85),((short int)390),((short int)-84),
  ((short int)254),((short int)327),((short int)257),((short int)335),
  ((short int)491),((short int)147),((short int)476),((short int)105),
  ((short int)54),((short int)77),((short int)437),((short int)370),
  ((short int)421),((short int)314),((short int)449),((short int)342),
  ((short int)329),((short int)126),((short int)673),((short int)292),
  ((short int)571),((short int)388),((short int)243),((short int)193),
  ((short int)653),((short int)320),((short int)621),((short int)280),
  ((short int)194),((short int)380),((short int)517),((short int)581),
  ((short int)45),((short int)323),((short int)111),((short int)422),
  ((short int)489),((short int)395),((short int)734),((short int)534),
  ((short int)622),((short int)546),((short int)486),((short int)502),
  ((short int)318),((short int)572),((short int)189),((short int)550),
  ((short int)385),((short int)422),((short int)-157),((short int)153),
  ((short int)-125),((short int)382),((short int)-197),((short int)386),
  ((short int)-263),((short int)334),((short int)228),((short int)697),
  ((short int)-188),((short int)1),((short int)51),((short int)297),
  ((short int)-507),((short int)213),((short int)-376),((short int)397),
  ((short int)-24),((short int)255),((short int)-547),((short int)89),
  ((short int)-502),((short int)-94),((short int)387),((short int)179),
  ((short int)-620),((short int)68),((short int)-684),((short int)112),
  ((short int)-642),((short int)-350),((short int)-260),((short int)172),
  ((short int)-438),((short int)-324),((short int)264),((short int)648),
  ((short int)-964),((short int)-4),((short int)-1121),((short int)7),
  ((short int)-134),((short int)134),((short int)-1133),((short int)-306),
  ((short int)143),((short int)96),((short int)-420),((short int)-497),
  ((short int)-1221),((short int)-350),((short int)-1527),((short int)-685),
  ((short int)-161),((short int)72),((short int)873),((short int)691),
  ((short int)732),((short int)283),((short int)921),((short int)353),
  ((short int)334),((short int)475),((short int)1095),((short int)821),
  ((short int)864),((short int)524),((short int)843),((short int)497),
  ((short int)714),((short int)711),((short int)788),((short int)750),
  ((short int)1076),((short int)714),((short int)1204),((short int)753) };

#line 1150 "vocoderSched.cc"
unsigned int __PRF_globalCounter[1619];

// channel class definitions /////////////////////////////////////////

OSNone::OSNone()
    : _specc::channel()
{   
}

OSNone::~OSNone(void)
{   
}

#line 23 "OSNone.sc"
void OSNone::ienter(void) {
}

#line 30 "OSNone.sc"
void OSNone::init(void) {
}

#line 26 "OSNone.sc"
void OSNone::ireturn(void) {
    _specc::notify(event(&intr), ((void*)0));
}

#line 38 "OSNone.sc"
void OSNone::par_end(int p) {
}

#line 36 "OSNone.sc"
int OSNone::par_start(void) { return  -1;
}

#line 56 "OSNone.sc"
void OSNone::post_wait(int me) {
}

#line 55 "OSNone.sc"
int OSNone::pre_wait() { return  -1;
}

#line 32 "OSNone.sc"
void OSNone::start(void) {
}

#line 43 "OSNone.sc"
void OSNone::task_activate(int tid) {
}

#line 40 "OSNone.sc"
int OSNone::task_create(const char *name, int type, unsigned long long int period, unsigned long long int wcet)
{    return  -1;
}

#line 51 "OSNone.sc"
void OSNone::task_endcycle(void) {
}
void OSNone::task_kill(int tid) {
}

#line 47 "OSNone.sc"
void OSNone::task_resume(int id) {
}

#line 45 "OSNone.sc"
void OSNone::task_sleep(void) {
}


void OSNone::task_terminate(void) {
}

#line 34 "OSNone.sc"
void OSNone::time_wait(long long int nsec) {
}

#line 19 "OSNone.sc"
void OSNone::wait_int(void) {
    _specc::wait(event(&intr), ((void*)0));
}

#line 1233 "vocoderSched.cc"
c_osnone_int_handshake::c_osnone_int_handshake(OSAPINone (&os))
    : _specc::channel(), os(os),
    f(false),
    w(false)
{   
}

c_osnone_int_handshake::~c_osnone_int_handshake(void)
{   
}

#line 71 "OSNone.sc"
void c_osnone_int_handshake::receive(void)
{   
    while( !f)
    {   
	os.wait_int();
    }
    f = false;
}

void c_osnone_int_handshake::send(void)
{   
    f = true;
}

#line 1260 "vocoderSched.cc"
c_double_handshake::c_double_handshake()
    : _specc::channel(),
    v(false),
    w(false)
{   
}

c_double_handshake::~c_double_handshake(void)
{   
}

#line 84 "c_double_handshake.sc"
void c_double_handshake::receive(void *d, unsigned long int l)
{   
    if ( !v)
    {   
	w = true;
	_specc::wait(event(&req), ((void*)0));
	w = false;
    }
    if (l != tmpl)
    {   
	abort();
    }
    memcpy(d, tmpd, l);
    v = false;
    _specc::notify(event(&ack), ((void*)0));
    _specc::wait(event(&ack), ((void*)0));
}

void c_double_handshake::send(const void *d, unsigned long int l)
{   
    tmpd = d;
    tmpl = l;
    v = true;
    if (w)
    {   
	_specc::notify(event(&req), ((void*)0));
    }
    _specc::wait(event(&ack), ((void*)0));
}

#line 1303 "vocoderSched.cc"
c_handshake::c_handshake()
    : _specc::channel(),
    f(false),
    w(false)
{   
}

c_handshake::~c_handshake(void)
{   
}

#line 62 "c_handshake.sc"
void c_handshake::receive(void)
{   
    if ( !f)
    {   
	w = true;
	_specc::wait(event(&e), ((void*)0));
	w = false;
    }
    f = false;
}

void c_handshake::send(void)
{   
    if (w)
    {   
	_specc::notify(event(&e), ((void*)0));
    }
    f = true;
}

#line 1336 "vocoderSched.cc"
c_int_queue::c_int_queue(const unsigned long int (&size))
    : _specc::channel(), size(size),
    buffer(0),
    n(0ul),
    p(0ul),
    wr(0ul),
    ws(0ul)
{   
}

c_int_queue::~c_int_queue(void)
{   
}

void c_int_queue::cleanup(void)
{   
    if ( !n)
    {   
	free(buffer);
	buffer = 0;
    }
}

void c_int_queue::receive(int *d)
{   
    while( !n)
    {   
	wr++ ;
	_specc::wait(event(&r), ((void*)0));
	wr-- ;
    }
    if (n <= p)
    {   
	 *d = buffer[p - n];
    }
    else 
    {   
	 *d = buffer[p + size - n];
    }
    n-- ;
    if (ws)
    {   
	_specc::notify(event(&s), ((void*)0));
    }
    cleanup();
}

void c_int_queue::send(int d)
{   
    while(n >= size)
    {   
	ws++ ;
	_specc::wait(event(&s), ((void*)0));
	ws-- ;
    }
    setup();
    buffer[p] = d;
    p++ ;
    if (p >= size)
    {   
	p = 0;
    }
    n++ ;
    if (wr)
    {   
	_specc::notify(event(&r), ((void*)0));
    }
}

void c_int_queue::setup(void)
{   
    if ( !buffer)
    {   
	if ( !(buffer = (int *)malloc(sizeof(int) * size)))
	{   
	    exit(1);
	}
    }
}

c_short_int__10__queue::c_short_int__10__queue(const unsigned long int (&size))
    : _specc::channel(), size(size),
    buffer(0),
    n(0ul),
    p(0ul),
    wr(0ul),
    ws(0ul)
{   
}

c_short_int__10__queue::~c_short_int__10__queue(void)
{   
}

void c_short_int__10__queue::cleanup(void)
{   
    if ( !n)
    {   
	free(buffer);
	buffer = 0;
    }
}

void c_short_int__10__queue::receive(short int (*d)[10])
{   
    while( !n)
    {   
	wr++ ;
	_specc::wait(event(&r), ((void*)0));
	wr-- ;
    }
    if (n <= p)
    {   
	{ unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<10;_scc_index_0++) ( *d)[_scc_index_0] = (buffer[p - 
	    n])[_scc_index_0]; }
    }
    else 
    {   
	{ unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<10;_scc_index_0++) ( *d)[_scc_index_0] = (buffer[p + 
	    size - n])[_scc_index_0]; }
    }
    n-- ;
    if (ws)
    {   
	_specc::notify(event(&s), ((void*)0));
    }
    cleanup();
}

void c_short_int__10__queue::send(short int d[10])
{   
    while(n >= size)
    {   
	ws++ ;
	_specc::wait(event(&s), ((void*)0));
	ws-- ;
    }
    setup();
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<10;_scc_index_0++) (buffer[p])[_scc_index_0] = (d)[_scc_index_0]; }
    p++ ;
    if (p >= size)
    {   
	p = 0;
    }
    n++ ;
    if (wr)
    {   
	_specc::notify(event(&r), ((void*)0));
    }
}

void c_short_int__10__queue::setup(void)
{   
    if ( !buffer)
    {   
	if ( !(buffer = (short int (*)[10])malloc(sizeof(short int [10]) * 
		    size)))
	{   
	    exit(1);
	}
    }
}

c_short_int__40__queue::c_short_int__40__queue(const unsigned long int (&size))
    : _specc::channel(), size(size),
    buffer(0),
    n(0ul),
    p(0ul),
    wr(0ul),
    ws(0ul)
{   
}

c_short_int__40__queue::~c_short_int__40__queue(void)
{   
}

void c_short_int__40__queue::cleanup(void)
{   
    if ( !n)
    {   
	free(buffer);
	buffer = 0;
    }
}

void c_short_int__40__queue::receive(short int (*d)[40])
{   
    while( !n)
    {   
	wr++ ;
	_specc::wait(event(&r), ((void*)0));
	wr-- ;
    }
    if (n <= p)
    {   
	{ unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<40;_scc_index_0++) ( *d)[_scc_index_0] = (buffer[p - 
	    n])[_scc_index_0]; }
    }
    else 
    {   
	{ unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<40;_scc_index_0++) ( *d)[_scc_index_0] = (buffer[p + 
	    size - n])[_scc_index_0]; }
    }
    n-- ;
    if (ws)
    {   
	_specc::notify(event(&s), ((void*)0));
    }
    cleanup();
}

void c_short_int__40__queue::send(short int d[40])
{   
    while(n >= size)
    {   
	ws++ ;
	_specc::wait(event(&s), ((void*)0));
	ws-- ;
    }
    setup();
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<40;_scc_index_0++) (buffer[p])[_scc_index_0] = (d)[_scc_index_0]; }
    p++ ;
    if (p >= size)
    {   
	p = 0;
    }
    n++ ;
    if (wr)
    {   
	_specc::notify(event(&r), ((void*)0));
    }
}

void c_short_int__40__queue::setup(void)
{   
    if ( !buffer)
    {   
	if ( !(buffer = (short int (*)[40])malloc(sizeof(short int [40]) * 
		    size)))
	{   
	    exit(1);
	}
    }
}

c_short_int_queue::c_short_int_queue(const unsigned long int (&size))
    : _specc::channel(), size(size),
    buffer(0),
    n(0ul),
    p(0ul),
    wr(0ul),
    ws(0ul)
{   
}

c_short_int_queue::~c_short_int_queue(void)
{   
}

void c_short_int_queue::cleanup(void)
{   
    if ( !n)
    {   
	free(buffer);
	buffer = 0;
    }
}

void c_short_int_queue::receive(short int *d)
{   
    while( !n)
    {   
	wr++ ;
	_specc::wait(event(&r), ((void*)0));
	wr-- ;
    }
    if (n <= p)
    {   
	 *d = buffer[p - n];
    }
    else 
    {   
	 *d = buffer[p + size - n];
    }
    n-- ;
    if (ws)
    {   
	_specc::notify(event(&s), ((void*)0));
    }
    cleanup();
}

void c_short_int_queue::send(short int d)
{   
    while(n >= size)
    {   
	ws++ ;
	_specc::wait(event(&s), ((void*)0));
	ws-- ;
    }
    setup();
    buffer[p] = d;
    p++ ;
    if (p >= size)
    {   
	p = 0;
    }
    n++ ;
    if (wr)
    {   
	_specc::notify(event(&r), ((void*)0));
    }
}

void c_short_int_queue::setup(void)
{   
    if ( !buffer)
    {   
	if ( !(buffer = (short int *)malloc(sizeof(short int) * size)))
	{   
	    exit(1);
	}
    }
}

// behavior class definitions ////////////////////////////////////////

Closed_Loop_Seq1::Closed_Loop_Seq1(unsigned int _idcnt, int (&i_subfr), short int (&h1)[40], short int *(&p_exc), short int *(&p_speech), short int *(&p_h1), short int *(&p_exc_i), short int *(&p_speech_i))
    : _specc::behavior(_idcnt), i_subfr(i_subfr), h1(h1), p_exc(p_exc), p_speech(p_speech), p_h1(p_h1), p_exc_i(p_exc_i), p_speech_i(p_speech_i)
{   
}

Closed_Loop_Seq1::~Closed_Loop_Seq1(void)
{   
}

void Closed_Loop_Seq1::main(void)

#line 31 "src/closed_loop/closed_loop.sc"
{   

#line 1679 "vocoderSched.cc"
    __PRF_globalCounter[205u]++ ;
    _specc::waitfor(0ull);

#line 32 "src/closed_loop/closed_loop.sc"
    p_h1 = (short int *)h1;
    p_exc_i = p_exc + i_subfr;
    p_speech_i = p_speech + i_subfr;
}

#line 1689 "vocoderSched.cc"
Closed_Loop_Seq2::Closed_Loop_Seq2(unsigned int _idcnt, short int (&Aq)[11], short int (&Ap1)[11], short int (&Ap2)[11], short int (&h1)[40], short int *(&p_speech_i), short int (&res2)[40], short int *(&p_exc_i), short int (&xn)[40], short int (&mem_err)[10], short int (&mem_w0)[10], short int (&txdtx_ctrl), short int (&CN_excitation_gain), bool (&reset_flag))
    : _specc::behavior(_idcnt), Aq(Aq), Ap1(Ap1), Ap2(Ap2), h1(h1), p_speech_i(p_speech_i), res2(res2), p_exc_i(p_exc_i), xn(xn), mem_err(mem_err), mem_w0(mem_w0), txdtx_ctrl(txdtx_ctrl), CN_excitation_gain(CN_excitation_gain), reset_flag(reset_flag),
    find_targetvec(_IDcnt, Aq, Ap1, Ap2, p_speech_i, res2, p_exc_i, xn, mem_err, mem_w0, txdtx_ctrl, CN_excitation_gain),
    imp_resp(_IDcnt, Aq, Ap1, Ap2, h1, txdtx_ctrl, reset_flag)
{   
}

Closed_Loop_Seq2::~Closed_Loop_Seq2(void)
{   
}

void Closed_Loop_Seq2::main(void)
{   
    __PRF_globalCounter[206u]++ ;
    imp_resp.main();
    find_targetvec.main();
}

void Closed_Loop_Seq2::reset(void)

#line 62 "src/closed_loop/closed_loop.sc"
{   

#line 1713 "vocoderSched.cc"
    __PRF_globalCounter[207u]++ ;

#line 63 "src/closed_loop/closed_loop.sc"
    imp_resp.reset();
}

#line 1720 "vocoderSched.cc"
Closed_Loop_Seq3::Closed_Loop_Seq3(unsigned int _idcnt, int (&i_subfr), int (&pit_flag), short int (&t0_min_2), short int (&t0_max_2), short int (&t0_min), short int (&t0_max))
    : _specc::behavior(_idcnt), i_subfr(i_subfr), pit_flag(pit_flag), t0_min_2(t0_min_2), t0_max_2(t0_max_2), t0_min(t0_min), t0_max(t0_max)
{   
}

Closed_Loop_Seq3::~Closed_Loop_Seq3(void)
{   
}

void Closed_Loop_Seq3::main(void)

#line 88 "src/closed_loop/closed_loop.sc"
{   

#line 1735 "vocoderSched.cc"
    __PRF_globalCounter[208u]++ ;
    _specc::waitfor(16666ull);

#line 94 "src/closed_loop/closed_loop.sc"
    pit_flag = i_subfr;

#line 1742 "vocoderSched.cc"
    __PRF_globalCounter[209u]++ ;

#line 97 "src/closed_loop/closed_loop.sc"
    if (sub(i_subfr, 80) == 0)
    {   

#line 1749 "vocoderSched.cc"
	__PRF_globalCounter[210u]++ ;

#line 99 "src/closed_loop/closed_loop.sc"
	pit_flag = 0;
	t0_min = t0_min_2;
	t0_max = t0_max_2;
    }
}

#line 1759 "vocoderSched.cc"
Closed_Loop_Seq4::Closed_Loop_Seq4(unsigned int _idcnt, short int (&ana_content), short int (&T0), short int (&ana)[2], short int (&t0))
    : _specc::behavior(_idcnt), ana_content(ana_content), T0(T0), ana(ana), t0(t0)
{   
}

Closed_Loop_Seq4::~Closed_Loop_Seq4(void)
{   
}

void Closed_Loop_Seq4::main(void)

#line 115 "src/closed_loop/closed_loop.sc"
{   

#line 1774 "vocoderSched.cc"
    __PRF_globalCounter[211u]++ ;
    _specc::waitfor(33333ull);

#line 116 "src/closed_loop/closed_loop.sc"
    ana[0] = ana_content;
    t0 = T0;
}

#line 1783 "vocoderSched.cc"
Closed_Loop_Seq5::Closed_Loop_Seq5(unsigned int _idcnt, short int (&gain_pit))
    : _specc::behavior(_idcnt), gain_pit(gain_pit)
{   
}

Closed_Loop_Seq5::~Closed_Loop_Seq5(void)
{   
}

void Closed_Loop_Seq5::main(void)

#line 125 "src/closed_loop/closed_loop.sc"
{   

#line 1798 "vocoderSched.cc"
    __PRF_globalCounter[212u]++ ;
    _specc::waitfor(0ull);

#line 126 "src/closed_loop/closed_loop.sc"
    gain_pit = 0;
}

#line 1806 "vocoderSched.cc"
Closed_Loop_Seq6::Closed_Loop_Seq6(unsigned int _idcnt, short int (&y1b)[40], short int (&gain_pitch), short int (&ana_content), short int (&y1)[40], short int (&gain_pit), short int (&ana)[2])
    : _specc::behavior(_idcnt), y1b(y1b), gain_pitch(gain_pitch), ana_content(ana_content), y1(y1), gain_pit(gain_pit), ana(ana)
{   
}

Closed_Loop_Seq6::~Closed_Loop_Seq6(void)
{   
}

void Closed_Loop_Seq6::main(void)

#line 140 "src/closed_loop/closed_loop.sc"
{   

#line 1821 "vocoderSched.cc"
    __PRF_globalCounter[213u]++ ;
    _specc::waitfor(33333ull);

#line 141 "src/closed_loop/closed_loop.sc"
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<40;_scc_index_0++) (y1)[_scc_index_0] = (y1b)[_scc_index_0]; }
    gain_pit = gain_pitch;
    ana[1] = ana_content;
}

#line 1831 "vocoderSched.cc"
Compute_CN_Excitation_Gain::Compute_CN_Excitation_Gain(unsigned int _idcnt, short int (&res2)[40], short int (&c_gain))
    : _specc::behavior(_idcnt), res2(res2), c_gain(c_gain)
{   
}

Compute_CN_Excitation_Gain::~Compute_CN_Excitation_Gain(void)
{   
}

void Compute_CN_Excitation_Gain::main(void)

#line 32 "src/closed_loop/compute_CN_excitation_gain.sc"
{   
    int i;
    short int gain;
    short int norm; short int norm1;
    bool overfl;
    int L_temp;

#line 1851 "vocoderSched.cc"
    __PRF_globalCounter[254u]++ ;
    _specc::waitfor(0ull);

#line 41 "src/closed_loop/compute_CN_excitation_gain.sc"
    norm = 0;

#line 1858 "vocoderSched.cc"
    __PRF_globalCounter[255u]++ ;

#line 42 "src/closed_loop/compute_CN_excitation_gain.sc"
    do 
    {   

#line 1865 "vocoderSched.cc"
	__PRF_globalCounter[256u]++ ;

#line 44 "src/closed_loop/compute_CN_excitation_gain.sc"
	overfl = 0;

	L_temp = 0l;

#line 1873 "vocoderSched.cc"
	__PRF_globalCounter[257u]++ ;

#line 47 "src/closed_loop/compute_CN_excitation_gain.sc"
	for(i = 0; i < 40; i++ )
	{   

#line 1880 "vocoderSched.cc"
	    __PRF_globalCounter[258u]++ ;

#line 49 "src/closed_loop/compute_CN_excitation_gain.sc"
	    gain = shr(res2[i], norm);
	    L_temp = L_mac(L_temp, gain, gain);
	}

#line 1888 "vocoderSched.cc"
	__PRF_globalCounter[259u]++ ;

#line 54 "src/closed_loop/compute_CN_excitation_gain.sc"
	if (L_sub(L_temp, (int)2147483647l) == 0)
	{   

#line 1895 "vocoderSched.cc"
	    __PRF_globalCounter[260u]++ ;

#line 56 "src/closed_loop/compute_CN_excitation_gain.sc"
	    norm = add(norm, 1);
	    overfl = 1;
	}
    }
    while(
	overfl != 0);

#line 1906 "vocoderSched.cc"
    __PRF_globalCounter[261u]++ ;

#line 63 "src/closed_loop/compute_CN_excitation_gain.sc"
    L_temp = L_add(L_temp, 1l);




    norm1 = norm_l(L_temp);
    gain = extract_h(L_shl(L_temp, norm1));
    L_temp = L_mult(gain, gain);
    L_temp = L_sub(805306368l, L_shr(L_temp, 3));
    L_temp = L_add(L_temp, L_mult(24576, gain));

    gain = extract_h(L_temp);

#line 1923 "vocoderSched.cc"
    __PRF_globalCounter[262u]++ ;

#line 76 "src/closed_loop/compute_CN_excitation_gain.sc"
    if ((norm1 & 1) != 0)
    {   

#line 1930 "vocoderSched.cc"
	__PRF_globalCounter[263u]++ ;

#line 78 "src/closed_loop/compute_CN_excitation_gain.sc"
	gain = mult_r(gain, 23170);
	norm1 = sub(norm1, 1);
    }

#line 1938 "vocoderSched.cc"
    __PRF_globalCounter[264u]++ ;

#line 83 "src/closed_loop/compute_CN_excitation_gain.sc"
    gain = mult_r(gain, 10362);



    norm1 = shr(norm1, 1);
    norm1 = sub(norm1, norm);

#line 1949 "vocoderSched.cc"
    __PRF_globalCounter[265u]++ ;

#line 91 "src/closed_loop/compute_CN_excitation_gain.sc"
    if (norm1 >= 0)
    {   

#line 1956 "vocoderSched.cc"
	__PRF_globalCounter[266u]++ ;

#line 93 "src/closed_loop/compute_CN_excitation_gain.sc"
	c_gain = shr(gain, norm1);
    }
    else 
    {   

#line 1965 "vocoderSched.cc"
	__PRF_globalCounter[267u]++ ;

#line 97 "src/closed_loop/compute_CN_excitation_gain.sc"
	c_gain = shl(gain, abs_s(norm1));
    }
}

#line 1973 "vocoderSched.cc"
Convolve::Convolve(unsigned int _idcnt, short int *(&x), short int *(&h), short int (&y)[40])
    : _specc::behavior(_idcnt), x(x), h(h), y(y)
{   
}

Convolve::~Convolve(void)
{   
}

void Convolve::main(void)

#line 31 "src/closed_loop/convolve.sc"
{   
    short int *p_y;

#line 1989 "vocoderSched.cc"
    __PRF_globalCounter[268u]++ ;
    _specc::waitfor(0ull);

#line 34 "src/closed_loop/convolve.sc"
    p_y =  &y[0];
    Convolut(x, h, p_y, 40);
}

#line 1998 "vocoderSched.cc"
Enc_Lag6::Enc_Lag6(unsigned int _idcnt, short int (&T0), short int (&T0_frac), short int (&T0_min), short int (&T0_max), int (&pit_flag), short int (&index))
    : _specc::behavior(_idcnt), T0(T0), T0_frac(T0_frac), T0_min(T0_min), T0_max(T0_max), pit_flag(pit_flag), index(index)
{   
}

Enc_Lag6::~Enc_Lag6(void)
{   
}

void Enc_Lag6::main(void)

#line 56 "src/closed_loop/enc_lag6.sc"
{   
    short int i;

#line 2014 "vocoderSched.cc"
    __PRF_globalCounter[306u]++ ;
    _specc::waitfor(50000ull);
    __PRF_globalCounter[307u]++ ;

#line 60 "src/closed_loop/enc_lag6.sc"
    if (pit_flag == 0)
    {   

#line 2023 "vocoderSched.cc"
	__PRF_globalCounter[308u]++ ;

#line 65 "src/closed_loop/enc_lag6.sc"
	if (sub(T0, 94) <= 0)
	{   

#line 2030 "vocoderSched.cc"
	    __PRF_globalCounter[309u]++ ;

#line 68 "src/closed_loop/enc_lag6.sc"
	    i = add(add(T0, T0), T0);
	    index = add(sub(add(i, i), 105), T0_frac);
	}
	else  {

#line 2039 "vocoderSched.cc"
	    __PRF_globalCounter[310u]++ ;

#line 72 "src/closed_loop/enc_lag6.sc"
	    T0_frac = 0;
	    index = add(T0, 368);
	}

#line 2047 "vocoderSched.cc"
	__PRF_globalCounter[311u]++ ;

#line 78 "src/closed_loop/enc_lag6.sc"
	T0_min = sub(T0, 5);

#line 2053 "vocoderSched.cc"
	__PRF_globalCounter[312u]++ ;

#line 80 "src/closed_loop/enc_lag6.sc"
	if (sub(T0_min, 18) < 0)
	{   

#line 2060 "vocoderSched.cc"
	    __PRF_globalCounter[313u]++ ;

#line 82 "src/closed_loop/enc_lag6.sc"
	    T0_min = 18;
	}

#line 2067 "vocoderSched.cc"
	__PRF_globalCounter[314u]++ ;

#line 84 "src/closed_loop/enc_lag6.sc"
	T0_max = add(T0_min, 9);

#line 2073 "vocoderSched.cc"
	__PRF_globalCounter[315u]++ ;

#line 86 "src/closed_loop/enc_lag6.sc"
	if (sub(T0_max, 143) > 0)
	{   

#line 2080 "vocoderSched.cc"
	    __PRF_globalCounter[316u]++ ;

#line 88 "src/closed_loop/enc_lag6.sc"
	    T0_max = 143;
	    T0_min = sub(T0_max, 9);
	}
    }
    else 
    {   

#line 2091 "vocoderSched.cc"
	__PRF_globalCounter[317u]++ ;

#line 95 "src/closed_loop/enc_lag6.sc"
	i = sub(T0, T0_min);
	i = add(add(i, i), i);
	index = add(add(add(i, i), 3), T0_frac);
    }
}

#line 2101 "vocoderSched.cc"
Find_Targetvec_Copy1::Find_Targetvec_Copy1(unsigned int _idcnt, short int (&res2b)[40], short int (&mem_err)[10], short int (&res2)[40], short int *(&p_exc_i), short int (&mem)[10])
    : _specc::behavior(_idcnt), res2b(res2b), mem_err(mem_err), res2(res2), p_exc_i(p_exc_i), mem(mem)
{   
}

Find_Targetvec_Copy1::~Find_Targetvec_Copy1(void)
{   
}

void Find_Targetvec_Copy1::main(void)

#line 21 "src/closed_loop/find_targetvec.sc"
{   

#line 2116 "vocoderSched.cc"
    __PRF_globalCounter[369u]++ ;
    _specc::waitfor(0ull);

#line 22 "src/closed_loop/find_targetvec.sc"
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<40;_scc_index_0++) (res2)[_scc_index_0] = (res2b)[_scc_index_0]; }
    Copy(res2b, (short int *)p_exc_i, 40);
    Copy(mem_err, (short int *)mem, 10);
}

#line 2126 "vocoderSched.cc"
Find_Targetvec_Copy2::Find_Targetvec_Copy2(unsigned int _idcnt, short int (&mem_err)[10], short int (&error)[40], short int (&mem_error)[50], short int *(&p_error))
    : _specc::behavior(_idcnt), mem_err(mem_err), error(error), mem_error(mem_error), p_error(p_error)
{   
}

Find_Targetvec_Copy2::~Find_Targetvec_Copy2(void)
{   
}

void Find_Targetvec_Copy2::main(void)

#line 37 "src/closed_loop/find_targetvec.sc"
{   

#line 2141 "vocoderSched.cc"
    __PRF_globalCounter[370u]++ ;
    _specc::waitfor(0ull);

#line 38 "src/closed_loop/find_targetvec.sc"
    p_error = mem_error + 10;
    Copy(mem_err, mem_error, 10);
    Copy(error, (short int *)p_error, 40);
}

#line 2151 "vocoderSched.cc"
Find_Targetvec::Find_Targetvec(unsigned int _idcnt, short int (&Aq)[11], short int (&Ap1)[11], short int (&Ap2)[11], short int *(&p_speech_i), short int (&res2)[40], short int *(&p_exc_i), short int (&xn)[40], short int (&mem_err)[10], short int (&mem_w0)[10], short int (&txdtx_ctrl), short int (&CN_excitation_gain))
    : _specc::behavior(_idcnt), Aq(Aq), Ap1(Ap1), Ap2(Ap2), p_speech_i(p_speech_i), res2(res2), p_exc_i(p_exc_i), xn(xn), mem_err(mem_err), mem_w0(mem_w0), txdtx_ctrl(txdtx_ctrl), CN_excitation_gain(CN_excitation_gain),
    _scc_const_port_0(false),
    _scc_const_port_1(false),
    compute_CN_excitation_gain(_IDcnt, res2b, CN_excitation_gain),
    copy1(_IDcnt, res2b, mem_err, res2, p_exc_i, mem),
    copy2(_IDcnt, mem_err, error, mem_error, p_error),
    residu_1(_IDcnt, Aq, p_speech_i, res2b),
    residu_2(_IDcnt, Ap1, p_error, xn2),
    syn_filt_1(_IDcnt, Aq, res2b, error, mem, _scc_const_port_0),
    syn_filt_2(_IDcnt, Ap2, xn2, xn, mem_w0, _scc_const_port_1)
{   
}

Find_Targetvec::~Find_Targetvec(void)
{   
}

void Find_Targetvec::main(void)

#line 80 "src/closed_loop/find_targetvec.sc"
{   

#line 2175 "vocoderSched.cc"
    __PRF_globalCounter[361u]++ ;
    { enum { _scc_state_0, _scc_state_residu_1, _scc_state_copy1, _scc_state_compute_CN_excitation_gain, _scc_state_syn_filt_1, _scc_state_copy2, _scc_state_residu_2, _scc_state_syn_filt_2 } _scc_next_state = _scc_state_residu_1; do switch(_scc_next_state) {

#line 83 "src/closed_loop/find_targetvec.sc"
	    case _scc_state_residu_1: { residu_1.main(); if (__PRF_globalCounter[362u]++  , 1u) { _scc_next_state = _scc_state_copy1; break; } }


	    case _scc_state_copy1: { copy1.main(); if (__PRF_globalCounter[363u]++  , (txdtx_ctrl & 1) == 0) { _scc_next_state = _scc_state_compute_CN_excitation_gain; break; }
		{ _scc_next_state = _scc_state_syn_filt_1; break; } }

#line 93 "src/closed_loop/find_targetvec.sc"
	    case _scc_state_compute_CN_excitation_gain: { compute_CN_excitation_gain.main(); if (__PRF_globalCounter[364u]++  , 1u) { _scc_next_state = _scc_state_0; break; } }


	    case _scc_state_syn_filt_1: { syn_filt_1.main(); if (__PRF_globalCounter[365u]++  , 1u) { _scc_next_state = _scc_state_copy2; break; } }

	    case _scc_state_copy2: { copy2.main(); if (__PRF_globalCounter[366u]++  , 1u) { _scc_next_state = _scc_state_residu_2; break; } }

	    case _scc_state_residu_2: { residu_2.main(); if (__PRF_globalCounter[367u]++  , 1u) { _scc_next_state = _scc_state_syn_filt_2; break; } }
	    case _scc_state_syn_filt_2: { syn_filt_2.main(); if (__PRF_globalCounter[368u]++  , 1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

#line 2199 "vocoderSched.cc"
G_Pitch::G_Pitch(unsigned int _idcnt, short int (&xn)[40], short int (&y1)[40], short int (&gain))
    : _specc::behavior(_idcnt), xn(xn), y1(y1), gain(gain)
{   
}

G_Pitch::~G_Pitch(void)
{   
}

void G_Pitch::main(void)

#line 35 "src/closed_loop/g_pitch.sc"
{   
    int i;
    short int g;
    short int exp_xy; short int exp_yy; short int xy; short int yy;
    int s;

    short int scaled_y1[80];

#line 2220 "vocoderSched.cc"
    __PRF_globalCounter[375u]++ ;
    _specc::waitfor(6333333ull);
    __PRF_globalCounter[376u]++ ;

#line 45 "src/closed_loop/g_pitch.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 2229 "vocoderSched.cc"
	__PRF_globalCounter[377u]++ ;

#line 47 "src/closed_loop/g_pitch.sc"
	scaled_y1[i] = shr(y1[i], 2);
    }

#line 2236 "vocoderSched.cc"
    __PRF_globalCounter[378u]++ ;

#line 52 "src/closed_loop/g_pitch.sc"
    s = 0l;

#line 2242 "vocoderSched.cc"
    __PRF_globalCounter[379u]++ ;

#line 53 "src/closed_loop/g_pitch.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 2249 "vocoderSched.cc"
	__PRF_globalCounter[380u]++ ;

#line 55 "src/closed_loop/g_pitch.sc"
	s = L_mac(s, y1[i], y1[i]);
    }

#line 2256 "vocoderSched.cc"
    __PRF_globalCounter[381u]++ ;

#line 58 "src/closed_loop/g_pitch.sc"
    if (L_sub(s, (int)2147483647l) != 0l)
    {   

#line 2263 "vocoderSched.cc"
	__PRF_globalCounter[382u]++ ;

#line 60 "src/closed_loop/g_pitch.sc"
	s = L_add(s, 1l);
	exp_yy = norm_l(s);
	yy = round(L_shl(s, exp_yy));
    }
    else 
    {   

#line 2274 "vocoderSched.cc"
	__PRF_globalCounter[383u]++ ;

#line 66 "src/closed_loop/g_pitch.sc"
	s = 1l;

#line 2280 "vocoderSched.cc"
	__PRF_globalCounter[384u]++ ;

#line 67 "src/closed_loop/g_pitch.sc"
	for(i = 0; i < 40; i++ )
	{   

#line 2287 "vocoderSched.cc"
	    __PRF_globalCounter[385u]++ ;

#line 69 "src/closed_loop/g_pitch.sc"
	    s = L_mac(s, scaled_y1[i], scaled_y1[i]);
	}

#line 2294 "vocoderSched.cc"
	__PRF_globalCounter[386u]++ ;

#line 71 "src/closed_loop/g_pitch.sc"
	exp_yy = norm_l(s);
	yy = round(L_shl(s, exp_yy));
	exp_yy = sub(exp_yy, 4);
    }

#line 2303 "vocoderSched.cc"
    __PRF_globalCounter[387u]++ ;

#line 79 "src/closed_loop/g_pitch.sc"
    clear_overflow();

    s = 1l;

#line 2311 "vocoderSched.cc"
    __PRF_globalCounter[388u]++ ;

#line 82 "src/closed_loop/g_pitch.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 2318 "vocoderSched.cc"
	__PRF_globalCounter[389u]++ ;

#line 85 "src/closed_loop/g_pitch.sc"
	clear_carry();

	s = L_macNs(s, xn[i], y1[i]);

#line 2326 "vocoderSched.cc"
	__PRF_globalCounter[390u]++ ;

#line 91 "src/closed_loop/g_pitch.sc"
	if (overflow())
	{   

#line 2333 "vocoderSched.cc"
	    __PRF_globalCounter[391u]++ ;

#line 93 "src/closed_loop/g_pitch.sc"
	    break;
	}
    }

#line 2341 "vocoderSched.cc"
    __PRF_globalCounter[392u]++ ;

#line 99 "src/closed_loop/g_pitch.sc"
    if ( !overflow())
    {   

#line 2348 "vocoderSched.cc"
	__PRF_globalCounter[393u]++ ;

#line 101 "src/closed_loop/g_pitch.sc"
	exp_xy = norm_l(s);
	xy = round(L_shl(s, exp_xy));
    }
    else 
    {   

#line 2358 "vocoderSched.cc"
	__PRF_globalCounter[394u]++ ;

#line 106 "src/closed_loop/g_pitch.sc"
	s = 1l;

#line 2364 "vocoderSched.cc"
	__PRF_globalCounter[395u]++ ;

#line 107 "src/closed_loop/g_pitch.sc"
	for(i = 0; i < 40; i++ )
	{   

#line 2371 "vocoderSched.cc"
	    __PRF_globalCounter[396u]++ ;

#line 109 "src/closed_loop/g_pitch.sc"
	    s = L_mac(s, xn[i], scaled_y1[i]);
	}

#line 2378 "vocoderSched.cc"
	__PRF_globalCounter[397u]++ ;

#line 111 "src/closed_loop/g_pitch.sc"
	exp_xy = norm_l(s);
	xy = round(L_shl(s, exp_xy));
	exp_xy = sub(exp_xy, 2);
    }

#line 2387 "vocoderSched.cc"
    __PRF_globalCounter[398u]++ ;

#line 118 "src/closed_loop/g_pitch.sc"
    i = sub(xy, 4);

#line 2393 "vocoderSched.cc"
    __PRF_globalCounter[399u]++ ;

#line 121 "src/closed_loop/g_pitch.sc"
    if (i < 0)
    {   

#line 2400 "vocoderSched.cc"
	__PRF_globalCounter[400u]++ ;

#line 124 "src/closed_loop/g_pitch.sc"
	gain = 0;
	return ;
    }

#line 2408 "vocoderSched.cc"
    __PRF_globalCounter[401u]++ ;

#line 130 "src/closed_loop/g_pitch.sc"
    xy = shr(xy, 1);
    g = div_s(xy, yy);

    i = add(exp_xy, 3 - 1);
    i = sub(i, exp_yy);

    g = shr(g, i);

#line 2420 "vocoderSched.cc"
    __PRF_globalCounter[402u]++ ;

#line 141 "src/closed_loop/g_pitch.sc"
    if (sub(g, 4915) > 0)
    {   

#line 2427 "vocoderSched.cc"
	__PRF_globalCounter[403u]++ ;

#line 143 "src/closed_loop/g_pitch.sc"
	g = 4915;
    }

#line 2434 "vocoderSched.cc"
    __PRF_globalCounter[404u]++ ;

#line 146 "src/closed_loop/g_pitch.sc"
    gain = g;
}

#line 2441 "vocoderSched.cc"
Imp_Resp_Init::Imp_Resp_Init(unsigned int _idcnt, bool (&reset_flag), short int (&zero)[10], short int (&ai_zero)[40])
    : _specc::behavior(_idcnt), reset_flag(reset_flag), zero(zero), ai_zero(ai_zero)
{   
}

Imp_Resp_Init::~Imp_Resp_Init(void)
{   
}

void Imp_Resp_Init::init(void)

#line 20 "src/closed_loop/imp_resp.sc"
{   

#line 2456 "vocoderSched.cc"
    __PRF_globalCounter[449u]++ ;

#line 21 "src/closed_loop/imp_resp.sc"
    Set_zero((short int *)zero, 10);
    Set_zero((short int *)ai_zero, 40);
}

#line 2464 "vocoderSched.cc"
void Imp_Resp_Init::main(void)

#line 31 "src/closed_loop/imp_resp.sc"
{   

#line 2470 "vocoderSched.cc"
    __PRF_globalCounter[450u]++ ;
    _specc::waitfor(166666ull);
    __PRF_globalCounter[451u]++ ;

#line 32 "src/closed_loop/imp_resp.sc"
    if (reset_flag)

#line 2478 "vocoderSched.cc"
    {   
	__PRF_globalCounter[452u]++ ;

#line 32 "src/closed_loop/imp_resp.sc"
	init();
    }
}

#line 2487 "vocoderSched.cc"
void Imp_Resp_Init::reset(void)

#line 26 "src/closed_loop/imp_resp.sc"
{   

#line 2493 "vocoderSched.cc"
    __PRF_globalCounter[453u]++ ;

#line 27 "src/closed_loop/imp_resp.sc"
    init();
}

#line 2500 "vocoderSched.cc"
Imp_Resp_Seq1::Imp_Resp_Seq1(unsigned int _idcnt, short int (&Ap1)[11], short int (&ai_zero)[40])
    : _specc::behavior(_idcnt), Ap1(Ap1), ai_zero(ai_zero)
{   
}

Imp_Resp_Seq1::~Imp_Resp_Seq1(void)
{   
}

void Imp_Resp_Seq1::main(void)

#line 43 "src/closed_loop/imp_resp.sc"
{   

#line 2515 "vocoderSched.cc"
    __PRF_globalCounter[454u]++ ;
    _specc::waitfor(16666ull);

#line 44 "src/closed_loop/imp_resp.sc"
    Copy(Ap1, (short int *)ai_zero, (10 + 1));
}

#line 2523 "vocoderSched.cc"
Imp_Resp::Imp_Resp(unsigned int _idcnt, short int (&Aq)[11], short int (&Ap1)[11], short int (&Ap2)[11], short int (&h1)[40], short int (&txdtx_ctrl), bool (&reset_flag))
    : _specc::behavior(_idcnt), Aq(Aq), Ap1(Ap1), Ap2(Ap2), h1(h1), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag),
    _scc_const_port_0(false),
    _scc_const_port_1(false),
    init(_IDcnt, reset_flag, zero, ai_zero),
    seq1(_IDcnt, Ap1, ai_zero),
    syn_filt_1(_IDcnt, Aq, ai_zero, h1b, zero, _scc_const_port_0),
    syn_filt_2(_IDcnt, Ap2, h1b, h1, zero, _scc_const_port_1)
{   
}

Imp_Resp::~Imp_Resp(void)
{   
}

void Imp_Resp::main(void)

#line 80 "src/closed_loop/imp_resp.sc"
{   

#line 2544 "vocoderSched.cc"
    __PRF_globalCounter[443u]++ ;
    { enum { _scc_state_0, _scc_state_init, _scc_state_seq1, _scc_state_syn_filt_1, _scc_state_syn_filt_2 } _scc_next_state = _scc_state_init; do switch(_scc_next_state) {

#line 84 "src/closed_loop/imp_resp.sc"
	    case _scc_state_init: { init.main(); if (__PRF_globalCounter[444u]++  , (txdtx_ctrl & 1) != 0) { _scc_next_state = _scc_state_seq1; break; }
		{ _scc_next_state = _scc_state_0; break; } }


	    case _scc_state_seq1: { seq1.main(); if (__PRF_globalCounter[445u]++  , 1u) { _scc_next_state = _scc_state_syn_filt_1; break; } }

	    case _scc_state_syn_filt_1: { syn_filt_1.main(); if (__PRF_globalCounter[446u]++  , 1u) { _scc_next_state = _scc_state_syn_filt_2; break; } }
	    case _scc_state_syn_filt_2: { syn_filt_2.main(); if (__PRF_globalCounter[447u]++  , 1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

#line 2560 "vocoderSched.cc"
void Imp_Resp::reset(void)

#line 74 "src/closed_loop/imp_resp.sc"
{   

#line 2566 "vocoderSched.cc"
    __PRF_globalCounter[448u]++ ;
    init.reset();
}

Par_Weight::Par_Weight(unsigned int _idcnt, short int (&A)[11], short int (&Ap1)[11], short int (&Ap2)[11])
    : _specc::behavior(_idcnt), A(A), Ap1(Ap1), Ap2(Ap2),
    weight_1(_IDcnt, A, F_gamma1, Ap1),
    weight_2(_IDcnt, A, F_gamma2, Ap2)
{   
}

Par_Weight::~Par_Weight(void)
{   
}

void Par_Weight::main(void)
{   
    __PRF_globalCounter[612u]++ ;
    weight_1.main();
    weight_2.main();
}

Pitch_Fr6::Pitch_Fr6(unsigned int _idcnt, short int *(&exc), short int (&xn)[40], short int *(&h1), short int (&t0_min), short int (&t0_max), int (&i_subfr), short int (&pit_frac), short int (&lag))
    : _specc::behavior(_idcnt), exc(exc), xn(xn), h1(h1), t0_min(t0_min), t0_max(t0_max), i_subfr(i_subfr), pit_frac(pit_frac), lag(lag)
{   
}

Pitch_Fr6::~Pitch_Fr6(void)
{   
}

short int Pitch_Fr6::Interpol_6(

#line 175 "src/closed_loop/pitch_fr6.sc"
    short int *x, 
    short int frac)

{   
    int i; int k;
    short int *x1; short int *x2;
    const short int *c1; const short int *c2;
    int s;




    const short int inter_6[25] = { 

      ((short int)29519),
      ((short int)28316),((short int)24906),((short int)19838),((short int)13896),((short int)7945),((short int)2755),
      ((short int)-1127),((short int)-3459),((short int)-4304),((short int)-3969),((short int)-2899),((short int)-1561),
      ((short int)-336),((short int)534),((short int)970),((short int)1023),((short int)823),((short int)516),
      ((short int)220),((short int)0),((short int)-131),((short int)-194),((short int)-215),((short int)0) };

#line 2621 "vocoderSched.cc"
    __PRF_globalCounter[636u]++ ;

#line 198 "src/closed_loop/pitch_fr6.sc"
    if (frac < 0)
    {   

#line 2628 "vocoderSched.cc"
	__PRF_globalCounter[637u]++ ;

#line 200 "src/closed_loop/pitch_fr6.sc"
	frac = add(frac, 6);
	x-- ;
    }

#line 2636 "vocoderSched.cc"
    __PRF_globalCounter[638u]++ ;

#line 203 "src/closed_loop/pitch_fr6.sc"
    x1 =  &x[0];
    x2 =  &x[1];
    c1 =  &inter_6[frac];
    c2 =  &inter_6[sub(6, frac)];

    s = 0;

#line 2647 "vocoderSched.cc"
    __PRF_globalCounter[639u]++ ;

#line 209 "src/closed_loop/pitch_fr6.sc"
    for(i = 0 , k = 0; i < 4; i++  , k += 6)
    {   

#line 2654 "vocoderSched.cc"
	__PRF_globalCounter[640u]++ ;

#line 211 "src/closed_loop/pitch_fr6.sc"
	s = L_mac(s, x1[ -i], c1[k]);
	s = L_mac(s, x2[i], c2[k]);
    }

#line 2662 "vocoderSched.cc"
    __PRF_globalCounter[641u]++ ;

#line 215 "src/closed_loop/pitch_fr6.sc"
    return round(s);
}

#line 2669 "vocoderSched.cc"
void Pitch_Fr6::Norm_Corr(

#line 62 "src/closed_loop/pitch_fr6.sc"
    short int NC_exc[], short int NC_xn[], short int NC_h[], 
    short int t_min, short int t_max, short int corr_norm[])
{   
    int i; int j; int k;
    short int corr__h; short int corr__l; short int norm__h; short int norm__l;
    int s;


    short int excf[80];
    short int h_fac; short int *s_excf; short int scaled_excf[80]; short int scaling;

#line 2684 "vocoderSched.cc"
    __PRF_globalCounter[642u]++ ;

#line 75 "src/closed_loop/pitch_fr6.sc"
    k =  -t_min;



    Convolut( &NC_exc[k], NC_h, excf, 40);

#line 2694 "vocoderSched.cc"
    __PRF_globalCounter[643u]++ ;

#line 83 "src/closed_loop/pitch_fr6.sc"
    for(j = 0; j < 40; j++ )
    {   

#line 2701 "vocoderSched.cc"
	__PRF_globalCounter[644u]++ ;

#line 85 "src/closed_loop/pitch_fr6.sc"
	scaled_excf[j] = shr(excf[j], 2);
    }

#line 2708 "vocoderSched.cc"
    __PRF_globalCounter[645u]++ ;

#line 90 "src/closed_loop/pitch_fr6.sc"
    s = 0;

#line 2714 "vocoderSched.cc"
    __PRF_globalCounter[646u]++ ;

#line 91 "src/closed_loop/pitch_fr6.sc"
    for(j = 0; j < 40; j++ )
    {   

#line 2721 "vocoderSched.cc"
	__PRF_globalCounter[647u]++ ;

#line 93 "src/closed_loop/pitch_fr6.sc"
	s = L_mac(s, excf[j], excf[j]);
    }

#line 2728 "vocoderSched.cc"
    __PRF_globalCounter[648u]++ ;

#line 96 "src/closed_loop/pitch_fr6.sc"
    if (L_sub(s, 67108864l) <= 0)
    {   

#line 2735 "vocoderSched.cc"
	__PRF_globalCounter[649u]++ ;

#line 98 "src/closed_loop/pitch_fr6.sc"
	s_excf = excf;
	h_fac = 15 - 12;
	scaling = 0;
    }
    else 
    {   

#line 2746 "vocoderSched.cc"
	__PRF_globalCounter[650u]++ ;

#line 105 "src/closed_loop/pitch_fr6.sc"
	s_excf = scaled_excf;
	h_fac = 15 - 12 - 2;
	scaling = 2;
    }

#line 2755 "vocoderSched.cc"
    __PRF_globalCounter[651u]++ ;

#line 112 "src/closed_loop/pitch_fr6.sc"
    for(i = t_min; i <= t_max; i++ )
    {   

#line 2762 "vocoderSched.cc"
	__PRF_globalCounter[652u]++ ;

#line 116 "src/closed_loop/pitch_fr6.sc"
	s = 0;

#line 2768 "vocoderSched.cc"
	__PRF_globalCounter[653u]++ ;

#line 117 "src/closed_loop/pitch_fr6.sc"
	for(j = 0; j < 40; j++ )
	{   

#line 2775 "vocoderSched.cc"
	    __PRF_globalCounter[654u]++ ;

#line 119 "src/closed_loop/pitch_fr6.sc"
	    s = L_mac(s, s_excf[j], s_excf[j]);
	}

#line 2782 "vocoderSched.cc"
	__PRF_globalCounter[655u]++ ;

#line 122 "src/closed_loop/pitch_fr6.sc"
	s = Inv_sqrt(s);
	L_Extract(s,  &norm__h,  &norm__l);



	s = 0;

#line 2793 "vocoderSched.cc"
	__PRF_globalCounter[656u]++ ;

#line 128 "src/closed_loop/pitch_fr6.sc"
	for(j = 0; j < 40; j++ )
	{   

#line 2800 "vocoderSched.cc"
	    __PRF_globalCounter[657u]++ ;

#line 130 "src/closed_loop/pitch_fr6.sc"
	    s = L_mac(s, NC_xn[j], s_excf[j]);
	}

#line 2807 "vocoderSched.cc"
	__PRF_globalCounter[658u]++ ;

#line 132 "src/closed_loop/pitch_fr6.sc"
	L_Extract(s,  &corr__h,  &corr__l);



	s = Mpy_32(corr__h, corr__l, norm__h, norm__l);

	corr_norm[i] = extract_h(L_shl(s, 16));

#line 2819 "vocoderSched.cc"
	__PRF_globalCounter[659u]++ ;

#line 144 "src/closed_loop/pitch_fr6.sc"
	if (i != t_max)
	{   

#line 2826 "vocoderSched.cc"
	    __PRF_globalCounter[660u]++ ;

#line 146 "src/closed_loop/pitch_fr6.sc"
	    k-- ;

#line 2832 "vocoderSched.cc"
	    __PRF_globalCounter[661u]++ ;

#line 147 "src/closed_loop/pitch_fr6.sc"
	    for(j = 40 - 1; j > 0; j-- )
	    {   

#line 2839 "vocoderSched.cc"
		__PRF_globalCounter[662u]++ ;

#line 149 "src/closed_loop/pitch_fr6.sc"
		s = L_mult(NC_exc[k], NC_h[j]);
		s = L_shl(s, h_fac);
		s_excf[j] = add(extract_h(s), s_excf[j - 1]);
	    }

#line 2848 "vocoderSched.cc"
	    __PRF_globalCounter[663u]++ ;

#line 153 "src/closed_loop/pitch_fr6.sc"
	    s_excf[0] = shr(NC_exc[k], scaling);
	}
    }

#line 2856 "vocoderSched.cc"
    __PRF_globalCounter[664u]++ ;

#line 156 "src/closed_loop/pitch_fr6.sc"
    return ;
}

#line 2863 "vocoderSched.cc"
void Pitch_Fr6::main(void)

#line 224 "src/closed_loop/pitch_fr6.sc"
{   
    int i;
    short int l;
    short int t_max; short int t_min;
    short int frac; short int max;
    short int *corr;
    short int corr_int;
    short int corr_v[40];

#line 2876 "vocoderSched.cc"
    __PRF_globalCounter[665u]++ ;
    _specc::waitfor(139366666ull);

#line 235 "src/closed_loop/pitch_fr6.sc"
    t_min = sub(t0_min, 4);
    t_max = add(t0_max, 4);

    corr =  &corr_v[ -t_min];




    Norm_Corr(exc,  &xn[0], h1, t_min, t_max, corr);



    max = corr[t0_min];
    l = t0_min;

#line 2896 "vocoderSched.cc"
    __PRF_globalCounter[666u]++ ;

#line 250 "src/closed_loop/pitch_fr6.sc"
    for(i = t0_min + 1; i <= t0_max; i++ )
    {   

#line 2903 "vocoderSched.cc"
	__PRF_globalCounter[667u]++ ;

#line 253 "src/closed_loop/pitch_fr6.sc"
	if (sub(corr[i], max) >= 0)
	{   

#line 2910 "vocoderSched.cc"
	    __PRF_globalCounter[668u]++ ;

#line 255 "src/closed_loop/pitch_fr6.sc"
	    max = corr[i];
	    l = i;
	}
    }

#line 2919 "vocoderSched.cc"
    __PRF_globalCounter[669u]++ ;

#line 263 "src/closed_loop/pitch_fr6.sc"
    if ((i_subfr == 0) && (sub(l, 94) > 0))
    {   

#line 2926 "vocoderSched.cc"
	__PRF_globalCounter[670u]++ ;

#line 265 "src/closed_loop/pitch_fr6.sc"
	pit_frac = 0;

	lag = l;
	return ;
    }

#line 2936 "vocoderSched.cc"
    __PRF_globalCounter[671u]++ ;

#line 273 "src/closed_loop/pitch_fr6.sc"
    max = Interpol_6( &corr[l],  -3);
    frac =  -3;

#line 2943 "vocoderSched.cc"
    __PRF_globalCounter[672u]++ ;

#line 276 "src/closed_loop/pitch_fr6.sc"
    for(i =  -2; i <= 3; i++ )
    {   

#line 2950 "vocoderSched.cc"
	__PRF_globalCounter[673u]++ ;

#line 278 "src/closed_loop/pitch_fr6.sc"
	corr_int = Interpol_6( &corr[l], i);

#line 2956 "vocoderSched.cc"
	__PRF_globalCounter[674u]++ ;

#line 280 "src/closed_loop/pitch_fr6.sc"
	if (sub(corr_int, max) > 0)
	{   

#line 2963 "vocoderSched.cc"
	    __PRF_globalCounter[675u]++ ;

#line 282 "src/closed_loop/pitch_fr6.sc"
	    max = corr_int;
	    frac = i;
	}
    }

#line 2972 "vocoderSched.cc"
    __PRF_globalCounter[676u]++ ;

#line 290 "src/closed_loop/pitch_fr6.sc"
    if (sub(frac,  -3) == 0)
    {   

#line 2979 "vocoderSched.cc"
	__PRF_globalCounter[677u]++ ;

#line 292 "src/closed_loop/pitch_fr6.sc"
	frac = 3;
	l = sub(l, 1);
    }

#line 2987 "vocoderSched.cc"
    __PRF_globalCounter[678u]++ ;

#line 295 "src/closed_loop/pitch_fr6.sc"
    pit_frac = frac;


    lag = l;
}

#line 2997 "vocoderSched.cc"
Pred_Lt_6::Pred_Lt_6(unsigned int _idcnt, short int *(&exc), short int (&T0), short int (&frac))
    : _specc::behavior(_idcnt), exc(exc), T0(T0), frac(frac)
{   
}

Pred_Lt_6::~Pred_Lt_6(void)
{   
}

void Pred_Lt_6::main(void)

#line 38 "src/closed_loop/pred_lt_6.sc"
{   
    int i; int j; int k;
    short int f;
    short int *x0; short int *x1; short int *x2;
    const short int *c1; const short int *c2;
    int s;


    const short int inter_6[61] = { 
      ((short int)29443),
      ((short int)28346),((short int)25207),((short int)20449),((short int)14701),((short int)8693),((short int)3143),
      ((short int)-1352),((short int)-4402),((short int)-5865),((short int)-5850),((short int)-4673),((short int)-2783),
      ((short int)-672),((short int)1211),((short int)2536),((short int)3130),((short int)2991),((short int)2259),
      ((short int)1170),((short int)0),((short int)-1001),((short int)-1652),((short int)-1868),((short int)-1666),
      ((short int)-1147),((short int)-464),((short int)218),((short int)756),((short int)1060),((short int)1099),
      ((short int)904),((short int)550),((short int)135),((short int)-245),((short int)-514),((short int)-634),
      ((short int)-602),((short int)-451),((short int)-231),((short int)0),((short int)191),((short int)308),
      ((short int)340),((short int)296),((short int)198),((short int)78),((short int)-36),((short int)-120),
      ((short int)-163),((short int)-165),((short int)-132),((short int)-79),((short int)-19),((short int)34),
      ((short int)73),((short int)91),((short int)89),((short int)70),((short int)38),((short int)0) };

#line 3031 "vocoderSched.cc"
    __PRF_globalCounter[750u]++ ;
    _specc::waitfor(44733333ull);

#line 61 "src/closed_loop/pred_lt_6.sc"
    x0 =  &exc[ -T0];

    f = negate(frac);

#line 3040 "vocoderSched.cc"
    __PRF_globalCounter[751u]++ ;

#line 65 "src/closed_loop/pred_lt_6.sc"
    if (f < 0)
    {   

#line 3047 "vocoderSched.cc"
	__PRF_globalCounter[752u]++ ;

#line 67 "src/closed_loop/pred_lt_6.sc"
	f = add(f, 6);
	x0-- ;
    }

#line 3055 "vocoderSched.cc"
    __PRF_globalCounter[753u]++ ;

#line 70 "src/closed_loop/pred_lt_6.sc"
    for(j = 0; j < 40; j++ )
    {   

#line 3062 "vocoderSched.cc"
	__PRF_globalCounter[754u]++ ;

#line 72 "src/closed_loop/pred_lt_6.sc"
	x1 = x0++ ;
	x2 = x0;
	c1 =  &inter_6[f];
	c2 =  &inter_6[sub(6, f)];

	s = 0;

#line 3073 "vocoderSched.cc"
	__PRF_globalCounter[755u]++ ;

#line 78 "src/closed_loop/pred_lt_6.sc"
	for(i = 0 , k = 0; i < 10; i++  , k += 6)
	{   

#line 3080 "vocoderSched.cc"
	    __PRF_globalCounter[756u]++ ;

#line 80 "src/closed_loop/pred_lt_6.sc"
	    s = L_mac(s, x1[ -i], c1[k]);
	    s = L_mac(s, x2[i], c2[k]);
	}

#line 3088 "vocoderSched.cc"
	__PRF_globalCounter[757u]++ ;

#line 84 "src/closed_loop/pred_lt_6.sc"
	exc[j] = round(s);
    }
}

#line 3096 "vocoderSched.cc"
Q_Gain_Pitch::Q_Gain_Pitch(unsigned int _idcnt, short int (&gain), short int (&index))
    : _specc::behavior(_idcnt), gain(gain), index(index)
{   
}

Q_Gain_Pitch::~Q_Gain_Pitch(void)
{   
}

void Q_Gain_Pitch::main(void)

#line 24 "src/closed_loop/q_gain_pitch.sc"
{   
    int i;
    short int err; short int err_min; short int gain_q14;




    const short int qua_gain_pitch[16] = { 

      ((short int)0),((short int)3277),((short int)6556),((short int)8192),((short int)9830),((short int)11469),((short int)12288),((short int)13107),
      ((short int)13926),((short int)14746),((short int)15565),((short int)16384),((short int)17203),((short int)18022),((short int)18842),((short int)19661) };

#line 3121 "vocoderSched.cc"
    __PRF_globalCounter[831u]++ ;
    _specc::waitfor(1100000ull);

#line 38 "src/closed_loop/q_gain_pitch.sc"
    gain_q14 = shl(gain, 2);

    err_min = abs_s(sub(gain_q14, qua_gain_pitch[0]));
    index = 0;

#line 3131 "vocoderSched.cc"
    __PRF_globalCounter[832u]++ ;

#line 43 "src/closed_loop/q_gain_pitch.sc"
    for(i = 1; i < 16; i++ )
    {   

#line 3138 "vocoderSched.cc"
	__PRF_globalCounter[833u]++ ;

#line 45 "src/closed_loop/q_gain_pitch.sc"
	err = abs_s(sub(gain_q14, qua_gain_pitch[i]));

#line 3144 "vocoderSched.cc"
	__PRF_globalCounter[834u]++ ;

#line 48 "src/closed_loop/q_gain_pitch.sc"
	if (sub(err, err_min) < 0)
	{   

#line 3151 "vocoderSched.cc"
	    __PRF_globalCounter[835u]++ ;

#line 50 "src/closed_loop/q_gain_pitch.sc"
	    err_min = err;
	    index = i;
	}
    }

#line 3160 "vocoderSched.cc"
    __PRF_globalCounter[836u]++ ;

#line 55 "src/closed_loop/q_gain_pitch.sc"
    gain = shr(qua_gain_pitch[index], 2);
}

#line 3167 "vocoderSched.cc"
Build_Code::Build_Code(unsigned int _idcnt, short int (&codvec)[10], short int (&sign)[40], short int (&cod)[40], short int (&h)[40], short int (&y)[40], short int (&indx)[10])
    : _specc::behavior(_idcnt), codvec(codvec), sign(sign), cod(cod), h(h), y(y), indx(indx)
{   
}

Build_Code::~Build_Code(void)
{   
}

void Build_Code::main(void)

#line 29 "src/codebook/build_code.sc"
{   
    int i; int k;
    short int _sign[10]; short int code[40]; short int index; short int indices[10]; short int j; short int track;
    int p0; int p1; int p2; int p3; int p4; int p5; int p6; int p7; int p8; int p9;
    int s;

#line 3186 "vocoderSched.cc"
    __PRF_globalCounter[143u]++ ;
    _specc::waitfor(1560000ull);
    __PRF_globalCounter[144u]++ ;

#line 35 "src/codebook/build_code.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 3195 "vocoderSched.cc"
	__PRF_globalCounter[145u]++ ;

#line 37 "src/codebook/build_code.sc"
	code[i] = 0;
    }

#line 3202 "vocoderSched.cc"
    __PRF_globalCounter[146u]++ ;

#line 39 "src/codebook/build_code.sc"
    for(i = 0; i < 5; i++ )
    {   

#line 3209 "vocoderSched.cc"
	__PRF_globalCounter[147u]++ ;

#line 41 "src/codebook/build_code.sc"
	indices[i] =  -1;
    }

#line 3216 "vocoderSched.cc"
    __PRF_globalCounter[148u]++ ;

#line 44 "src/codebook/build_code.sc"
    for(k = 0; k < 10; k++ )
    {   

#line 3223 "vocoderSched.cc"
	__PRF_globalCounter[149u]++ ;

#line 47 "src/codebook/build_code.sc"
	i = codvec[k];

	j = sign[i];

	index = mult(i, 6554);

	track = sub(i, extract_l(L_shr(L_mult(index, 5), 1)));

#line 3235 "vocoderSched.cc"
	__PRF_globalCounter[150u]++ ;

#line 55 "src/codebook/build_code.sc"
	if (j > 0)
	{   

#line 3242 "vocoderSched.cc"
	    __PRF_globalCounter[151u]++ ;

#line 57 "src/codebook/build_code.sc"
	    code[i] = add(code[i], 4096);
	    _sign[k] = 8192;
	}
	else 

	{   

#line 3253 "vocoderSched.cc"
	    __PRF_globalCounter[152u]++ ;

#line 63 "src/codebook/build_code.sc"
	    code[i] = sub(code[i], 4096);
	    _sign[k] =  -8192;
	    index = add(index, 8);
	}

#line 3262 "vocoderSched.cc"
	__PRF_globalCounter[153u]++ ;

#line 69 "src/codebook/build_code.sc"
	if (indices[track] < 0)
	{   

#line 3269 "vocoderSched.cc"
	    __PRF_globalCounter[154u]++ ;

#line 71 "src/codebook/build_code.sc"
	    indices[track] = index;
	}
	else 
	{   

#line 3278 "vocoderSched.cc"
	    __PRF_globalCounter[155u]++ ;

#line 76 "src/codebook/build_code.sc"
	    if (((index ^ indices[track]) & 8) == 0)
	    {   

#line 3285 "vocoderSched.cc"
		__PRF_globalCounter[156u]++ ;

#line 81 "src/codebook/build_code.sc"
		if (sub(indices[track], index) <= 0)
		{   

#line 3292 "vocoderSched.cc"
		    __PRF_globalCounter[157u]++ ;

#line 83 "src/codebook/build_code.sc"
		    indices[track + 5] = index;
		}
		else 
		{   

#line 3301 "vocoderSched.cc"
		    __PRF_globalCounter[158u]++ ;

#line 87 "src/codebook/build_code.sc"
		    indices[track + 5] = indices[track];

		    indices[track] = index;
		}
	    }
	    else 
	    {   

#line 3313 "vocoderSched.cc"
		__PRF_globalCounter[159u]++ ;

#line 97 "src/codebook/build_code.sc"
		if (sub((indices[track] & 7), (index & 7)) <= 0)
		{   

#line 3320 "vocoderSched.cc"
		    __PRF_globalCounter[160u]++ ;

#line 99 "src/codebook/build_code.sc"
		    indices[track + 5] = indices[track];

		    indices[track] = index;
		}
		else 
		{   

#line 3331 "vocoderSched.cc"
		    __PRF_globalCounter[161u]++ ;

#line 105 "src/codebook/build_code.sc"
		    indices[track + 5] = index;
		}
	    }
	}
    }

#line 3341 "vocoderSched.cc"
    __PRF_globalCounter[162u]++ ;

#line 111 "src/codebook/build_code.sc"
    p0 =  -codvec[0];
    p1 =  -codvec[1];
    p2 =  -codvec[2];
    p3 =  -codvec[3];
    p4 =  -codvec[4];
    p5 =  -codvec[5];
    p6 =  -codvec[6];
    p7 =  -codvec[7];
    p8 =  -codvec[8];
    p9 =  -codvec[9];

#line 3356 "vocoderSched.cc"
    __PRF_globalCounter[163u]++ ;

#line 122 "src/codebook/build_code.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 3363 "vocoderSched.cc"
	__PRF_globalCounter[164u]++ ;

#line 124 "src/codebook/build_code.sc"
	s = 0;

#line 3369 "vocoderSched.cc"
	__PRF_globalCounter[165u]++ ;

#line 125 "src/codebook/build_code.sc"
	if (p0 >= 0)

#line 3375 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[166u]++ ;

#line 125 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p0], _sign[0]);
	}

#line 3383 "vocoderSched.cc"
	__PRF_globalCounter[167u]++ ;

#line 126 "src/codebook/build_code.sc"
	if (p1 >= 0)

#line 3389 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[168u]++ ;

#line 126 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p1], _sign[1]);
	}

#line 3397 "vocoderSched.cc"
	__PRF_globalCounter[169u]++ ;

#line 127 "src/codebook/build_code.sc"
	if (p2 >= 0)

#line 3403 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[170u]++ ;

#line 127 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p2], _sign[2]);
	}

#line 3411 "vocoderSched.cc"
	__PRF_globalCounter[171u]++ ;

#line 128 "src/codebook/build_code.sc"
	if (p3 >= 0)

#line 3417 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[172u]++ ;

#line 128 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p3], _sign[3]);
	}

#line 3425 "vocoderSched.cc"
	__PRF_globalCounter[173u]++ ;

#line 129 "src/codebook/build_code.sc"
	if (p4 >= 0)

#line 3431 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[174u]++ ;

#line 129 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p4], _sign[4]);
	}

#line 3439 "vocoderSched.cc"
	__PRF_globalCounter[175u]++ ;

#line 130 "src/codebook/build_code.sc"
	if (p5 >= 0)

#line 3445 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[176u]++ ;

#line 130 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p5], _sign[5]);
	}

#line 3453 "vocoderSched.cc"
	__PRF_globalCounter[177u]++ ;

#line 131 "src/codebook/build_code.sc"
	if (p6 >= 0)

#line 3459 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[178u]++ ;

#line 131 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p6], _sign[6]);
	}

#line 3467 "vocoderSched.cc"
	__PRF_globalCounter[179u]++ ;

#line 132 "src/codebook/build_code.sc"
	if (p7 >= 0)

#line 3473 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[180u]++ ;

#line 132 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p7], _sign[7]);
	}

#line 3481 "vocoderSched.cc"
	__PRF_globalCounter[181u]++ ;

#line 133 "src/codebook/build_code.sc"
	if (p8 >= 0)

#line 3487 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[182u]++ ;

#line 133 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p8], _sign[8]);
	}

#line 3495 "vocoderSched.cc"
	__PRF_globalCounter[183u]++ ;

#line 134 "src/codebook/build_code.sc"
	if (p9 >= 0)

#line 3501 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[184u]++ ;

#line 134 "src/codebook/build_code.sc"
	    s = L_mac(s, h[p9], _sign[9]);
	}

#line 3509 "vocoderSched.cc"
	__PRF_globalCounter[185u]++ ;

#line 135 "src/codebook/build_code.sc"
	p0++ ;
	p1++ ;
	p2++ ;
	p3++ ;
	p4++ ;
	p5++ ;
	p6++ ;
	p7++ ;
	p8++ ;
	p9++ ;
	y[i] = round(s);
    }

#line 3526 "vocoderSched.cc"
    __PRF_globalCounter[186u]++ ;

#line 149 "src/codebook/build_code.sc"
    for(i = 0; i < 10; i++ )

#line 3532 "vocoderSched.cc"
    {   
	__PRF_globalCounter[187u]++ ;

#line 150 "src/codebook/build_code.sc"
	indx[i] = indices[i];
    }

#line 3540 "vocoderSched.cc"
    __PRF_globalCounter[188u]++ ;

#line 151 "src/codebook/build_code.sc"
    for(i = 0; i < 40; i++ )

#line 3546 "vocoderSched.cc"
    {   
	__PRF_globalCounter[189u]++ ;

#line 152 "src/codebook/build_code.sc"
	cod[i] = code[i];
    }
}

#line 3555 "vocoderSched.cc"
Code_10i40_35bits::Code_10i40_35bits(unsigned int _idcnt, short int (&x)[40], short int (&cn)[40], short int (&h)[40], short int (&cod)[40], short int (&y)[40], short int (&prm)[10])
    : _specc::behavior(_idcnt), x(x), cn(cn), h(h), cod(cod), y(y), prm(prm),
    build_code(_IDcnt, codevec, sign, cod, h, y, indx),
    cor_h(_IDcnt, h, sign, rr),
    cor_h_x(_IDcnt, h, x, dn),
    q_p(_IDcnt, indx, prm),
    search_10i40(_IDcnt, dn2, rr, ipos, pos_max, codevec),
    set_sign(_IDcnt, dn, dn2, cn, sign, pos_max, ipos)
{   
}

Code_10i40_35bits::~Code_10i40_35bits(void)
{   
}

void Code_10i40_35bits::main(void)

#line 46 "src/codebook/code_10i40_35bits.sc"
{   

#line 3576 "vocoderSched.cc"
    __PRF_globalCounter[232u]++ ;

#line 47 "src/codebook/code_10i40_35bits.sc"
    cor_h_x.main();
    set_sign.main();
    cor_h.main();
    search_10i40.main();
    build_code.main();
    q_p.main();
}

#line 3588 "vocoderSched.cc"
Codebook_Seq1::Codebook_Seq1(unsigned int _idcnt, short int (&xn)[40], short int (&y1)[40], short int (&gain_pit), short int (&exc)[40], short int (&h1)[40], short int (&T0), short int (&res2)[40], short int (&xn2)[40], short int (&h1b)[40], short int (&res2b)[40])
    : _specc::behavior(_idcnt), xn(xn), y1(y1), gain_pit(gain_pit), exc(exc), h1(h1), T0(T0), res2(res2), xn2(xn2), h1b(h1b), res2b(res2b),
    filter_h(_IDcnt, T0, gain_pit, h1, h1b),
    upd_res(_IDcnt, res2, exc, gain_pit, res2b),
    upd_target(_IDcnt, xn, y1, gain_pit, xn2)
{   
}

Codebook_Seq1::~Codebook_Seq1(void)
{   
}

void Codebook_Seq1::main(void)
{   
    __PRF_globalCounter[242u]++ ;
    upd_target.main();
    upd_res.main();
    filter_h.main();
}

Codebook_Seq2::Codebook_Seq2(unsigned int _idcnt, short int (&T0), short int (&gain_pit), short int (&codeb)[40], short int (&xn2)[40], short int (&y2)[40], short int (&code)[40], short int (&gain_code))
    : _specc::behavior(_idcnt), T0(T0), gain_pit(gain_pit), codeb(codeb), xn2(xn2), y2(y2), code(code), gain_code(gain_code),
    filter_c(_IDcnt, T0, gain_pit, codeb, code),
    g_code(_IDcnt, xn2, y2, gain_code)
{   
}

Codebook_Seq2::~Codebook_Seq2(void)
{   
}

void Codebook_Seq2::main(void)
{   
    __PRF_globalCounter[243u]++ ;
    filter_c.main();
    g_code.main();
}

Cor_h::Cor_h(unsigned int _idcnt, short int (&h)[40], short int (&sign)[40], short int (&rr)[1600])
    : _specc::behavior(_idcnt), h(h), sign(sign), rr(rr)
{   
}

Cor_h::~Cor_h(void)
{   
}

void Cor_h::main(void)

#line 30 "src/codebook/cor_h.sc"
{   
    int dec; int i; int k; int l;
    short int h2[40]; short int j; short int tmp;
    int s;

#line 3644 "vocoderSched.cc"
    __PRF_globalCounter[270u]++ ;
    _specc::waitfor(58600000ull);

#line 36 "src/codebook/cor_h.sc"
    s = 2;

#line 3651 "vocoderSched.cc"
    __PRF_globalCounter[271u]++ ;

#line 37 "src/codebook/cor_h.sc"
    for(i = 0; i < 40; i++ )

#line 3657 "vocoderSched.cc"
    {   
	__PRF_globalCounter[272u]++ ;

#line 38 "src/codebook/cor_h.sc"
	s = L_mac(s, h[i], h[i]);
    }

#line 3665 "vocoderSched.cc"
    __PRF_globalCounter[273u]++ ;

#line 40 "src/codebook/cor_h.sc"
    j = sub(extract_h(s), 32767);

#line 3671 "vocoderSched.cc"
    __PRF_globalCounter[274u]++ ;

#line 42 "src/codebook/cor_h.sc"
    if (j == 0)
    {   

#line 3678 "vocoderSched.cc"
	__PRF_globalCounter[275u]++ ;

#line 44 "src/codebook/cor_h.sc"
	for(i = 0; i < 40; i++ )
	{   

#line 3685 "vocoderSched.cc"
	    __PRF_globalCounter[276u]++ ;

#line 46 "src/codebook/cor_h.sc"
	    h2[i] = shr(h[i], 1);
	}
    }
    else 
    {   

#line 3695 "vocoderSched.cc"
	__PRF_globalCounter[277u]++ ;

#line 51 "src/codebook/cor_h.sc"
	s = L_shr(s, 1);
	k = extract_h(L_shl(Inv_sqrt(s), 7));
	k = mult(k, 32440);

#line 3703 "vocoderSched.cc"
	__PRF_globalCounter[278u]++ ;

#line 55 "src/codebook/cor_h.sc"
	for(i = 0; i < 40; i++ )
	{   

#line 3710 "vocoderSched.cc"
	    __PRF_globalCounter[279u]++ ;

#line 57 "src/codebook/cor_h.sc"
	    h2[i] = round(L_shl(L_mult(h[i], k), 9));
	}
    }

#line 3718 "vocoderSched.cc"
    __PRF_globalCounter[280u]++ ;

#line 64 "src/codebook/cor_h.sc"
    s = 0;
    i = 40 - 1;

#line 3725 "vocoderSched.cc"
    __PRF_globalCounter[281u]++ ;

#line 66 "src/codebook/cor_h.sc"
    for(k = 0; k < 40; k++  , i-- )
    {   

#line 3732 "vocoderSched.cc"
	__PRF_globalCounter[282u]++ ;

#line 68 "src/codebook/cor_h.sc"
	s = L_mac(s, h2[k], h2[k]);
	rr[i * 40 + i] = round(s);
    }

#line 3740 "vocoderSched.cc"
    __PRF_globalCounter[283u]++ ;

#line 72 "src/codebook/cor_h.sc"
    for(dec = 1; dec < 40; dec++ )
    {   

#line 3747 "vocoderSched.cc"
	__PRF_globalCounter[284u]++ ;

#line 74 "src/codebook/cor_h.sc"
	s = 0;
	l = 40 - 1;
	i = l - dec;

#line 3755 "vocoderSched.cc"
	__PRF_globalCounter[285u]++ ;

#line 77 "src/codebook/cor_h.sc"
	for(k = 0; k < (40 - dec); k++  , i--  , l-- )
	{   

#line 3762 "vocoderSched.cc"
	    __PRF_globalCounter[286u]++ ;

#line 79 "src/codebook/cor_h.sc"
	    s = L_mac(s, h2[k], h2[k + dec]);
	    tmp = mult(round(s), mult(sign[i], sign[l]));
	    rr[l * 40 + i] = tmp;
	    rr[i * 40 + l] = tmp;
	}
    }
}

#line 3774 "vocoderSched.cc"
Cor_h_x::Cor_h_x(unsigned int _idcnt, short int (&h)[40], short int (&x)[40], short int (&dn)[40])
    : _specc::behavior(_idcnt), h(h), x(x), dn(dn)
{   
}

Cor_h_x::~Cor_h_x(void)
{   
}

void Cor_h_x::main(void)

#line 30 "src/codebook/cor_h_x.sc"
{   
    int i; int k;
    short int j;
    int max; int s; int tot; int y32[40];

#line 3792 "vocoderSched.cc"
    __PRF_globalCounter[287u]++ ;
    _specc::waitfor(18120000ull);

#line 37 "src/codebook/cor_h_x.sc"
    tot = 5;

#line 3799 "vocoderSched.cc"
    __PRF_globalCounter[288u]++ ;

#line 39 "src/codebook/cor_h_x.sc"
    for(k = 0; k < 5; k++ )
    {   

#line 3806 "vocoderSched.cc"
	__PRF_globalCounter[289u]++ ;

#line 41 "src/codebook/cor_h_x.sc"
	max = 0;

#line 3812 "vocoderSched.cc"
	__PRF_globalCounter[290u]++ ;

#line 42 "src/codebook/cor_h_x.sc"
	for(i = k; i < 40; i += 5)
	{   

#line 3819 "vocoderSched.cc"
	    __PRF_globalCounter[291u]++ ;

#line 44 "src/codebook/cor_h_x.sc"
	    s = 0;

#line 3825 "vocoderSched.cc"
	    __PRF_globalCounter[292u]++ ;

#line 45 "src/codebook/cor_h_x.sc"
	    for(j = i; j < 40; j++ )

#line 3831 "vocoderSched.cc"
	    {   
		__PRF_globalCounter[293u]++ ;

#line 46 "src/codebook/cor_h_x.sc"
		s = L_mac(s, x[j], h[j - i]);
	    }

#line 3839 "vocoderSched.cc"
	    __PRF_globalCounter[294u]++ ;

#line 48 "src/codebook/cor_h_x.sc"
	    y32[i] = s;

	    s = L_abs(s);

#line 3847 "vocoderSched.cc"
	    __PRF_globalCounter[295u]++ ;

#line 52 "src/codebook/cor_h_x.sc"
	    if (L_sub(s, max) > (int)0l)

#line 3853 "vocoderSched.cc"
	    {   
		__PRF_globalCounter[296u]++ ;

#line 53 "src/codebook/cor_h_x.sc"
		max = s;
	    }
	}

#line 3862 "vocoderSched.cc"
	__PRF_globalCounter[297u]++ ;

#line 55 "src/codebook/cor_h_x.sc"
	tot = L_add(tot, L_shr(max, 1));
    }

#line 3869 "vocoderSched.cc"
    __PRF_globalCounter[298u]++ ;

#line 58 "src/codebook/cor_h_x.sc"
    j = sub(norm_l(tot), 2);

#line 3875 "vocoderSched.cc"
    __PRF_globalCounter[299u]++ ;

#line 60 "src/codebook/cor_h_x.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 3882 "vocoderSched.cc"
	__PRF_globalCounter[300u]++ ;

#line 62 "src/codebook/cor_h_x.sc"
	dn[i] = round(L_shl(y32[i], j));
    }
}

#line 3890 "vocoderSched.cc"
Gain_Code::Gain_Code(unsigned int _idcnt, short int (&xn2)[40], short int (&y2)[40], short int (&gain))
    : _specc::behavior(_idcnt), xn2(xn2), y2(y2), gain(gain)
{   
}

Gain_Code::~Gain_Code(void)
{   
}

void Gain_Code::main(void)

#line 23 "src/codebook/g_code.sc"
{   
    int i;
    short int g;
    short int j;
    short int exp_xy; short int exp_yy; short int xy; short int yy;
    short int scal_y2[40];
    int s;

#line 3911 "vocoderSched.cc"
    __PRF_globalCounter[405u]++ ;
    _specc::waitfor(1230000ull);
    __PRF_globalCounter[406u]++ ;

#line 33 "src/codebook/g_code.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 3920 "vocoderSched.cc"
	__PRF_globalCounter[407u]++ ;

#line 35 "src/codebook/g_code.sc"
	scal_y2[i] = shr(y2[i], 1);
    }

#line 3927 "vocoderSched.cc"
    __PRF_globalCounter[408u]++ ;

#line 40 "src/codebook/g_code.sc"
    s = 1l;

#line 3933 "vocoderSched.cc"
    __PRF_globalCounter[409u]++ ;

#line 41 "src/codebook/g_code.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 3940 "vocoderSched.cc"
	__PRF_globalCounter[410u]++ ;

#line 43 "src/codebook/g_code.sc"
	s = L_mac(s, xn2[i], scal_y2[i]);
    }

#line 3947 "vocoderSched.cc"
    __PRF_globalCounter[411u]++ ;

#line 45 "src/codebook/g_code.sc"
    exp_xy = norm_l(s);
    xy = extract_h(L_shl(s, exp_xy));

#line 3954 "vocoderSched.cc"
    __PRF_globalCounter[412u]++ ;

#line 50 "src/codebook/g_code.sc"
    if (xy <= 0) {

#line 3960 "vocoderSched.cc"
	__PRF_globalCounter[413u]++ ;

#line 51 "src/codebook/g_code.sc"
	gain = ((short int)0);
	return ;
    }

#line 3968 "vocoderSched.cc"
    __PRF_globalCounter[414u]++ ;

#line 57 "src/codebook/g_code.sc"
    s = 0l;

#line 3974 "vocoderSched.cc"
    __PRF_globalCounter[415u]++ ;

#line 58 "src/codebook/g_code.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 3981 "vocoderSched.cc"
	__PRF_globalCounter[416u]++ ;

#line 60 "src/codebook/g_code.sc"
	s = L_mac(s, scal_y2[i], scal_y2[i]);
    }

#line 3988 "vocoderSched.cc"
    __PRF_globalCounter[417u]++ ;

#line 62 "src/codebook/g_code.sc"
    exp_yy = norm_l(s);
    yy = extract_h(L_shl(s, exp_yy));



    xy = shr(xy, 1);
    g = div_s(xy, yy);


    j = add(exp_xy, 5);
    j = sub(j, exp_yy);

    gain = shr(g, j);
}

#line 4007 "vocoderSched.cc"
Pitch_Contr::Pitch_Contr(unsigned int _idcnt, short int (&x)[40], short int (&v)[40], short int (&gain_pit), short int (&y)[40])
    : _specc::behavior(_idcnt), x(x), v(v), gain_pit(gain_pit), y(y)
{   
}

Pitch_Contr::~Pitch_Contr(void)
{   
}

void Pitch_Contr::main(void)

#line 25 "src/codebook/pitch_contr.sc"
{   
    int i;
    int L_temp;
    short int temp_gain_pit;

#line 4025 "vocoderSched.cc"
    __PRF_globalCounter[633u]++ ;
    _specc::waitfor(410000ull);

#line 30 "src/codebook/pitch_contr.sc"
    temp_gain_pit = gain_pit;

#line 4032 "vocoderSched.cc"
    __PRF_globalCounter[634u]++ ;

#line 32 "src/codebook/pitch_contr.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 4039 "vocoderSched.cc"
	__PRF_globalCounter[635u]++ ;

#line 34 "src/codebook/pitch_contr.sc"
	L_temp = L_mult(v[i], temp_gain_pit);
	L_temp = L_shl(L_temp, 3);
	y[i] = sub(x[i], extract_h(L_temp));
    }
}

#line 4049 "vocoderSched.cc"
Prefilter::Prefilter(unsigned int _idcnt, short int (&T0), short int (&gain_pit), short int (&x_in)[40], short int (&x_out)[40])
    : _specc::behavior(_idcnt), T0(T0), gain_pit(gain_pit), x_in(x_in), x_out(x_out)
{   
}

Prefilter::~Prefilter(void)
{   
}

void Prefilter::main(void)

#line 26 "src/codebook/prefilter.sc"
{   
    int i;
    short int temp;
    short int pit_sharp;
    short int x[40];
    short int temp_T0;
    short int temp_gain_pit;

#line 4070 "vocoderSched.cc"
    __PRF_globalCounter[758u]++ ;
    _specc::waitfor(480000ull);

#line 34 "src/codebook/prefilter.sc"
    temp_T0 = T0;
    temp_gain_pit = gain_pit;

#line 4078 "vocoderSched.cc"
    __PRF_globalCounter[759u]++ ;

#line 38 "src/codebook/prefilter.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 4085 "vocoderSched.cc"
	__PRF_globalCounter[760u]++ ;

#line 40 "src/codebook/prefilter.sc"
	x[i] = x_in[i];
    }

#line 4092 "vocoderSched.cc"
    __PRF_globalCounter[761u]++ ;

#line 43 "src/codebook/prefilter.sc"
    pit_sharp = shl(temp_gain_pit, 3);

#line 4098 "vocoderSched.cc"
    __PRF_globalCounter[762u]++ ;

#line 45 "src/codebook/prefilter.sc"
    for(i = T0; i < 40; i++ )
    {   

#line 4105 "vocoderSched.cc"
	__PRF_globalCounter[763u]++ ;

#line 47 "src/codebook/prefilter.sc"
	temp = mult(x[i - T0], pit_sharp);
	x[i] = add(x[i], temp);
    }

#line 4113 "vocoderSched.cc"
    __PRF_globalCounter[764u]++ ;

#line 51 "src/codebook/prefilter.sc"
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<40;_scc_index_0++) (x_out)[_scc_index_0] = (x)[_scc_index_0]; }
}

#line 4120 "vocoderSched.cc"
Q_p::Q_p(unsigned int _idcnt, short int (&indi)[10], short int (&indo)[10])
    : _specc::behavior(_idcnt), indi(indi), indo(indo)
{   
}

Q_p::~Q_p(void)
{   
}

void Q_p::main(void)

#line 18 "src/codebook/q_p.sc"
{   
    static const short int gray[8] = { ((short int)0),((short int)1),((short int)3),((short int)2),((short int)6),((short int)4),((short int)5),((short int)7) };
    short int tmp;
    int i;

#line 4138 "vocoderSched.cc"
    __PRF_globalCounter[938u]++ ;
    _specc::waitfor(310000ull);
    __PRF_globalCounter[939u]++ ;

#line 23 "src/codebook/q_p.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 4147 "vocoderSched.cc"
	__PRF_globalCounter[940u]++ ;

#line 25 "src/codebook/q_p.sc"
	tmp = indi[i];

#line 4153 "vocoderSched.cc"
	__PRF_globalCounter[941u]++ ;

#line 27 "src/codebook/q_p.sc"
	if (i < 5)
	{   

#line 4160 "vocoderSched.cc"
	    __PRF_globalCounter[942u]++ ;

#line 29 "src/codebook/q_p.sc"
	    tmp = (tmp & 8) | gray[tmp & 7];
	}
	else 

	{   

#line 4170 "vocoderSched.cc"
	    __PRF_globalCounter[943u]++ ;

#line 34 "src/codebook/q_p.sc"
	    tmp = gray[tmp & 7];
	}

#line 4177 "vocoderSched.cc"
	__PRF_globalCounter[944u]++ ;

#line 37 "src/codebook/q_p.sc"
	indo[i] = tmp;
    }
}

#line 4185 "vocoderSched.cc"
Search_10i40::Search_10i40(unsigned int _idcnt, short int (&dn)[40], short int (&rr)[1600], short int (&ipos)[10], short int (&pos_max)[5], short int (&codvec)[10])
    : _specc::behavior(_idcnt), dn(dn), rr(rr), ipos(ipos), pos_max(pos_max), codvec(codvec)
{   
}

Search_10i40::~Search_10i40(void)
{   
}

void Search_10i40::main(void)

#line 39 "src/codebook/search_10i40.sc"
{   
    short int i0; short int i1;
    int i2; int i3; int i4; int i5; int i6; int i7; int i8; int i9;
    int i; int j; int k;
    short int ia; short int ib; short int pos;
    short int ps; short int ps0; short int ps1; short int ps2; short int psk; short int sq; short int sq2;
    short int alp; short int alp_16; short int alpk;
    short int rrv[40];
    int alp0; int alp1; int alp2; int s;
    short int my_ipos[10];

#line 4209 "vocoderSched.cc"
    __PRF_globalCounter[963u]++ ;
    _specc::waitfor(151470000ull);
    __PRF_globalCounter[964u]++ ;

#line 51 "src/codebook/search_10i40.sc"
    for(i = 0; i < 10; i++ ) {

#line 4217 "vocoderSched.cc"
	__PRF_globalCounter[965u]++ ;

#line 52 "src/codebook/search_10i40.sc"
	my_ipos[i] = ipos[i];
    }

#line 4224 "vocoderSched.cc"
    __PRF_globalCounter[966u]++ ;

#line 57 "src/codebook/search_10i40.sc"
    i0 = pos_max[my_ipos[0]];

#line 64 "src/codebook/search_10i40.sc"
    psk =  -1;
    alpk = 1;

#line 4234 "vocoderSched.cc"
    __PRF_globalCounter[967u]++ ;

#line 66 "src/codebook/search_10i40.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 4241 "vocoderSched.cc"
	__PRF_globalCounter[968u]++ ;

#line 68 "src/codebook/search_10i40.sc"
	codvec[i] = i;
    }

#line 4248 "vocoderSched.cc"
    __PRF_globalCounter[969u]++ ;

#line 71 "src/codebook/search_10i40.sc"
    for(i = 1; i < 5; i++ )
    {   

#line 4255 "vocoderSched.cc"
	__PRF_globalCounter[970u]++ ;

#line 73 "src/codebook/search_10i40.sc"
	i1 = pos_max[my_ipos[1]];
	ps0 = add(dn[i0], dn[i1]);
	alp0 = L_mult(rr[i0 * 40 + i0], (short int)(32768l / 16));
	alp0 = L_mac(alp0, rr[i1 * 40 + i1], (short int)(32768l / 16));
	alp0 = L_mac(alp0, rr[i0 * 40 + i1], (short int)(32768l / 8));

#line 4265 "vocoderSched.cc"
	__PRF_globalCounter[971u]++ ;

#line 89 "src/codebook/search_10i40.sc"
	for(i3 = my_ipos[3]; i3 < 40; i3 += 5)
	{   

#line 4272 "vocoderSched.cc"
	    __PRF_globalCounter[972u]++ ;

#line 91 "src/codebook/search_10i40.sc"
	    s = L_mult(rr[i3 * 40 + i3], (short int)(32768l / 8));
	    s = L_mac(s, rr[i0 * 40 + i3], (short int)(32768l / 4));
	    s = L_mac(s, rr[i1 * 40 + i3], (short int)(32768l / 4));
	    rrv[i3] = round(s);
	}

#line 4282 "vocoderSched.cc"
	__PRF_globalCounter[973u]++ ;

#line 98 "src/codebook/search_10i40.sc"
	sq =  -1;
	alp = 1;
	ps = 0;
	ia = my_ipos[2];
	ib = my_ipos[3];

#line 4292 "vocoderSched.cc"
	__PRF_globalCounter[974u]++ ;

#line 110 "src/codebook/search_10i40.sc"
	for(i2 = my_ipos[2]; i2 < 40; i2 += 5)
	{   

#line 4299 "vocoderSched.cc"
	    __PRF_globalCounter[975u]++ ;

#line 113 "src/codebook/search_10i40.sc"
	    ps1 = add(ps0, dn[i2]);


	    alp1 = L_mac(alp0, rr[i2 * 40 + i2], (short int)(32768l / 16));

	    alp1 = L_mac(alp1, rr[i0 * 40 + i2], (short int)(32768l / 8));

	    alp1 = L_mac(alp1, rr[i1 * 40 + i2], (short int)(32768l / 8));

#line 4312 "vocoderSched.cc"
	    __PRF_globalCounter[976u]++ ;

#line 127 "src/codebook/search_10i40.sc"
	    for(i3 = my_ipos[3]; i3 < 40; i3 += 5)
	    {   

#line 4319 "vocoderSched.cc"
		__PRF_globalCounter[977u]++ ;

#line 130 "src/codebook/search_10i40.sc"
		ps2 = add(ps1, dn[i3]);


		alp2 = L_mac(alp1, rrv[i3], (short int)(32768l / 2));

		alp2 = L_mac(alp2, rr[i2 * 40 + i3], (short int)(32768l / 8));

		sq2 = mult(ps2, ps2);

		alp_16 = round(alp2);

		s = L_msu(L_mult(alp, sq2), sq, alp_16);

#line 4336 "vocoderSched.cc"
		__PRF_globalCounter[978u]++ ;

#line 143 "src/codebook/search_10i40.sc"
		if (s > 0)
		{   

#line 4343 "vocoderSched.cc"
		    __PRF_globalCounter[979u]++ ;

#line 145 "src/codebook/search_10i40.sc"
		    sq = sq2;
		    ps = ps2;
		    alp = alp_16;
		    ia = i2;
		    ib = i3;
		}
	    }
	}

#line 4356 "vocoderSched.cc"
	__PRF_globalCounter[980u]++ ;

#line 153 "src/codebook/search_10i40.sc"
	i2 = ia;
	i3 = ib;

#line 160 "src/codebook/search_10i40.sc"
	ps0 = ps;
	alp0 = L_mult(alp, (short int)(32768l / 2));

#line 4367 "vocoderSched.cc"
	__PRF_globalCounter[981u]++ ;

#line 166 "src/codebook/search_10i40.sc"
	for(i5 = my_ipos[5]; i5 < 40; i5 += 5)
	{   

#line 4374 "vocoderSched.cc"
	    __PRF_globalCounter[982u]++ ;

#line 168 "src/codebook/search_10i40.sc"
	    s = L_mult(rr[i5 * 40 + i5], (short int)(32768l / 8));
	    s = L_mac(s, rr[i0 * 40 + i5], (short int)(32768l / 4));
	    s = L_mac(s, rr[i1 * 40 + i5], (short int)(32768l / 4));
	    s = L_mac(s, rr[i2 * 40 + i5], (short int)(32768l / 4));
	    s = L_mac(s, rr[i3 * 40 + i5], (short int)(32768l / 4));
	    rrv[i5] = round(s);
	}

#line 4386 "vocoderSched.cc"
	__PRF_globalCounter[983u]++ ;

#line 177 "src/codebook/search_10i40.sc"
	sq =  -1;
	alp = 1;
	ps = 0;
	ia = my_ipos[4];
	ib = my_ipos[5];

#line 4396 "vocoderSched.cc"
	__PRF_globalCounter[984u]++ ;

#line 186 "src/codebook/search_10i40.sc"
	for(i4 = my_ipos[4]; i4 < 40; i4 += 5)
	{   

#line 4403 "vocoderSched.cc"
	    __PRF_globalCounter[985u]++ ;

#line 188 "src/codebook/search_10i40.sc"
	    ps1 = add(ps0, dn[i4]);

	    alp1 = L_mac(alp0, rr[i4 * 40 + i4], (short int)(32768l / 32));
	    alp1 = L_mac(alp1, rr[i0 * 40 + i4], (short int)(32768l / 16));
	    alp1 = L_mac(alp1, rr[i1 * 40 + i4], (short int)(32768l / 16));
	    alp1 = L_mac(alp1, rr[i2 * 40 + i4], (short int)(32768l / 16));
	    alp1 = L_mac(alp1, rr[i3 * 40 + i4], (short int)(32768l / 16));

#line 4415 "vocoderSched.cc"
	    __PRF_globalCounter[986u]++ ;

#line 199 "src/codebook/search_10i40.sc"
	    for(i5 = my_ipos[5]; i5 < 40; i5 += 5)
	    {   

#line 4422 "vocoderSched.cc"
		__PRF_globalCounter[987u]++ ;

#line 201 "src/codebook/search_10i40.sc"
		ps2 = add(ps1, dn[i5]);

		alp2 = L_mac(alp1, rrv[i5], (short int)(32768l / 4));
		alp2 = L_mac(alp2, rr[i4 * 40 + i5], (short int)(32768l / 16));

		sq2 = mult(ps2, ps2);

		alp_16 = round(alp2);

		s = L_msu(L_mult(alp, sq2), sq, alp_16);

#line 4437 "vocoderSched.cc"
		__PRF_globalCounter[988u]++ ;

#line 213 "src/codebook/search_10i40.sc"
		if (s > 0)
		{   

#line 4444 "vocoderSched.cc"
		    __PRF_globalCounter[989u]++ ;

#line 215 "src/codebook/search_10i40.sc"
		    sq = sq2;
		    ps = ps2;
		    alp = alp_16;
		    ia = i4;
		    ib = i5;
		}
	    }
	}

#line 4457 "vocoderSched.cc"
	__PRF_globalCounter[990u]++ ;

#line 223 "src/codebook/search_10i40.sc"
	i4 = ia;
	i5 = ib;

#line 230 "src/codebook/search_10i40.sc"
	ps0 = ps;
	alp0 = L_mult(alp, (short int)(32768l / 2));

#line 4468 "vocoderSched.cc"
	__PRF_globalCounter[991u]++ ;

#line 237 "src/codebook/search_10i40.sc"
	for(i7 = my_ipos[7]; i7 < 40; i7 += 5)
	{   

#line 4475 "vocoderSched.cc"
	    __PRF_globalCounter[992u]++ ;

#line 239 "src/codebook/search_10i40.sc"
	    s = L_mult(rr[i7 * 40 + i7], (short int)(32768l / 16));
	    s = L_mac(s, rr[i0 * 40 + i7], (short int)(32768l / 8));
	    s = L_mac(s, rr[i1 * 40 + i7], (short int)(32768l / 8));
	    s = L_mac(s, rr[i2 * 40 + i7], (short int)(32768l / 8));
	    s = L_mac(s, rr[i3 * 40 + i7], (short int)(32768l / 8));
	    s = L_mac(s, rr[i4 * 40 + i7], (short int)(32768l / 8));
	    s = L_mac(s, rr[i5 * 40 + i7], (short int)(32768l / 8));
	    rrv[i7] = round(s);
	}

#line 4489 "vocoderSched.cc"
	__PRF_globalCounter[993u]++ ;

#line 250 "src/codebook/search_10i40.sc"
	sq =  -1;
	alp = 1;
	ps = 0;
	ia = my_ipos[6];
	ib = my_ipos[7];

#line 4499 "vocoderSched.cc"
	__PRF_globalCounter[994u]++ ;

#line 258 "src/codebook/search_10i40.sc"
	for(i6 = my_ipos[6]; i6 < 40; i6 += 5)
	{   

#line 4506 "vocoderSched.cc"
	    __PRF_globalCounter[995u]++ ;

#line 260 "src/codebook/search_10i40.sc"
	    ps1 = add(ps0, dn[i6]);

	    alp1 = L_mac(alp0, rr[i6 * 40 + i6], (short int)(32768l / 64));
	    alp1 = L_mac(alp1, rr[i0 * 40 + i6], (short int)(32768l / 32));
	    alp1 = L_mac(alp1, rr[i1 * 40 + i6], (short int)(32768l / 32));
	    alp1 = L_mac(alp1, rr[i2 * 40 + i6], (short int)(32768l / 32));
	    alp1 = L_mac(alp1, rr[i3 * 40 + i6], (short int)(32768l / 32));
	    alp1 = L_mac(alp1, rr[i4 * 40 + i6], (short int)(32768l / 32));
	    alp1 = L_mac(alp1, rr[i5 * 40 + i6], (short int)(32768l / 32));

#line 4520 "vocoderSched.cc"
	    __PRF_globalCounter[996u]++ ;

#line 273 "src/codebook/search_10i40.sc"
	    for(i7 = my_ipos[7]; i7 < 40; i7 += 5)
	    {   

#line 4527 "vocoderSched.cc"
		__PRF_globalCounter[997u]++ ;

#line 275 "src/codebook/search_10i40.sc"
		ps2 = add(ps1, dn[i7]);

		alp2 = L_mac(alp1, rrv[i7], (short int)(32768l / 4));
		alp2 = L_mac(alp2, rr[i6 * 40 + i7], (short int)(32768l / 32));

		sq2 = mult(ps2, ps2);

		alp_16 = round(alp2);

		s = L_msu(L_mult(alp, sq2), sq, alp_16);

#line 4542 "vocoderSched.cc"
		__PRF_globalCounter[998u]++ ;

#line 287 "src/codebook/search_10i40.sc"
		if (s > 0)
		{   

#line 4549 "vocoderSched.cc"
		    __PRF_globalCounter[999u]++ ;

#line 289 "src/codebook/search_10i40.sc"
		    sq = sq2;
		    ps = ps2;
		    alp = alp_16;
		    ia = i6;
		    ib = i7;
		}
	    }
	}

#line 4562 "vocoderSched.cc"
	__PRF_globalCounter[1000u]++ ;

#line 297 "src/codebook/search_10i40.sc"
	i6 = ia;
	i7 = ib;

#line 304 "src/codebook/search_10i40.sc"
	ps0 = ps;
	alp0 = L_mult(alp, (short int)(32768l / 2));

#line 4573 "vocoderSched.cc"
	__PRF_globalCounter[1001u]++ ;

#line 311 "src/codebook/search_10i40.sc"
	for(i9 = my_ipos[9]; i9 < 40; i9 += 5)
	{   

#line 4580 "vocoderSched.cc"
	    __PRF_globalCounter[1002u]++ ;

#line 313 "src/codebook/search_10i40.sc"
	    s = L_mult(rr[i9 * 40 + i9], (short int)(32768l / 16));
	    s = L_mac(s, rr[i0 * 40 + i9], (short int)(32768l / 8));
	    s = L_mac(s, rr[i1 * 40 + i9], (short int)(32768l / 8));
	    s = L_mac(s, rr[i2 * 40 + i9], (short int)(32768l / 8));
	    s = L_mac(s, rr[i3 * 40 + i9], (short int)(32768l / 8));
	    s = L_mac(s, rr[i4 * 40 + i9], (short int)(32768l / 8));
	    s = L_mac(s, rr[i5 * 40 + i9], (short int)(32768l / 8));
	    s = L_mac(s, rr[i6 * 40 + i9], (short int)(32768l / 8));
	    s = L_mac(s, rr[i7 * 40 + i9], (short int)(32768l / 8));
	    rrv[i9] = round(s);
	}

#line 4596 "vocoderSched.cc"
	__PRF_globalCounter[1003u]++ ;

#line 326 "src/codebook/search_10i40.sc"
	sq =  -1;
	alp = 1;
	ps = 0;
	ia = my_ipos[8];
	ib = my_ipos[9];

#line 4606 "vocoderSched.cc"
	__PRF_globalCounter[1004u]++ ;

#line 333 "src/codebook/search_10i40.sc"
	for(i8 = my_ipos[8]; i8 < 40; i8 += 5)
	{   

#line 4613 "vocoderSched.cc"
	    __PRF_globalCounter[1005u]++ ;

#line 335 "src/codebook/search_10i40.sc"
	    ps1 = add(ps0, dn[i8]);

	    alp1 = L_mac(alp0, rr[i8 * 40 + i8], (short int)(32768l / 128));
	    alp1 = L_mac(alp1, rr[i0 * 40 + i8], (short int)(32768l / 64));
	    alp1 = L_mac(alp1, rr[i1 * 40 + i8], (short int)(32768l / 64));
	    alp1 = L_mac(alp1, rr[i2 * 40 + i8], (short int)(32768l / 64));
	    alp1 = L_mac(alp1, rr[i3 * 40 + i8], (short int)(32768l / 64));
	    alp1 = L_mac(alp1, rr[i4 * 40 + i8], (short int)(32768l / 64));
	    alp1 = L_mac(alp1, rr[i5 * 40 + i8], (short int)(32768l / 64));
	    alp1 = L_mac(alp1, rr[i6 * 40 + i8], (short int)(32768l / 64));
	    alp1 = L_mac(alp1, rr[i7 * 40 + i8], (short int)(32768l / 64));

#line 4629 "vocoderSched.cc"
	    __PRF_globalCounter[1006u]++ ;

#line 350 "src/codebook/search_10i40.sc"
	    for(i9 = my_ipos[9]; i9 < 40; i9 += 5)
	    {   

#line 4636 "vocoderSched.cc"
		__PRF_globalCounter[1007u]++ ;

#line 352 "src/codebook/search_10i40.sc"
		ps2 = add(ps1, dn[i9]);

		alp2 = L_mac(alp1, rrv[i9], (short int)(32768l / 8));
		alp2 = L_mac(alp2, rr[i8 * 40 + i9], (short int)(32768l / 64));

		sq2 = mult(ps2, ps2);

		alp_16 = round(alp2);

		s = L_msu(L_mult(alp, sq2), sq, alp_16);

#line 4651 "vocoderSched.cc"
		__PRF_globalCounter[1008u]++ ;

#line 364 "src/codebook/search_10i40.sc"
		if (s > 0)
		{   

#line 4658 "vocoderSched.cc"
		    __PRF_globalCounter[1009u]++ ;

#line 366 "src/codebook/search_10i40.sc"
		    sq = sq2;
		    ps = ps2;
		    alp = alp_16;
		    ia = i8;
		    ib = i9;
		}
	    }
	}

#line 4671 "vocoderSched.cc"
	__PRF_globalCounter[1010u]++ ;

#line 379 "src/codebook/search_10i40.sc"
	s = L_msu(L_mult(alpk, sq), psk, alp);

#line 4677 "vocoderSched.cc"
	__PRF_globalCounter[1011u]++ ;

#line 382 "src/codebook/search_10i40.sc"
	if (s > 0)
	{   

#line 4684 "vocoderSched.cc"
	    __PRF_globalCounter[1012u]++ ;

#line 384 "src/codebook/search_10i40.sc"
	    psk = sq;
	    alpk = alp;
	    codvec[0] = i0;
	    codvec[1] = i1;
	    codvec[2] = i2;
	    codvec[3] = i3;
	    codvec[4] = i4;
	    codvec[5] = i5;
	    codvec[6] = i6;
	    codvec[7] = i7;
	    codvec[8] = ia;
	    codvec[9] = ib;
	}

#line 4702 "vocoderSched.cc"
	__PRF_globalCounter[1013u]++ ;

#line 401 "src/codebook/search_10i40.sc"
	pos = my_ipos[1];

#line 4708 "vocoderSched.cc"
	__PRF_globalCounter[1014u]++ ;

#line 402 "src/codebook/search_10i40.sc"
	for(j = 1 , k = 2; k < 10; j++  , k++ )
	{   

#line 4715 "vocoderSched.cc"
	    __PRF_globalCounter[1015u]++ ;

#line 404 "src/codebook/search_10i40.sc"
	    my_ipos[j] = my_ipos[k];
	}

#line 4722 "vocoderSched.cc"
	__PRF_globalCounter[1016u]++ ;

#line 406 "src/codebook/search_10i40.sc"
	my_ipos[10 - 1] = pos;
    }
}

#line 4730 "vocoderSched.cc"
Set_Sign::Set_Sign(unsigned int _idcnt, short int (&dn)[40], short int (&dn2)[40], short int (&cn)[40], short int (&sign)[40], short int (&pos_max)[5], short int (&ipos)[10])
    : _specc::behavior(_idcnt), dn(dn), dn2(dn2), cn(cn), sign(sign), pos_max(pos_max), ipos(ipos)
{   
}

Set_Sign::~Set_Sign(void)
{   
}

void Set_Sign::main(void)

#line 41 "src/codebook/set_sign.sc"
{   
    int i; int j;
    short int cor; short int ipos0; short int k_cn; short int k_dn; short int max; short int max_of_all; short int pos; short int val;
    short int en[40];
    int s;

#line 4749 "vocoderSched.cc"
    __PRF_globalCounter[1019u]++ ;
    _specc::waitfor(2230000ull);

#line 49 "src/codebook/set_sign.sc"
    s = 256;

#line 4756 "vocoderSched.cc"
    __PRF_globalCounter[1020u]++ ;

#line 50 "src/codebook/set_sign.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 4763 "vocoderSched.cc"
	__PRF_globalCounter[1021u]++ ;

#line 52 "src/codebook/set_sign.sc"
	s = L_mac(s, cn[i], cn[i]);
    }

#line 4770 "vocoderSched.cc"
    __PRF_globalCounter[1022u]++ ;

#line 54 "src/codebook/set_sign.sc"
    s = Inv_sqrt(s);
    k_cn = extract_h(L_shl(s, 5));

    s = 256;

#line 4779 "vocoderSched.cc"
    __PRF_globalCounter[1023u]++ ;

#line 58 "src/codebook/set_sign.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 4786 "vocoderSched.cc"
	__PRF_globalCounter[1024u]++ ;

#line 60 "src/codebook/set_sign.sc"
	s = L_mac(s, dn[i], dn[i]);
    }

#line 4793 "vocoderSched.cc"
    __PRF_globalCounter[1025u]++ ;

#line 62 "src/codebook/set_sign.sc"
    s = Inv_sqrt(s);
    k_dn = extract_h(L_shl(s, 5));

#line 4800 "vocoderSched.cc"
    __PRF_globalCounter[1026u]++ ;

#line 65 "src/codebook/set_sign.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 4807 "vocoderSched.cc"
	__PRF_globalCounter[1027u]++ ;

#line 67 "src/codebook/set_sign.sc"
	val = dn[i];
	cor = round(L_shl(L_mac(L_mult(k_cn, cn[i]), k_dn, val), 10));

#line 4814 "vocoderSched.cc"
	__PRF_globalCounter[1028u]++ ;

#line 71 "src/codebook/set_sign.sc"
	if (cor >= 0)
	{   

#line 4821 "vocoderSched.cc"
	    __PRF_globalCounter[1029u]++ ;

#line 73 "src/codebook/set_sign.sc"
	    sign[i] = 32767;
	}
	else 
	{   

#line 4830 "vocoderSched.cc"
	    __PRF_globalCounter[1030u]++ ;

#line 77 "src/codebook/set_sign.sc"
	    sign[i] =  -32767;
	    cor = negate(cor);
	    val = negate(val);
	}

#line 4839 "vocoderSched.cc"
	__PRF_globalCounter[1031u]++ ;

#line 82 "src/codebook/set_sign.sc"
	dn2[i] = val;
	en[i] = cor;
    }

#line 4847 "vocoderSched.cc"
    __PRF_globalCounter[1032u]++ ;

#line 86 "src/codebook/set_sign.sc"
    max_of_all =  -1;

#line 4853 "vocoderSched.cc"
    __PRF_globalCounter[1033u]++ ;

#line 87 "src/codebook/set_sign.sc"
    for(i = 0; i < 5; i++ )
    {   

#line 4860 "vocoderSched.cc"
	__PRF_globalCounter[1034u]++ ;

#line 89 "src/codebook/set_sign.sc"
	max =  -1;

#line 4866 "vocoderSched.cc"
	__PRF_globalCounter[1035u]++ ;

#line 91 "src/codebook/set_sign.sc"
	for(j = i; j < 40; j += 5)
	{   

#line 4873 "vocoderSched.cc"
	    __PRF_globalCounter[1036u]++ ;

#line 93 "src/codebook/set_sign.sc"
	    cor = en[j];
	    val = sub(cor, max);

#line 4880 "vocoderSched.cc"
	    __PRF_globalCounter[1037u]++ ;

#line 96 "src/codebook/set_sign.sc"
	    if (val > 0)
	    {   

#line 4887 "vocoderSched.cc"
		__PRF_globalCounter[1038u]++ ;

#line 98 "src/codebook/set_sign.sc"
		max = cor;
		pos = j;
	    }
	}

#line 4896 "vocoderSched.cc"
	__PRF_globalCounter[1039u]++ ;

#line 103 "src/codebook/set_sign.sc"
	pos_max[i] = pos;
	val = sub(max, max_of_all);

#line 4903 "vocoderSched.cc"
	__PRF_globalCounter[1040u]++ ;

#line 106 "src/codebook/set_sign.sc"
	if (val > 0)
	{   

#line 4910 "vocoderSched.cc"
	    __PRF_globalCounter[1041u]++ ;

#line 108 "src/codebook/set_sign.sc"
	    max_of_all = max;

	    ipos0 = i;
	}
    }

#line 4920 "vocoderSched.cc"
    __PRF_globalCounter[1042u]++ ;

#line 118 "src/codebook/set_sign.sc"
    pos = ipos0;
    ipos[0] = ipos0;
    ipos[5] = ipos0;

#line 4928 "vocoderSched.cc"
    __PRF_globalCounter[1043u]++ ;

#line 122 "src/codebook/set_sign.sc"
    for(i = 1; i < 5; i++ )
    {   

#line 4935 "vocoderSched.cc"
	__PRF_globalCounter[1044u]++ ;

#line 124 "src/codebook/set_sign.sc"
	pos = add(pos, 1);

#line 4941 "vocoderSched.cc"
	__PRF_globalCounter[1045u]++ ;

#line 125 "src/codebook/set_sign.sc"
	if (sub(pos, 5) >= 0)
	{   

#line 4948 "vocoderSched.cc"
	    __PRF_globalCounter[1046u]++ ;

#line 127 "src/codebook/set_sign.sc"
	    pos = 0;
	}

#line 4955 "vocoderSched.cc"
	__PRF_globalCounter[1047u]++ ;

#line 129 "src/codebook/set_sign.sc"
	ipos[i] = pos;
	ipos[i + 5] = pos;
    }
}

#line 4964 "vocoderSched.cc"
Nop::Nop(unsigned int _idcnt)
    : _specc::behavior(_idcnt)
{   
}

Nop::~Nop(void)
{   
}

void Nop::main(void)

#line 14 "src/common/copy.sc"
{   

#line 4979 "vocoderSched.cc"
    __PRF_globalCounter[579u]++ ;
    _specc::waitfor(0ull);
}

CopySubfr::CopySubfr(unsigned int _idcnt, short int (&x)[40], short int (&y)[40])
    : _specc::behavior(_idcnt), x(x), y(y)
{   
}

CopySubfr::~CopySubfr(void)
{   
}

void CopySubfr::main(void)

#line 24 "src/common/copy.sc"
{   

#line 4998 "vocoderSched.cc"
    __PRF_globalCounter[269u]++ ;
    _specc::waitfor(0ull);

#line 25 "src/common/copy.sc"
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<40;_scc_index_0++) (y)[_scc_index_0] = (x)[_scc_index_0]; }
}

#line 5006 "vocoderSched.cc"
Residu::Residu(unsigned int _idcnt, short int (&a)[11], short int *(&x), short int (&y)[40])
    : _specc::behavior(_idcnt), a(a), x(x), y(y)
{   
}

Residu::~Residu(void)
{   
}

void Residu::main(void)

#line 28 "src/common/residu.sc"
{   
    int i; int j;
    int s;

#line 5023 "vocoderSched.cc"
    __PRF_globalCounter[945u]++ ;
    _specc::waitfor(31366666ull);
    __PRF_globalCounter[946u]++ ;

#line 32 "src/common/residu.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 5032 "vocoderSched.cc"
	__PRF_globalCounter[947u]++ ;

#line 34 "src/common/residu.sc"
	s = L_mult(x[i], a[0]);

#line 5038 "vocoderSched.cc"
	__PRF_globalCounter[948u]++ ;

#line 35 "src/common/residu.sc"
	for(j = 1; j <= 10; j++ )
	{   

#line 5045 "vocoderSched.cc"
	    __PRF_globalCounter[949u]++ ;

#line 37 "src/common/residu.sc"
	    s = L_mac(s, a[j], x[i - j]);
	}

#line 5052 "vocoderSched.cc"
	__PRF_globalCounter[950u]++ ;

#line 39 "src/common/residu.sc"
	s = L_shl(s, 3);
	y[i] = round(s);
    }
}

#line 5061 "vocoderSched.cc"
Syn_Filt::Syn_Filt(unsigned int _idcnt, short int (&a)[11], short int (&x)[40], short int (&y)[40], short int (&mem)[10], bool (&update))
    : _specc::behavior(_idcnt), a(a), x(x), y(y), mem(mem), update(update)
{   
}

Syn_Filt::~Syn_Filt(void)
{   
}

void Syn_Filt::main(void)

#line 28 "src/common/syn_filt.sc"
{   
    int i; int j;
    int s;
    short int tmp[80];
    short int *yy;

#line 5080 "vocoderSched.cc"
    __PRF_globalCounter[1080u]++ ;
    _specc::waitfor(34833333ull);

#line 36 "src/common/syn_filt.sc"
    yy = tmp;

#line 5087 "vocoderSched.cc"
    __PRF_globalCounter[1081u]++ ;

#line 38 "src/common/syn_filt.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 5094 "vocoderSched.cc"
	__PRF_globalCounter[1082u]++ ;

#line 40 "src/common/syn_filt.sc"
	 *yy++  = mem[i];
    }

#line 5101 "vocoderSched.cc"
    __PRF_globalCounter[1083u]++ ;

#line 45 "src/common/syn_filt.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 5108 "vocoderSched.cc"
	__PRF_globalCounter[1084u]++ ;

#line 47 "src/common/syn_filt.sc"
	s = L_mult(x[i], a[0]);

#line 5114 "vocoderSched.cc"
	__PRF_globalCounter[1085u]++ ;

#line 48 "src/common/syn_filt.sc"
	for(j = 1; j <= 10; j++ )
	{   

#line 5121 "vocoderSched.cc"
	    __PRF_globalCounter[1086u]++ ;

#line 50 "src/common/syn_filt.sc"
	    s = L_msu(s, a[j], yy[ -j]);
	}

#line 5128 "vocoderSched.cc"
	__PRF_globalCounter[1087u]++ ;

#line 52 "src/common/syn_filt.sc"
	s = L_shl(s, 3);
	 *yy++  = round(s);
    }

#line 5136 "vocoderSched.cc"
    __PRF_globalCounter[1088u]++ ;

#line 56 "src/common/syn_filt.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 5143 "vocoderSched.cc"
	__PRF_globalCounter[1089u]++ ;

#line 58 "src/common/syn_filt.sc"
	y[i] = tmp[i + 10];
    }

#line 5150 "vocoderSched.cc"
    __PRF_globalCounter[1090u]++ ;

#line 64 "src/common/syn_filt.sc"
    if (update != 0)
    {   

#line 5157 "vocoderSched.cc"
	__PRF_globalCounter[1091u]++ ;

#line 66 "src/common/syn_filt.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 5164 "vocoderSched.cc"
	    __PRF_globalCounter[1092u]++ ;

#line 68 "src/common/syn_filt.sc"
	    mem[i] = tmp[40 + i];
	}
    }
}

#line 5173 "vocoderSched.cc"
Weight_Ai::Weight_Ai(unsigned int _idcnt, short int (&a)[11], short int (&fac)[10], short int (&a_exp)[11])
    : _specc::behavior(_idcnt), a(a), fac(fac), a_exp(a_exp)
{   
}

Weight_Ai::~Weight_Ai(void)
{   
}

void Weight_Ai::main(void)

#line 25 "src/common/weight_ai.sc"
{   
    int i;

#line 5189 "vocoderSched.cc"
    __PRF_globalCounter[1393u]++ ;
    _specc::waitfor(716666ull);

#line 28 "src/common/weight_ai.sc"
    a_exp[0] = a[0];

#line 5196 "vocoderSched.cc"
    __PRF_globalCounter[1394u]++ ;

#line 29 "src/common/weight_ai.sc"
    for(i = 1; i <= 10; i++ )
    {   

#line 5203 "vocoderSched.cc"
	__PRF_globalCounter[1395u]++ ;

#line 31 "src/common/weight_ai.sc"
	a_exp[i] = round(L_mult(a[i], fac[i - 1]));
    }
}

#line 5211 "vocoderSched.cc"
Autocorr::Autocorr(unsigned int _idcnt, short int *(&x), short int (&wind)[240], short int (&r_h)[11], short int (&r_l)[11], short int (&norm))
    : _specc::behavior(_idcnt), x(x), wind(wind), r_h(r_h), r_l(r_l), norm(norm)
{   
}

Autocorr::~Autocorr(void)
{   
}

void Autocorr::main(void)

#line 32 "src/lp_analysis/autocorr.sc"
{   
    int i; int j;
    short int n;
    short int y[240];
    int sum;
    bool overfl;
    short int overfl_shft;

#line 5232 "vocoderSched.cc"
    __PRF_globalCounter[23u]++ ;
    _specc::waitfor(220816666ull);
    __PRF_globalCounter[24u]++ ;

#line 42 "src/lp_analysis/autocorr.sc"
    for(i = 0; i < 240; i++ )
    {   

#line 5241 "vocoderSched.cc"
	__PRF_globalCounter[25u]++ ;

#line 44 "src/lp_analysis/autocorr.sc"
	y[i] = mult_r(x[i], wind[i]);
    }

#line 5248 "vocoderSched.cc"
    __PRF_globalCounter[26u]++ ;

#line 49 "src/lp_analysis/autocorr.sc"
    overfl_shft = 0;

#line 5254 "vocoderSched.cc"
    __PRF_globalCounter[27u]++ ;

#line 51 "src/lp_analysis/autocorr.sc"
    do 
    {   

#line 5261 "vocoderSched.cc"
	__PRF_globalCounter[28u]++ ;

#line 53 "src/lp_analysis/autocorr.sc"
	overfl = 0;
	sum = 0l;

#line 5268 "vocoderSched.cc"
	__PRF_globalCounter[29u]++ ;

#line 56 "src/lp_analysis/autocorr.sc"
	for(i = 0; i < 240; i++ )
	{   

#line 5275 "vocoderSched.cc"
	    __PRF_globalCounter[30u]++ ;

#line 58 "src/lp_analysis/autocorr.sc"
	    sum = L_mac(sum, y[i], y[i]);
	}

#line 5282 "vocoderSched.cc"
	__PRF_globalCounter[31u]++ ;

#line 64 "src/lp_analysis/autocorr.sc"
	if (L_sub(sum, (int)2147483647l) == 0l)
	{   

#line 5289 "vocoderSched.cc"
	    __PRF_globalCounter[32u]++ ;

#line 66 "src/lp_analysis/autocorr.sc"
	    overfl_shft = add(overfl_shft, 4);
	    overfl = 1;

#line 5296 "vocoderSched.cc"
	    __PRF_globalCounter[33u]++ ;

#line 69 "src/lp_analysis/autocorr.sc"
	    for(i = 0; i < 240; i++ )
	    {   

#line 5303 "vocoderSched.cc"
		__PRF_globalCounter[34u]++ ;

#line 71 "src/lp_analysis/autocorr.sc"
		y[i] = shr(y[i], 2);
	    }
	}
    }
    while(
	overfl != 0);

#line 5314 "vocoderSched.cc"
    __PRF_globalCounter[35u]++ ;

#line 78 "src/lp_analysis/autocorr.sc"
    sum = L_add(sum, 1l);



    n = norm_l(sum);
    sum = L_shl(sum, n);
    L_Extract(sum,  &r_h[0],  &r_l[0]);

#line 5326 "vocoderSched.cc"
    __PRF_globalCounter[36u]++ ;

#line 88 "src/lp_analysis/autocorr.sc"
    for(i = 1; i <= 10; i++ )
    {   

#line 5333 "vocoderSched.cc"
	__PRF_globalCounter[37u]++ ;

#line 90 "src/lp_analysis/autocorr.sc"
	sum = 0;

#line 5339 "vocoderSched.cc"
	__PRF_globalCounter[38u]++ ;

#line 92 "src/lp_analysis/autocorr.sc"
	for(j = 0; j < 240 - i; j++ )
	{   

#line 5346 "vocoderSched.cc"
	    __PRF_globalCounter[39u]++ ;

#line 94 "src/lp_analysis/autocorr.sc"
	    sum = L_mac(sum, y[j], y[j + i]);
	}

#line 5353 "vocoderSched.cc"
	__PRF_globalCounter[40u]++ ;

#line 97 "src/lp_analysis/autocorr.sc"
	sum = L_shl(sum, n);
	L_Extract(sum,  &r_h[i],  &r_l[i]);
    }

#line 5361 "vocoderSched.cc"
    __PRF_globalCounter[41u]++ ;

#line 101 "src/lp_analysis/autocorr.sc"
    norm = sub(n, overfl_shft);
}

#line 5368 "vocoderSched.cc"
Az_Lsp::Az_Lsp(unsigned int _idcnt, short int (&a)[11], short int (&lsp)[10], short int (&old_lsp)[10])
    : _specc::behavior(_idcnt), a(a), lsp(lsp), old_lsp(old_lsp)
{   
}

Az_Lsp::~Az_Lsp(void)
{   
}

short int Az_Lsp::Chebps(

#line 65 "src/lp_analysis/az_lsp.sc"
    short int x, short int f[], short int n)
{   
    int i;
    short int cheb;
    short int b0_h; short int b0_l; short int b1_h; short int b1_l; short int b2_h; short int b2_l;
    int t0;

#line 5388 "vocoderSched.cc"
    __PRF_globalCounter[61u]++ ;

#line 72 "src/lp_analysis/az_lsp.sc"
    b2_h = 256;
    b2_l = 0;

    t0 = L_mult(x, 512);
    t0 = L_mac(t0, f[1], 8192);
    L_Extract(t0,  &b1_h,  &b1_l);

#line 5399 "vocoderSched.cc"
    __PRF_globalCounter[62u]++ ;

#line 79 "src/lp_analysis/az_lsp.sc"
    for(i = 2; i < n; i++ )
    {   

#line 5406 "vocoderSched.cc"
	__PRF_globalCounter[63u]++ ;

#line 81 "src/lp_analysis/az_lsp.sc"
	t0 = Mpy_32_16(b1_h, b1_l, x);
	t0 = L_shl(t0, 1);
	t0 = L_mac(t0, b2_h, (short int)32768);
	t0 = L_msu(t0, b2_l, 1);
	t0 = L_mac(t0, f[i], 8192);

	L_Extract(t0,  &b0_h,  &b0_l);

	b2_l = b1_l;
	b2_h = b1_h;
	b1_l = b0_l;
	b1_h = b0_h;
    }

#line 5424 "vocoderSched.cc"
    __PRF_globalCounter[64u]++ ;

#line 95 "src/lp_analysis/az_lsp.sc"
    t0 = Mpy_32_16(b1_h, b1_l, x);
    t0 = L_mac(t0, b2_h, (short int)32768);
    t0 = L_msu(t0, b2_l, 1);
    t0 = L_mac(t0, f[i], 4096);

    t0 = L_shl(t0, 6);

    cheb = extract_h(t0);

    return (cheb);
}

#line 5440 "vocoderSched.cc"
void Az_Lsp::main(void)

#line 109 "src/lp_analysis/az_lsp.sc"
{   
    int i; int j;
    int ip; int nf;
    short int xhigh; short int xint; short int xlow; short int xmid; short int yhigh; short int ylow; short int ymid;
    short int exp; short int sign; short int x; short int y;
    short int *coef;
    short int f1[6]; short int f2[6];
    int t0;

#line 128 "src/lp_analysis/az_lsp.sc"
    const short int grid[61] = { 

      ((short int)32760),((short int)32723),((short int)32588),((short int)32364),((short int)32051),((short int)31651),
      ((short int)31164),((short int)30591),((short int)29935),((short int)29196),((short int)28377),((short int)27481),
      ((short int)26509),((short int)25465),((short int)24351),((short int)23170),((short int)21926),((short int)20621),
      ((short int)19260),((short int)17846),((short int)16384),((short int)14876),((short int)13327),((short int)11743),
      ((short int)10125),((short int)8480),((short int)6812),((short int)5126),((short int)3425),((short int)1714),
      ((short int)0),((short int)-1714),((short int)-3425),((short int)-5126),((short int)-6812),((short int)-8480),
      ((short int)-10125),((short int)-11743),((short int)-13327),((short int)-14876),((short int)-16384),((short int)-17846),
      ((short int)-19260),((short int)-20621),((short int)-21926),((short int)-23170),((short int)-24351),((short int)-25465),
      ((short int)-26509),((short int)-27481),((short int)-28377),((short int)-29196),((short int)-29935),((short int)-30591),
      ((short int)-31164),((short int)-31651),((short int)-32051),((short int)-32364),((short int)-32588),((short int)-32723),
      ((short int)-32760) };

#line 5468 "vocoderSched.cc"
    __PRF_globalCounter[65u]++ ;
    _specc::waitfor(154333333ull);

#line 158 "src/lp_analysis/az_lsp.sc"
    f1[0] = 1024;
    f2[0] = 1024;

#line 5476 "vocoderSched.cc"
    __PRF_globalCounter[66u]++ ;

#line 161 "src/lp_analysis/az_lsp.sc"
    for(i = 0; i < 10 / 2; i++ )
    {   

#line 5483 "vocoderSched.cc"
	__PRF_globalCounter[67u]++ ;

#line 163 "src/lp_analysis/az_lsp.sc"
	t0 = L_mult(a[i + 1], 8192);
	t0 = L_mac(t0, a[10 - i], 8192);
	x = extract_h(t0);

	f1[i + 1] = sub(x, f1[i]);

	t0 = L_mult(a[i + 1], 8192);
	t0 = L_msu(t0, a[10 - i], 8192);
	x = extract_h(t0);

	f2[i + 1] = add(x, f2[i]);
    }

#line 5500 "vocoderSched.cc"
    __PRF_globalCounter[68u]++ ;

#line 180 "src/lp_analysis/az_lsp.sc"
    nf = 0;
    ip = 0;

    coef = f1;

    xlow = grid[0];
    ylow = Chebps(xlow, coef, 10 / 2);

    j = 0;

#line 5514 "vocoderSched.cc"
    __PRF_globalCounter[69u]++ ;

#line 191 "src/lp_analysis/az_lsp.sc"
    while((sub(nf, 10) < 0) && (sub(j, 60) < 0))
    {   

#line 5521 "vocoderSched.cc"
	__PRF_globalCounter[70u]++ ;

#line 193 "src/lp_analysis/az_lsp.sc"
	j++ ;
	xhigh = xlow;
	yhigh = ylow;
	xlow = grid[j];
	ylow = Chebps(xlow, coef, 10 / 2);

#line 5531 "vocoderSched.cc"
	__PRF_globalCounter[71u]++ ;

#line 201 "src/lp_analysis/az_lsp.sc"
	if (L_mult(ylow, yhigh) <= (int)0l)
	{   

#line 5538 "vocoderSched.cc"
	    __PRF_globalCounter[72u]++ ;

#line 206 "src/lp_analysis/az_lsp.sc"
	    for(i = 0; i < 4; i++ )
	    {   

#line 5545 "vocoderSched.cc"
		__PRF_globalCounter[73u]++ ;

#line 209 "src/lp_analysis/az_lsp.sc"
		xmid = add(shr(xlow, 1), shr(xhigh, 1));
		ymid = Chebps(xmid, coef, 10 / 2);

#line 5552 "vocoderSched.cc"
		__PRF_globalCounter[74u]++ ;

#line 214 "src/lp_analysis/az_lsp.sc"
		if (L_mult(ylow, ymid) <= (int)0l)
		{   

#line 5559 "vocoderSched.cc"
		    __PRF_globalCounter[75u]++ ;

#line 216 "src/lp_analysis/az_lsp.sc"
		    yhigh = ymid;
		    xhigh = xmid;
		}
		else 
		{   

#line 5569 "vocoderSched.cc"
		    __PRF_globalCounter[76u]++ ;

#line 221 "src/lp_analysis/az_lsp.sc"
		    ylow = ymid;
		    xlow = xmid;
		}
	    }

#line 5578 "vocoderSched.cc"
	    __PRF_globalCounter[77u]++ ;

#line 231 "src/lp_analysis/az_lsp.sc"
	    x = sub(xhigh, xlow);
	    y = sub(yhigh, ylow);

#line 5585 "vocoderSched.cc"
	    __PRF_globalCounter[78u]++ ;

#line 235 "src/lp_analysis/az_lsp.sc"
	    if (y == 0)
	    {   

#line 5592 "vocoderSched.cc"
		__PRF_globalCounter[79u]++ ;

#line 237 "src/lp_analysis/az_lsp.sc"
		xint = xlow;
	    }
	    else 
	    {   

#line 5601 "vocoderSched.cc"
		__PRF_globalCounter[80u]++ ;

#line 241 "src/lp_analysis/az_lsp.sc"
		sign = y;
		y = abs_s(y);
		exp = norm_s(y);
		y = shl(y, exp);
		y = div_s((short int)16383, y);
		t0 = L_mult(x, y);
		t0 = L_shr(t0, sub(20, exp));
		y = extract_l(t0);

#line 5614 "vocoderSched.cc"
		__PRF_globalCounter[81u]++ ;

#line 251 "src/lp_analysis/az_lsp.sc"
		if (sign < 0)

#line 5620 "vocoderSched.cc"
		{   
		    __PRF_globalCounter[82u]++ ;

#line 252 "src/lp_analysis/az_lsp.sc"
		    y = negate(y);
		}

#line 5628 "vocoderSched.cc"
		__PRF_globalCounter[83u]++ ;

#line 254 "src/lp_analysis/az_lsp.sc"
		t0 = L_mult(ylow, y);
		t0 = L_shr(t0, 11);
		xint = sub(xlow, extract_l(t0));
	    }

#line 5637 "vocoderSched.cc"
	    __PRF_globalCounter[84u]++ ;

#line 259 "src/lp_analysis/az_lsp.sc"
	    lsp[nf] = xint;
	    xlow = xint;
	    nf++ ;

#line 5645 "vocoderSched.cc"
	    __PRF_globalCounter[85u]++ ;

#line 264 "src/lp_analysis/az_lsp.sc"
	    if (ip == 0)
	    {   

#line 5652 "vocoderSched.cc"
		__PRF_globalCounter[86u]++ ;

#line 266 "src/lp_analysis/az_lsp.sc"
		ip = 1;
		coef = f2;
	    }
	    else 
	    {   

#line 5662 "vocoderSched.cc"
		__PRF_globalCounter[87u]++ ;

#line 271 "src/lp_analysis/az_lsp.sc"
		ip = 0;
		coef = f1;
	    }

#line 5670 "vocoderSched.cc"
	    __PRF_globalCounter[88u]++ ;

#line 274 "src/lp_analysis/az_lsp.sc"
	    ylow = Chebps(xlow, coef, 10 / 2);
	}
    }

#line 5678 "vocoderSched.cc"
    __PRF_globalCounter[89u]++ ;

#line 284 "src/lp_analysis/az_lsp.sc"
    if (sub(nf, 10) < 0)
    {   

#line 5685 "vocoderSched.cc"
	__PRF_globalCounter[90u]++ ;

#line 286 "src/lp_analysis/az_lsp.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 5692 "vocoderSched.cc"
	    __PRF_globalCounter[91u]++ ;

#line 288 "src/lp_analysis/az_lsp.sc"
	    lsp[i] = old_lsp[i];
	}
    }
}

#line 5701 "vocoderSched.cc"
Find_Az::Find_Az(unsigned int _idcnt, short int *(&x), short int (&wind)[240], short int (&r_h)[11], short int (&r_l)[11], short int (&old_A)[11], short int (&scal_fac), short int (&A_t)[11], short int (&rc)[4])
    : _specc::behavior(_idcnt), x(x), wind(wind), r_h(r_h), r_l(r_l), old_A(old_A), scal_fac(scal_fac), A_t(A_t), rc(rc),
    autocorr(_IDcnt, x, wind, r_h, r_l, scal_fac),
    lag_window(_IDcnt, r_h, r_l),
    levinson(_IDcnt, r_h, r_l, old_A, A_t, rc)
{   
}

Find_Az::~Find_Az(void)
{   
}

void Find_Az::main(void)

#line 27 "src/lp_analysis/find_az.sc"
{   

#line 5719 "vocoderSched.cc"
    __PRF_globalCounter[359u]++ ;

#line 28 "src/lp_analysis/find_az.sc"
    autocorr.main();
    lag_window.main();
    levinson.main();
}

#line 5728 "vocoderSched.cc"
Int_Lpc::Int_Lpc(unsigned int _idcnt, short int (&lsp_old)[10], short int (&lsp_mid)[10], short int (&lsp_new)[10], short int (&A)[4][11])
    : _specc::behavior(_idcnt), lsp_old(lsp_old), lsp_mid(lsp_mid), lsp_new(lsp_new), A(A)
{   
}

Int_Lpc::~Int_Lpc(void)
{   
}

void Int_Lpc::main(void)

#line 43 "src/lp_analysis/int_lpc.sc"
{   
    int i;
    short int lsp[10];

#line 5745 "vocoderSched.cc"
    __PRF_globalCounter[455u]++ ;
    _specc::waitfor(1066666ull);
    __PRF_globalCounter[456u]++ ;

#line 49 "src/lp_analysis/int_lpc.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 5754 "vocoderSched.cc"
	__PRF_globalCounter[457u]++ ;

#line 51 "src/lp_analysis/int_lpc.sc"
	lsp[i] = add(shr(lsp_mid[i], 1), shr(lsp_old[i], 1));
    }

#line 5761 "vocoderSched.cc"
    __PRF_globalCounter[458u]++ ;

#line 55 "src/lp_analysis/int_lpc.sc"
    Lsp_Az(lsp, (short int *)(A[0]));
    Lsp_Az(lsp_mid, (short int *)(A[1]));

#line 5768 "vocoderSched.cc"
    __PRF_globalCounter[459u]++ ;

#line 58 "src/lp_analysis/int_lpc.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 5775 "vocoderSched.cc"
	__PRF_globalCounter[460u]++ ;

#line 60 "src/lp_analysis/int_lpc.sc"
	lsp[i] = add(shr(lsp_mid[i], 1), shr(lsp_new[i], 1));
    }

#line 5782 "vocoderSched.cc"
    __PRF_globalCounter[461u]++ ;

#line 64 "src/lp_analysis/int_lpc.sc"
    Lsp_Az(lsp, (short int *)(A[2]));
    Lsp_Az(lsp_new, (short int *)(A[3]));
}

#line 5790 "vocoderSched.cc"
Int_Lpc2::Int_Lpc2(unsigned int _idcnt, short int (&lsp_old)[10], short int (&lsp_mid)[10], short int (&lsp_new)[10], short int (&A)[4][11])
    : _specc::behavior(_idcnt), lsp_old(lsp_old), lsp_mid(lsp_mid), lsp_new(lsp_new), A(A)
{   
}

Int_Lpc2::~Int_Lpc2(void)
{   
}

void Int_Lpc2::main(void)

#line 91 "src/lp_analysis/int_lpc.sc"
{   
    int i;
    short int lsp[10];

#line 5807 "vocoderSched.cc"
    __PRF_globalCounter[462u]++ ;
    _specc::waitfor(1066666ull);
    __PRF_globalCounter[463u]++ ;

#line 97 "src/lp_analysis/int_lpc.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 5816 "vocoderSched.cc"
	__PRF_globalCounter[464u]++ ;

#line 99 "src/lp_analysis/int_lpc.sc"
	lsp[i] = add(shr(lsp_mid[i], 1), shr(lsp_old[i], 1));
    }

#line 5823 "vocoderSched.cc"
    __PRF_globalCounter[465u]++ ;

#line 103 "src/lp_analysis/int_lpc.sc"
    Lsp_Az(lsp, (short int *)(A[0]));

#line 5829 "vocoderSched.cc"
    __PRF_globalCounter[466u]++ ;

#line 105 "src/lp_analysis/int_lpc.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 5836 "vocoderSched.cc"
	__PRF_globalCounter[467u]++ ;

#line 107 "src/lp_analysis/int_lpc.sc"
	lsp[i] = add(shr(lsp_mid[i], 1), shr(lsp_new[i], 1));
    }

#line 5843 "vocoderSched.cc"
    __PRF_globalCounter[468u]++ ;

#line 110 "src/lp_analysis/int_lpc.sc"
    Lsp_Az(lsp, (short int *)(A[2]));
}

#line 5850 "vocoderSched.cc"
Lag_Window::Lag_Window(unsigned int _idcnt, short int (&r_h)[11], short int (&r_l)[11])
    : _specc::behavior(_idcnt), r_h(r_h), r_l(r_l)
{   
}

Lag_Window::~Lag_Window(void)
{   
}

void Lag_Window::main(void)

#line 29 "src/lp_analysis/lag_wind.sc"
{   
    int i;
    int x;

#line 53 "src/lp_analysis/lag_wind.sc"
    const short int lag_h[10] = { 

      ((short int)32728),((short int)32619),((short int)32438),((short int)32187),((short int)31867),((short int)31480),((short int)31029),((short int)30517),((short int)29946),((short int)29321) };


    const short int lag_l[10] = { 

      ((short int)11904),((short int)17280),((short int)30720),((short int)25856),((short int)24192),((short int)28992),((short int)24384),((short int)7360),((short int)19520),((short int)14784) };

#line 5877 "vocoderSched.cc"
    __PRF_globalCounter[484u]++ ;
    _specc::waitfor(866666ull);
    __PRF_globalCounter[485u]++ ;

#line 66 "src/lp_analysis/lag_wind.sc"
    for(i = 1; i <= 10; i++ )
    {   

#line 5886 "vocoderSched.cc"
	__PRF_globalCounter[486u]++ ;

#line 68 "src/lp_analysis/lag_wind.sc"
	x = Mpy_32(r_h[i], r_l[i], lag_h[i - 1], lag_l[i - 1]);
	L_Extract(x,  &r_h[i],  &r_l[i]);
    }
}

#line 5895 "vocoderSched.cc"
Levinson::Levinson(unsigned int _idcnt, short int (&Rh)[11], short int (&Rl)[11], short int (&old_A)[11], short int (&A)[11], short int (&rc)[4])
    : _specc::behavior(_idcnt), Rh(Rh), Rl(Rl), old_A(old_A), A(A), rc(rc)
{   
}

Levinson::~Levinson(void)
{   
}

void Levinson::main(void)

#line 55 "src/lp_analysis/levinson.sc"
{   
    int i; int j;
    short int t0_n;
    short int hi; short int lo;
    short int Kh; short int Kl;
    short int alp_exp; short int alp_h; short int alp_l;
    short int Ah[11]; short int Al[11];
    short int Anh[11]; short int Anl[11];

    int t0; int t1; int t2;

#line 5919 "vocoderSched.cc"
    __PRF_globalCounter[490u]++ ;
    _specc::waitfor(17333333ull);

#line 69 "src/lp_analysis/levinson.sc"
    t1 = L_Comp(Rh[1], Rl[1]);
    t2 = L_abs(t1);
    t0 = Div_32(t2, Rh[0], Rl[0]);

#line 5928 "vocoderSched.cc"
    __PRF_globalCounter[491u]++ ;

#line 73 "src/lp_analysis/levinson.sc"
    if (t1 > 0)

#line 5934 "vocoderSched.cc"
    {   
	__PRF_globalCounter[492u]++ ;

#line 74 "src/lp_analysis/levinson.sc"
	t0 = L_negate(t0);
    }

#line 5942 "vocoderSched.cc"
    __PRF_globalCounter[493u]++ ;

#line 75 "src/lp_analysis/levinson.sc"
    L_Extract(t0,  &Kh,  &Kl);

    rc[0] = round(t0);

    t0 = L_shr(t0, 4);
    L_Extract(t0,  &Ah[1],  &Al[1]);



    t0 = Mpy_32(Kh, Kl, Kh, Kl);
    t0 = L_abs(t0);
    t0 = L_sub((int)2147483647l, t0);
    L_Extract(t0,  &hi,  &lo);
    t0 = Mpy_32(Rh[0], Rl[0], hi, lo);



    alp_exp = norm_l(t0);
    t0 = L_shl(t0, alp_exp);
    L_Extract(t0,  &alp_h,  &alp_l);

#line 5967 "vocoderSched.cc"
    __PRF_globalCounter[494u]++ ;

#line 100 "src/lp_analysis/levinson.sc"
    for(i = 2; i <= 10; i++ )
    {   

#line 5974 "vocoderSched.cc"
	__PRF_globalCounter[495u]++ ;

#line 104 "src/lp_analysis/levinson.sc"
	t0 = 0;

#line 5980 "vocoderSched.cc"
	__PRF_globalCounter[496u]++ ;

#line 105 "src/lp_analysis/levinson.sc"
	for(j = 1; j < i; j++ )
	{   

#line 5987 "vocoderSched.cc"
	    __PRF_globalCounter[497u]++ ;

#line 107 "src/lp_analysis/levinson.sc"
	    t0 = L_add(t0, Mpy_32(Rh[j], Rl[j], Ah[i - j], Al[i - j]));
	}

#line 5994 "vocoderSched.cc"
	__PRF_globalCounter[498u]++ ;

#line 109 "src/lp_analysis/levinson.sc"
	t0 = L_shl(t0, 4);

	t1 = L_Comp(Rh[i], Rl[i]);
	t0 = L_add(t0, t1);



	t1 = L_abs(t0);
	t2 = Div_32(t1, alp_h, alp_l);

#line 6008 "vocoderSched.cc"
	__PRF_globalCounter[499u]++ ;

#line 119 "src/lp_analysis/levinson.sc"
	if (t0 > 0)

#line 6014 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[500u]++ ;

#line 120 "src/lp_analysis/levinson.sc"
	    t2 = L_negate(t2);
	}

#line 6022 "vocoderSched.cc"
	__PRF_globalCounter[501u]++ ;

#line 121 "src/lp_analysis/levinson.sc"
	t2 = L_shl(t2, alp_exp);
	L_Extract(t2,  &Kh,  &Kl);

#line 6029 "vocoderSched.cc"
	__PRF_globalCounter[502u]++ ;

#line 125 "src/lp_analysis/levinson.sc"
	if (sub(i, 5) < 0)
	{   

#line 6036 "vocoderSched.cc"
	    __PRF_globalCounter[503u]++ ;

#line 127 "src/lp_analysis/levinson.sc"
	    rc[i - 1] = round(t2);
	}

#line 6043 "vocoderSched.cc"
	__PRF_globalCounter[504u]++ ;

#line 132 "src/lp_analysis/levinson.sc"
	if (sub(abs_s(Kh), 32750) > 0)
	{   

#line 6050 "vocoderSched.cc"
	    __PRF_globalCounter[505u]++ ;

#line 134 "src/lp_analysis/levinson.sc"
	    for(j = 0; j <= 10; j++ )
	    {   

#line 6057 "vocoderSched.cc"
		__PRF_globalCounter[506u]++ ;

#line 136 "src/lp_analysis/levinson.sc"
		A[j] = old_A[j];
	    }

#line 6064 "vocoderSched.cc"
	    __PRF_globalCounter[507u]++ ;

#line 139 "src/lp_analysis/levinson.sc"
	    for(j = 0; j < 4; j++ )
	    {   

#line 6071 "vocoderSched.cc"
		__PRF_globalCounter[508u]++ ;

#line 141 "src/lp_analysis/levinson.sc"
		rc[j] = 0;
	    }

#line 6078 "vocoderSched.cc"
	    __PRF_globalCounter[509u]++ ;

#line 144 "src/lp_analysis/levinson.sc"
	    return ;
	}

#line 6085 "vocoderSched.cc"
	__PRF_globalCounter[510u]++ ;

#line 152 "src/lp_analysis/levinson.sc"
	for(j = 1; j < i; j++ )
	{   

#line 6092 "vocoderSched.cc"
	    __PRF_globalCounter[511u]++ ;

#line 154 "src/lp_analysis/levinson.sc"
	    t0 = Mpy_32(Kh, Kl, Ah[i - j], Al[i - j]);
	    t0 = L_mac(t0, Ah[j], 32767);
	    t0 = L_mac(t0, Al[j], 1);
	    L_Extract(t0,  &Anh[j],  &Anl[j]);
	}

#line 6102 "vocoderSched.cc"
	__PRF_globalCounter[512u]++ ;

#line 159 "src/lp_analysis/levinson.sc"
	t2 = L_shr(t2, 4);
	L_Extract(t2,  &Anh[i],  &Anl[i]);



	t0 = Mpy_32(Kh, Kl, Kh, Kl);
	t0 = L_abs(t0);
	t0 = L_sub((int)2147483647l, t0);
	L_Extract(t0,  &hi,  &lo);
	t0 = Mpy_32(alp_h, alp_l, hi, lo);



	t0_n = norm_l(t0);
	t0 = L_shl(t0, t0_n);
	L_Extract(t0,  &alp_h,  &alp_l);
	alp_exp = add(alp_exp, t0_n);

#line 6124 "vocoderSched.cc"
	__PRF_globalCounter[513u]++ ;

#line 180 "src/lp_analysis/levinson.sc"
	for(j = 1; j <= i; j++ )
	{   

#line 6131 "vocoderSched.cc"
	    __PRF_globalCounter[514u]++ ;

#line 182 "src/lp_analysis/levinson.sc"
	    Ah[j] = Anh[j];
	    Al[j] = Anl[j];
	}
    }

#line 6140 "vocoderSched.cc"
    __PRF_globalCounter[515u]++ ;

#line 187 "src/lp_analysis/levinson.sc"
    A[0] = 4096;

#line 6146 "vocoderSched.cc"
    __PRF_globalCounter[516u]++ ;

#line 188 "src/lp_analysis/levinson.sc"
    for(i = 1; i <= 10; i++ )
    {   

#line 6153 "vocoderSched.cc"
	__PRF_globalCounter[517u]++ ;

#line 190 "src/lp_analysis/levinson.sc"
	t0 = L_Comp(Ah[i], Al[i]);
	A[i] = old_A[i] = round(L_shl(t0, 1));
    }
}

#line 6162 "vocoderSched.cc"
LP_Analysis_Init::LP_Analysis_Init(unsigned int _idcnt, bool (&reset_flag), short int (&old_A)[11], short int (&lsp_old)[10], short int (&lsp_old_q)[10])
    : _specc::behavior(_idcnt), reset_flag(reset_flag), old_A(old_A), lsp_old(lsp_old), lsp_old_q(lsp_old_q)
{   
}

LP_Analysis_Init::~LP_Analysis_Init(void)
{   
}

void LP_Analysis_Init::init(void)

#line 27 "src/lp_analysis/lp_analysis.sc"
{   
    int i;
    short int lsp[10];

#line 6179 "vocoderSched.cc"
    __PRF_globalCounter[473u]++ ;

#line 31 "src/lp_analysis/lp_analysis.sc"
    old_A[0] = 4096;

#line 6185 "vocoderSched.cc"
    __PRF_globalCounter[474u]++ ;

#line 32 "src/lp_analysis/lp_analysis.sc"
    for(i = 1; i < (10 + 1); i++ )
    {   

#line 6192 "vocoderSched.cc"
	__PRF_globalCounter[475u]++ ;

#line 34 "src/lp_analysis/lp_analysis.sc"
	old_A[i] = 0;
    }

#line 6199 "vocoderSched.cc"
    __PRF_globalCounter[476u]++ ;

#line 38 "src/lp_analysis/lp_analysis.sc"
    lsp[0] = 30000;
    lsp[1] = 26000;
    lsp[2] = 21000;
    lsp[3] = 15000;
    lsp[4] = 8000;
    lsp[5] = 0;
    lsp[6] =  -8000;
    lsp[7] =  -15000;
    lsp[8] =  -21000;
    lsp[9] =  -26000;

    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<10;_scc_index_0++) (lsp_old)[_scc_index_0] = (lsp)[_scc_index_0]; }
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<10;_scc_index_0++) (lsp_old_q)[_scc_index_0] = (lsp)[_scc_index_0]; }
}

#line 6218 "vocoderSched.cc"
void LP_Analysis_Init::main(void)

#line 60 "src/lp_analysis/lp_analysis.sc"
{   

#line 6224 "vocoderSched.cc"
    __PRF_globalCounter[477u]++ ;
    _specc::waitfor(0ull);
    __PRF_globalCounter[478u]++ ;

#line 61 "src/lp_analysis/lp_analysis.sc"
    if (reset_flag)

#line 6232 "vocoderSched.cc"
    {   
	__PRF_globalCounter[479u]++ ;

#line 61 "src/lp_analysis/lp_analysis.sc"
	init();
    }
}

#line 6241 "vocoderSched.cc"
void LP_Analysis_Init::reset(void)

#line 55 "src/lp_analysis/lp_analysis.sc"
{   

#line 6247 "vocoderSched.cc"
    __PRF_globalCounter[480u]++ ;

#line 56 "src/lp_analysis/lp_analysis.sc"
    init();
}

#line 6254 "vocoderSched.cc"
LP_Analysis_Seq1::LP_Analysis_Seq1(unsigned int _idcnt, short int *(&p_window), short int (&r_h)[11], short int (&r_l)[11], short int (&old_A)[11], short int (&scal_fac), short int (&A_t_1)[11], short int (&A_t_3)[11], short int (&rc)[4])
    : _specc::behavior(_idcnt), p_window(p_window), r_h(r_h), r_l(r_l), old_A(old_A), scal_fac(scal_fac), A_t_1(A_t_1), A_t_3(A_t_3), rc(rc),
    find_az_1(_IDcnt, p_window, window_160_80, dummy_r_h, dummy_r_l, old_A, dummy_fac, A_t_1, dummy_rc),
    find_az_2(_IDcnt, p_window, window_232_8, r_h, r_l, old_A, scal_fac, A_t_3, rc)
{   
}

LP_Analysis_Seq1::~LP_Analysis_Seq1(void)
{   
}

void LP_Analysis_Seq1::main(void)
{   
    __PRF_globalCounter[481u]++ ;
    find_az_1.main();
    find_az_2.main();
}

LP_Analysis_Seq2::LP_Analysis_Seq2(unsigned int _idcnt, short int (&lsp_old)[10], short int (&lsp_mid)[10], short int (&lsp_new)[10], short int (&lsp_old_q)[10], short int (&lsp_mid_q)[10], short int (&lsp_new_q)[10], short int (&A_t)[4][11], short int (&Aq_t)[4][11], short int (&ana)[57], short int (&txdtx_ctrl), bool (&reset_flag))
    : _specc::behavior(_idcnt), lsp_old(lsp_old), lsp_mid(lsp_mid), lsp_new(lsp_new), lsp_old_q(lsp_old_q), lsp_mid_q(lsp_mid_q), lsp_new_q(lsp_new_q), A_t(A_t), Aq_t(Aq_t), ana(ana), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag),
    int_lpc2(_IDcnt, lsp_old, lsp_mid, lsp_new, A_t),
    q_plsf_and_intlpc(_IDcnt, Aq_t, lsp_old, lsp_mid, lsp_new, lsp_old_q, lsp_mid_q, lsp_new_q, ana, txdtx_ctrl, reset_flag)
{   
}

LP_Analysis_Seq2::~LP_Analysis_Seq2(void)
{   
}

void LP_Analysis_Seq2::main(void)
{   
    __PRF_globalCounter[482u]++ ;
    int_lpc2.main();
    q_plsf_and_intlpc.main();
}

void LP_Analysis_Seq2::reset(void)

#line 125 "src/lp_analysis/lp_analysis.sc"
{   

#line 6296 "vocoderSched.cc"
    __PRF_globalCounter[483u]++ ;

#line 126 "src/lp_analysis/lp_analysis.sc"
    q_plsf_and_intlpc.reset();
}

#line 6303 "vocoderSched.cc"
LP_Analysis_Copy1::LP_Analysis_Copy1(unsigned int _idcnt, short int (&A_t_1)[11], short int (&A_t_3)[11], short int (&A_t)[4][11])
    : _specc::behavior(_idcnt), A_t_1(A_t_1), A_t_3(A_t_3), A_t(A_t)
{   
}

LP_Analysis_Copy1::~LP_Analysis_Copy1(void)
{   
}

void LP_Analysis_Copy1::main(void)

#line 148 "src/lp_analysis/lp_analysis.sc"
{   

#line 6318 "vocoderSched.cc"
    __PRF_globalCounter[471u]++ ;
    _specc::waitfor(0ull);

#line 149 "src/lp_analysis/lp_analysis.sc"
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<11;_scc_index_0++) (A_t[1])[_scc_index_0] = (A_t_1)[_scc_index_0]; }
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<11;_scc_index_0++) (A_t[3])[_scc_index_0] = (A_t_3)[_scc_index_0]; }
}

#line 6327 "vocoderSched.cc"
LP_Analysis_Copy2::LP_Analysis_Copy2(unsigned int _idcnt, short int (&A)[4][11], short int (&A_t)[4][11])
    : _specc::behavior(_idcnt), A(A), A_t(A_t)
{   
}

LP_Analysis_Copy2::~LP_Analysis_Copy2(void)
{   
}

void LP_Analysis_Copy2::main(void)

#line 162 "src/lp_analysis/lp_analysis.sc"
{   

#line 6342 "vocoderSched.cc"
    __PRF_globalCounter[472u]++ ;
    _specc::waitfor(0ull);

#line 163 "src/lp_analysis/lp_analysis.sc"
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<4;_scc_index_0++) { unsigned int _scc_index_1; for(_scc_index_1=0;_scc_index_1<11;_scc_index_1++) (A_t)[_scc_index_0][_scc_index_1] = (A)[_scc_index_0][_scc_index_1]; } }
}

#line 6350 "vocoderSched.cc"
No_Speech_Upd::No_Speech_Upd(unsigned int _idcnt, short int (&A_t)[4][11], short int (&Aq_t)[4][11], short int (&lsp_old)[10], short int (&lsp_new)[10], short int (&lsp_old_q)[10], short int (&lsp_new_q)[10], short int (&txdtx_ctrl))
    : _specc::behavior(_idcnt), A_t(A_t), Aq_t(Aq_t), lsp_old(lsp_old), lsp_new(lsp_new), lsp_old_q(lsp_old_q), lsp_new_q(lsp_new_q), txdtx_ctrl(txdtx_ctrl)
{   
}

No_Speech_Upd::~No_Speech_Upd(void)
{   
}

void No_Speech_Upd::main(void)

#line 16 "src/lp_analysis/no_speech_upd.sc"
{   
    int i; int j;

#line 6366 "vocoderSched.cc"
    __PRF_globalCounter[569u]++ ;
    _specc::waitfor(700000ull);
    __PRF_globalCounter[570u]++ ;

#line 19 "src/lp_analysis/no_speech_upd.sc"
    if ((txdtx_ctrl & 1) == 0)
    {   

#line 6375 "vocoderSched.cc"
	__PRF_globalCounter[571u]++ ;

#line 22 "src/lp_analysis/no_speech_upd.sc"
	for(i = 0; i < (10 + 1); i++ )
	{   

#line 6382 "vocoderSched.cc"
	    __PRF_globalCounter[572u]++ ;

#line 24 "src/lp_analysis/no_speech_upd.sc"
	    for(j = 0; j < 4; j++ )

#line 6388 "vocoderSched.cc"
	    {   
		__PRF_globalCounter[573u]++ ;

#line 25 "src/lp_analysis/no_speech_upd.sc"
		Aq_t[j][i] = A_t[j][i];
	    }
	}

#line 6397 "vocoderSched.cc"
	__PRF_globalCounter[574u]++ ;

#line 29 "src/lp_analysis/no_speech_upd.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 6404 "vocoderSched.cc"
	    __PRF_globalCounter[575u]++ ;

#line 31 "src/lp_analysis/no_speech_upd.sc"
	    lsp_old[i] = lsp_new[i];
	    lsp_old_q[i] = lsp_new[i];
	}
    }
    else 
    {   

#line 6415 "vocoderSched.cc"
	__PRF_globalCounter[576u]++ ;

#line 38 "src/lp_analysis/no_speech_upd.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 6422 "vocoderSched.cc"
	    __PRF_globalCounter[577u]++ ;

#line 40 "src/lp_analysis/no_speech_upd.sc"
	    lsp_old[i] = lsp_new[i];
	    lsp_old_q[i] = lsp_new_q[i];
	}
    }
}

#line 6432 "vocoderSched.cc"
Nodtx_Setflags::Nodtx_Setflags(unsigned int _idcnt, bool (&VAD_flag), short int (&txdtx_ctrl))
    : _specc::behavior(_idcnt), VAD_flag(VAD_flag), txdtx_ctrl(txdtx_ctrl)
{   
}

Nodtx_Setflags::~Nodtx_Setflags(void)
{   
}

void Nodtx_Setflags::main(void)

#line 12 "src/lp_analysis/nodtx_setflags.sc"
{   

#line 6447 "vocoderSched.cc"
    __PRF_globalCounter[578u]++ ;
    _specc::waitfor(50000ull);

#line 13 "src/lp_analysis/nodtx_setflags.sc"
    VAD_flag = 1;
    txdtx_ctrl = 2 | 1;
}

#line 6456 "vocoderSched.cc"
Q_Plsf_5::Q_Plsf_5(unsigned int _idcnt, short int (&lsp1)[10], short int (&lsp2)[10], short int (&lsp1_q)[10], short int (&lsp2_q)[10], short int (&indice)[57], short int (&txdtx_ctrl), bool (&reset_flag))
    : _specc::behavior(_idcnt), lsp1(lsp1), lsp2(lsp2), lsp1_q(lsp1_q), lsp2_q(lsp2_q), indice(indice), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag)
{   
}

Q_Plsf_5::~Q_Plsf_5(void)
{   
}

void Q_Plsf_5::Lsf_wt(

#line 432 "src/lp_analysis/q_plsf_5.sc"
    short int *lsf, 
    short int *wf)
{   
    short int temp;
    int i;

#line 6475 "vocoderSched.cc"
    __PRF_globalCounter[837u]++ ;

#line 440 "src/lp_analysis/q_plsf_5.sc"
    wf[0] = lsf[1];

#line 6481 "vocoderSched.cc"
    __PRF_globalCounter[838u]++ ;

#line 441 "src/lp_analysis/q_plsf_5.sc"
    for(i = 1; i < 9; i++ )
    {   

#line 6488 "vocoderSched.cc"
	__PRF_globalCounter[839u]++ ;

#line 443 "src/lp_analysis/q_plsf_5.sc"
	wf[i] = sub(lsf[i + 1], lsf[i - 1]);
    }

#line 6495 "vocoderSched.cc"
    __PRF_globalCounter[840u]++ ;

#line 446 "src/lp_analysis/q_plsf_5.sc"
    wf[9] = sub(16384, lsf[8]);

#line 6501 "vocoderSched.cc"
    __PRF_globalCounter[841u]++ ;

#line 448 "src/lp_analysis/q_plsf_5.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 6508 "vocoderSched.cc"
	__PRF_globalCounter[842u]++ ;

#line 450 "src/lp_analysis/q_plsf_5.sc"
	temp = sub(wf[i], 1843);

#line 6514 "vocoderSched.cc"
	__PRF_globalCounter[843u]++ ;

#line 452 "src/lp_analysis/q_plsf_5.sc"
	if (temp < 0)
	{   

#line 6521 "vocoderSched.cc"
	    __PRF_globalCounter[844u]++ ;

#line 454 "src/lp_analysis/q_plsf_5.sc"
	    wf[i] = sub(3427, mult(wf[i], 28160));
	}
	else 
	{   

#line 6530 "vocoderSched.cc"
	    __PRF_globalCounter[845u]++ ;

#line 458 "src/lp_analysis/q_plsf_5.sc"
	    wf[i] = sub(1843, mult(temp, 6242));
	}

#line 6537 "vocoderSched.cc"
	__PRF_globalCounter[846u]++ ;

#line 461 "src/lp_analysis/q_plsf_5.sc"
	wf[i] = shl(wf[i], 3);
    }

#line 6544 "vocoderSched.cc"
    __PRF_globalCounter[847u]++ ;

#line 463 "src/lp_analysis/q_plsf_5.sc"
    return ;
}

#line 6551 "vocoderSched.cc"
short int Q_Plsf_5::Vq_subvec(

#line 239 "src/lp_analysis/q_plsf_5.sc"
    short int *lsf_r1, 
    short int *lsf_r2, 
    const short int *dico, 
    short int *wf1, 
    short int *wf2, 
    short int dico_size)

{   
    int i;
    short int index; short int temp;
    const short int *p_dico;
    int dist; int dist_min;

#line 6568 "vocoderSched.cc"
    __PRF_globalCounter[848u]++ ;

#line 252 "src/lp_analysis/q_plsf_5.sc"
    dist_min = (int)2147483647l;
    p_dico = dico;

#line 6575 "vocoderSched.cc"
    __PRF_globalCounter[849u]++ ;

#line 255 "src/lp_analysis/q_plsf_5.sc"
    for(i = 0; i < dico_size; i++ )
    {   

#line 6582 "vocoderSched.cc"
	__PRF_globalCounter[850u]++ ;

#line 257 "src/lp_analysis/q_plsf_5.sc"
	temp = sub(lsf_r1[0],  *p_dico++ );
	temp = mult(wf1[0], temp);
	dist = L_mult(temp, temp);

	temp = sub(lsf_r1[1],  *p_dico++ );
	temp = mult(wf1[1], temp);
	dist = L_mac(dist, temp, temp);

	temp = sub(lsf_r2[0],  *p_dico++ );
	temp = mult(wf2[0], temp);
	dist = L_mac(dist, temp, temp);

	temp = sub(lsf_r2[1],  *p_dico++ );
	temp = mult(wf2[1], temp);
	dist = L_mac(dist, temp, temp);

#line 6602 "vocoderSched.cc"
	__PRF_globalCounter[851u]++ ;

#line 274 "src/lp_analysis/q_plsf_5.sc"
	if (L_sub(dist, dist_min) < (int)0)
	{   

#line 6609 "vocoderSched.cc"
	    __PRF_globalCounter[852u]++ ;

#line 276 "src/lp_analysis/q_plsf_5.sc"
	    dist_min = dist;
	    index = i;
	}
    }

#line 6618 "vocoderSched.cc"
    __PRF_globalCounter[853u]++ ;

#line 283 "src/lp_analysis/q_plsf_5.sc"
    p_dico =  &dico[shl(index, 2)];
    lsf_r1[0] =  *p_dico++ ;
    lsf_r1[1] =  *p_dico++ ;
    lsf_r2[0] =  *p_dico++ ;
    lsf_r2[1] =  *p_dico++ ;

    return index;
}

#line 6631 "vocoderSched.cc"
short int Q_Plsf_5::Vq_subvec_s(

#line 297 "src/lp_analysis/q_plsf_5.sc"
    short int *lsf_r1, 
    short int *lsf_r2, 
    const short int *dico, 
    short int *wf1, 
    short int *wf2, 
    short int dico_size)
{   
    int i;
    short int index; short int temp;
    bool sign;
    const short int *p_dico;
    int dist; int dist_min;

#line 6648 "vocoderSched.cc"
    __PRF_globalCounter[854u]++ ;

#line 310 "src/lp_analysis/q_plsf_5.sc"
    dist_min = (int)2147483647l;
    p_dico = dico;

#line 6655 "vocoderSched.cc"
    __PRF_globalCounter[855u]++ ;

#line 313 "src/lp_analysis/q_plsf_5.sc"
    for(i = 0; i < dico_size; i++ )
    {   

#line 6662 "vocoderSched.cc"
	__PRF_globalCounter[856u]++ ;

#line 317 "src/lp_analysis/q_plsf_5.sc"
	temp = sub(lsf_r1[0],  *p_dico++ );
	temp = mult(wf1[0], temp);
	dist = L_mult(temp, temp);

	temp = sub(lsf_r1[1],  *p_dico++ );
	temp = mult(wf1[1], temp);
	dist = L_mac(dist, temp, temp);

	temp = sub(lsf_r2[0],  *p_dico++ );
	temp = mult(wf2[0], temp);
	dist = L_mac(dist, temp, temp);

	temp = sub(lsf_r2[1],  *p_dico++ );
	temp = mult(wf2[1], temp);
	dist = L_mac(dist, temp, temp);

#line 6682 "vocoderSched.cc"
	__PRF_globalCounter[857u]++ ;

#line 334 "src/lp_analysis/q_plsf_5.sc"
	if (L_sub(dist, dist_min) < (int)0)
	{   

#line 6689 "vocoderSched.cc"
	    __PRF_globalCounter[858u]++ ;

#line 336 "src/lp_analysis/q_plsf_5.sc"
	    dist_min = dist;
	    index = i;
	    sign = 0;
	}

#line 6698 "vocoderSched.cc"
	__PRF_globalCounter[859u]++ ;

#line 344 "src/lp_analysis/q_plsf_5.sc"
	p_dico -= 4;
	temp = add(lsf_r1[0],  *p_dico++ );
	temp = mult(wf1[0], temp);
	dist = L_mult(temp, temp);

	temp = add(lsf_r1[1],  *p_dico++ );
	temp = mult(wf1[1], temp);
	dist = L_mac(dist, temp, temp);

	temp = add(lsf_r2[0],  *p_dico++ );
	temp = mult(wf2[0], temp);
	dist = L_mac(dist, temp, temp);

	temp = add(lsf_r2[1],  *p_dico++ );
	temp = mult(wf2[1], temp);
	dist = L_mac(dist, temp, temp);

#line 6719 "vocoderSched.cc"
	__PRF_globalCounter[860u]++ ;

#line 362 "src/lp_analysis/q_plsf_5.sc"
	if (L_sub(dist, dist_min) < (int)0)
	{   

#line 6726 "vocoderSched.cc"
	    __PRF_globalCounter[861u]++ ;

#line 364 "src/lp_analysis/q_plsf_5.sc"
	    dist_min = dist;
	    index = i;
	    sign = 1;
	}
    }

#line 6736 "vocoderSched.cc"
    __PRF_globalCounter[862u]++ ;

#line 372 "src/lp_analysis/q_plsf_5.sc"
    p_dico =  &dico[shl(index, 2)];

#line 6742 "vocoderSched.cc"
    __PRF_globalCounter[863u]++ ;

#line 374 "src/lp_analysis/q_plsf_5.sc"
    if (sign == 0)
    {   

#line 6749 "vocoderSched.cc"
	__PRF_globalCounter[864u]++ ;

#line 376 "src/lp_analysis/q_plsf_5.sc"
	lsf_r1[0] =  *p_dico++ ;
	lsf_r1[1] =  *p_dico++ ;
	lsf_r2[0] =  *p_dico++ ;
	lsf_r2[1] =  *p_dico++ ;
    }
    else 
    {   

#line 6761 "vocoderSched.cc"
	__PRF_globalCounter[865u]++ ;

#line 383 "src/lp_analysis/q_plsf_5.sc"
	lsf_r1[0] = negate( *p_dico++ );
	lsf_r1[1] = negate( *p_dico++ );
	lsf_r2[0] = negate( *p_dico++ );
	lsf_r2[1] = negate( *p_dico++ );
    }

#line 6771 "vocoderSched.cc"
    __PRF_globalCounter[866u]++ ;

#line 389 "src/lp_analysis/q_plsf_5.sc"
    index = shl(index, 1);
    index = add(index, sign);

    return index;
}

#line 6781 "vocoderSched.cc"
void Q_Plsf_5::aver_lsf_history(

#line 209 "src/lp_analysis/q_plsf_5.sc"
    short int lsf_old[7][10], 
    short int lsf1[10], 
    short int lsf2[10], 
    short int lsf_aver[10])

{   
    int i; int j;
    int L_temp;

#line 6794 "vocoderSched.cc"
    __PRF_globalCounter[867u]++ ;

#line 218 "src/lp_analysis/q_plsf_5.sc"
    for(j = 0; j < 10; j++ )
    {   

#line 6801 "vocoderSched.cc"
	__PRF_globalCounter[868u]++ ;

#line 220 "src/lp_analysis/q_plsf_5.sc"
	L_temp = L_mult(16383, lsf1[j]);
	L_temp = L_mac(L_temp, 16383, lsf2[j]);
	L_temp = L_mult((32767 / (7 + 1)), extract_h(L_temp));

#line 6809 "vocoderSched.cc"
	__PRF_globalCounter[869u]++ ;

#line 224 "src/lp_analysis/q_plsf_5.sc"
	for(i = 0; i < 7; i++ )
	{   

#line 6816 "vocoderSched.cc"
	    __PRF_globalCounter[870u]++ ;

#line 226 "src/lp_analysis/q_plsf_5.sc"
	    L_temp = L_mac(L_temp, (32767 / (7 + 1)), lsf_old[i][j]);
	}

#line 6823 "vocoderSched.cc"
	__PRF_globalCounter[871u]++ ;

#line 229 "src/lp_analysis/q_plsf_5.sc"
	lsf_aver[j] = extract_h(L_temp);
    }

#line 6830 "vocoderSched.cc"
    __PRF_globalCounter[872u]++ ;

#line 232 "src/lp_analysis/q_plsf_5.sc"
    return ;
}

#line 6837 "vocoderSched.cc"
void Q_Plsf_5::init(void)

#line 67 "src/lp_analysis/q_plsf_5.sc"
{   
    int i;

#line 6844 "vocoderSched.cc"
    __PRF_globalCounter[873u]++ ;

#line 70 "src/lp_analysis/q_plsf_5.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 6851 "vocoderSched.cc"
	__PRF_globalCounter[874u]++ ;

#line 72 "src/lp_analysis/q_plsf_5.sc"
	past_r2_q[i] = 0;
    }

#line 6858 "vocoderSched.cc"
    __PRF_globalCounter[875u]++ ;

#line 75 "src/lp_analysis/q_plsf_5.sc"
    for(i = 0; i < 7; i++ )
    {   

#line 6865 "vocoderSched.cc"
	__PRF_globalCounter[876u]++ ;

#line 77 "src/lp_analysis/q_plsf_5.sc"
	lsf_old_tx[i][0] = 1384;
	lsf_old_tx[i][1] = 2077;
	lsf_old_tx[i][2] = 3420;
	lsf_old_tx[i][3] = 5108;
	lsf_old_tx[i][4] = 6742;
	lsf_old_tx[i][5] = 8122;
	lsf_old_tx[i][6] = 9863;
	lsf_old_tx[i][7] = 11092;
	lsf_old_tx[i][8] = 12714;
	lsf_old_tx[i][9] = 13701;
    }
}

#line 6882 "vocoderSched.cc"
void Q_Plsf_5::lsf_lsp(

#line 485 "src/lp_analysis/q_plsf_5.sc"
    short int lsf[], 
    short int *lsp, 
    short int m)

{   
    int i; int ind;
    short int offset;
    int L_tmp;

#line 6895 "vocoderSched.cc"
    __PRF_globalCounter[877u]++ ;

#line 494 "src/lp_analysis/q_plsf_5.sc"
    for(i = 0; i < m; i++ )
    {   

#line 6902 "vocoderSched.cc"
	__PRF_globalCounter[878u]++ ;

#line 496 "src/lp_analysis/q_plsf_5.sc"
	ind = shr(lsf[i], 8);
	offset = lsf[i] & 255;



	L_tmp = L_mult(sub(table[ind + 1], table[ind]), offset);
	lsp[i] = add(table[ind], extract_l(L_shr(L_tmp, 9)));
    }

#line 6915 "vocoderSched.cc"
    __PRF_globalCounter[879u]++ ;

#line 505 "src/lp_analysis/q_plsf_5.sc"
    return ;
}

#line 6922 "vocoderSched.cc"
void Q_Plsf_5::lsp_lsf(

#line 509 "src/lp_analysis/q_plsf_5.sc"
    short int lsp[], 
    short int *lsf, 
    short int m)

{   
    int i; int ind;
    int L_tmp;

#line 6934 "vocoderSched.cc"
    __PRF_globalCounter[880u]++ ;

#line 517 "src/lp_analysis/q_plsf_5.sc"
    ind = 63;

#line 6940 "vocoderSched.cc"
    __PRF_globalCounter[881u]++ ;

#line 519 "src/lp_analysis/q_plsf_5.sc"
    for(i = m - 1; i >= 0; i-- )
    {   

#line 6947 "vocoderSched.cc"
	__PRF_globalCounter[882u]++ ;

#line 523 "src/lp_analysis/q_plsf_5.sc"
	while(sub(table[ind], lsp[i]) < 0)
	{   

#line 6954 "vocoderSched.cc"
	    __PRF_globalCounter[883u]++ ;

#line 525 "src/lp_analysis/q_plsf_5.sc"
	    ind-- ;
	}

#line 6961 "vocoderSched.cc"
	__PRF_globalCounter[884u]++ ;

#line 532 "src/lp_analysis/q_plsf_5.sc"
	L_tmp = L_mult(sub(lsp[i], table[ind]), slope[ind]);

	lsf[i] = round(L_shl(L_tmp, 3));
	lsf[i] = add(lsf[i], shl(ind, 8));
    }

#line 6971 "vocoderSched.cc"
    __PRF_globalCounter[885u]++ ;

#line 537 "src/lp_analysis/q_plsf_5.sc"
    return ;
}

#line 6978 "vocoderSched.cc"
void Q_Plsf_5::main(void)

#line 579 "src/lp_analysis/q_plsf_5.sc"
{   
    int i;
    short int lsf1[10]; short int lsf2[10]; short int lsf_p[10]; short int lsf_r1[10]; short int lsf_r2[10]; short int wf1[10]; short int wf2[10];
    short int lsf1_q[10]; short int lsf2_q[10];
    short int lsf_aver[10];
    static short int lsf_p_CN[10];

#line 6989 "vocoderSched.cc"
    __PRF_globalCounter[886u]++ ;
    _specc::waitfor(293533333ull);
    __PRF_globalCounter[887u]++ ;

#line 586 "src/lp_analysis/q_plsf_5.sc"
    if (reset_flag)

#line 6997 "vocoderSched.cc"
    {   
	__PRF_globalCounter[888u]++ ;

#line 586 "src/lp_analysis/q_plsf_5.sc"
	init();
    }

#line 7005 "vocoderSched.cc"
    __PRF_globalCounter[889u]++ ;

#line 590 "src/lp_analysis/q_plsf_5.sc"
    lsp_lsf(lsp1, lsf1, 10);
    lsp_lsf(lsp2, lsf2, 10);

#line 7012 "vocoderSched.cc"
    __PRF_globalCounter[890u]++ ;

#line 596 "src/lp_analysis/q_plsf_5.sc"
    if ((txdtx_ctrl & 1) == 0 && 
	(txdtx_ctrl & 8) != 0)
    {   

#line 7020 "vocoderSched.cc"
	__PRF_globalCounter[891u]++ ;

#line 599 "src/lp_analysis/q_plsf_5.sc"
	update_lsf_p_CN(lsf_old_tx, lsf_p_CN);
    }

#line 7027 "vocoderSched.cc"
    __PRF_globalCounter[892u]++ ;

#line 602 "src/lp_analysis/q_plsf_5.sc"
    if ((txdtx_ctrl & 16) != 0)
    {   

#line 7034 "vocoderSched.cc"
	__PRF_globalCounter[893u]++ ;

#line 607 "src/lp_analysis/q_plsf_5.sc"
	aver_lsf_history(lsf_old_tx, lsf1, lsf2, lsf_aver);
    }

#line 7041 "vocoderSched.cc"
    __PRF_globalCounter[894u]++ ;

#line 613 "src/lp_analysis/q_plsf_5.sc"
    if ((txdtx_ctrl & 1) == 0)
    {   

#line 7048 "vocoderSched.cc"
	__PRF_globalCounter[895u]++ ;

#line 615 "src/lp_analysis/q_plsf_5.sc"
	update_lsf_history(lsf1, lsf2, lsf_old_tx);
    }

#line 7055 "vocoderSched.cc"
    __PRF_globalCounter[896u]++ ;

#line 618 "src/lp_analysis/q_plsf_5.sc"
    if ((txdtx_ctrl & 16) != 0)
    {   

#line 7062 "vocoderSched.cc"
	__PRF_globalCounter[897u]++ ;

#line 624 "src/lp_analysis/q_plsf_5.sc"
	Lsf_wt(lsf_aver, wf2);

#line 7068 "vocoderSched.cc"
	__PRF_globalCounter[898u]++ ;

#line 625 "src/lp_analysis/q_plsf_5.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 7075 "vocoderSched.cc"
	    __PRF_globalCounter[899u]++ ;

#line 627 "src/lp_analysis/q_plsf_5.sc"
	    wf1[i] = 0;
	    lsf1[i] = lsf_aver[i];
	    lsf2[i] = lsf_aver[i];
	}
    }
    else 
    {   

#line 7087 "vocoderSched.cc"
	__PRF_globalCounter[900u]++ ;

#line 636 "src/lp_analysis/q_plsf_5.sc"
	Lsf_wt(lsf1, wf1);
	Lsf_wt(lsf2, wf2);
    }

#line 7095 "vocoderSched.cc"
    __PRF_globalCounter[901u]++ ;

#line 643 "src/lp_analysis/q_plsf_5.sc"
    if ((txdtx_ctrl & 1) != 0)
    {   

#line 7102 "vocoderSched.cc"
	__PRF_globalCounter[902u]++ ;

#line 645 "src/lp_analysis/q_plsf_5.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 7109 "vocoderSched.cc"
	    __PRF_globalCounter[903u]++ ;

#line 647 "src/lp_analysis/q_plsf_5.sc"
	    lsf_p[i] = add(mean_lsf[i], mult(past_r2_q[i], 21299));

	    lsf_r1[i] = sub(lsf1[i], lsf_p[i]);
	    lsf_r2[i] = sub(lsf2[i], lsf_p[i]);
	}
    }
    else 
    {   

#line 7122 "vocoderSched.cc"
	__PRF_globalCounter[904u]++ ;

#line 655 "src/lp_analysis/q_plsf_5.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 7129 "vocoderSched.cc"
	    __PRF_globalCounter[905u]++ ;

#line 657 "src/lp_analysis/q_plsf_5.sc"
	    lsf_r1[i] = sub(lsf1[i], lsf_p_CN[i]);
	    lsf_r2[i] = sub(lsf2[i], lsf_p_CN[i]);
	}
    }

#line 7138 "vocoderSched.cc"
    __PRF_globalCounter[906u]++ ;

#line 664 "src/lp_analysis/q_plsf_5.sc"
    indice[0] = Vq_subvec( &lsf_r1[0],  &lsf_r2[0], dico1_lsf,  &
	wf1[0],  &wf2[0], 128);


    indice[1] = Vq_subvec( &lsf_r1[2],  &lsf_r2[2], dico2_lsf,  &
	wf1[2],  &wf2[2], 256);


    indice[2] = Vq_subvec_s( &lsf_r1[4],  &lsf_r2[4], dico3_lsf,  &
	wf1[4],  &wf2[4], 256);


    indice[3] = Vq_subvec( &lsf_r1[6],  &lsf_r2[6], dico4_lsf,  &
	wf1[6],  &wf2[6], 256);


    indice[4] = Vq_subvec( &lsf_r1[8],  &lsf_r2[8], dico5_lsf,  &
	wf1[8],  &wf2[8], 64);

#line 7161 "vocoderSched.cc"
    __PRF_globalCounter[907u]++ ;

#line 689 "src/lp_analysis/q_plsf_5.sc"
    if ((txdtx_ctrl & 1) != 0)
    {   

#line 7168 "vocoderSched.cc"
	__PRF_globalCounter[908u]++ ;

#line 691 "src/lp_analysis/q_plsf_5.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 7175 "vocoderSched.cc"
	    __PRF_globalCounter[909u]++ ;

#line 693 "src/lp_analysis/q_plsf_5.sc"
	    lsf1_q[i] = add(lsf_r1[i], lsf_p[i]);
	    lsf2_q[i] = add(lsf_r2[i], lsf_p[i]);
	    past_r2_q[i] = lsf_r2[i];
	}

#line 7184 "vocoderSched.cc"
	__PRF_globalCounter[910u]++ ;

#line 700 "src/lp_analysis/q_plsf_5.sc"
	reorder_lsf(lsf1_q, 205, 10);
	reorder_lsf(lsf2_q, 205, 10);

#line 7191 "vocoderSched.cc"
	__PRF_globalCounter[911u]++ ;

#line 707 "src/lp_analysis/q_plsf_5.sc"
	if ((txdtx_ctrl & 4) != 0)
	{   

#line 7198 "vocoderSched.cc"
	    __PRF_globalCounter[912u]++ ;

#line 709 "src/lp_analysis/q_plsf_5.sc"
	    update_lsf_history(lsf1_q, lsf2_q, lsf_old_tx);
	}

#line 7205 "vocoderSched.cc"
	__PRF_globalCounter[913u]++ ;

#line 713 "src/lp_analysis/q_plsf_5.sc"
	lsf_lsp(lsf1_q,  &lsp1_q[0], 10);
	lsf_lsp(lsf2_q,  &lsp2_q[0], 10);
    }
    else 
    {   

#line 7215 "vocoderSched.cc"
	__PRF_globalCounter[914u]++ ;

#line 718 "src/lp_analysis/q_plsf_5.sc"
	for(i = 0; i < 10; i++ )
	{   

#line 7222 "vocoderSched.cc"
	    __PRF_globalCounter[915u]++ ;

#line 720 "src/lp_analysis/q_plsf_5.sc"
	    past_r2_q[i] = 0;
	}
    }
}

#line 7231 "vocoderSched.cc"
void Q_Plsf_5::reorder_lsf(

#line 555 "src/lp_analysis/q_plsf_5.sc"
    short int *lsf, 
    short int min_dist, 
    short int n)

{   
    int i;
    short int lsf_min;

#line 7243 "vocoderSched.cc"
    __PRF_globalCounter[916u]++ ;

#line 563 "src/lp_analysis/q_plsf_5.sc"
    lsf_min = min_dist;

#line 7249 "vocoderSched.cc"
    __PRF_globalCounter[917u]++ ;

#line 564 "src/lp_analysis/q_plsf_5.sc"
    for(i = 0; i < n; i++ )
    {   

#line 7256 "vocoderSched.cc"
	__PRF_globalCounter[918u]++ ;

#line 567 "src/lp_analysis/q_plsf_5.sc"
	if (sub(lsf[i], lsf_min) < 0)
	{   

#line 7263 "vocoderSched.cc"
	    __PRF_globalCounter[919u]++ ;

#line 569 "src/lp_analysis/q_plsf_5.sc"
	    lsf[i] = lsf_min;
	}

#line 7270 "vocoderSched.cc"
	__PRF_globalCounter[920u]++ ;

#line 571 "src/lp_analysis/q_plsf_5.sc"
	lsf_min = add(lsf[i], min_dist);
    }
}

#line 7278 "vocoderSched.cc"
void Q_Plsf_5::reset(void)

#line 92 "src/lp_analysis/q_plsf_5.sc"
{   

#line 7284 "vocoderSched.cc"
    __PRF_globalCounter[921u]++ ;

#line 93 "src/lp_analysis/q_plsf_5.sc"
    init();
}

#line 7291 "vocoderSched.cc"
void Q_Plsf_5::update_lsf_history(

#line 120 "src/lp_analysis/q_plsf_5.sc"
    short int lsf1[10], 
    short int lsf2[10], 
    short int lsf_old[7][10])

{   
    int i; int j;
    short int temp;

#line 7303 "vocoderSched.cc"
    __PRF_globalCounter[922u]++ ;

#line 131 "src/lp_analysis/q_plsf_5.sc"
    for(i = 7 - 1; i > 0; i-- )
    {   

#line 7310 "vocoderSched.cc"
	__PRF_globalCounter[923u]++ ;

#line 133 "src/lp_analysis/q_plsf_5.sc"
	for(j = 0; j < 10; j++ )
	{   

#line 7317 "vocoderSched.cc"
	    __PRF_globalCounter[924u]++ ;

#line 135 "src/lp_analysis/q_plsf_5.sc"
	    lsf_old[i][j] = lsf_old[i - 1][j];
	}
    }

#line 7325 "vocoderSched.cc"
    __PRF_globalCounter[925u]++ ;

#line 141 "src/lp_analysis/q_plsf_5.sc"
    for(i = 0; i < 10; i++ )
    {   

#line 7332 "vocoderSched.cc"
	__PRF_globalCounter[926u]++ ;

#line 143 "src/lp_analysis/q_plsf_5.sc"
	temp = add(shr(lsf1[i], 1), shr(lsf2[i], 1));
	lsf_old[0][i] = temp;
    }

#line 7340 "vocoderSched.cc"
    __PRF_globalCounter[927u]++ ;

#line 147 "src/lp_analysis/q_plsf_5.sc"
    return ;
}

#line 7347 "vocoderSched.cc"
void Q_Plsf_5::update_lsf_p_CN(

#line 168 "src/lp_analysis/q_plsf_5.sc"
    short int lsf_old[7][10], 
    short int lsf_p_CN[10])

{   
    int i; int j;
    int L_temp;

#line 7358 "vocoderSched.cc"
    __PRF_globalCounter[928u]++ ;

#line 175 "src/lp_analysis/q_plsf_5.sc"
    for(j = 0; j < 10; j++ )
    {   

#line 7365 "vocoderSched.cc"
	__PRF_globalCounter[929u]++ ;

#line 177 "src/lp_analysis/q_plsf_5.sc"
	L_temp = L_mult((32767 / 7), lsf_old[0][j]);

#line 7371 "vocoderSched.cc"
	__PRF_globalCounter[930u]++ ;

#line 178 "src/lp_analysis/q_plsf_5.sc"
	for(i = 1; i < 7; i++ )
	{   

#line 7378 "vocoderSched.cc"
	    __PRF_globalCounter[931u]++ ;

#line 180 "src/lp_analysis/q_plsf_5.sc"
	    L_temp = L_mac(L_temp, (32767 / 7), lsf_old[i][j]);
	}

#line 7385 "vocoderSched.cc"
	__PRF_globalCounter[932u]++ ;

#line 182 "src/lp_analysis/q_plsf_5.sc"
	lsf_p_CN[j] = round(L_temp);
    }

#line 7392 "vocoderSched.cc"
    __PRF_globalCounter[933u]++ ;

#line 185 "src/lp_analysis/q_plsf_5.sc"
    return ;
}

#line 7399 "vocoderSched.cc"
Q_Plsf_And_Intlpc::Q_Plsf_And_Intlpc(unsigned int _idcnt, short int (&Aq_t)[4][11], short int (&lsp_old)[10], short int (&lsp_mid)[10], short int (&lsp_new)[10], short int (&lsp_old_q)[10], short int (&lsp_mid_q)[10], short int (&lsp_new_q)[10], short int (&ana)[57], short int (&txdtx_ctrl), bool (&reset_flag))
    : _specc::behavior(_idcnt), Aq_t(Aq_t), lsp_old(lsp_old), lsp_mid(lsp_mid), lsp_new(lsp_new), lsp_old_q(lsp_old_q), lsp_mid_q(lsp_mid_q), lsp_new_q(lsp_new_q), ana(ana), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag),
    int_lpc(_IDcnt, lsp_old_q, lsp_mid_q, lsp_new_q, Aq_t),
    q_plsf_5(_IDcnt, lsp_mid, lsp_new, lsp_mid_q, lsp_new_q, ana, txdtx_ctrl, reset_flag)
{   
}

Q_Plsf_And_Intlpc::~Q_Plsf_And_Intlpc(void)
{   
}

void Q_Plsf_And_Intlpc::main(void)

#line 38 "src/lp_analysis/q_plsf_and_intlpc.sc"
{   

#line 7416 "vocoderSched.cc"
    __PRF_globalCounter[934u]++ ;

#line 39 "src/lp_analysis/q_plsf_and_intlpc.sc"
    { enum { _scc_state_0, _scc_state_q_plsf_5, _scc_state_int_lpc } _scc_next_state = _scc_state_q_plsf_5; do switch(_scc_next_state) {


	    case _scc_state_q_plsf_5: { q_plsf_5.main(); if (__PRF_globalCounter[935u]++  , (txdtx_ctrl & 1) == 0) { _scc_next_state = _scc_state_0; break; } }



	    case _scc_state_int_lpc: { int_lpc.main(); if (__PRF_globalCounter[936u]++  , 1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

#line 7431 "vocoderSched.cc"
void Q_Plsf_And_Intlpc::reset(void)

#line 32 "src/lp_analysis/q_plsf_and_intlpc.sc"
{   

#line 7437 "vocoderSched.cc"
    __PRF_globalCounter[937u]++ ;

#line 33 "src/lp_analysis/q_plsf_and_intlpc.sc"
    q_plsf_5.reset();
}

#line 7444 "vocoderSched.cc"
TX_Dtx::TX_Dtx(unsigned int _idcnt, bool (&VAD_flag), short int (&txdtx_ctrl), int (&L_pn_seed_tx), bool (&reset_flag))
    : _specc::behavior(_idcnt), VAD_flag(VAD_flag), txdtx_ctrl(txdtx_ctrl), L_pn_seed_tx(L_pn_seed_tx), reset_flag(reset_flag)
{   
}

TX_Dtx::~TX_Dtx(void)
{   
}

void TX_Dtx::init(void)

#line 67 "src/lp_analysis/tx_dtx.sc"
{   

#line 7459 "vocoderSched.cc"
    __PRF_globalCounter[1158u]++ ;

#line 68 "src/lp_analysis/tx_dtx.sc"
    txdtx_hangover = 7;
    txdtx_N_elapsed = 32767;

    L_pn_seed_tx = 1887529304l;
}

#line 7469 "vocoderSched.cc"
void TX_Dtx::main(void)

#line 80 "src/lp_analysis/tx_dtx.sc"
{   

#line 7475 "vocoderSched.cc"
    __PRF_globalCounter[1159u]++ ;
    _specc::waitfor(0ull);
    __PRF_globalCounter[1160u]++ ;

#line 81 "src/lp_analysis/tx_dtx.sc"
    if (reset_flag)

#line 7483 "vocoderSched.cc"
    {   
	__PRF_globalCounter[1161u]++ ;

#line 81 "src/lp_analysis/tx_dtx.sc"
	init();
    }

#line 7491 "vocoderSched.cc"
    __PRF_globalCounter[1162u]++ ;

#line 86 "src/lp_analysis/tx_dtx.sc"
    txdtx_N_elapsed = add(txdtx_N_elapsed, 1);

#line 7497 "vocoderSched.cc"
    __PRF_globalCounter[1163u]++ ;

#line 91 "src/lp_analysis/tx_dtx.sc"
    if (sub(VAD_flag, 1) == 0)
    {   

#line 7504 "vocoderSched.cc"
	__PRF_globalCounter[1164u]++ ;

#line 93 "src/lp_analysis/tx_dtx.sc"
	txdtx_hangover = 7;
	txdtx_ctrl = 1 | 2;
    }
    else 
    {   

#line 7514 "vocoderSched.cc"
	__PRF_globalCounter[1165u]++ ;

#line 99 "src/lp_analysis/tx_dtx.sc"
	if (txdtx_hangover == 0)
	{   

#line 7521 "vocoderSched.cc"
	    __PRF_globalCounter[1166u]++ ;

#line 103 "src/lp_analysis/tx_dtx.sc"
	    txdtx_N_elapsed = 0;

#line 7527 "vocoderSched.cc"
	    __PRF_globalCounter[1167u]++ ;

#line 107 "src/lp_analysis/tx_dtx.sc"
	    if ((txdtx_ctrl & 4) != 0)
	    {   

#line 7534 "vocoderSched.cc"
		__PRF_globalCounter[1168u]++ ;

#line 109 "src/lp_analysis/tx_dtx.sc"
		txdtx_ctrl = 8 | 
		16;
		L_pn_seed_tx = 1887529304l;
	    }
	    else 
	    {   

#line 7545 "vocoderSched.cc"
		__PRF_globalCounter[1169u]++ ;

#line 115 "src/lp_analysis/tx_dtx.sc"
		txdtx_ctrl = 16;
	    }
	}
	else 
	{   

#line 7555 "vocoderSched.cc"
	    __PRF_globalCounter[1170u]++ ;

#line 121 "src/lp_analysis/tx_dtx.sc"
	    txdtx_hangover = sub(txdtx_hangover, 1);

#line 7561 "vocoderSched.cc"
	    __PRF_globalCounter[1171u]++ ;

#line 131 "src/lp_analysis/tx_dtx.sc"
	    if (sub(add(txdtx_N_elapsed, txdtx_hangover), 
		    (24 + 7 - 1)) < 0)
	    {   

#line 7569 "vocoderSched.cc"
		__PRF_globalCounter[1172u]++ ;

#line 135 "src/lp_analysis/tx_dtx.sc"
		txdtx_ctrl = 32;
	    }
	    else 
	    {   

#line 7578 "vocoderSched.cc"
		__PRF_globalCounter[1173u]++ ;

#line 140 "src/lp_analysis/tx_dtx.sc"
		if ((txdtx_ctrl & 4) != 0)
		{   

#line 7585 "vocoderSched.cc"
		    __PRF_globalCounter[1174u]++ ;

#line 142 "src/lp_analysis/tx_dtx.sc"
		    txdtx_ctrl = 8 | 
		    4 | 
		    1;
		}
		else 
		{   

#line 7596 "vocoderSched.cc"
		    __PRF_globalCounter[1175u]++ ;

#line 148 "src/lp_analysis/tx_dtx.sc"
		    txdtx_ctrl = 4 | 
		    1;
		}
	    }
	}
    }
}

#line 7608 "vocoderSched.cc"
void TX_Dtx::reset(void)

#line 75 "src/lp_analysis/tx_dtx.sc"
{   

#line 7614 "vocoderSched.cc"
    __PRF_globalCounter[1176u]++ ;

#line 76 "src/lp_analysis/tx_dtx.sc"
    init();
}

#line 7621 "vocoderSched.cc"
VAD_Computation::VAD_Computation(unsigned int _idcnt, short int (&r_h)[11], short int (&r_l)[11], short int (&scal_acf), short int (&rc)[4], bool (&ptch), bool (&vad), bool (&reset_flag))
    : _specc::behavior(_idcnt), r_h(r_h), r_l(r_l), scal_acf(scal_acf), rc(rc), ptch(ptch), vad(vad), reset_flag(reset_flag)
{   
}

VAD_Computation::~VAD_Computation(void)
{   
}

void VAD_Computation::Acf_averaging(

#line 272 "src/lp_analysis/vad_comp.sc"
    short int Aa_r_h[], 
    short int Aa_r_l[], 
    short int Aa_scal_acf, 
    int L_av0[], 
    int L_av1[])

{   
    int L_temp;
    short int scale;
    int i;

#line 7645 "vocoderSched.cc"
    __PRF_globalCounter[1185u]++ ;

#line 283 "src/lp_analysis/vad_comp.sc"
    scale = add(9, Aa_scal_acf);

#line 7651 "vocoderSched.cc"
    __PRF_globalCounter[1186u]++ ;

#line 285 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i <= 8; i++ )
    {   

#line 7658 "vocoderSched.cc"
	__PRF_globalCounter[1187u]++ ;

#line 287 "src/lp_analysis/vad_comp.sc"
	L_temp = L_shr(L_Comp(Aa_r_h[i], Aa_r_l[i]), scale);
	L_av0[i] = L_add(L_sacf[i], L_temp);
	L_av0[i] = L_add(L_sacf[i + 9], L_av0[i]);
	L_av0[i] = L_add(L_sacf[i + 18], L_av0[i]);
	L_sacf[pt_sacf + i] = L_temp;
	L_av1[i] = L_sav0[pt_sav0 + i];
	L_sav0[pt_sav0 + i] = L_av0[i];
    }

#line 7671 "vocoderSched.cc"
    __PRF_globalCounter[1188u]++ ;

#line 299 "src/lp_analysis/vad_comp.sc"
    if (sub(pt_sacf, 18) == 0)
    {   

#line 7678 "vocoderSched.cc"
	__PRF_globalCounter[1189u]++ ;

#line 301 "src/lp_analysis/vad_comp.sc"
	pt_sacf = 0;
    }
    else 
    {   

#line 7687 "vocoderSched.cc"
	__PRF_globalCounter[1190u]++ ;

#line 305 "src/lp_analysis/vad_comp.sc"
	pt_sacf = add(pt_sacf, 9);
    }

#line 7694 "vocoderSched.cc"
    __PRF_globalCounter[1191u]++ ;

#line 309 "src/lp_analysis/vad_comp.sc"
    if (sub(pt_sav0, 27) == 0)
    {   

#line 7701 "vocoderSched.cc"
	__PRF_globalCounter[1192u]++ ;

#line 311 "src/lp_analysis/vad_comp.sc"
	pt_sav0 = 0;
    }
    else 
    {   

#line 7710 "vocoderSched.cc"
	__PRF_globalCounter[1193u]++ ;

#line 315 "src/lp_analysis/vad_comp.sc"
	pt_sav0 = add(pt_sav0, 9);
    }

#line 7717 "vocoderSched.cc"
    __PRF_globalCounter[1194u]++ ;

#line 318 "src/lp_analysis/vad_comp.sc"
    return ;
}

#line 7724 "vocoderSched.cc"
void VAD_Computation::Compute_rav1(

#line 493 "src/lp_analysis/vad_comp.sc"
    short int aav1[], 
    short int rav1[], 
    short int *scal_rav1)

{   
    int L_work[9];
    int i; int k;

#line 7736 "vocoderSched.cc"
    __PRF_globalCounter[1195u]++ ;

#line 503 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i <= 8; i++ )
    {   

#line 7743 "vocoderSched.cc"
	__PRF_globalCounter[1196u]++ ;

#line 505 "src/lp_analysis/vad_comp.sc"
	L_work[i] = 0l;

#line 7749 "vocoderSched.cc"
	__PRF_globalCounter[1197u]++ ;

#line 507 "src/lp_analysis/vad_comp.sc"
	for(k = 0; k <= 8 - i; k++ )
	{   

#line 7756 "vocoderSched.cc"
	    __PRF_globalCounter[1198u]++ ;

#line 509 "src/lp_analysis/vad_comp.sc"
	    L_work[i] = L_mac(L_work[i], aav1[k], aav1[k + i]);
	}
    }

#line 7764 "vocoderSched.cc"
    __PRF_globalCounter[1199u]++ ;

#line 514 "src/lp_analysis/vad_comp.sc"
    if (L_work[0] == 0l)
    {   

#line 7771 "vocoderSched.cc"
	__PRF_globalCounter[1200u]++ ;

#line 516 "src/lp_analysis/vad_comp.sc"
	 *scal_rav1 = 0;
    }
    else 
    {   

#line 7780 "vocoderSched.cc"
	__PRF_globalCounter[1201u]++ ;

#line 520 "src/lp_analysis/vad_comp.sc"
	 *scal_rav1 = norm_l(L_work[0]);
    }

#line 7787 "vocoderSched.cc"
    __PRF_globalCounter[1202u]++ ;

#line 523 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i <= 8; i++ )
    {   

#line 7794 "vocoderSched.cc"
	__PRF_globalCounter[1203u]++ ;

#line 525 "src/lp_analysis/vad_comp.sc"
	rav1[i] = extract_h(L_shl(L_work[i],  *scal_rav1));
    }

#line 7801 "vocoderSched.cc"
    __PRF_globalCounter[1204u]++ ;

#line 528 "src/lp_analysis/vad_comp.sc"
    return ;
}

#line 7808 "vocoderSched.cc"
void VAD_Computation::Energy_computation(

#line 208 "src/lp_analysis/vad_comp.sc"
    short int Ec_r_h[], 
    short int Ec_scal_acf, 
    struct _fp *acf0, 
    struct _fp *pvad)

{   
    int i;
    short int norm_prod; short int temp;
    int L_temp;

#line 7822 "vocoderSched.cc"
    __PRF_globalCounter[1205u]++ ;

#line 222 "src/lp_analysis/vad_comp.sc"
    acf0->e = sub(32, Ec_scal_acf);
    acf0->m = Ec_r_h[0] & 32760;



    pvad->e = add(acf0->e, 14);
    pvad->e = sub(pvad->e, scal_rvad);

    L_temp = 0l;

#line 7836 "vocoderSched.cc"
    __PRF_globalCounter[1206u]++ ;

#line 232 "src/lp_analysis/vad_comp.sc"
    for(i = 1; i <= 8; i++ )
    {   

#line 7843 "vocoderSched.cc"
	__PRF_globalCounter[1207u]++ ;

#line 234 "src/lp_analysis/vad_comp.sc"
	temp = shr(Ec_r_h[i], 3);
	L_temp = L_mac(L_temp, temp, rvad[i]);
    }

#line 7851 "vocoderSched.cc"
    __PRF_globalCounter[1208u]++ ;

#line 238 "src/lp_analysis/vad_comp.sc"
    temp = shr(Ec_r_h[0], 3);
    L_temp = L_add(L_temp, L_shr(L_mult(temp, rvad[0]), 1));

#line 7858 "vocoderSched.cc"
    __PRF_globalCounter[1209u]++ ;

#line 242 "src/lp_analysis/vad_comp.sc"
    if (L_temp <= 0l)
    {   

#line 7865 "vocoderSched.cc"
	__PRF_globalCounter[1210u]++ ;

#line 244 "src/lp_analysis/vad_comp.sc"
	L_temp = 1l;
    }

#line 7872 "vocoderSched.cc"
    __PRF_globalCounter[1211u]++ ;

#line 246 "src/lp_analysis/vad_comp.sc"
    norm_prod = norm_l(L_temp);
    pvad->e = sub(pvad->e, norm_prod);
    pvad->m = extract_h(L_shl(L_temp, norm_prod));


    return ;
}

#line 7884 "vocoderSched.cc"
void VAD_Computation::Predictor_values(

#line 550 "src/lp_analysis/vad_comp.sc"
    int L_av1[], 
    short int rav1[], 
    short int *scal_rav1)

{   
    short int aav1[9]; short int vpar[8];

#line 7895 "vocoderSched.cc"
    __PRF_globalCounter[1212u]++ ;

#line 557 "src/lp_analysis/vad_comp.sc"
    Schur_recursion(L_av1, vpar);
    Step_up(8, vpar, aav1);
    Compute_rav1(aav1, rav1, scal_rav1);

    return ;
}

#line 7906 "vocoderSched.cc"
void VAD_Computation::Schur_recursion(

#line 336 "src/lp_analysis/vad_comp.sc"
    int L_av1[], 
    short int vpar[])

{   
    short int acf[9]; short int kk[9]; short int pp[9]; short int temp;
    int i; int k; int m; int n;

#line 7917 "vocoderSched.cc"
    __PRF_globalCounter[1213u]++ ;

#line 346 "src/lp_analysis/vad_comp.sc"
    if (L_av1[0] == 0)
    {   

#line 7924 "vocoderSched.cc"
	__PRF_globalCounter[1214u]++ ;

#line 348 "src/lp_analysis/vad_comp.sc"
	for(i = 0; i < 8; i++ )
	{   

#line 7931 "vocoderSched.cc"
	    __PRF_globalCounter[1215u]++ ;

#line 350 "src/lp_analysis/vad_comp.sc"
	    vpar[i] = 0;
	}

#line 7938 "vocoderSched.cc"
	__PRF_globalCounter[1216u]++ ;

#line 352 "src/lp_analysis/vad_comp.sc"
	return ;
    }

#line 7945 "vocoderSched.cc"
    __PRF_globalCounter[1217u]++ ;

#line 354 "src/lp_analysis/vad_comp.sc"
    temp = norm_l(L_av1[0]);

#line 7951 "vocoderSched.cc"
    __PRF_globalCounter[1218u]++ ;

#line 356 "src/lp_analysis/vad_comp.sc"
    for(k = 0; k <= 8; k++ )
    {   

#line 7958 "vocoderSched.cc"
	__PRF_globalCounter[1219u]++ ;

#line 358 "src/lp_analysis/vad_comp.sc"
	acf[k] = extract_h(L_shl(L_av1[k], temp));
    }

#line 7965 "vocoderSched.cc"
    __PRF_globalCounter[1220u]++ ;

#line 363 "src/lp_analysis/vad_comp.sc"
    for(i = 1; i <= 7; i++ )
    {   

#line 7972 "vocoderSched.cc"
	__PRF_globalCounter[1221u]++ ;

#line 365 "src/lp_analysis/vad_comp.sc"
	kk[9 - i] = acf[i];
    }

#line 7979 "vocoderSched.cc"
    __PRF_globalCounter[1222u]++ ;

#line 368 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i <= 8; i++ )
    {   

#line 7986 "vocoderSched.cc"
	__PRF_globalCounter[1223u]++ ;

#line 370 "src/lp_analysis/vad_comp.sc"
	pp[i] = acf[i];
    }

#line 7993 "vocoderSched.cc"
    __PRF_globalCounter[1224u]++ ;

#line 375 "src/lp_analysis/vad_comp.sc"
    for(n = 0; n < 8; n++ )
    {   

#line 8000 "vocoderSched.cc"
	__PRF_globalCounter[1225u]++ ;

#line 378 "src/lp_analysis/vad_comp.sc"
	if ((pp[0] == 0) || 
	    (sub(pp[0], abs_s(pp[1])) < 0))
	{   

#line 8008 "vocoderSched.cc"
	    __PRF_globalCounter[1226u]++ ;

#line 381 "src/lp_analysis/vad_comp.sc"
	    for(i = n; i < 8; i++ )
	    {   

#line 8015 "vocoderSched.cc"
		__PRF_globalCounter[1227u]++ ;

#line 383 "src/lp_analysis/vad_comp.sc"
		vpar[i] = 0;
	    }

#line 8022 "vocoderSched.cc"
	    __PRF_globalCounter[1228u]++ ;

#line 385 "src/lp_analysis/vad_comp.sc"
	    return ;
	}

#line 8029 "vocoderSched.cc"
	__PRF_globalCounter[1229u]++ ;

#line 387 "src/lp_analysis/vad_comp.sc"
	vpar[n] = div_s(abs_s(pp[1]), pp[0]);

#line 8035 "vocoderSched.cc"
	__PRF_globalCounter[1230u]++ ;

#line 390 "src/lp_analysis/vad_comp.sc"
	if (pp[1] > 0)
	{   

#line 8042 "vocoderSched.cc"
	    __PRF_globalCounter[1231u]++ ;

#line 392 "src/lp_analysis/vad_comp.sc"
	    vpar[n] = negate(vpar[n]);
	}

#line 8049 "vocoderSched.cc"
	__PRF_globalCounter[1232u]++ ;

#line 395 "src/lp_analysis/vad_comp.sc"
	if (n == 7)
	{   

#line 8056 "vocoderSched.cc"
	    __PRF_globalCounter[1233u]++ ;

#line 397 "src/lp_analysis/vad_comp.sc"
	    return ;
	}

#line 8063 "vocoderSched.cc"
	__PRF_globalCounter[1234u]++ ;

#line 401 "src/lp_analysis/vad_comp.sc"
	pp[0] = add(pp[0], mult_r(pp[1], vpar[n]));

#line 8069 "vocoderSched.cc"
	__PRF_globalCounter[1235u]++ ;

#line 403 "src/lp_analysis/vad_comp.sc"
	for(m = 1; m <= 7 - n; m++ )
	{   

#line 8076 "vocoderSched.cc"
	    __PRF_globalCounter[1236u]++ ;

#line 405 "src/lp_analysis/vad_comp.sc"
	    pp[m] = add(pp[1 + m], mult_r(kk[9 - m], vpar[n]));

	    kk[9 - m] = add(kk[9 - m], mult_r(pp[1 + m], vpar[n]));
	}
    }

#line 8086 "vocoderSched.cc"
    __PRF_globalCounter[1237u]++ ;

#line 412 "src/lp_analysis/vad_comp.sc"
    return ;
}

#line 8093 "vocoderSched.cc"
bool VAD_Computation::Spectral_comparison(

#line 585 "src/lp_analysis/vad_comp.sc"
    short int rav1[], 
    short int scal_rav1, 
    int L_av0[])

{   
    int L_dm; int L_sump; int L_temp;
    short int divshift; short int sav0[9]; short int shift; short int temp;
    bool stat;
    int i;

#line 8107 "vocoderSched.cc"
    __PRF_globalCounter[1238u]++ ;

#line 598 "src/lp_analysis/vad_comp.sc"
    if (L_av0[0] == 0l)
    {   

#line 8114 "vocoderSched.cc"
	__PRF_globalCounter[1239u]++ ;

#line 600 "src/lp_analysis/vad_comp.sc"
	for(i = 0; i <= 8; i++ )
	{   

#line 8121 "vocoderSched.cc"
	    __PRF_globalCounter[1240u]++ ;

#line 602 "src/lp_analysis/vad_comp.sc"
	    sav0[i] = 4095;
	}
    }
    else 
    {   

#line 8131 "vocoderSched.cc"
	__PRF_globalCounter[1241u]++ ;

#line 607 "src/lp_analysis/vad_comp.sc"
	shift = sub(norm_l(L_av0[0]), 3);

#line 8137 "vocoderSched.cc"
	__PRF_globalCounter[1242u]++ ;

#line 608 "src/lp_analysis/vad_comp.sc"
	for(i = 0; i <= 8; i++ )
	{   

#line 8144 "vocoderSched.cc"
	    __PRF_globalCounter[1243u]++ ;

#line 610 "src/lp_analysis/vad_comp.sc"
	    sav0[i] = extract_h(L_shl(L_av0[i], shift));
	}
    }

#line 8152 "vocoderSched.cc"
    __PRF_globalCounter[1244u]++ ;

#line 616 "src/lp_analysis/vad_comp.sc"
    L_sump = 0l;

#line 8158 "vocoderSched.cc"
    __PRF_globalCounter[1245u]++ ;

#line 617 "src/lp_analysis/vad_comp.sc"
    for(i = 1; i <= 8; i++ )
    {   

#line 8165 "vocoderSched.cc"
	__PRF_globalCounter[1246u]++ ;

#line 619 "src/lp_analysis/vad_comp.sc"
	L_sump = L_mac(L_sump, rav1[i], sav0[i]);
    }

#line 8172 "vocoderSched.cc"
    __PRF_globalCounter[1247u]++ ;

#line 625 "src/lp_analysis/vad_comp.sc"
    if (L_sump < 0l)
    {   

#line 8179 "vocoderSched.cc"
	__PRF_globalCounter[1248u]++ ;

#line 627 "src/lp_analysis/vad_comp.sc"
	L_temp = L_negate(L_sump);
    }
    else 
    {   

#line 8188 "vocoderSched.cc"
	__PRF_globalCounter[1249u]++ ;

#line 631 "src/lp_analysis/vad_comp.sc"
	L_temp = L_sump;
    }

#line 8195 "vocoderSched.cc"
    __PRF_globalCounter[1250u]++ ;

#line 635 "src/lp_analysis/vad_comp.sc"
    if (L_temp == 0l)
    {   

#line 8202 "vocoderSched.cc"
	__PRF_globalCounter[1251u]++ ;

#line 637 "src/lp_analysis/vad_comp.sc"
	L_dm = 0l;
	shift = 0;
    }
    else 
    {   

#line 8212 "vocoderSched.cc"
	__PRF_globalCounter[1252u]++ ;

#line 642 "src/lp_analysis/vad_comp.sc"
	sav0[0] = shl(sav0[0], 3);
	shift = norm_l(L_temp);
	temp = extract_h(L_shl(L_temp, shift));

#line 8220 "vocoderSched.cc"
	__PRF_globalCounter[1253u]++ ;

#line 647 "src/lp_analysis/vad_comp.sc"
	if (sub(sav0[0], temp) >= 0)
	{   

#line 8227 "vocoderSched.cc"
	    __PRF_globalCounter[1254u]++ ;

#line 649 "src/lp_analysis/vad_comp.sc"
	    divshift = 0;
	    temp = div_s(temp, sav0[0]);
	}
	else 
	{   

#line 8237 "vocoderSched.cc"
	    __PRF_globalCounter[1255u]++ ;

#line 654 "src/lp_analysis/vad_comp.sc"
	    divshift = 1;
	    temp = sub(temp, sav0[0]);
	    temp = div_s(temp, sav0[0]);
	}

#line 8246 "vocoderSched.cc"
	__PRF_globalCounter[1256u]++ ;

#line 660 "src/lp_analysis/vad_comp.sc"
	if (sub(divshift, 1) == 0)
	{   

#line 8253 "vocoderSched.cc"
	    __PRF_globalCounter[1257u]++ ;

#line 662 "src/lp_analysis/vad_comp.sc"
	    L_dm = 32768l;
	}
	else 
	{   

#line 8262 "vocoderSched.cc"
	    __PRF_globalCounter[1258u]++ ;

#line 666 "src/lp_analysis/vad_comp.sc"
	    L_dm = 0l;
	}

#line 8269 "vocoderSched.cc"
	__PRF_globalCounter[1259u]++ ;

#line 669 "src/lp_analysis/vad_comp.sc"
	L_dm = L_shl(L_add(L_dm, L_deposit_l(temp)), 1);

#line 8275 "vocoderSched.cc"
	__PRF_globalCounter[1260u]++ ;

#line 672 "src/lp_analysis/vad_comp.sc"
	if (L_sump < 0l)
	{   

#line 8282 "vocoderSched.cc"
	    __PRF_globalCounter[1261u]++ ;

#line 674 "src/lp_analysis/vad_comp.sc"
	    L_dm = L_negate(L_dm);
	}
    }

#line 8290 "vocoderSched.cc"
    __PRF_globalCounter[1262u]++ ;

#line 680 "src/lp_analysis/vad_comp.sc"
    L_dm = L_shl(L_dm, 14);
    L_dm = L_shr(L_dm, shift);
    L_dm = L_add(L_dm, L_shl(L_deposit_l(rav1[0]), 11));
    L_dm = L_shr(L_dm, scal_rav1);



    L_temp = L_sub(L_dm, L_lastdm);
    L_lastdm = L_dm;

#line 8304 "vocoderSched.cc"
    __PRF_globalCounter[1263u]++ ;

#line 691 "src/lp_analysis/vad_comp.sc"
    if (L_temp < 0l)
    {   

#line 8311 "vocoderSched.cc"
	__PRF_globalCounter[1264u]++ ;

#line 693 "src/lp_analysis/vad_comp.sc"
	L_temp = L_negate(L_temp);
    }

#line 8318 "vocoderSched.cc"
    __PRF_globalCounter[1265u]++ ;

#line 697 "src/lp_analysis/vad_comp.sc"
    L_temp = L_sub(L_temp, 3670l);

#line 8324 "vocoderSched.cc"
    __PRF_globalCounter[1266u]++ ;

#line 700 "src/lp_analysis/vad_comp.sc"
    if (L_temp < 0l)
    {   

#line 8331 "vocoderSched.cc"
	__PRF_globalCounter[1267u]++ ;

#line 702 "src/lp_analysis/vad_comp.sc"
	stat = 1;
    }
    else 
    {   

#line 8340 "vocoderSched.cc"
	__PRF_globalCounter[1268u]++ ;

#line 706 "src/lp_analysis/vad_comp.sc"
	stat = 0;
    }

#line 8347 "vocoderSched.cc"
    __PRF_globalCounter[1269u]++ ;

#line 709 "src/lp_analysis/vad_comp.sc"
    return stat;
}

#line 8354 "vocoderSched.cc"
void VAD_Computation::Step_up(

#line 433 "src/lp_analysis/vad_comp.sc"
    short int np, 
    short int vpar[], 
    short int aav1[])

{   
    int L_coef[9]; int L_work[9];
    short int temp;
    int i; int m;

#line 8367 "vocoderSched.cc"
    __PRF_globalCounter[1270u]++ ;

#line 444 "src/lp_analysis/vad_comp.sc"
    L_coef[0] = 536870912l;
    L_coef[1] = L_shl(L_deposit_l(vpar[0]), 14);

#line 8374 "vocoderSched.cc"
    __PRF_globalCounter[1271u]++ ;

#line 449 "src/lp_analysis/vad_comp.sc"
    for(m = 2; m <= np; m++ )
    {   

#line 8381 "vocoderSched.cc"
	__PRF_globalCounter[1272u]++ ;

#line 451 "src/lp_analysis/vad_comp.sc"
	for(i = 1; i < m; i++ )
	{   

#line 8388 "vocoderSched.cc"
	    __PRF_globalCounter[1273u]++ ;

#line 453 "src/lp_analysis/vad_comp.sc"
	    temp = extract_h(L_coef[m - i]);
	    L_work[i] = L_mac(L_coef[i], vpar[m - 1], temp);
	}

#line 8396 "vocoderSched.cc"
	__PRF_globalCounter[1274u]++ ;

#line 457 "src/lp_analysis/vad_comp.sc"
	for(i = 1; i < m; i++ )
	{   

#line 8403 "vocoderSched.cc"
	    __PRF_globalCounter[1275u]++ ;

#line 459 "src/lp_analysis/vad_comp.sc"
	    L_coef[i] = L_work[i];
	}

#line 8410 "vocoderSched.cc"
	__PRF_globalCounter[1276u]++ ;

#line 462 "src/lp_analysis/vad_comp.sc"
	L_coef[m] = L_shl(L_deposit_l(vpar[m - 1]), 14);
    }

#line 8417 "vocoderSched.cc"
    __PRF_globalCounter[1277u]++ ;

#line 467 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i <= np; i++ )
    {   

#line 8424 "vocoderSched.cc"
	__PRF_globalCounter[1278u]++ ;

#line 469 "src/lp_analysis/vad_comp.sc"
	aav1[i] = extract_h(L_shr(L_coef[i], 3));
    }

#line 8431 "vocoderSched.cc"
    __PRF_globalCounter[1279u]++ ;

#line 472 "src/lp_analysis/vad_comp.sc"
    return ;
}

#line 8438 "vocoderSched.cc"
void VAD_Computation::Threshold_adaptation(

#line 738 "src/lp_analysis/vad_comp.sc"
    bool stat, 
    bool Ta_ptch, 
    bool tone, 
    short int rav1[], 
    short int scal_rav1, 
    struct _fp pvad, 
    struct _fp acf0)

{   
    bool comp; bool comp2;
    int L_temp;
    short int temp;
    struct _fp p_temp;
    int i;

#line 8457 "vocoderSched.cc"
    __PRF_globalCounter[1280u]++ ;

#line 753 "src/lp_analysis/vad_comp.sc"
    comp = 0;

#line 8463 "vocoderSched.cc"
    __PRF_globalCounter[1281u]++ ;

#line 758 "src/lp_analysis/vad_comp.sc"
    if (sub(acf0.e, 17) < 0)
    {   

#line 8470 "vocoderSched.cc"
	__PRF_globalCounter[1282u]++ ;

#line 760 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8477 "vocoderSched.cc"
    __PRF_globalCounter[1283u]++ ;

#line 763 "src/lp_analysis/vad_comp.sc"
    if ((sub(acf0.e, 17) == 0) && (sub(acf0.m, 32500) < 0))
    {   

#line 8484 "vocoderSched.cc"
	__PRF_globalCounter[1284u]++ ;

#line 765 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8491 "vocoderSched.cc"
    __PRF_globalCounter[1285u]++ ;

#line 768 "src/lp_analysis/vad_comp.sc"
    if (comp == 1)
    {   

#line 8498 "vocoderSched.cc"
	__PRF_globalCounter[1286u]++ ;

#line 770 "src/lp_analysis/vad_comp.sc"
	thvad.e = 19;
	thvad.m = 21667;

	return ;
    }

#line 8508 "vocoderSched.cc"
    __PRF_globalCounter[1287u]++ ;

#line 778 "src/lp_analysis/vad_comp.sc"
    if (Ta_ptch == 1)
    {   

#line 8515 "vocoderSched.cc"
	__PRF_globalCounter[1288u]++ ;

#line 780 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8522 "vocoderSched.cc"
    __PRF_globalCounter[1289u]++ ;

#line 783 "src/lp_analysis/vad_comp.sc"
    if (stat == 0)
    {   

#line 8529 "vocoderSched.cc"
	__PRF_globalCounter[1290u]++ ;

#line 785 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8536 "vocoderSched.cc"
    __PRF_globalCounter[1291u]++ ;

#line 788 "src/lp_analysis/vad_comp.sc"
    if (tone == 1)
    {   

#line 8543 "vocoderSched.cc"
	__PRF_globalCounter[1292u]++ ;

#line 790 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8550 "vocoderSched.cc"
    __PRF_globalCounter[1293u]++ ;

#line 793 "src/lp_analysis/vad_comp.sc"
    if (comp == 1)
    {   

#line 8557 "vocoderSched.cc"
	__PRF_globalCounter[1294u]++ ;

#line 795 "src/lp_analysis/vad_comp.sc"
	adaptcount = 0;
	return ;
    }

#line 8565 "vocoderSched.cc"
    __PRF_globalCounter[1295u]++ ;

#line 800 "src/lp_analysis/vad_comp.sc"
    adaptcount = add(adaptcount, 1);

#line 8571 "vocoderSched.cc"
    __PRF_globalCounter[1296u]++ ;

#line 802 "src/lp_analysis/vad_comp.sc"
    if (sub(adaptcount, 8) <= 0)
    {   

#line 8578 "vocoderSched.cc"
	__PRF_globalCounter[1297u]++ ;

#line 804 "src/lp_analysis/vad_comp.sc"
	return ;
    }

#line 8585 "vocoderSched.cc"
    __PRF_globalCounter[1298u]++ ;

#line 808 "src/lp_analysis/vad_comp.sc"
    thvad.m = sub(thvad.m, shr(thvad.m, 5));

#line 8591 "vocoderSched.cc"
    __PRF_globalCounter[1299u]++ ;

#line 811 "src/lp_analysis/vad_comp.sc"
    if (sub(thvad.m, 16384) < 0)
    {   

#line 8598 "vocoderSched.cc"
	__PRF_globalCounter[1300u]++ ;

#line 813 "src/lp_analysis/vad_comp.sc"
	thvad.m = shl(thvad.m, 1);
	thvad.e = sub(thvad.e, 1);
    }

#line 8606 "vocoderSched.cc"
    __PRF_globalCounter[1301u]++ ;

#line 818 "src/lp_analysis/vad_comp.sc"
    L_temp = L_mult(pvad.m, 17203);
    L_temp = L_shr(L_temp, 15);
    p_temp.e = add(pvad.e, 1);

#line 8614 "vocoderSched.cc"
    __PRF_globalCounter[1302u]++ ;

#line 823 "src/lp_analysis/vad_comp.sc"
    if (L_temp > 32767l)
    {   

#line 8621 "vocoderSched.cc"
	__PRF_globalCounter[1303u]++ ;

#line 825 "src/lp_analysis/vad_comp.sc"
	L_temp = L_shr(L_temp, 1);
	p_temp.e = add(p_temp.e, 1);
    }

#line 8629 "vocoderSched.cc"
    __PRF_globalCounter[1304u]++ ;

#line 828 "src/lp_analysis/vad_comp.sc"
    p_temp.m = extract_l(L_temp);

#line 8635 "vocoderSched.cc"
    __PRF_globalCounter[1305u]++ ;

#line 833 "src/lp_analysis/vad_comp.sc"
    if (sub(thvad.e, p_temp.e) < 0)
    {   

#line 8642 "vocoderSched.cc"
	__PRF_globalCounter[1306u]++ ;

#line 835 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8649 "vocoderSched.cc"
    __PRF_globalCounter[1307u]++ ;

#line 838 "src/lp_analysis/vad_comp.sc"
    if ((sub(thvad.e, p_temp.e) == 0) && 
	(sub(thvad.m, p_temp.m) < 0))
    {   

#line 8657 "vocoderSched.cc"
	__PRF_globalCounter[1308u]++ ;

#line 841 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8664 "vocoderSched.cc"
    __PRF_globalCounter[1309u]++ ;

#line 846 "src/lp_analysis/vad_comp.sc"
    if (comp == 1)
    {   

#line 8671 "vocoderSched.cc"
	__PRF_globalCounter[1310u]++ ;

#line 850 "src/lp_analysis/vad_comp.sc"
	L_temp = L_add(L_deposit_l(thvad.m), 
	    L_deposit_l(shr(thvad.m, 4)));

#line 8678 "vocoderSched.cc"
	__PRF_globalCounter[1311u]++ ;

#line 854 "src/lp_analysis/vad_comp.sc"
	if (L_sub(L_temp, 32767l) > 0)
	{   

#line 8685 "vocoderSched.cc"
	    __PRF_globalCounter[1312u]++ ;

#line 856 "src/lp_analysis/vad_comp.sc"
	    thvad.m = extract_l(L_shr(L_temp, 1));
	    thvad.e = add(thvad.e, 1);
	}
	else 
	{   

#line 8695 "vocoderSched.cc"
	    __PRF_globalCounter[1313u]++ ;

#line 861 "src/lp_analysis/vad_comp.sc"
	    thvad.m = extract_l(L_temp);
	}

#line 8702 "vocoderSched.cc"
	__PRF_globalCounter[1314u]++ ;

#line 864 "src/lp_analysis/vad_comp.sc"
	comp2 = 0;

#line 8708 "vocoderSched.cc"
	__PRF_globalCounter[1315u]++ ;

#line 867 "src/lp_analysis/vad_comp.sc"
	if (sub(p_temp.e, thvad.e) < 0)
	{   

#line 8715 "vocoderSched.cc"
	    __PRF_globalCounter[1316u]++ ;

#line 869 "src/lp_analysis/vad_comp.sc"
	    comp2 = 1;
	}

#line 8722 "vocoderSched.cc"
	__PRF_globalCounter[1317u]++ ;

#line 872 "src/lp_analysis/vad_comp.sc"
	if ((sub(p_temp.e, thvad.e) == 0) && 
	    (sub(p_temp.m, thvad.m) < 0))
	{   

#line 8730 "vocoderSched.cc"
	    __PRF_globalCounter[1318u]++ ;

#line 875 "src/lp_analysis/vad_comp.sc"
	    comp2 = 1;
	}

#line 8737 "vocoderSched.cc"
	__PRF_globalCounter[1319u]++ ;

#line 878 "src/lp_analysis/vad_comp.sc"
	if (comp2 == 1)
	{   

#line 8744 "vocoderSched.cc"
	    __PRF_globalCounter[1320u]++ ;

#line 880 "src/lp_analysis/vad_comp.sc"
	    thvad.e = p_temp.e;
	    thvad.m = p_temp.m;
	}
    }

#line 8753 "vocoderSched.cc"
    __PRF_globalCounter[1321u]++ ;

#line 887 "src/lp_analysis/vad_comp.sc"
    if (sub(pvad.e, 27) == 0)
    {   

#line 8760 "vocoderSched.cc"
	__PRF_globalCounter[1322u]++ ;

#line 889 "src/lp_analysis/vad_comp.sc"
	L_temp = L_add(L_deposit_l(pvad.m), L_deposit_l(16927));
	p_temp.m = extract_l(L_shr(L_temp, 1));
	p_temp.e = add(pvad.e, 1);
    }
    else 
    {   

#line 8771 "vocoderSched.cc"
	__PRF_globalCounter[1323u]++ ;

#line 896 "src/lp_analysis/vad_comp.sc"
	if (sub(pvad.e, 27) > 0)
	{   

#line 8778 "vocoderSched.cc"
	    __PRF_globalCounter[1324u]++ ;

#line 898 "src/lp_analysis/vad_comp.sc"
	    temp = sub(pvad.e, 27);
	    temp = shr(16927, temp);
	    L_temp = L_add(L_deposit_l(pvad.m), L_deposit_l(temp));

#line 8786 "vocoderSched.cc"
	    __PRF_globalCounter[1325u]++ ;

#line 903 "src/lp_analysis/vad_comp.sc"
	    if (L_sub(L_temp, 32767l) > 0)
	    {   

#line 8793 "vocoderSched.cc"
		__PRF_globalCounter[1326u]++ ;

#line 905 "src/lp_analysis/vad_comp.sc"
		p_temp.e = add(pvad.e, 1);
		p_temp.m = extract_l(L_shr(L_temp, 1));
	    }
	    else 

	    {   

#line 8804 "vocoderSched.cc"
		__PRF_globalCounter[1327u]++ ;

#line 911 "src/lp_analysis/vad_comp.sc"
		p_temp.e = pvad.e;
		p_temp.m = extract_l(L_temp);
	    }
	}
	else 
	{   

#line 8815 "vocoderSched.cc"
	    __PRF_globalCounter[1328u]++ ;

#line 917 "src/lp_analysis/vad_comp.sc"
	    temp = sub(27, pvad.e);
	    temp = shr(pvad.m, temp);
	    L_temp = L_add(L_deposit_l(16927), L_deposit_l(temp));

#line 8823 "vocoderSched.cc"
	    __PRF_globalCounter[1329u]++ ;

#line 922 "src/lp_analysis/vad_comp.sc"
	    if (L_sub(L_temp, 32767l) > 0)
	    {   

#line 8830 "vocoderSched.cc"
		__PRF_globalCounter[1330u]++ ;

#line 924 "src/lp_analysis/vad_comp.sc"
		p_temp.e = add(27, 1);
		p_temp.m = extract_l(L_shr(L_temp, 1));
	    }
	    else 

	    {   

#line 8841 "vocoderSched.cc"
		__PRF_globalCounter[1331u]++ ;

#line 930 "src/lp_analysis/vad_comp.sc"
		p_temp.e = 27;
		p_temp.m = extract_l(L_temp);
	    }
	}
    }

#line 8851 "vocoderSched.cc"
    __PRF_globalCounter[1332u]++ ;

#line 938 "src/lp_analysis/vad_comp.sc"
    comp = 0;

#line 8857 "vocoderSched.cc"
    __PRF_globalCounter[1333u]++ ;

#line 941 "src/lp_analysis/vad_comp.sc"
    if (sub(thvad.e, p_temp.e) > 0)
    {   

#line 8864 "vocoderSched.cc"
	__PRF_globalCounter[1334u]++ ;

#line 943 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8871 "vocoderSched.cc"
    __PRF_globalCounter[1335u]++ ;

#line 946 "src/lp_analysis/vad_comp.sc"
    if ((sub(thvad.e, p_temp.e) == 0) && 
	(sub(thvad.m, p_temp.m) > 0))
    {   

#line 8879 "vocoderSched.cc"
	__PRF_globalCounter[1336u]++ ;

#line 949 "src/lp_analysis/vad_comp.sc"
	comp = 1;
    }

#line 8886 "vocoderSched.cc"
    __PRF_globalCounter[1337u]++ ;

#line 952 "src/lp_analysis/vad_comp.sc"
    if (comp == 1)
    {   

#line 8893 "vocoderSched.cc"
	__PRF_globalCounter[1338u]++ ;

#line 954 "src/lp_analysis/vad_comp.sc"
	thvad.e = p_temp.e;
	thvad.m = p_temp.m;
    }

#line 8901 "vocoderSched.cc"
    __PRF_globalCounter[1339u]++ ;

#line 959 "src/lp_analysis/vad_comp.sc"
    scal_rvad = scal_rav1;

#line 8907 "vocoderSched.cc"
    __PRF_globalCounter[1340u]++ ;

#line 961 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i <= 8; i++ )
    {   

#line 8914 "vocoderSched.cc"
	__PRF_globalCounter[1341u]++ ;

#line 963 "src/lp_analysis/vad_comp.sc"
	rvad[i] = rav1[i];
    }

#line 8921 "vocoderSched.cc"
    __PRF_globalCounter[1342u]++ ;

#line 968 "src/lp_analysis/vad_comp.sc"
    adaptcount = 9;

    return ;
}

#line 8930 "vocoderSched.cc"
void VAD_Computation::Tone_detection(

#line 991 "src/lp_analysis/vad_comp.sc"
    short int Td_rc[], 
    bool *tone)

{   
    int L_den; int L_num; int L_temp;
    short int a[3]; short int prederr; short int temp;
    int i;

#line 8942 "vocoderSched.cc"
    __PRF_globalCounter[1343u]++ ;

#line 999 "src/lp_analysis/vad_comp.sc"
     *tone = 0;



    Step_up(2, Td_rc, a);



    temp = shl(a[1], 3);
    L_den = L_mult(temp, temp);



    L_temp = L_shl(L_deposit_h(a[2]), 3);
    L_num = L_sub(L_temp, L_den);

#line 8962 "vocoderSched.cc"
    __PRF_globalCounter[1344u]++ ;

#line 1018 "src/lp_analysis/vad_comp.sc"
    if (L_num <= 0)
    {   

#line 8969 "vocoderSched.cc"
	__PRF_globalCounter[1345u]++ ;

#line 1020 "src/lp_analysis/vad_comp.sc"
	return ;
    }

#line 8976 "vocoderSched.cc"
    __PRF_globalCounter[1346u]++ ;

#line 1023 "src/lp_analysis/vad_comp.sc"
    if (a[1] < 0)
    {   

#line 8983 "vocoderSched.cc"
	__PRF_globalCounter[1347u]++ ;

#line 1025 "src/lp_analysis/vad_comp.sc"
	temp = extract_h(L_den);
	L_den = L_mult(temp, 3189);

	L_temp = L_sub(L_num, L_den);

#line 8992 "vocoderSched.cc"
	__PRF_globalCounter[1348u]++ ;

#line 1031 "src/lp_analysis/vad_comp.sc"
	if (L_temp < 0)
	{   

#line 8999 "vocoderSched.cc"
	    __PRF_globalCounter[1349u]++ ;

#line 1033 "src/lp_analysis/vad_comp.sc"
	    return ;
	}
    }

#line 9007 "vocoderSched.cc"
    __PRF_globalCounter[1350u]++ ;

#line 1038 "src/lp_analysis/vad_comp.sc"
    prederr = 32767;

#line 9013 "vocoderSched.cc"
    __PRF_globalCounter[1351u]++ ;

#line 1040 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i < 4; i++ )
    {   

#line 9020 "vocoderSched.cc"
	__PRF_globalCounter[1352u]++ ;

#line 1042 "src/lp_analysis/vad_comp.sc"
	temp = mult(Td_rc[i], Td_rc[i]);
	temp = sub(32767, temp);
	prederr = mult(prederr, temp);
    }

#line 9029 "vocoderSched.cc"
    __PRF_globalCounter[1353u]++ ;

#line 1049 "src/lp_analysis/vad_comp.sc"
    temp = sub(prederr, 1464);

#line 9035 "vocoderSched.cc"
    __PRF_globalCounter[1354u]++ ;

#line 1052 "src/lp_analysis/vad_comp.sc"
    if (temp < 0)
    {   

#line 9042 "vocoderSched.cc"
	__PRF_globalCounter[1355u]++ ;

#line 1054 "src/lp_analysis/vad_comp.sc"
	 *tone = 1;
    }

#line 9049 "vocoderSched.cc"
    __PRF_globalCounter[1356u]++ ;

#line 1056 "src/lp_analysis/vad_comp.sc"
    return ;
}

#line 9056 "vocoderSched.cc"
bool VAD_Computation::Vad_decision(

#line 1077 "src/lp_analysis/vad_comp.sc"
    struct _fp pvad)

{   
    bool vvad;

#line 9065 "vocoderSched.cc"
    __PRF_globalCounter[1357u]++ ;

#line 1083 "src/lp_analysis/vad_comp.sc"
    if (sub(pvad.e, thvad.e) > 0)
    {   

#line 9072 "vocoderSched.cc"
	__PRF_globalCounter[1358u]++ ;

#line 1085 "src/lp_analysis/vad_comp.sc"
	vvad = 1;
    }
    else 

#line 9080 "vocoderSched.cc"
    {   
	__PRF_globalCounter[1359u]++ ;

#line 1087 "src/lp_analysis/vad_comp.sc"
	if ((sub(pvad.e, thvad.e) == 0) && 
	    (sub(pvad.m, thvad.m) > 0))
	{   

#line 9089 "vocoderSched.cc"
	    __PRF_globalCounter[1360u]++ ;

#line 1090 "src/lp_analysis/vad_comp.sc"
	    vvad = 1;
	}
	else 
	{   

#line 9098 "vocoderSched.cc"
	    __PRF_globalCounter[1361u]++ ;

#line 1094 "src/lp_analysis/vad_comp.sc"
	    vvad = 0;
	}
    }

#line 9106 "vocoderSched.cc"
    __PRF_globalCounter[1362u]++ ;

#line 1097 "src/lp_analysis/vad_comp.sc"
    return vvad;
}

#line 9113 "vocoderSched.cc"
bool VAD_Computation::Vad_hangover(

#line 1117 "src/lp_analysis/vad_comp.sc"
    bool vvad)

{   

#line 9121 "vocoderSched.cc"
    __PRF_globalCounter[1363u]++ ;

#line 1121 "src/lp_analysis/vad_comp.sc"
    if (vvad == 1)
    {   

#line 9128 "vocoderSched.cc"
	__PRF_globalCounter[1364u]++ ;

#line 1123 "src/lp_analysis/vad_comp.sc"
	burstcount = add(burstcount, 1);
    }
    else 
    {   

#line 9137 "vocoderSched.cc"
	__PRF_globalCounter[1365u]++ ;

#line 1127 "src/lp_analysis/vad_comp.sc"
	burstcount = 0;
    }

#line 9144 "vocoderSched.cc"
    __PRF_globalCounter[1366u]++ ;

#line 1131 "src/lp_analysis/vad_comp.sc"
    if (sub(burstcount, 3) >= 0)
    {   

#line 9151 "vocoderSched.cc"
	__PRF_globalCounter[1367u]++ ;

#line 1133 "src/lp_analysis/vad_comp.sc"
	hangcount = 10;
	burstcount = 3;
    }

#line 9159 "vocoderSched.cc"
    __PRF_globalCounter[1368u]++ ;

#line 1137 "src/lp_analysis/vad_comp.sc"
    if (hangcount >= 0)
    {   

#line 9166 "vocoderSched.cc"
	__PRF_globalCounter[1369u]++ ;

#line 1139 "src/lp_analysis/vad_comp.sc"
	hangcount = sub(hangcount, 1);
	return 1;
    }

#line 9174 "vocoderSched.cc"
    __PRF_globalCounter[1370u]++ ;

#line 1142 "src/lp_analysis/vad_comp.sc"
    return vvad;
}

#line 9181 "vocoderSched.cc"
void VAD_Computation::init(void)

#line 143 "src/lp_analysis/vad_comp.sc"
{   
    int i;

#line 9188 "vocoderSched.cc"
    __PRF_globalCounter[1371u]++ ;

#line 147 "src/lp_analysis/vad_comp.sc"
    rvad[0] = 24576;

#line 9194 "vocoderSched.cc"
    __PRF_globalCounter[1372u]++ ;

#line 148 "src/lp_analysis/vad_comp.sc"
    for(i = 1; i < 9; i++ )
    {   

#line 9201 "vocoderSched.cc"
	__PRF_globalCounter[1373u]++ ;

#line 150 "src/lp_analysis/vad_comp.sc"
	rvad[i] = 0;
    }

#line 9208 "vocoderSched.cc"
    __PRF_globalCounter[1374u]++ ;

#line 152 "src/lp_analysis/vad_comp.sc"
    scal_rvad = 7;


    thvad.e = 20;
    thvad.m = 27083;

#line 9218 "vocoderSched.cc"
    __PRF_globalCounter[1375u]++ ;

#line 159 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i < 27; i++ )
    {   

#line 9225 "vocoderSched.cc"
	__PRF_globalCounter[1376u]++ ;

#line 161 "src/lp_analysis/vad_comp.sc"
	L_sacf[i] = 0l;
    }

#line 9232 "vocoderSched.cc"
    __PRF_globalCounter[1377u]++ ;

#line 163 "src/lp_analysis/vad_comp.sc"
    for(i = 0; i < 36; i++ )
    {   

#line 9239 "vocoderSched.cc"
	__PRF_globalCounter[1378u]++ ;

#line 165 "src/lp_analysis/vad_comp.sc"
	L_sav0[i] = 0l;
    }

#line 9246 "vocoderSched.cc"
    __PRF_globalCounter[1379u]++ ;

#line 167 "src/lp_analysis/vad_comp.sc"
    pt_sacf = 0;
    pt_sav0 = 0;


    L_lastdm = 0l;


    adaptcount = 0;


    burstcount = 0;
    hangcount =  -1;
}

#line 9264 "vocoderSched.cc"
void VAD_Computation::main(void)

#line 1149 "src/lp_analysis/vad_comp.sc"
{   
    int L_av0[9]; int L_av1[9];
    short int rav1[9]; short int scal_rav1;
    bool stat; bool tone;
    bool vvad;
    struct _fp acf0; struct _fp pvad;

#line 9275 "vocoderSched.cc"
    __PRF_globalCounter[1380u]++ ;
    _specc::waitfor(0ull);
    __PRF_globalCounter[1381u]++ ;

#line 1156 "src/lp_analysis/vad_comp.sc"
    if (reset_flag)

#line 9283 "vocoderSched.cc"
    {   
	__PRF_globalCounter[1382u]++ ;

#line 1156 "src/lp_analysis/vad_comp.sc"
	init();
    }

#line 9291 "vocoderSched.cc"
    __PRF_globalCounter[1383u]++ ;

#line 1158 "src/lp_analysis/vad_comp.sc"
    Energy_computation(r_h, scal_acf,  &acf0,  &pvad);
    Acf_averaging(r_h, r_l, scal_acf, L_av0, L_av1);
    Predictor_values(L_av1, rav1,  &scal_rav1);
    stat = Spectral_comparison(rav1, scal_rav1, L_av0);
    Tone_detection(rc,  &tone);
    Threshold_adaptation(stat, ptch, tone, rav1, scal_rav1, pvad, acf0);
    vvad = Vad_decision(pvad);
    vad = Vad_hangover(vvad);
}

#line 9305 "vocoderSched.cc"
void VAD_Computation::reset(void)

#line 183 "src/lp_analysis/vad_comp.sc"
{   

#line 9311 "vocoderSched.cc"
    __PRF_globalCounter[1384u]++ ;

#line 184 "src/lp_analysis/vad_comp.sc"
    init();
}

#line 9318 "vocoderSched.cc"
Vad_Lp::Vad_Lp(unsigned int _idcnt, short int (&r_l)[11], short int (&r_h)[11], short int (&scal_fac), short int (&rc)[4], bool (&ptch), short int (&txdtx_ctrl), bool (&dtx_mode), int (&L_pn_seed_tx), bool (&reset_flag))
    : _specc::behavior(_idcnt), r_l(r_l), r_h(r_h), scal_fac(scal_fac), rc(rc), ptch(ptch), txdtx_ctrl(txdtx_ctrl), dtx_mode(dtx_mode), L_pn_seed_tx(L_pn_seed_tx), reset_flag(reset_flag),
    nodtx_setflags(_IDcnt, VAD_flag, txdtx_ctrl),
    nop(_IDcnt),
    tx_dtx(_IDcnt, VAD_flag, txdtx_ctrl, L_pn_seed_tx, reset_flag),
    vad_computation(_IDcnt, r_h, r_l, scal_fac, rc, ptch, VAD_flag, reset_flag)
{   
}

Vad_Lp::~Vad_Lp(void)
{   
}

void Vad_Lp::main(void)

#line 44 "src/lp_analysis/vad_lp.sc"
{   

#line 9337 "vocoderSched.cc"
    __PRF_globalCounter[1385u]++ ;

#line 46 "src/lp_analysis/vad_lp.sc"
    { enum { _scc_state_0, _scc_state_nop, _scc_state_nodtx_setflags, _scc_state_vad_computation, _scc_state_tx_dtx } _scc_next_state = _scc_state_nop; do switch(_scc_next_state) {


	    case _scc_state_nop: { nop.main(); if (__PRF_globalCounter[1386u]++  , dtx_mode == 1) { _scc_next_state = _scc_state_vad_computation; break; } }

#line 56 "src/lp_analysis/vad_lp.sc"
	    case _scc_state_nodtx_setflags: { nodtx_setflags.main(); if (__PRF_globalCounter[1387u]++  , 1u) { _scc_next_state = _scc_state_0; break; } }


	    case _scc_state_vad_computation: { vad_computation.main(); if (__PRF_globalCounter[1388u]++  , 1u) { _scc_next_state = _scc_state_tx_dtx; break; } }




	    case _scc_state_tx_dtx: { tx_dtx.main(); if (__PRF_globalCounter[1389u]++  , 1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

#line 9359 "vocoderSched.cc"
void Vad_Lp::reset(void)

#line 37 "src/lp_analysis/vad_lp.sc"
{   

#line 9365 "vocoderSched.cc"
    __PRF_globalCounter[1390u]++ ;

#line 38 "src/lp_analysis/vad_lp.sc"
    vad_computation.reset();
    tx_dtx.reset();
}

#line 9373 "vocoderSched.cc"
Encoder_Monitor::Encoder_Monitor(unsigned int _idcnt, i_receiver (&serial_bits), i_receiver (&c_txdtx_ctrl))
    : _specc::behavior(_idcnt), serial_bits(serial_bits), c_txdtx_ctrl(c_txdtx_ctrl)
{   
}

Encoder_Monitor::~Encoder_Monitor(void)
{   
}

void Encoder_Monitor::main(void)

#line 57 "src/monitor.sc"
{   
    unsigned long long int f_start = 0ull;

    int i;



    short int tmp[244];
    short int txdtx_ctrl;

#line 9396 "vocoderSched.cc"
    __PRF_globalCounter[323u]++ ;

#line 72 "src/monitor.sc"
    while(true)
    {   

#line 9403 "vocoderSched.cc"
	__PRF_globalCounter[324u]++ ;
	__PRF_channelAccess(51256u, sizeof (tmp));

#line 83 "src/monitor.sc"
	serial_bits.receive(tmp, sizeof (tmp));

#line 9410 "vocoderSched.cc"
	__PRF_channelAccess(51257u, sizeof (txdtx_ctrl));

#line 84 "src/monitor.sc"
	c_txdtx_ctrl.receive( &txdtx_ctrl, sizeof (txdtx_ctrl));

#line 93 "src/monitor.sc"
	f_start += 20000000000ull;

#line 100 "src/monitor.sc"
	printf("\t\t encoding delay = %2.2f ms", (double)
	    (now() - f_start) / (double)1.000000000000000e+09);

#line 9423 "vocoderSched.cc"
	__PRF_globalCounter[325u]++ ;

#line 106 "src/monitor.sc"
	for(i = 0; i < 244; i++ )

#line 9429 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[326u]++ ;

#line 107 "src/monitor.sc"
	    fprintf(f_serial, "%d", tmp[i]);
	}

#line 9437 "vocoderSched.cc"
	__PRF_globalCounter[327u]++ ;

#line 111 "src/monitor.sc"
	fprintf(f_serial, "%d", (int)((txdtx_ctrl & 2) != 0));
	fprintf(f_serial, "%d", (int)((txdtx_ctrl & 1) != 0));
	fprintf(f_serial, "\n");
    }
}

#line 9447 "vocoderSched.cc"
int Encoder_Monitor::open_serial_outfile(

#line 42 "src/monitor.sc"
    char *serialfile_name)
{   

#line 9454 "vocoderSched.cc"
    __PRF_globalCounter[328u]++ ;

#line 45 "src/monitor.sc"
    if ((f_serial = fopen(serialfile_name, "w")) == 0)
    {   

#line 9461 "vocoderSched.cc"
	__PRF_globalCounter[329u]++ ;

#line 47 "src/monitor.sc"
	printf("Error opening output bitstream file %s !!\n", 
	    serialfile_name);
	return (0);
    }

#line 9470 "vocoderSched.cc"
    __PRF_globalCounter[330u]++ ;

#line 51 "src/monitor.sc"
    printf(" Output bitstream file:  %s\n", serialfile_name);
    return (1);
}

#line 9478 "vocoderSched.cc"
Get_Minmax::Get_Minmax(unsigned int _idcnt, short int (&T_op), short int (&T0_min), short int (&T0_max), short int (&txdtx_ctrl))
    : _specc::behavior(_idcnt), T_op(T_op), T0_min(T0_min), T0_max(T0_max), txdtx_ctrl(txdtx_ctrl)
{   
}

Get_Minmax::~Get_Minmax(void)
{   
}

void Get_Minmax::main(void)

#line 16 "src/open_loop/get_minmax.sc"
{   
    short int max; short int min;

#line 9494 "vocoderSched.cc"
    __PRF_globalCounter[418u]++ ;
    _specc::waitfor(66666ull);
    __PRF_globalCounter[419u]++ ;

#line 19 "src/open_loop/get_minmax.sc"
    if ((txdtx_ctrl & 1) != 0)
    {   

#line 9503 "vocoderSched.cc"
	__PRF_globalCounter[420u]++ ;

#line 23 "src/open_loop/get_minmax.sc"
	min = sub(T_op, 3);

#line 9509 "vocoderSched.cc"
	__PRF_globalCounter[421u]++ ;

#line 25 "src/open_loop/get_minmax.sc"
	if (sub(min, 18) < 0)
	{   

#line 9516 "vocoderSched.cc"
	    __PRF_globalCounter[422u]++ ;

#line 27 "src/open_loop/get_minmax.sc"
	    min = 18;
	}

#line 9523 "vocoderSched.cc"
	__PRF_globalCounter[423u]++ ;

#line 30 "src/open_loop/get_minmax.sc"
	max = add(min, 6);

#line 9529 "vocoderSched.cc"
	__PRF_globalCounter[424u]++ ;

#line 32 "src/open_loop/get_minmax.sc"
	if (sub(max, 143) > 0)
	{   

#line 9536 "vocoderSched.cc"
	    __PRF_globalCounter[425u]++ ;

#line 34 "src/open_loop/get_minmax.sc"
	    max = 143;
	    min = sub(max, 6);
	}

#line 9544 "vocoderSched.cc"
	__PRF_globalCounter[426u]++ ;

#line 38 "src/open_loop/get_minmax.sc"
	T0_min = min;
	T0_max = max;
    }
}

#line 9553 "vocoderSched.cc"
Ol_Lag_Est_Seq1::Ol_Lag_Est_Seq1(unsigned int _idcnt, short int *(&wsp), short int *(&p_wsp_1), short int *(&p_wsp_2))
    : _specc::behavior(_idcnt), wsp(wsp), p_wsp_1(p_wsp_1), p_wsp_2(p_wsp_2)
{   
}

Ol_Lag_Est_Seq1::~Ol_Lag_Est_Seq1(void)
{   
}

void Ol_Lag_Est_Seq1::main(void)

#line 17 "src/open_loop/ol_lag_est.sc"
{   

#line 9568 "vocoderSched.cc"
    __PRF_globalCounter[589u]++ ;
    _specc::waitfor(0ull);

#line 18 "src/open_loop/ol_lag_est.sc"
    p_wsp_1 = wsp;
    p_wsp_2 = wsp + 80;
}

#line 9577 "vocoderSched.cc"
Ol_Lag_Est_Seq2::Ol_Lag_Est_Seq2(unsigned int _idcnt, short int *(&p_wsp_1), short int *(&p_wsp_2), short int (&T_op_1), short int (&T_op_2))
    : _specc::behavior(_idcnt), p_wsp_1(p_wsp_1), p_wsp_2(p_wsp_2), T_op_1(T_op_1), T_op_2(T_op_2),
    pitch_ol_1(_IDcnt, p_wsp_1, T_op_1),
    pitch_ol_2(_IDcnt, p_wsp_2, T_op_2)
{   
}

Ol_Lag_Est_Seq2::~Ol_Lag_Est_Seq2(void)
{   
}

void Ol_Lag_Est_Seq2::main(void)
{   
    __PRF_globalCounter[590u]++ ;
    pitch_ol_1.main();
    pitch_ol_2.main();
}

Ol_Lag_Est_Seq3::Ol_Lag_Est_Seq3(unsigned int _idcnt, short int (&T_op_1), short int (&T_op_2), short int (&T0_min_1), short int (&T0_max_1), short int (&T0_min_2), short int (&T0_max_2), short int (&txdtx_ctrl))
    : _specc::behavior(_idcnt), T_op_1(T_op_1), T_op_2(T_op_2), T0_min_1(T0_min_1), T0_max_1(T0_max_1), T0_min_2(T0_min_2), T0_max_2(T0_max_2), txdtx_ctrl(txdtx_ctrl),
    minmax_1(_IDcnt, T_op_1, T0_min_1, T0_max_1, txdtx_ctrl),
    minmax_2(_IDcnt, T_op_2, T0_min_2, T0_max_2, txdtx_ctrl)
{   
}

Ol_Lag_Est_Seq3::~Ol_Lag_Est_Seq3(void)
{   
}

void Ol_Lag_Est_Seq3::main(void)
{   
    __PRF_globalCounter[591u]++ ;
    minmax_1.main();
    minmax_2.main();
}

Ol_Lag_Est_Seq4::Ol_Lag_Est_Seq4(unsigned int _idcnt, short int (&lags)[2], short int (&T_op_1))
    : _specc::behavior(_idcnt), lags(lags), T_op_1(T_op_1)
{   
}

Ol_Lag_Est_Seq4::~Ol_Lag_Est_Seq4(void)
{   
}

void Ol_Lag_Est_Seq4::main(void)

#line 70 "src/open_loop/ol_lag_est.sc"
{   

#line 9628 "vocoderSched.cc"
    __PRF_globalCounter[592u]++ ;
    _specc::waitfor(16666ull);

#line 71 "src/open_loop/ol_lag_est.sc"
    lags[0] = T_op_1;
}

#line 9636 "vocoderSched.cc"
Ol_Lag_Est_Seq5::Ol_Lag_Est_Seq5(unsigned int _idcnt, short int (&lags)[2], short int (&T_op_2))
    : _specc::behavior(_idcnt), lags(lags), T_op_2(T_op_2)
{   
}

Ol_Lag_Est_Seq5::~Ol_Lag_Est_Seq5(void)
{   
}

void Ol_Lag_Est_Seq5::main(void)

#line 79 "src/open_loop/ol_lag_est.sc"
{   

#line 9651 "vocoderSched.cc"
    __PRF_globalCounter[593u]++ ;
    _specc::waitfor(0ull);

#line 80 "src/open_loop/ol_lag_est.sc"
    lags[1] = T_op_2;
}

#line 9659 "vocoderSched.cc"
Ol_Lag_Est::Ol_Lag_Est(unsigned int _idcnt, short int *(&wsp), short int (&T0_min_1), short int (&T0_max_1), short int (&T0_min_2), short int (&T0_max_2), bool (&ptch), short int (&txdtx_ctrl), bool (&dtx_mode), bool (&reset_flag))
    : _specc::behavior(_idcnt), wsp(wsp), T0_min_1(T0_min_1), T0_max_1(T0_max_1), T0_min_2(T0_min_2), T0_max_2(T0_max_2), ptch(ptch), txdtx_ctrl(txdtx_ctrl), dtx_mode(dtx_mode), reset_flag(reset_flag),
    periodicity_update(_IDcnt, lags, ptch, dtx_mode, reset_flag),
    seq1(_IDcnt, wsp, p_wsp_1, p_wsp_2),
    seq2(_IDcnt, p_wsp_1, p_wsp_2, T_op_1, T_op_2),
    seq3(_IDcnt, T_op_1, T_op_2, T0_min_1, T0_max_1, T0_min_2, T0_max_2, txdtx_ctrl),
    seq4(_IDcnt, lags, T_op_1),
    seq5(_IDcnt, lags, T_op_2)
{   
}

Ol_Lag_Est::~Ol_Lag_Est(void)
{   
}

void Ol_Lag_Est::main(void)

#line 122 "src/open_loop/ol_lag_est.sc"
{   

#line 9680 "vocoderSched.cc"
    __PRF_globalCounter[581u]++ ;

#line 123 "src/open_loop/ol_lag_est.sc"
    { enum { _scc_state_0, _scc_state_seq1, _scc_state_seq2, _scc_state_seq3, _scc_state_seq4, _scc_state_seq5, _scc_state_periodicity_update } _scc_next_state = _scc_state_seq1; do switch(_scc_next_state) {

	    case _scc_state_seq1: { seq1.main(); if (__PRF_globalCounter[582u]++  , 1u) { _scc_next_state = _scc_state_seq2; break; } }

	    case _scc_state_seq2: { seq2.main(); if (__PRF_globalCounter[583u]++  , 1u) { _scc_next_state = _scc_state_seq3; break; } }

	    case _scc_state_seq3: { seq3.main(); if (__PRF_globalCounter[584u]++  , 1u) { _scc_next_state = _scc_state_seq4; break; } }


	    case _scc_state_seq4: { seq4.main(); if (__PRF_globalCounter[585u]++  , dtx_mode == 1) { _scc_next_state = _scc_state_seq5; break; }
		{ _scc_next_state = _scc_state_periodicity_update; break; } }


	    case _scc_state_seq5: { seq5.main(); if (__PRF_globalCounter[586u]++  , 1u) { _scc_next_state = _scc_state_periodicity_update; break; } }

	    case _scc_state_periodicity_update: { periodicity_update.main(); if (__PRF_globalCounter[587u]++  , 1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

#line 9703 "vocoderSched.cc"
void Ol_Lag_Est::reset(void)

#line 116 "src/open_loop/ol_lag_est.sc"
{   

#line 9709 "vocoderSched.cc"
    __PRF_globalCounter[588u]++ ;

#line 117 "src/open_loop/ol_lag_est.sc"
    periodicity_update.reset();
}

#line 9716 "vocoderSched.cc"
Open_Loop_Init::Open_Loop_Init(unsigned int _idcnt, int (&i), bool (&reset_flag), short int (&mem_w)[10])
    : _specc::behavior(_idcnt), i(i), reset_flag(reset_flag), mem_w(mem_w)
{   
}

Open_Loop_Init::~Open_Loop_Init(void)
{   
}

void Open_Loop_Init::init(void)

#line 22 "src/open_loop/open_loop.sc"
{   

#line 9731 "vocoderSched.cc"
    __PRF_globalCounter[606u]++ ;

#line 23 "src/open_loop/open_loop.sc"
    Set_zero(mem_w, 10);
}

#line 9738 "vocoderSched.cc"
void Open_Loop_Init::main(void)

#line 32 "src/open_loop/open_loop.sc"
{   

#line 9744 "vocoderSched.cc"
    __PRF_globalCounter[607u]++ ;
    _specc::waitfor(366666ull);
    __PRF_globalCounter[608u]++ ;

#line 33 "src/open_loop/open_loop.sc"
    if (reset_flag)

#line 9752 "vocoderSched.cc"
    {   
	__PRF_globalCounter[609u]++ ;

#line 33 "src/open_loop/open_loop.sc"
	init();
    }

#line 9760 "vocoderSched.cc"
    __PRF_globalCounter[610u]++ ;

#line 35 "src/open_loop/open_loop.sc"
    i = 0;
}

#line 9767 "vocoderSched.cc"
void Open_Loop_Init::reset(void)

#line 27 "src/open_loop/open_loop.sc"
{   

#line 9773 "vocoderSched.cc"
    __PRF_globalCounter[611u]++ ;

#line 28 "src/open_loop/open_loop.sc"
    init();
}

#line 9780 "vocoderSched.cc"
Open_Loop_Body1::Open_Loop_Body1(unsigned int _idcnt, short int (&A_t)[4][11], short int *(&p_speech), int (&i), short int (&A_t_i)[11], short int *(&p_speech_i))
    : _specc::behavior(_idcnt), A_t(A_t), p_speech(p_speech), i(i), A_t_i(A_t_i), p_speech_i(p_speech_i)
{   
}

Open_Loop_Body1::~Open_Loop_Body1(void)
{   
}

void Open_Loop_Body1::main(void)

#line 49 "src/open_loop/open_loop.sc"
{   

#line 9795 "vocoderSched.cc"
    __PRF_globalCounter[603u]++ ;
    _specc::waitfor(33333ull);

#line 50 "src/open_loop/open_loop.sc"
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<11;_scc_index_0++) (A_t_i)[_scc_index_0] = (A_t[i])[_scc_index_0]; }
    p_speech_i = p_speech + i * 40;
}

#line 9804 "vocoderSched.cc"
Open_Loop_Body2::Open_Loop_Body2(unsigned int _idcnt, short int (&A_t_i)[11], short int (&ap1)[11], short int (&ap2)[11])
    : _specc::behavior(_idcnt), A_t_i(A_t_i), ap1(ap1), ap2(ap2),
    weight_ai_1(_IDcnt, A_t_i, F_gamma1, ap1),
    weight_ai_2(_IDcnt, A_t_i, F_gamma2, ap2)
{   
}

Open_Loop_Body2::~Open_Loop_Body2(void)
{   
}

void Open_Loop_Body2::main(void)
{   
    __PRF_globalCounter[604u]++ ;
    weight_ai_1.main();
    weight_ai_2.main();
}

Open_Loop_End::Open_Loop_End(unsigned int _idcnt, int (&i), short int (&wsp_i)[40], short int *(&wsp))
    : _specc::behavior(_idcnt), i(i), wsp_i(wsp_i), wsp(wsp)
{   
}

Open_Loop_End::~Open_Loop_End(void)
{   
}

void Open_Loop_End::main(void)

#line 82 "src/open_loop/open_loop.sc"
{   

#line 9837 "vocoderSched.cc"
    __PRF_globalCounter[605u]++ ;
    _specc::waitfor(50000ull);

#line 83 "src/open_loop/open_loop.sc"
    Copy(wsp_i, (short int *)wsp + i * 40, 40);
    i++ ;
}

#line 9846 "vocoderSched.cc"
Period_Upd::Period_Upd(unsigned int _idcnt, short int (&lags)[2], bool (&ptch), bool (&dtx_mode), bool (&reset_flag))
    : _specc::behavior(_idcnt), lags(lags), ptch(ptch), dtx_mode(dtx_mode), reset_flag(reset_flag)
{   
}

Period_Upd::~Period_Upd(void)
{   
}

void Period_Upd::init(void)

#line 43 "src/open_loop/period_upd.sc"
{   

#line 9861 "vocoderSched.cc"
    __PRF_globalCounter[613u]++ ;

#line 45 "src/open_loop/period_upd.sc"
    oldlagcount = 0;
    veryoldlagcount = 0;
    oldlag = 18;
}

#line 9870 "vocoderSched.cc"
void Period_Upd::main(void)

#line 56 "src/open_loop/period_upd.sc"
{   
    short int lagcount; short int maxlag; short int minlag; short int temp;
    int i;

#line 9878 "vocoderSched.cc"
    __PRF_globalCounter[614u]++ ;
    _specc::waitfor(0ull);
    __PRF_globalCounter[615u]++ ;

#line 60 "src/open_loop/period_upd.sc"
    if (reset_flag)

#line 9886 "vocoderSched.cc"
    {   
	__PRF_globalCounter[616u]++ ;

#line 60 "src/open_loop/period_upd.sc"
	init();
    }

#line 9894 "vocoderSched.cc"
    __PRF_globalCounter[617u]++ ;

#line 62 "src/open_loop/period_upd.sc"
    if (dtx_mode != 1)

#line 9900 "vocoderSched.cc"
    {   
	__PRF_globalCounter[618u]++ ;

#line 62 "src/open_loop/period_upd.sc"
	return ;
    }

#line 9908 "vocoderSched.cc"
    __PRF_globalCounter[619u]++ ;

#line 66 "src/open_loop/period_upd.sc"
    lagcount = 0;

#line 9914 "vocoderSched.cc"
    __PRF_globalCounter[620u]++ ;

#line 68 "src/open_loop/period_upd.sc"
    for(i = 0; i <= 1; i++ )
    {   

#line 9921 "vocoderSched.cc"
	__PRF_globalCounter[621u]++ ;

#line 73 "src/open_loop/period_upd.sc"
	if (sub(oldlag, lags[i]) > 0)
	{   

#line 9928 "vocoderSched.cc"
	    __PRF_globalCounter[622u]++ ;

#line 75 "src/open_loop/period_upd.sc"
	    minlag = lags[i];
	    maxlag = oldlag;
	}
	else 
	{   

#line 9938 "vocoderSched.cc"
	    __PRF_globalCounter[623u]++ ;

#line 80 "src/open_loop/period_upd.sc"
	    minlag = oldlag;
	    maxlag = lags[i];
	}

#line 9946 "vocoderSched.cc"
	__PRF_globalCounter[624u]++ ;

#line 84 "src/open_loop/period_upd.sc"
	temp = sub(maxlag, minlag);

#line 9952 "vocoderSched.cc"
	__PRF_globalCounter[625u]++ ;

#line 87 "src/open_loop/period_upd.sc"
	if (sub(temp, 2) < 0)
	{   

#line 9959 "vocoderSched.cc"
	    __PRF_globalCounter[626u]++ ;

#line 89 "src/open_loop/period_upd.sc"
	    lagcount = add(lagcount, 1);
	}

#line 9966 "vocoderSched.cc"
	__PRF_globalCounter[627u]++ ;

#line 93 "src/open_loop/period_upd.sc"
	oldlag = lags[i];
    }

#line 9973 "vocoderSched.cc"
    __PRF_globalCounter[628u]++ ;

#line 98 "src/open_loop/period_upd.sc"
    veryoldlagcount = oldlagcount;

    oldlagcount = lagcount;



    temp = add(oldlagcount, veryoldlagcount);

#line 9985 "vocoderSched.cc"
    __PRF_globalCounter[629u]++ ;

#line 107 "src/open_loop/period_upd.sc"
    if (sub(temp, 4) >= 0)
    {   

#line 9992 "vocoderSched.cc"
	__PRF_globalCounter[630u]++ ;

#line 109 "src/open_loop/period_upd.sc"
	ptch = 1;
    }
    else 
    {   

#line 10001 "vocoderSched.cc"
	__PRF_globalCounter[631u]++ ;

#line 113 "src/open_loop/period_upd.sc"
	ptch = 0;
    }
}

#line 10009 "vocoderSched.cc"
void Period_Upd::reset(void)

#line 51 "src/open_loop/period_upd.sc"
{   

#line 10015 "vocoderSched.cc"
    __PRF_globalCounter[632u]++ ;

#line 52 "src/open_loop/period_upd.sc"
    init();
}

#line 10022 "vocoderSched.cc"
Pitch_Ol::Pitch_Ol(unsigned int _idcnt, short int *(&sig), short int (&p_max1))
    : _specc::behavior(_idcnt), sig(sig), p_max1(p_max1)
{   
}

Pitch_Ol::~Pitch_Ol(void)
{   
}

short int Pitch_Ol::Lag_max(

#line 57 "src/open_loop/pitch_ol.sc"
    short int scal_sig[], 
    short int scal_fac, 
    short int L_frame, 
    short int lag_max, 
    short int lag_min, 
    short int *cor_max)
{   
    int i; int j;
    short int *p; short int *p1;
    int max; int t0;
    short int ener_h; short int ener_l; short int max_h; short int max_l;
    short int p_max;

#line 10048 "vocoderSched.cc"
    __PRF_globalCounter[679u]++ ;

#line 70 "src/open_loop/pitch_ol.sc"
    max = (int)2147483648ul;

#line 10054 "vocoderSched.cc"
    __PRF_globalCounter[680u]++ ;

#line 72 "src/open_loop/pitch_ol.sc"
    for(i = lag_max; i >= lag_min; i-- )
    {   

#line 10061 "vocoderSched.cc"
	__PRF_globalCounter[681u]++ ;

#line 74 "src/open_loop/pitch_ol.sc"
	p = scal_sig;
	p1 =  &scal_sig[ -i];
	t0 = 0;

#line 10069 "vocoderSched.cc"
	__PRF_globalCounter[682u]++ ;

#line 78 "src/open_loop/pitch_ol.sc"
	for(j = 0; j < L_frame; j++  , p++  , p1++ )
	{   

#line 10076 "vocoderSched.cc"
	    __PRF_globalCounter[683u]++ ;

#line 80 "src/open_loop/pitch_ol.sc"
	    t0 = L_mac(t0,  *p,  *p1);
	}

#line 10083 "vocoderSched.cc"
	__PRF_globalCounter[684u]++ ;

#line 83 "src/open_loop/pitch_ol.sc"
	if (L_sub(t0, max) >= 0)
	{   

#line 10090 "vocoderSched.cc"
	    __PRF_globalCounter[685u]++ ;

#line 85 "src/open_loop/pitch_ol.sc"
	    max = t0;
	    p_max = i;
	}
    }

#line 10099 "vocoderSched.cc"
    __PRF_globalCounter[686u]++ ;

#line 92 "src/open_loop/pitch_ol.sc"
    t0 = 0;
    p =  &scal_sig[ -p_max];

#line 10106 "vocoderSched.cc"
    __PRF_globalCounter[687u]++ ;

#line 94 "src/open_loop/pitch_ol.sc"
    for(i = 0; i < L_frame; i++  , p++ )
    {   

#line 10113 "vocoderSched.cc"
	__PRF_globalCounter[688u]++ ;

#line 96 "src/open_loop/pitch_ol.sc"
	t0 = L_mac(t0,  *p,  *p);
    }

#line 10120 "vocoderSched.cc"
    __PRF_globalCounter[689u]++ ;

#line 100 "src/open_loop/pitch_ol.sc"
    t0 = Inv_sqrt(t0);
    t0 = L_shl(t0, 1);



    L_Extract(max,  &max_h,  &max_l);
    L_Extract(t0,  &ener_h,  &ener_l);

    t0 = Mpy_32(max_h, max_l, ener_h, ener_l);
    t0 = L_shr(t0, scal_fac);

     *cor_max = extract_h(L_shl(t0, 15));

    return (p_max);
}

#line 10140 "vocoderSched.cc"
void Pitch_Ol::main(void)

#line 119 "src/open_loop/pitch_ol.sc"
{   
    int i; int j;
    short int max1; short int max2; short int max3;
    short int p_max2; short int p_max3;
    int t0;




    short int scaled_signal[512];
    short int scal_fac; short int *scal_sig;

#line 10156 "vocoderSched.cc"
    __PRF_globalCounter[690u]++ ;
    _specc::waitfor(552633333ull);

#line 131 "src/open_loop/pitch_ol.sc"
    scal_sig =  &scaled_signal[143];

    t0 = 0l;

#line 10165 "vocoderSched.cc"
    __PRF_globalCounter[691u]++ ;

#line 134 "src/open_loop/pitch_ol.sc"
    for(i =  -143; i < 80; i++ )
    {   

#line 10172 "vocoderSched.cc"
	__PRF_globalCounter[692u]++ ;

#line 136 "src/open_loop/pitch_ol.sc"
	t0 = L_mac(t0, sig[i], sig[i]);
    }

#line 10179 "vocoderSched.cc"
    __PRF_globalCounter[693u]++ ;

#line 151 "src/open_loop/pitch_ol.sc"
    if (L_sub(t0, (int)2147483647l) == 0l)
    {   

#line 10186 "vocoderSched.cc"
	__PRF_globalCounter[694u]++ ;

#line 153 "src/open_loop/pitch_ol.sc"
	for(i =  -143; i < 80; i++ )
	{   

#line 10193 "vocoderSched.cc"
	    __PRF_globalCounter[695u]++ ;

#line 155 "src/open_loop/pitch_ol.sc"
	    scal_sig[i] = shr(sig[i], 3);
	}

#line 10200 "vocoderSched.cc"
	__PRF_globalCounter[696u]++ ;

#line 157 "src/open_loop/pitch_ol.sc"
	scal_fac = 3;
    }
    else 

#line 10208 "vocoderSched.cc"
    {   
	__PRF_globalCounter[697u]++ ;

#line 159 "src/open_loop/pitch_ol.sc"
	if (L_sub(t0, (int)1048576l) < (int)0)

	{   

#line 10217 "vocoderSched.cc"
	    __PRF_globalCounter[698u]++ ;

#line 162 "src/open_loop/pitch_ol.sc"
	    for(i =  -143; i < 80; i++ )
	    {   

#line 10224 "vocoderSched.cc"
		__PRF_globalCounter[699u]++ ;

#line 164 "src/open_loop/pitch_ol.sc"
		scal_sig[i] = shl(sig[i], 3);
	    }

#line 10231 "vocoderSched.cc"
	    __PRF_globalCounter[700u]++ ;

#line 166 "src/open_loop/pitch_ol.sc"
	    scal_fac =  -3;
	}
	else 
	{   

#line 10240 "vocoderSched.cc"
	    __PRF_globalCounter[701u]++ ;

#line 170 "src/open_loop/pitch_ol.sc"
	    for(i =  -143; i < 80; i++ )
	    {   

#line 10247 "vocoderSched.cc"
		__PRF_globalCounter[702u]++ ;

#line 172 "src/open_loop/pitch_ol.sc"
		scal_sig[i] = sig[i];
	    }

#line 10254 "vocoderSched.cc"
	    __PRF_globalCounter[703u]++ ;

#line 174 "src/open_loop/pitch_ol.sc"
	    scal_fac = 0;
	}
    }

#line 10262 "vocoderSched.cc"
    __PRF_globalCounter[704u]++ ;

#line 188 "src/open_loop/pitch_ol.sc"
    j = shl(18, 2);
    p_max1 = Lag_max(scal_sig, scal_fac, 80, 143, j,  &max1);

    i = sub(j, 1);
    j = shl(18, 1);
    p_max2 = Lag_max(scal_sig, scal_fac, 80, i, j,  &max2);

    i = sub(j, 1);
    p_max3 = Lag_max(scal_sig, scal_fac, 80, i, 18,  &max3);

#line 10276 "vocoderSched.cc"
    __PRF_globalCounter[705u]++ ;

#line 203 "src/open_loop/pitch_ol.sc"
    if (sub(mult(max1, 27853), max2) < 0)
    {   

#line 10283 "vocoderSched.cc"
	__PRF_globalCounter[706u]++ ;

#line 205 "src/open_loop/pitch_ol.sc"
	max1 = max2;
	p_max1 = p_max2;
    }

#line 10291 "vocoderSched.cc"
    __PRF_globalCounter[707u]++ ;

#line 209 "src/open_loop/pitch_ol.sc"
    if (sub(mult(max1, 27853), max3) < 0)
    {   

#line 10298 "vocoderSched.cc"
	__PRF_globalCounter[708u]++ ;

#line 211 "src/open_loop/pitch_ol.sc"
	p_max1 = p_max3;
    }
}

#line 10306 "vocoderSched.cc"
Cn_Encoder::Cn_Encoder(unsigned int _idcnt, short int (&paramsi)[57], short int (&paramso)[57], short int (&txdtx_ctrl), bool (&reset_flag))
    : _specc::behavior(_idcnt), paramsi(paramsi), paramso(paramso), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag)
{   
}

Cn_Encoder::~Cn_Encoder(void)
{   
}

void Cn_Encoder::init(void)

#line 43 "src/processing/cn_encoding.sc"
{   
    int i;

#line 10322 "vocoderSched.cc"
    __PRF_globalCounter[214u]++ ;

#line 46 "src/processing/cn_encoding.sc"
    for(i = 0; i < 6; i++ )
    {   

#line 10329 "vocoderSched.cc"
	__PRF_globalCounter[215u]++ ;

#line 48 "src/processing/cn_encoding.sc"
	old_CN_mem_tx[i] = 0;
    }
}

#line 10337 "vocoderSched.cc"
void Cn_Encoder::main(void)

#line 58 "src/processing/cn_encoding.sc"
{   
    int i;

#line 10344 "vocoderSched.cc"
    __PRF_globalCounter[216u]++ ;
    _specc::waitfor(0ull);
    __PRF_globalCounter[217u]++ ;

#line 61 "src/processing/cn_encoding.sc"
    if (reset_flag)

#line 10352 "vocoderSched.cc"
    {   
	__PRF_globalCounter[218u]++ ;

#line 61 "src/processing/cn_encoding.sc"
	init();
    }

#line 10360 "vocoderSched.cc"
    __PRF_globalCounter[219u]++ ;

#line 63 "src/processing/cn_encoding.sc"
    if ((txdtx_ctrl & 1) == 0)
    {   

#line 10367 "vocoderSched.cc"
	__PRF_globalCounter[220u]++ ;

#line 65 "src/processing/cn_encoding.sc"
	if ((txdtx_ctrl & 16) != 0)
	{   

#line 10374 "vocoderSched.cc"
	    __PRF_globalCounter[221u]++ ;

#line 71 "src/processing/cn_encoding.sc"
	    for(i = 0; i < 5; i++ )
	    {   

#line 10381 "vocoderSched.cc"
		__PRF_globalCounter[222u]++ ;

#line 73 "src/processing/cn_encoding.sc"
		old_CN_mem_tx[i] = paramsi[i];
	    }

#line 10388 "vocoderSched.cc"
	    __PRF_globalCounter[223u]++ ;

#line 76 "src/processing/cn_encoding.sc"
	    old_CN_mem_tx[5] = paramsi[56];
	}

#line 10395 "vocoderSched.cc"
	__PRF_globalCounter[224u]++ ;

#line 78 "src/processing/cn_encoding.sc"
	if ((txdtx_ctrl & 32) != 0)
	{   

#line 10402 "vocoderSched.cc"
	    __PRF_globalCounter[225u]++ ;

#line 81 "src/processing/cn_encoding.sc"
	    for(i = 0; i < 5; i++ )
	    {   

#line 10409 "vocoderSched.cc"
		__PRF_globalCounter[226u]++ ;

#line 83 "src/processing/cn_encoding.sc"
		paramso[i] = old_CN_mem_tx[i];
	    }

#line 10416 "vocoderSched.cc"
	    __PRF_globalCounter[227u]++ ;

#line 85 "src/processing/cn_encoding.sc"
	    paramso[17] = old_CN_mem_tx[5];
	    paramso[30] = old_CN_mem_tx[5];
	    paramso[43] = old_CN_mem_tx[5];
	    paramso[56] = old_CN_mem_tx[5];
	}

#line 10426 "vocoderSched.cc"
	__PRF_globalCounter[228u]++ ;

#line 92 "src/processing/cn_encoding.sc"
	for(i = 0; i < 12; i++ )
	{   

#line 10433 "vocoderSched.cc"
	    __PRF_globalCounter[229u]++ ;

#line 94 "src/processing/cn_encoding.sc"
	    paramso[i + 5] = 0;
	    paramso[i + 18] = 0;
	    paramso[i + 31] = 0;
	    paramso[i + 44] = 0;
	}
    }
    else  {

#line 10445 "vocoderSched.cc"
	__PRF_globalCounter[230u]++ ;

#line 101 "src/processing/cn_encoding.sc"
	{ unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<57;_scc_index_0++) (paramso)[_scc_index_0] = (paramsi)[_scc_index_0]; }
    }
}

#line 10453 "vocoderSched.cc"
void Cn_Encoder::reset(void)

#line 53 "src/processing/cn_encoding.sc"
{   

#line 10459 "vocoderSched.cc"
    __PRF_globalCounter[231u]++ ;

#line 54 "src/processing/cn_encoding.sc"
    init();
}

#line 10466 "vocoderSched.cc"
Filter_And_Scale::Filter_And_Scale(unsigned int _idcnt, bool (&reset_flag), short int (&input_frame)[160], short int (&sig_out)[160])
    : _specc::behavior(_idcnt), reset_flag(reset_flag), input_frame(input_frame), sig_out(sig_out)
{   
}

Filter_And_Scale::~Filter_And_Scale(void)
{   
}

void Filter_And_Scale::init(void)

#line 44 "src/processing/filter_and_scale.sc"
{   

#line 10481 "vocoderSched.cc"
    __PRF_globalCounter[352u]++ ;

#line 45 "src/processing/filter_and_scale.sc"
    y2_hi = 0;
    y2_lo = 0;
    y1_hi = 0;
    y1_lo = 0;
    x0 = 0;
    x1 = 0;
}

#line 10493 "vocoderSched.cc"
void Filter_And_Scale::main(void)

#line 71 "src/processing/filter_and_scale.sc"
{   
    int i;
    short int x2;
    int L_tmp;


    const short int b[3] = { ((short int)1899),((short int)-3798),((short int)1899) };
    const short int a[3] = { ((short int)4096),((short int)7807),((short int)-3733) };

#line 10506 "vocoderSched.cc"
    __PRF_globalCounter[353u]++ ;
    _specc::waitfor(40033333ull);
    __PRF_globalCounter[354u]++ ;

#line 80 "src/processing/filter_and_scale.sc"
    if (reset_flag)

#line 10514 "vocoderSched.cc"
    {   
	__PRF_globalCounter[355u]++ ;

#line 80 "src/processing/filter_and_scale.sc"
	init();
    }

#line 10522 "vocoderSched.cc"
    __PRF_globalCounter[356u]++ ;

#line 83 "src/processing/filter_and_scale.sc"
    for(i = 0; i < 160; i++ )
    {   

#line 10529 "vocoderSched.cc"
	__PRF_globalCounter[357u]++ ;

#line 85 "src/processing/filter_and_scale.sc"
	x2 = x1;
	x1 = x0;

	x0 = (input_frame[i] << 3);

#line 97 "src/processing/filter_and_scale.sc"
	L_tmp = Mpy_32_16(y1_hi, y1_lo, a[1]);
	L_tmp = L_add(L_tmp, Mpy_32_16(y2_hi, y2_lo, a[2]));
	L_tmp = L_mac(L_tmp, x0, b[0]);
	L_tmp = L_mac(L_tmp, x1, b[1]);
	L_tmp = L_mac(L_tmp, x2, b[2]);
	L_tmp = L_shl(L_tmp, 3);
	sig_out[i] = round(L_tmp);

	y2_hi = y1_hi;
	y2_lo = y1_lo;
	L_Extract(L_tmp,  &y1_hi,  &y1_lo);
    }
}

#line 10553 "vocoderSched.cc"
void Filter_And_Scale::reset(void)

#line 54 "src/processing/filter_and_scale.sc"
{   

#line 10559 "vocoderSched.cc"
    __PRF_globalCounter[358u]++ ;

#line 55 "src/processing/filter_and_scale.sc"
    init();
}

#line 10566 "vocoderSched.cc"
Encoder_Homingframe_Test::Encoder_Homingframe_Test(unsigned int _idcnt, short int (&input_frame)[160], bool (&j))
    : _specc::behavior(_idcnt), input_frame(input_frame), j(j)
{   
}

Encoder_Homingframe_Test::~Encoder_Homingframe_Test(void)
{   
}

void Encoder_Homingframe_Test::main(void)

#line 48 "src/processing/homing_test.sc"
{   
    bool f;
    int i;

#line 10583 "vocoderSched.cc"
    __PRF_globalCounter[318u]++ ;
    _specc::waitfor(83333ull);
    __PRF_globalCounter[319u]++ ;

#line 53 "src/processing/homing_test.sc"
    for(i = 0; i < 160; i++ )
    {   

#line 10592 "vocoderSched.cc"
	__PRF_globalCounter[320u]++ ;

#line 55 "src/processing/homing_test.sc"
	if ( !(f = input_frame[i] == 1))

#line 10598 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[321u]++ ;

#line 56 "src/processing/homing_test.sc"
	    break;
	}
    }

#line 10607 "vocoderSched.cc"
    __PRF_globalCounter[322u]++ ;

#line 59 "src/processing/homing_test.sc"
    j = f;
}

#line 10614 "vocoderSched.cc"
Decoder_Homingframe_Test::Decoder_Homingframe_Test(unsigned int _idcnt, short int *(&parm), int (&nbr_of_params), bool (&res))
    : _specc::behavior(_idcnt), parm(parm), nbr_of_params(nbr_of_params), res(res)
{   
}

Decoder_Homingframe_Test::~Decoder_Homingframe_Test(void)
{   
}

void Decoder_Homingframe_Test::main(void)

#line 154 "src/processing/homing_test.sc"
{   
    short int i; short int j;

    const short int dhf_mask[57] = { 

      ((short int)4),
      ((short int)47),
      ((short int)180),
      ((short int)144),
      ((short int)62),

      ((short int)342),
      ((short int)11),
      ((short int)0),
      ((short int)1),
      ((short int)15),
      ((short int)1),
      ((short int)13),
      ((short int)0),
      ((short int)3),
      ((short int)0),
      ((short int)3),
      ((short int)0),
      ((short int)3),

      ((short int)54),
      ((short int)1),
      ((short int)8),
      ((short int)8),
      ((short int)5),
      ((short int)8),
      ((short int)1),
      ((short int)0),
      ((short int)0),
      ((short int)1),
      ((short int)1),
      ((short int)0),
      ((short int)0),

      ((short int)342),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),

      ((short int)54),
      ((short int)11),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0),
      ((short int)0) };

#line 10694 "vocoderSched.cc"
    __PRF_globalCounter[301u]++ ;

#line 221 "src/processing/homing_test.sc"
    for(i = 0; i < nbr_of_params; i++ )
    {   

#line 10701 "vocoderSched.cc"
	__PRF_globalCounter[302u]++ ;

#line 223 "src/processing/homing_test.sc"
	j = parm[i] ^ dhf_mask[i];

#line 10707 "vocoderSched.cc"
	__PRF_globalCounter[303u]++ ;

#line 225 "src/processing/homing_test.sc"
	if (j)

#line 10713 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[304u]++ ;

#line 226 "src/processing/homing_test.sc"
	    break;
	}
    }

#line 10722 "vocoderSched.cc"
    __PRF_globalCounter[305u]++ ;

#line 229 "src/processing/homing_test.sc"
    res =  !j;
}

#line 10729 "vocoderSched.cc"
Post_Process_Seq1::Post_Process_Seq1(unsigned int _idcnt, short int (&txdtx_ctrl), short int (&local_serial)[244], i_sender (&txdtx_ctrl_out), i_sender (&serial))
    : _specc::behavior(_idcnt), txdtx_ctrl(txdtx_ctrl), local_serial(local_serial), txdtx_ctrl_out(txdtx_ctrl_out), serial(serial)
{   
}

Post_Process_Seq1::~Post_Process_Seq1(void)
{   
}

void Post_Process_Seq1::main(void)

#line 31 "src/processing/post_process.sc"
{   

#line 10744 "vocoderSched.cc"
    __PRF_globalCounter[741u]++ ;
    _specc::waitfor(33333ull);
    __PRF_channelAccess(114131u, sizeof (local_serial));

#line 43 "src/processing/post_process.sc"
    serial.send(local_serial, sizeof (local_serial));

#line 10752 "vocoderSched.cc"
    __PRF_channelAccess(114130u, sizeof (txdtx_ctrl));

#line 44 "src/processing/post_process.sc"
    txdtx_ctrl_out.send( &txdtx_ctrl, sizeof (txdtx_ctrl));
}

#line 10759 "vocoderSched.cc"
Pre_Process_Seq1::Pre_Process_Seq1(unsigned int _idcnt, bool (&reset_next), bool (&reset_flag), bool (&reset_flag_1), bool (&reset_flag_2), i_receiver (&speech_frame), i_receiver (&dtx_mode), short int (&local_speech_frame)[160], bool (&local_dtx_mode))
    : _specc::behavior(_idcnt), reset_next(reset_next), reset_flag(reset_flag), reset_flag_1(reset_flag_1), reset_flag_2(reset_flag_2), speech_frame(speech_frame), dtx_mode(dtx_mode), local_speech_frame(local_speech_frame), local_dtx_mode(local_dtx_mode)
{   
}

Pre_Process_Seq1::~Pre_Process_Seq1(void)
{   
}

void Pre_Process_Seq1::main(void)

#line 43 "src/processing/pre_process.sc"
{   

    static bool First = true;

#line 10776 "vocoderSched.cc"
    __PRF_globalCounter[744u]++ ;
    _specc::waitfor(66666ull);

#line 52 "src/processing/pre_process.sc"
    reset_flag_1 = reset_flag = reset_next;
    reset_flag_2 = reset_flag;

#line 10784 "vocoderSched.cc"
    __PRF_globalCounter[745u]++ ;

#line 55 "src/processing/pre_process.sc"
    if (reset_next == 1)
    {   

#line 10791 "vocoderSched.cc"
	__PRF_globalCounter[746u]++ ;

#line 62 "src/processing/pre_process.sc"
	reset_next = 0;
    }

#line 10798 "vocoderSched.cc"
    __PRF_globalCounter[747u]++ ;

#line 73 "src/processing/pre_process.sc"
    if (First)
    {   

#line 10805 "vocoderSched.cc"
	__PRF_globalCounter[748u]++ ;
	__PRF_channelAccess(116261u, sizeof (local_dtx_mode));

#line 75 "src/processing/pre_process.sc"
	dtx_mode.receive( &local_dtx_mode, sizeof (local_dtx_mode));
	First = false;
    }

#line 10814 "vocoderSched.cc"
    __PRF_globalCounter[749u]++ ;
    __PRF_channelAccess(116260u, sizeof (local_speech_frame));

#line 93 "src/processing/pre_process.sc"
    speech_frame.receive(local_speech_frame, sizeof (local_speech_frame));
}

#line 10822 "vocoderSched.cc"
Prm2bits_12k2::Prm2bits_12k2(unsigned int _idcnt, short int (&prm)[57], short int (&serial)[244])
    : _specc::behavior(_idcnt), prm(prm), serial(serial)
{   
}

Prm2bits_12k2::~Prm2bits_12k2(void)
{   
}

void Prm2bits_12k2::Int2bin(

#line 58 "src/processing/prm2bits.sc"
    short int value, 
    short int no_of_bits, 
    short int *bitstream)

{   
    short int *pt_bitstream;
    short int bt; short int i;

#line 10843 "vocoderSched.cc"
    __PRF_globalCounter[765u]++ ;

#line 66 "src/processing/prm2bits.sc"
    pt_bitstream =  &bitstream[no_of_bits];

#line 10849 "vocoderSched.cc"
    __PRF_globalCounter[766u]++ ;

#line 68 "src/processing/prm2bits.sc"
    for(i = 0; i < no_of_bits; i++ )
    {   

#line 10856 "vocoderSched.cc"
	__PRF_globalCounter[767u]++ ;

#line 70 "src/processing/prm2bits.sc"
	bt = value & 1;

#line 10862 "vocoderSched.cc"
	__PRF_globalCounter[768u]++ ;

#line 72 "src/processing/prm2bits.sc"
	if (bt == 0)
	{   

#line 10869 "vocoderSched.cc"
	    __PRF_globalCounter[769u]++ ;

#line 74 "src/processing/prm2bits.sc"
	     * --pt_bitstream = 0;
	}
	else 
	{   

#line 10878 "vocoderSched.cc"
	    __PRF_globalCounter[770u]++ ;

#line 78 "src/processing/prm2bits.sc"
	     * --pt_bitstream = 1;
	}

#line 10885 "vocoderSched.cc"
	__PRF_globalCounter[771u]++ ;

#line 80 "src/processing/prm2bits.sc"
	value = shr(value, 1);
    }
}

#line 10893 "vocoderSched.cc"
void Prm2bits_12k2::main(void)

#line 86 "src/processing/prm2bits.sc"
{   




    short int i;
    short int *p_bits;

    short int bits[244];

    static const short int bitno[57] = { 

      ((short int)7),((short int)8),((short int)9),((short int)8),((short int)6),
      ((short int)9),((short int)4),((short int)4),((short int)4),((short int)4),((short int)4),((short int)4),((short int)3),((short int)3),((short int)3),((short int)3),((short int)3),((short int)5),
      ((short int)6),((short int)4),((short int)4),((short int)4),((short int)4),((short int)4),((short int)4),((short int)3),((short int)3),((short int)3),((short int)3),((short int)3),((short int)5),
      ((short int)9),((short int)4),((short int)4),((short int)4),((short int)4),((short int)4),((short int)4),((short int)3),((short int)3),((short int)3),((short int)3),((short int)3),((short int)5),
      ((short int)6),((short int)4),((short int)4),((short int)4),((short int)4),((short int)4),((short int)4),((short int)3),((short int)3),((short int)3),((short int)3),((short int)3),((short int)5) };

#line 10915 "vocoderSched.cc"
    __PRF_globalCounter[772u]++ ;
    _specc::waitfor(40466666ull);

#line 105 "src/processing/prm2bits.sc"
    p_bits = bits;

#line 10922 "vocoderSched.cc"
    __PRF_globalCounter[773u]++ ;

#line 107 "src/processing/prm2bits.sc"
    for(i = 0; i < 57; i++ )
    {   

#line 10929 "vocoderSched.cc"
	__PRF_globalCounter[774u]++ ;

#line 109 "src/processing/prm2bits.sc"
	Int2bin(prm[i], bitno[i], p_bits);
	p_bits += bitno[i];
    }

#line 10937 "vocoderSched.cc"
    __PRF_globalCounter[775u]++ ;

#line 113 "src/processing/prm2bits.sc"
    for(i = 0; i < (244); i++ )
    {   

#line 10944 "vocoderSched.cc"
	__PRF_globalCounter[776u]++ ;

#line 115 "src/processing/prm2bits.sc"
	serial[i] = bits[i];
    }
}

#line 10952 "vocoderSched.cc"
Sid_Codeword_Encoder::Sid_Codeword_Encoder(unsigned int _idcnt, short int (&ser1)[244], short int (&txdtx_ctrl), short int (&ser2)[244])
    : _specc::behavior(_idcnt), ser1(ser1), txdtx_ctrl(txdtx_ctrl), ser2(ser2)
{   
}

Sid_Codeword_Encoder::~Sid_Codeword_Encoder(void)
{   
}

void Sid_Codeword_Encoder::main(void)

#line 38 "src/processing/sid_codeword_encode.sc"
{   
    int i;


    const short int SID_codeword_bit_idx[95] = { 

      ((short int)45),((short int)46),((short int)48),((short int)49),((short int)50),((short int)51),((short int)52),((short int)53),((short int)54),((short int)55),
      ((short int)56),((short int)57),((short int)58),((short int)59),((short int)60),((short int)61),((short int)62),((short int)63),((short int)64),((short int)65),
      ((short int)66),((short int)67),((short int)68),((short int)94),((short int)95),((short int)96),((short int)98),((short int)99),((short int)100),((short int)101),
      ((short int)102),((short int)103),((short int)104),((short int)105),((short int)106),((short int)107),((short int)108),((short int)109),((short int)110),((short int)111),
      ((short int)112),((short int)113),((short int)114),((short int)115),((short int)116),((short int)117),((short int)118),((short int)148),((short int)149),((short int)150),
      ((short int)151),((short int)152),((short int)153),((short int)154),((short int)155),((short int)156),((short int)157),((short int)158),((short int)159),((short int)160),
      ((short int)161),((short int)162),((short int)163),((short int)164),((short int)165),((short int)166),((short int)167),((short int)168),((short int)169),((short int)170),
      ((short int)171),((short int)196),((short int)197),((short int)198),((short int)199),((short int)200),((short int)201),((short int)202),((short int)203),((short int)204),
      ((short int)205),((short int)206),((short int)207),((short int)208),((short int)209),((short int)212),((short int)213),((short int)214),((short int)215),((short int)216),
      ((short int)217),((short int)218),((short int)219),((short int)220),((short int)221) };

#line 10982 "vocoderSched.cc"
    __PRF_globalCounter[1049u]++ ;
    _specc::waitfor(12233333ull);
    __PRF_globalCounter[1050u]++ ;

#line 58 "src/processing/sid_codeword_encode.sc"
    for(i = 0; i < 244; i++ )

#line 10990 "vocoderSched.cc"
    {   
	__PRF_globalCounter[1051u]++ ;

#line 59 "src/processing/sid_codeword_encode.sc"
	ser2[i] = ser1[i];
    }

#line 10998 "vocoderSched.cc"
    __PRF_globalCounter[1052u]++ ;

#line 66 "src/processing/sid_codeword_encode.sc"
    if ((txdtx_ctrl & 1) == 0)
    {   

#line 11005 "vocoderSched.cc"
	__PRF_globalCounter[1053u]++ ;

#line 68 "src/processing/sid_codeword_encode.sc"
	for(i = 0; i < 95; i++ )
	{   

#line 11012 "vocoderSched.cc"
	    __PRF_globalCounter[1054u]++ ;

#line 70 "src/processing/sid_codeword_encode.sc"
	    ser2[SID_codeword_bit_idx[i]] = 1;
	}
    }
}

#line 11021 "vocoderSched.cc"
Encoder_Stimulus::Encoder_Stimulus(unsigned int _idcnt, i_sender (&speech_samples), i_sender (&dtx_mode))
    : _specc::behavior(_idcnt), speech_samples(speech_samples), dtx_mode(dtx_mode),
    f_speech(0)
{   
}

Encoder_Stimulus::~Encoder_Stimulus(void)
{   
}

void Encoder_Stimulus::main()

#line 67 "src/stimulus.sc"
{   
    int frame_count = 0;
    int sample_count;
    unsigned long long int f_start = 0ull;


    short int samples[160];


    short int tmp[160];

#line 11046 "vocoderSched.cc"
    __PRF_globalCounter[331u]++ ;
    __PRF_channelAccess(52129u, sizeof (local_dtx_mode));

#line 85 "src/stimulus.sc"
    dtx_mode.send( &local_dtx_mode, sizeof (local_dtx_mode));



    f_start = now();

#line 11057 "vocoderSched.cc"
    __PRF_globalCounter[332u]++ ;

#line 92 "src/stimulus.sc"
    while(fread(samples, sizeof(short int), 160, f_speech) == 160)
    {   

#line 11064 "vocoderSched.cc"
	__PRF_globalCounter[333u]++ ;

#line 95 "src/stimulus.sc"
	_specc::waitfor(20000000000ull - ((now() - f_start) % 20000000000ull));
	f_start = now();




	printf("\nframe=%3d ",  ++frame_count);

#line 11076 "vocoderSched.cc"
	__PRF_globalCounter[334u]++ ;

#line 106 "src/stimulus.sc"
	for(sample_count = 0; sample_count < 160; sample_count++ ) {

#line 11082 "vocoderSched.cc"
	    __PRF_globalCounter[335u]++ ;

#line 107 "src/stimulus.sc"
	    samples[sample_count] = order_bytes(samples[sample_count]);

	    tmp[sample_count] = ((short)((_bit(samples[sample_count])(15,3))).toInt());
	}

#line 11091 "vocoderSched.cc"
	__PRF_globalCounter[336u]++ ;
	__PRF_channelAccess(52128u, sizeof (tmp));

#line 122 "src/stimulus.sc"
	speech_samples.send(tmp, sizeof (tmp));
    }

#line 11099 "vocoderSched.cc"
    __PRF_globalCounter[337u]++ ;

#line 130 "src/stimulus.sc"
    fclose(f_speech);
    f_speech = 0;

#line 151 "src/stimulus.sc"
    _specc::waitfor(20000000000ull);



    printf("\n\ndone, %d frames encoded\n\n", frame_count);


    exit(0);
}

#line 11117 "vocoderSched.cc"
int Encoder_Stimulus::open_speech_infile(

#line 45 "src/stimulus.sc"
    char *speechfile_name, bool dtx)
{   
    struct _IO_FILE *f;

#line 11125 "vocoderSched.cc"
    __PRF_globalCounter[338u]++ ;

#line 49 "src/stimulus.sc"
    local_dtx_mode = dtx;

#line 11131 "vocoderSched.cc"
    __PRF_globalCounter[339u]++ ;

#line 56 "src/stimulus.sc"
    if ((f = fopen(speechfile_name, "rb")) == 0)
    {   

#line 11138 "vocoderSched.cc"
	__PRF_globalCounter[340u]++ ;

#line 58 "src/stimulus.sc"
	printf("Error opening input file  %s !!\n", speechfile_name);
	return (0);
    }

#line 11146 "vocoderSched.cc"
    __PRF_globalCounter[341u]++ ;

#line 61 "src/stimulus.sc"
    printf(" Input speech file:  %s\n", speechfile_name);
    f_speech = f;
    return (1);
}

#line 11155 "vocoderSched.cc"
Ex_Syn_Upd_Sh_Init::Ex_Syn_Upd_Sh_Init(unsigned int _idcnt, bool (&reset_flag), short int (&mem_syn)[10])
    : _specc::behavior(_idcnt), reset_flag(reset_flag), mem_syn(mem_syn)
{   
}

Ex_Syn_Upd_Sh_Init::~Ex_Syn_Upd_Sh_Init(void)
{   
}

void Ex_Syn_Upd_Sh_Init::init(void)

#line 25 "src/update/ex_syn_upd_sh.sc"
{   

#line 11170 "vocoderSched.cc"
    __PRF_globalCounter[344u]++ ;

#line 26 "src/update/ex_syn_upd_sh.sc"
    Set_zero((short int *)mem_syn, 10);
}

#line 11177 "vocoderSched.cc"
void Ex_Syn_Upd_Sh_Init::main(void)

#line 35 "src/update/ex_syn_upd_sh.sc"
{   

#line 11183 "vocoderSched.cc"
    __PRF_globalCounter[345u]++ ;
    _specc::waitfor(83333ull);
    __PRF_globalCounter[346u]++ ;

#line 36 "src/update/ex_syn_upd_sh.sc"
    if (reset_flag)

#line 11191 "vocoderSched.cc"
    {   
	__PRF_globalCounter[347u]++ ;

#line 36 "src/update/ex_syn_upd_sh.sc"
	init();
    }
}

#line 11200 "vocoderSched.cc"
void Ex_Syn_Upd_Sh_Init::reset(void)

#line 30 "src/update/ex_syn_upd_sh.sc"
{   

#line 11206 "vocoderSched.cc"
    __PRF_globalCounter[348u]++ ;

#line 31 "src/update/ex_syn_upd_sh.sc"
    init();
}

#line 11213 "vocoderSched.cc"
Ex_Syn_Upd_Sh::Ex_Syn_Upd_Sh(unsigned int _idcnt, short int (&gain_pit), short int (&gain_code), short int (&exc_i)[40], short int (&speech_i)[40], short int (&synth_i)[40], short int (&xn)[40], short int (&y1)[40], short int (&y2)[40], short int (&code)[40], short int (&Aq)[11], short int (&mem_err)[10], short int (&mem_w0)[10], short int (&txdtx_ctrl), bool (&reset_flag))
    : _specc::behavior(_idcnt), gain_pit(gain_pit), gain_code(gain_code), exc_i(exc_i), speech_i(speech_i), synth_i(synth_i), xn(xn), y1(y1), y2(y2), code(code), Aq(Aq), mem_err(mem_err), mem_w0(mem_w0), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag),
    _scc_const_port_0(true),
    copy(_IDcnt, synth, synth_i),
    excitation(_IDcnt, gain_pit, gain_code, code, exc_i),
    init(_IDcnt, reset_flag, mem_syn),
    syn_filt(_IDcnt, Aq, exc_i, synth, mem_syn, _scc_const_port_0),
    upd_mem(_IDcnt, speech_i, synth, xn, y1, y2, gain_pit, gain_code, mem_err, mem_w0, txdtx_ctrl)
{   
}

Ex_Syn_Upd_Sh::~Ex_Syn_Upd_Sh(void)
{   
}

void Ex_Syn_Upd_Sh::main(void)

#line 81 "src/update/ex_syn_upd_sh.sc"
{   

#line 11234 "vocoderSched.cc"
    __PRF_globalCounter[342u]++ ;

#line 82 "src/update/ex_syn_upd_sh.sc"
    init.main();

    excitation.main();

    syn_filt.main();
    upd_mem.main();

    copy.main();
}

#line 11248 "vocoderSched.cc"
void Ex_Syn_Upd_Sh::reset(void)

#line 75 "src/update/ex_syn_upd_sh.sc"
{   

#line 11254 "vocoderSched.cc"
    __PRF_globalCounter[343u]++ ;

#line 76 "src/update/ex_syn_upd_sh.sc"
    init.reset();
}

#line 11261 "vocoderSched.cc"
Excitation::Excitation(unsigned int _idcnt, short int (&gain_pit), short int (&gain_code), short int (&code)[40], short int (&exc_i)[40])
    : _specc::behavior(_idcnt), gain_pit(gain_pit), gain_code(gain_code), code(code), exc_i(exc_i)
{   
}

Excitation::~Excitation(void)
{   
}

void Excitation::main(void)

#line 18 "src/update/excitation.sc"
{   
    int i;
    int L_temp;

#line 11278 "vocoderSched.cc"
    __PRF_globalCounter[349u]++ ;
    _specc::waitfor(4033333ull);
    __PRF_globalCounter[350u]++ ;

#line 30 "src/update/excitation.sc"
    for(i = 0; i < 40; i++ )
    {   

#line 11287 "vocoderSched.cc"
	__PRF_globalCounter[351u]++ ;

#line 34 "src/update/excitation.sc"
	L_temp = L_mult(exc_i[i], gain_pit);
	L_temp = L_mac(L_temp, code[i], gain_code);
	L_temp = L_shl(L_temp, 3);
	exc_i[i] = round(L_temp);
    }
}

#line 11298 "vocoderSched.cc"
Q_Gain_Code::Q_Gain_Code(unsigned int _idcnt, int (&i_subfr), short int (&code)[40], short int (&gain), short int (&qgain), short int (&txdtx_ctrl), short int (&CN_excitation_gain), short int (&index), bool (&reset_flag))
    : _specc::behavior(_idcnt), i_subfr(i_subfr), code(code), gain(gain), qgain(qgain), txdtx_ctrl(txdtx_ctrl), CN_excitation_gain(CN_excitation_gain), index(index), reset_flag(reset_flag)
{   
}

Q_Gain_Code::~Q_Gain_Code(void)
{   
}

short int Q_Gain_Code::Aver_gain_code_history(

#line 150 "src/update/q_gain_code.sc"
    short int CN_exc_gain, 
    short int gain_code_old[28])

{   
    int i;
    int L_ret;

#line 11318 "vocoderSched.cc"
    __PRF_globalCounter[777u]++ ;

#line 157 "src/update/q_gain_code.sc"
    L_ret = L_mult(1136, CN_exc_gain);

#line 11324 "vocoderSched.cc"
    __PRF_globalCounter[778u]++ ;

#line 159 "src/update/q_gain_code.sc"
    for(i = 0; i < (4 * 7); i++ )
    {   

#line 11331 "vocoderSched.cc"
	__PRF_globalCounter[779u]++ ;

#line 161 "src/update/q_gain_code.sc"
	L_ret = L_mac(L_ret, 1136, gain_code_old[i]);
    }

#line 11338 "vocoderSched.cc"
    __PRF_globalCounter[780u]++ ;

#line 163 "src/update/q_gain_code.sc"
    return extract_h(L_ret);
}

#line 11345 "vocoderSched.cc"
void Q_Gain_Code::Update_gain_code_history_tx(

#line 106 "src/update/q_gain_code.sc"
    short int new_gain_code)

{   

#line 11353 "vocoderSched.cc"
    __PRF_globalCounter[781u]++ ;

#line 111 "src/update/q_gain_code.sc"
    gain_code_old_tx[buf_p_tx] = new_gain_code;

#line 11359 "vocoderSched.cc"
    __PRF_globalCounter[782u]++ ;

#line 114 "src/update/q_gain_code.sc"
    if (sub(buf_p_tx, (4 * 7 - 1)) == 0)
    {   

#line 11366 "vocoderSched.cc"
	__PRF_globalCounter[783u]++ ;

#line 116 "src/update/q_gain_code.sc"
	buf_p_tx = 0;
    }
    else 
    {   

#line 11375 "vocoderSched.cc"
	__PRF_globalCounter[784u]++ ;

#line 120 "src/update/q_gain_code.sc"
	buf_p_tx = add(buf_p_tx, 1);
    }

#line 11382 "vocoderSched.cc"
    __PRF_globalCounter[785u]++ ;

#line 123 "src/update/q_gain_code.sc"
    return ;
}

#line 11389 "vocoderSched.cc"
void Q_Gain_Code::init(void)

#line 55 "src/update/q_gain_code.sc"
{   
    int i;

#line 11396 "vocoderSched.cc"
    __PRF_globalCounter[786u]++ ;

#line 58 "src/update/q_gain_code.sc"
    for(i = 0; i < 4; i++ )
    {   

#line 11403 "vocoderSched.cc"
	__PRF_globalCounter[787u]++ ;

#line 60 "src/update/q_gain_code.sc"
	past_qua_en[i] =  -2381;
    }

#line 11410 "vocoderSched.cc"
    __PRF_globalCounter[788u]++ ;

#line 62 "src/update/q_gain_code.sc"
    pred[0] = 44;
    pred[1] = 37;
    pred[2] = 22;
    pred[3] = 12;

#line 11419 "vocoderSched.cc"
    __PRF_globalCounter[789u]++ ;

#line 67 "src/update/q_gain_code.sc"
    for(i = 0; i < 4 * 7; i++ )
    {   

#line 11426 "vocoderSched.cc"
	__PRF_globalCounter[790u]++ ;

#line 69 "src/update/q_gain_code.sc"
	gain_code_old_tx[i] = 0;
    }

#line 11433 "vocoderSched.cc"
    __PRF_globalCounter[791u]++ ;

#line 72 "src/update/q_gain_code.sc"
    buf_p_tx = 0;
}

#line 11440 "vocoderSched.cc"
void Q_Gain_Code::main(void)

#line 211 "src/update/q_gain_code.sc"
{   
    int i;
    short int err; short int err_min; short int exp; short int frac; short int gcode0;
    int ener; int ener_code;
    short int aver_gain;
    static short int gcode0_CN;

#line 222 "src/update/q_gain_code.sc"
    const short int qua_gain_code[32] = { 

      ((short int)159),((short int)206),((short int)268),((short int)349),((short int)419),((short int)482),((short int)554),((short int)637),
      ((short int)733),((short int)842),((short int)969),((short int)1114),((short int)1281),((short int)1473),((short int)1694),((short int)1948),
      ((short int)2241),((short int)2577),((short int)2963),((short int)3408),((short int)3919),((short int)4507),((short int)5183),((short int)5960),
      ((short int)6855),((short int)7883),((short int)9065),((short int)10425),((short int)12510),((short int)16263),((short int)21142),((short int)27485) };

#line 11459 "vocoderSched.cc"
    __PRF_globalCounter[792u]++ ;
    _specc::waitfor(4733333ull);
    __PRF_globalCounter[793u]++ ;

#line 230 "src/update/q_gain_code.sc"
    if (reset_flag)

#line 11467 "vocoderSched.cc"
    {   
	__PRF_globalCounter[794u]++ ;

#line 230 "src/update/q_gain_code.sc"
	init();
    }

#line 11475 "vocoderSched.cc"
    __PRF_globalCounter[795u]++ ;

#line 232 "src/update/q_gain_code.sc"
    if ((txdtx_ctrl & 1) != 0)
    {   

#line 11482 "vocoderSched.cc"
	__PRF_globalCounter[796u]++ ;

#line 244 "src/update/q_gain_code.sc"
	ener_code = 0;

#line 11488 "vocoderSched.cc"
	__PRF_globalCounter[797u]++ ;

#line 245 "src/update/q_gain_code.sc"
	for(i = 0; i < 40; i++ )
	{   

#line 11495 "vocoderSched.cc"
	    __PRF_globalCounter[798u]++ ;

#line 247 "src/update/q_gain_code.sc"
	    ener_code = L_mac(ener_code, code[i], code[i]);
	}

#line 11502 "vocoderSched.cc"
	__PRF_globalCounter[799u]++ ;

#line 250 "src/update/q_gain_code.sc"
	ener_code = L_mult(round(ener_code), 26214);


	Log2(ener_code,  &exp,  &frac);
	ener_code = L_Comp(sub(exp, 30), frac);



	ener = 783741l;

#line 11516 "vocoderSched.cc"
	__PRF_globalCounter[800u]++ ;

#line 259 "src/update/q_gain_code.sc"
	for(i = 0; i < 4; i++ )
	{   

#line 11523 "vocoderSched.cc"
	    __PRF_globalCounter[801u]++ ;

#line 261 "src/update/q_gain_code.sc"
	    ener = L_mac(ener, past_qua_en[i], pred[i]);
	}

#line 11530 "vocoderSched.cc"
	__PRF_globalCounter[802u]++ ;

#line 272 "src/update/q_gain_code.sc"
	ener = L_shr(L_sub(ener, ener_code), 1);
	L_Extract(ener,  &exp,  &frac);

	gcode0 = extract_l(Pow2(exp, frac));

	gcode0 = shl(gcode0, 4);

#line 283 "src/update/q_gain_code.sc"
	err_min = abs_s(sub(gain, mult(gcode0, qua_gain_code[0])));
	index = 0;

#line 11545 "vocoderSched.cc"
	__PRF_globalCounter[803u]++ ;

#line 286 "src/update/q_gain_code.sc"
	for(i = 1; i < 32; i++ )
	{   

#line 11552 "vocoderSched.cc"
	    __PRF_globalCounter[804u]++ ;

#line 288 "src/update/q_gain_code.sc"
	    err = abs_s(sub(gain, mult(gcode0, qua_gain_code[i])));

#line 11558 "vocoderSched.cc"
	    __PRF_globalCounter[805u]++ ;

#line 291 "src/update/q_gain_code.sc"
	    if (sub(err, err_min) < 0)
	    {   

#line 11565 "vocoderSched.cc"
		__PRF_globalCounter[806u]++ ;

#line 293 "src/update/q_gain_code.sc"
		err_min = err;
		index = i;
	    }
	}

#line 11574 "vocoderSched.cc"
	__PRF_globalCounter[807u]++ ;

#line 298 "src/update/q_gain_code.sc"
	qgain = gcode0 = mult(gcode0, qua_gain_code[index]);

#line 11580 "vocoderSched.cc"
	__PRF_globalCounter[808u]++ ;

#line 309 "src/update/q_gain_code.sc"
	for(i = 3; i > 0; i-- )
	{   

#line 11587 "vocoderSched.cc"
	    __PRF_globalCounter[809u]++ ;

#line 311 "src/update/q_gain_code.sc"
	    past_qua_en[i] = past_qua_en[i - 1];
	}

#line 11594 "vocoderSched.cc"
	__PRF_globalCounter[810u]++ ;

#line 313 "src/update/q_gain_code.sc"
	Log2(L_deposit_l(qua_gain_code[index]),  &exp,  &frac);

	past_qua_en[0] = shr(frac, 5);
	past_qua_en[0] = add(past_qua_en[0], shl(sub(exp, 11), 10));


	Update_gain_code_history_tx(gcode0);
    }
    else 
    {   

#line 11609 "vocoderSched.cc"
	__PRF_globalCounter[811u]++ ;

#line 324 "src/update/q_gain_code.sc"
	if ((txdtx_ctrl & 8) != 0 && (i_subfr == 0))
	{   

#line 11616 "vocoderSched.cc"
	    __PRF_globalCounter[812u]++ ;

#line 326 "src/update/q_gain_code.sc"
	    gcode0_CN = update_gcode0_CN(gain_code_old_tx);
	    gcode0_CN = shl(gcode0_CN, 4);
	}

#line 11624 "vocoderSched.cc"
	__PRF_globalCounter[813u]++ ;

#line 329 "src/update/q_gain_code.sc"
	qgain = gcode0 = CN_excitation_gain;

#line 11630 "vocoderSched.cc"
	__PRF_globalCounter[814u]++ ;

#line 332 "src/update/q_gain_code.sc"
	if ((txdtx_ctrl & 16) != 0)
	{   

#line 11637 "vocoderSched.cc"
	    __PRF_globalCounter[815u]++ ;

#line 334 "src/update/q_gain_code.sc"
	    aver_gain = Aver_gain_code_history(CN_excitation_gain, 
		gain_code_old_tx);

#line 341 "src/update/q_gain_code.sc"
	    err_min = abs_s(sub(aver_gain, 
		    mult(gcode0_CN, qua_gain_code[0])));
	    index = 0;

#line 11649 "vocoderSched.cc"
	    __PRF_globalCounter[816u]++ ;

#line 345 "src/update/q_gain_code.sc"
	    for(i = 1; i < 32; i++ )
	    {   

#line 11656 "vocoderSched.cc"
		__PRF_globalCounter[817u]++ ;

#line 347 "src/update/q_gain_code.sc"
		err = abs_s(sub(aver_gain, 
			mult(gcode0_CN, qua_gain_code[i])));

#line 11663 "vocoderSched.cc"
		__PRF_globalCounter[818u]++ ;

#line 351 "src/update/q_gain_code.sc"
		if (sub(err, err_min) < 0)
		{   

#line 11670 "vocoderSched.cc"
		    __PRF_globalCounter[819u]++ ;

#line 353 "src/update/q_gain_code.sc"
		    err_min = err;
		    index = i;
		}
	    }
	}

#line 11680 "vocoderSched.cc"
	__PRF_globalCounter[820u]++ ;

#line 358 "src/update/q_gain_code.sc"
	Update_gain_code_history_tx(gcode0);

#line 11686 "vocoderSched.cc"
	__PRF_globalCounter[821u]++ ;

#line 365 "src/update/q_gain_code.sc"
	for(i = 0; i < 4; i++ )
	{   

#line 11693 "vocoderSched.cc"
	    __PRF_globalCounter[822u]++ ;

#line 367 "src/update/q_gain_code.sc"
	    past_qua_en[i] =  -2381;
	}
    }
}

#line 11702 "vocoderSched.cc"
void Q_Gain_Code::reset(void)

#line 78 "src/update/q_gain_code.sc"
{   

#line 11708 "vocoderSched.cc"
    __PRF_globalCounter[823u]++ ;

#line 79 "src/update/q_gain_code.sc"
    init();
}

#line 11715 "vocoderSched.cc"
short int Q_Gain_Code::update_gcode0_CN(

#line 187 "src/update/q_gain_code.sc"
    short int gain_code_old[28])

{   
    int i; int j;
    int L_ret; int L_temp;

#line 11725 "vocoderSched.cc"
    __PRF_globalCounter[824u]++ ;

#line 193 "src/update/q_gain_code.sc"
    L_ret = 0l;

#line 11731 "vocoderSched.cc"
    __PRF_globalCounter[825u]++ ;

#line 194 "src/update/q_gain_code.sc"
    for(i = 0; i < 7; i++ )
    {   

#line 11738 "vocoderSched.cc"
	__PRF_globalCounter[826u]++ ;

#line 196 "src/update/q_gain_code.sc"
	L_temp = L_mult(8191, gain_code_old[4 * i]);

#line 11744 "vocoderSched.cc"
	__PRF_globalCounter[827u]++ ;

#line 197 "src/update/q_gain_code.sc"
	for(j = 1; j < 4; j++ )
	{   

#line 11751 "vocoderSched.cc"
	    __PRF_globalCounter[828u]++ ;

#line 199 "src/update/q_gain_code.sc"
	    L_temp = L_mac(L_temp, 8191, gain_code_old[4 * i + j]);
	}

#line 11758 "vocoderSched.cc"
	__PRF_globalCounter[829u]++ ;

#line 201 "src/update/q_gain_code.sc"
	L_ret = L_mac(L_ret, (32767 / 7), extract_h(L_temp));
    }

#line 11765 "vocoderSched.cc"
    __PRF_globalCounter[830u]++ ;

#line 204 "src/update/q_gain_code.sc"
    return extract_h(L_ret);
}

#line 11772 "vocoderSched.cc"
Upd_Mem::Upd_Mem(unsigned int _idcnt, short int (&speech)[40], short int (&synth)[40], short int (&xn)[40], short int (&y1)[40], short int (&y2)[40], short int (&gain_pit), short int (&gain_code), short int (&mem_err)[10], short int (&mem_w0)[10], short int (&txdtx_ctrl))
    : _specc::behavior(_idcnt), speech(speech), synth(synth), xn(xn), y1(y1), y2(y2), gain_pit(gain_pit), gain_code(gain_code), mem_err(mem_err), mem_w0(mem_w0), txdtx_ctrl(txdtx_ctrl)
{   
}

Upd_Mem::~Upd_Mem(void)
{   
}

void Upd_Mem::main(void)

#line 23 "src/update/upd_mem.sc"
{   
    int i; int j;
    short int k;
    short int temp;

#line 11790 "vocoderSched.cc"
    __PRF_globalCounter[1177u]++ ;
    _specc::waitfor(1233333ull);
    __PRF_globalCounter[1178u]++ ;

#line 28 "src/update/upd_mem.sc"
    if ((txdtx_ctrl & 1) != 0)
    {   

#line 11799 "vocoderSched.cc"
	__PRF_globalCounter[1179u]++ ;

#line 31 "src/update/upd_mem.sc"
	for(i = 40 - 10 , j = 0; i < 40; i++  , j++ )
	{   

#line 11806 "vocoderSched.cc"
	    __PRF_globalCounter[1180u]++ ;

#line 33 "src/update/upd_mem.sc"
	    mem_err[j] = sub(speech[i], synth[i]);

	    temp = extract_h(L_shl(L_mult(y1[i], gain_pit), 3));
	    k = extract_h(L_shl(L_mult(y2[i], gain_code), 5));
	    mem_w0[j] = sub(xn[i], add(temp, k));
	}
    }
    else 
    {   

#line 11820 "vocoderSched.cc"
	__PRF_globalCounter[1181u]++ ;

#line 42 "src/update/upd_mem.sc"
	for(j = 0; j < 10; j++ )
	{   

#line 11827 "vocoderSched.cc"
	    __PRF_globalCounter[1182u]++ ;

#line 44 "src/update/upd_mem.sc"
	    mem_err[j] = 0;
	    mem_w0[j] = 0;
	}
    }
}

#line 11837 "vocoderSched.cc"
SpeechIn::SpeechIn(unsigned int _idcnt, i_receiver (&speech_in), i_sender (&speech), i_receiver (&dtx_mode_in), i_sender (&dtx_mode))
    : _specc::behavior(_idcnt), speech_in(speech_in), speech(speech), dtx_mode_in(dtx_mode_in), dtx_mode(dtx_mode)
{   
}

SpeechIn::~SpeechIn(void)
{   
}

void SpeechIn::main(void)

#line 288 "src/vocoder.sc"
{   
    short int samples[160];
    bool mode;

#line 11854 "vocoderSched.cc"
    __PRF_globalCounter[1055u]++ ;
    __PRF_channelAccess(133258u, sizeof (mode));

#line 292 "src/vocoder.sc"
    dtx_mode_in.receive( &mode, sizeof (mode));

#line 11861 "vocoderSched.cc"
    __PRF_channelAccess(133131u, sizeof (mode));

#line 293 "src/vocoder.sc"
    dtx_mode.send( &mode, sizeof (mode));

#line 11867 "vocoderSched.cc"
    __PRF_globalCounter[1056u]++ ;

#line 295 "src/vocoder.sc"
    while(true) {

#line 11873 "vocoderSched.cc"
	__PRF_globalCounter[1057u]++ ;
	__PRF_channelAccess(133256u, sizeof (samples));

#line 296 "src/vocoder.sc"
	speech_in.receive(samples, sizeof (samples));

#line 11880 "vocoderSched.cc"
	__PRF_channelAccess(133129u, sizeof (samples));

#line 297 "src/vocoder.sc"
	speech.send(samples, sizeof (samples));
    }
}

#line 11888 "vocoderSched.cc"
SerialOut::SerialOut(unsigned int _idcnt, i_receiver (&serial), i_sender (&serial_out), i_receiver (&txdtx), i_sender (&txdtx_out))
    : _specc::behavior(_idcnt), serial(serial), serial_out(serial_out), txdtx(txdtx), txdtx_out(txdtx_out)
{   
}

SerialOut::~SerialOut(void)
{   
}

void SerialOut::main(void)

#line 306 "src/vocoder.sc"
{   
    short int data[244];
    short int txdtx_ctrl;

#line 11905 "vocoderSched.cc"
    __PRF_globalCounter[1017u]++ ;

#line 310 "src/vocoder.sc"
    while(true) {

#line 11911 "vocoderSched.cc"
	__PRF_globalCounter[1018u]++ ;
	__PRF_channelAccess(129256u, sizeof (data));

#line 311 "src/vocoder.sc"
	serial.receive(data, sizeof (data));

#line 11918 "vocoderSched.cc"
	__PRF_channelAccess(129258u, sizeof (txdtx_ctrl));

#line 312 "src/vocoder.sc"
	txdtx.receive( &txdtx_ctrl, sizeof (txdtx_ctrl));

#line 11924 "vocoderSched.cc"
	__PRF_channelAccess(129129u, sizeof (data));

#line 313 "src/vocoder.sc"
	serial_out.send(data, sizeof (data));

#line 11930 "vocoderSched.cc"
	__PRF_channelAccess(129131u, sizeof (txdtx_ctrl));

#line 314 "src/vocoder.sc"
	txdtx_out.send( &txdtx_ctrl, sizeof (txdtx_ctrl));
    }
}

#line 11938 "vocoderSched.cc"
Vocoder::Vocoder(unsigned int _idcnt, i_receiver (&samples_in), i_receiver (&dtx_mode_in), i_sender (&serial_bits_out), i_sender (&txdtx_ctrl_out))
    : _specc::behavior(_idcnt), samples_in(samples_in), dtx_mode_in(dtx_mode_in), serial_bits_out(serial_bits_out), txdtx_ctrl_out(txdtx_ctrl_out),
    coder(++_IDcnt, speech_samples, dtx_mode, serial_bits, txdtx_ctrl),
    dtx_mode(),
    serial(++_IDcnt, serial_bits, serial_bits_out, txdtx_ctrl, txdtx_ctrl_out),
    serial_bits(),
    speech(++_IDcnt, samples_in, speech_samples, dtx_mode_in, dtx_mode),
    speech_samples(),
    txdtx_ctrl()
{   
}

Vocoder::~Vocoder(void)
{   
}

void Vocoder::main(void)

#line 377 "src/vocoder.sc"
{   
    { _specc::fork _scc_fork_0(&speech), _scc_fork_1(&coder), _scc_fork_2(&serial); _specc::par(

#line 11961 "vocoderSched.cc"
	    &_scc_fork_0, &_scc_fork_1, &_scc_fork_2, ((_specc::fork*)0));
    }
}

void Vocoder::reset(void)

#line 372 "src/vocoder.sc"
{   

#line 11971 "vocoderSched.cc"
    __PRF_globalCounter[1392u]++ ;
    coder.reset();
}

GSM_Vocoder::GSM_Vocoder(unsigned int _idcnt)
    : _specc::behavior(_idcnt),
    local_dtx_mode(),
    monitor(++_IDcnt, serial_bits, txdtx_ctrl),
    serial_bits(),
    speech_samples(),
    stimulus(++_IDcnt, speech_samples, local_dtx_mode),
    txdtx_ctrl(),
    vocoder(++_IDcnt, speech_samples, local_dtx_mode, serial_bits, txdtx_ctrl)
{   
}

GSM_Vocoder::~GSM_Vocoder(void)
{   
}

void GSM_Vocoder::main(void)

#line 475 "src/vocoder.sc"
{   
    { _specc::fork _scc_fork_0(&stimulus), _scc_fork_1(&vocoder), _scc_fork_2(&monitor); _specc::par(

	    &_scc_fork_0, 

	    &_scc_fork_1, 

#line 487 "src/vocoder.sc"
	    &_scc_fork_2, ((_specc::fork*)0));
    }
}

#line 12007 "vocoderSched.cc"
int GSM_Vocoder::open_serial_outfile(

#line 461 "src/vocoder.sc"
    char *serialfile_name) {

#line 12013 "vocoderSched.cc"
    __PRF_globalCounter[372u]++ ;

#line 462 "src/vocoder.sc"
    return monitor.open_serial_outfile(serialfile_name);
}

#line 12020 "vocoderSched.cc"
int GSM_Vocoder::open_speech_infile(

#line 429 "src/vocoder.sc"
    char *speechfile_name, bool dtx_mode) {

#line 12026 "vocoderSched.cc"
    __PRF_globalCounter[373u]++ ;

#line 430 "src/vocoder.sc"
    return stimulus.open_speech_infile(speechfile_name, dtx_mode);
}

#line 12033 "vocoderSched.cc"
void GSM_Vocoder::reset(void)

#line 466 "src/vocoder.sc"
{   

#line 12039 "vocoderSched.cc"
    __PRF_globalCounter[374u]++ ;

#line 468 "src/vocoder.sc"
    vocoder.reset();
}

#line 12046 "vocoderSched.cc"
Main::Main(unsigned int _idcnt)
    : _specc::class_type(_idcnt),
    testbench(_IDcnt)
{   
}

Main::~Main(void)
{   
}

int Main::main(

#line 62 "testbench.sc"
    int argc, char *argv[])
{   
    char *speech_in_file_name;
    char *serial_out_file_name;

#line 12065 "vocoderSched.cc"
    __PRF_startProfiler("vocoderSched.bb_counter.prf", "vocoderSched.fn_counter.prf", 
	__PRF_globalCounter, 1619u);
    __PRF_globalCounter[546u]++ ;

#line 76 "testbench.sc"
    dtx_mode = arg_handler(argc, argv,  &speech_in_file_name,  &
	serial_out_file_name);

#line 12074 "vocoderSched.cc"
    __PRF_globalCounter[547u]++ ;

#line 82 "testbench.sc"
    if ( !testbench.open_speech_infile(speech_in_file_name, dtx_mode))

#line 12080 "vocoderSched.cc"
    {   
	__PRF_globalCounter[548u]++ ;

#line 82 "testbench.sc"
	exit(1);
    }

#line 12088 "vocoderSched.cc"
    __PRF_globalCounter[549u]++ ;

#line 86 "testbench.sc"
    if ( !testbench.open_serial_outfile(serial_out_file_name))

#line 12094 "vocoderSched.cc"
    {   
	__PRF_globalCounter[550u]++ ;

#line 86 "testbench.sc"
	exit(1);
    }

#line 12102 "vocoderSched.cc"
    __PRF_globalCounter[551u]++ ;

#line 92 "testbench.sc"
    testbench.reset();

#line 99 "testbench.sc"
    testbench.main();

    return (0);
}

#line 12114 "vocoderSched.cc"
Main _scc_main(_IDcnt);

int main(int argc, char *argv[])
{   
    int _scc_main_return;
    
    _specc::start();
    _scc_main_return = _scc_main.main(argc, argv);
    _specc::end();
    return(_scc_main_return);
}

AR_FORK_Codebook::AR_FORK_Codebook(unsigned int _idcnt, i_short_int_receiver (&ar_cc__T0__DSP__HW), short int (&T0), i_short_int__40__receiver (&ar_cc__code__DSP__HW), short int (&code)[40], i_short_int__40__receiver (&ar_cc__exc_i__DSP__HW), short int (&exc_i)[40], i_short_int_receiver (&ar_cc__gain_pit__DSP__HW), short int (&gain_pit), i_short_int__40__receiver (&ar_cc__h1__DSP__HW), short int (&h1)[40], i_short_int__40__receiver (&ar_cc__res2__DSP__HW), short int (&res2)[40], i_short_int__40__receiver (&ar_cc__xn__DSP__HW), short int (&xn)[40], i_short_int__40__receiver (&ar_cc__y1__DSP__HW), short int (&y1)[40])
    : _specc::behavior(_idcnt), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), T0(T0), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), code(code), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), exc_i(exc_i), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), gain_pit(gain_pit), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), h1(h1), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), res2(res2), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), xn(xn), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), y1(y1)
{   
}

AR_FORK_Codebook::~AR_FORK_Codebook(void)
{   
}

void AR_FORK_Codebook::main(void)
{   
    __PRF_globalCounter[0u]++ ;
    ar_cc__T0__DSP__HW.receive( &T0);
    ar_cc__code__DSP__HW.receive( &code);
    ar_cc__exc_i__DSP__HW.receive( &exc_i);
    ar_cc__gain_pit__DSP__HW.receive( &gain_pit);
    ar_cc__h1__DSP__HW.receive( &h1);
    ar_cc__res2__DSP__HW.receive( &res2);
    ar_cc__xn__DSP__HW.receive( &xn);
    ar_cc__y1__DSP__HW.receive( &y1);
}

AR_FORK_Codebook_DSP::AR_FORK_Codebook_DSP(unsigned int _idcnt, i_short_int_sender (&ar_cc__T0__DSP__HW), short int (&T0), i_short_int__40__sender (&ar_cc__code__DSP__HW), short int (&code)[40], i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), short int (&exc_i)[40], i_short_int_sender (&ar_cc__gain_pit__DSP__HW), short int (&gain_pit), i_short_int__40__sender (&ar_cc__h1__DSP__HW), short int (&h1)[40], i_short_int__40__sender (&ar_cc__res2__DSP__HW), short int (&res2)[40], i_short_int__40__sender (&ar_cc__xn__DSP__HW), short int (&xn)[40], i_short_int__40__sender (&ar_cc__y1__DSP__HW), short int (&y1)[40])
    : _specc::behavior(_idcnt), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), T0(T0), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), code(code), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), exc_i(exc_i), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), gain_pit(gain_pit), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), h1(h1), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), res2(res2), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), xn(xn), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), y1(y1)
{   
}

AR_FORK_Codebook_DSP::~AR_FORK_Codebook_DSP(void)
{   
}

void AR_FORK_Codebook_DSP::main(void)
{   
    __PRF_globalCounter[1u]++ ;
    ar_cc__T0__DSP__HW.send(T0);
    ar_cc__code__DSP__HW.send(code);
    ar_cc__exc_i__DSP__HW.send(exc_i);
    ar_cc__gain_pit__DSP__HW.send(gain_pit);
    ar_cc__h1__DSP__HW.send(h1);
    ar_cc__res2__DSP__HW.send(res2);
    ar_cc__xn__DSP__HW.send(xn);
    ar_cc__y1__DSP__HW.send(y1);
}

AR_JOIN_Codebook::AR_JOIN_Codebook(unsigned int _idcnt, i_short_int__10__sender (&ar_cc__cb_ana__HW__DSP), short int (&cb_ana)[10], i_short_int__40__sender (&ar_cc__code__HW__DSP), short int (&code)[40], i_short_int_sender (&ar_cc__gain_code__HW__DSP), short int (&gain_code), i_short_int__40__sender (&ar_cc__y2__HW__DSP), short int (&y2)[40])
    : _specc::behavior(_idcnt), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), cb_ana(cb_ana), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), code(code), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), gain_code(gain_code), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP), y2(y2)
{   
}

AR_JOIN_Codebook::~AR_JOIN_Codebook(void)
{   
}

void AR_JOIN_Codebook::main(void)
{   
    __PRF_globalCounter[2u]++ ;
    ar_cc__cb_ana__HW__DSP.send(cb_ana);
    ar_cc__code__HW__DSP.send(code);
    ar_cc__gain_code__HW__DSP.send(gain_code);
    ar_cc__y2__HW__DSP.send(y2);
}

AR_JOIN_Codebook_DSP::AR_JOIN_Codebook_DSP(unsigned int _idcnt, i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), short int (&cb_ana)[10], i_short_int__40__receiver (&ar_cc__code__HW__DSP), short int (&code)[40], i_short_int_receiver (&ar_cc__gain_code__HW__DSP), short int (&gain_code), i_short_int__40__receiver (&ar_cc__y2__HW__DSP), short int (&y2)[40])
    : _specc::behavior(_idcnt), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), cb_ana(cb_ana), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), code(code), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), gain_code(gain_code), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP), y2(y2)
{   
}

AR_JOIN_Codebook_DSP::~AR_JOIN_Codebook_DSP(void)
{   
}

void AR_JOIN_Codebook_DSP::main(void)
{   
    __PRF_globalCounter[3u]++ ;
    ar_cc__cb_ana__HW__DSP.receive( &cb_ana);
    ar_cc__code__HW__DSP.receive( &code);
    ar_cc__gain_code__HW__DSP.receive( &gain_code);
    ar_cc__y2__HW__DSP.receive( &y2);
}

AR_RECV_int::AR_RECV_int(unsigned int _idcnt, i_int_receiver (&ch), int (&data))
    : _specc::behavior(_idcnt), ch(ch), data(data)
{   
}

AR_RECV_int::~AR_RECV_int(void)
{   
}

void AR_RECV_int::main(void)
{   
    __PRF_globalCounter[4u]++ ;
    ch.receive( &data);
}

AR_SEND_int::AR_SEND_int(unsigned int _idcnt, i_int_sender (&ch), int (&data))
    : _specc::behavior(_idcnt), ch(ch), data(data)
{   
}

AR_SEND_int::~AR_SEND_int(void)
{   
}

void AR_SEND_int::main(void)
{   
    __PRF_globalCounter[5u]++ ;
    ch.send(data);
}

AR_SEND_int_0::AR_SEND_int_0(unsigned int _idcnt, i_int_sender (&ch), int (&data))
    : _specc::behavior(_idcnt), ch(ch), data(data)
{   
}

AR_SEND_int_0::~AR_SEND_int_0(void)
{   
}

void AR_SEND_int_0::main(void)
{   
    __PRF_globalCounter[6u]++ ;
    ch.send(data);
}

AR_SEND_int_1::AR_SEND_int_1(unsigned int _idcnt, i_int_sender (&ch), int (&data))
    : _specc::behavior(_idcnt), ch(ch), data(data)
{   
}

AR_SEND_int_1::~AR_SEND_int_1(void)
{   
}

void AR_SEND_int_1::main(void)
{   
    __PRF_globalCounter[7u]++ ;
    ch.send(data);
}

AR_SEND_int_2::AR_SEND_int_2(unsigned int _idcnt, i_int_sender (&ch), int (&data))
    : _specc::behavior(_idcnt), ch(ch), data(data)
{   
}

AR_SEND_int_2::~AR_SEND_int_2(void)
{   
}

void AR_SEND_int_2::main(void)
{   
    __PRF_globalCounter[8u]++ ;
    ch.send(data);
}

AR_SEND_int_3::AR_SEND_int_3(unsigned int _idcnt, i_int_sender (&ch), int (&data))
    : _specc::behavior(_idcnt), ch(ch), data(data)
{   
}

AR_SEND_int_3::~AR_SEND_int_3(void)
{   
}

void AR_SEND_int_3::main(void)
{   
    __PRF_globalCounter[9u]++ ;
    ch.send(data);
}

AR_SEND_int_4::AR_SEND_int_4(unsigned int _idcnt, i_int_sender (&ch), int (&data))
    : _specc::behavior(_idcnt), ch(ch), data(data)
{   
}

AR_SEND_int_4::~AR_SEND_int_4(void)
{   
}

void AR_SEND_int_4::main(void)
{   
    __PRF_globalCounter[10u]++ ;
    ch.send(data);
}

AR_SEND_int_5::AR_SEND_int_5(unsigned int _idcnt, i_int_sender (&ch), int (&data))
    : _specc::behavior(_idcnt), ch(ch), data(data)
{   
}

AR_SEND_int_5::~AR_SEND_int_5(void)
{   
}

void AR_SEND_int_5::main(void)
{   
    __PRF_globalCounter[11u]++ ;
    ch.send(data);
}

AR_WR_Codebook::AR_WR_Codebook(unsigned int _idcnt, short int (&xn)[40], short int (&y1)[40], short int (&gain_pit), short int (&exc)[40], short int (&h1)[40], short int (&T0), short int (&res2)[40], short int (&code)[40], short int (&y2)[40], short int (&gain_code), short int (&ana)[10], i_short_int_receiver (&ar_cc__T0__DSP__HW), i_short_int__10__sender (&ar_cc__cb_ana__HW__DSP), i_short_int__40__receiver (&ar_cc__code__DSP__HW), i_short_int__40__sender (&ar_cc__code__HW__DSP), i_short_int__40__receiver (&ar_cc__exc_i__DSP__HW), i_short_int_sender (&ar_cc__gain_code__HW__DSP), i_short_int_receiver (&ar_cc__gain_pit__DSP__HW), i_short_int__40__receiver (&ar_cc__h1__DSP__HW), i_short_int__40__receiver (&ar_cc__res2__DSP__HW), i_short_int__40__receiver (&ar_cc__xn__DSP__HW), i_short_int__40__receiver (&ar_cc__y1__DSP__HW), i_short_int__40__sender (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), xn(xn), y1(y1), gain_pit(gain_pit), exc(exc), h1(h1), T0(T0), res2(res2), code(code), y2(y2), gain_code(gain_code), ana(ana), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    ar_fork_codebook(_IDcnt, ar_cc__T0__DSP__HW, T0, ar_cc__code__DSP__HW, code, ar_cc__exc_i__DSP__HW, exc, ar_cc__gain_pit__DSP__HW, gain_pit, ar_cc__h1__DSP__HW, h1, ar_cc__res2__DSP__HW, res2, ar_cc__xn__DSP__HW, xn, ar_cc__y1__DSP__HW, y1),
    ar_join_codebook(_IDcnt, ar_cc__cb_ana__HW__DSP, ana, ar_cc__code__HW__DSP, code, ar_cc__gain_code__HW__DSP, gain_code, ar_cc__y2__HW__DSP, y2),
    codebook(_IDcnt, xn, y1, gain_pit, exc, h1, T0, res2, code, y2, gain_code, ana)
{   
}

AR_WR_Codebook::~AR_WR_Codebook(void)
{   
}

void AR_WR_Codebook::main(void)
{   
    __PRF_globalCounter[12u]++ ;
    ar_fork_codebook.main();
    codebook.main();
    ar_join_codebook.main();
}

AR_WR_Codebook_CN::AR_WR_Codebook_CN(unsigned int _idcnt, short int (&code)[40], short int (&txdtx_ctrl), int (&L_pn_seed_tx), i_int_sender (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_sender (&ar_cc__ar_tid_Codebook_CN__DSP__HW), i_short_int_sender (&ar_cc__T0__DSP__HW), short int (&T0), i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), short int (&cb_ana)[10], i_short_int__40__sender (&ar_cc__code__DSP__HW), i_short_int__40__receiver (&ar_cc__code__HW__DSP), i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), short int (&exc_i)[40], i_short_int_receiver (&ar_cc__gain_code__HW__DSP), short int (&gain_code), i_short_int_sender (&ar_cc__gain_pit__DSP__HW), short int (&gain_pit), i_short_int__40__sender (&ar_cc__h1__DSP__HW), short int (&h1)[40], i_short_int__40__sender (&ar_cc__res2__DSP__HW), short int (&res2)[40], i_short_int__40__sender (&ar_cc__xn__DSP__HW), short int (&xn)[40], i_short_int__40__sender (&ar_cc__y1__DSP__HW), short int (&y1)[40], i_short_int__40__receiver (&ar_cc__y2__HW__DSP), short int (&y2)[40])
    : _specc::behavior(_idcnt), code(code), txdtx_ctrl(txdtx_ctrl), L_pn_seed_tx(L_pn_seed_tx), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), T0(T0), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), cb_ana(cb_ana), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), exc_i(exc_i), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), gain_code(gain_code), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), gain_pit(gain_pit), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), h1(h1), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), res2(res2), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), xn(xn), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), y1(y1), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP), y2(y2),
    _scc_const_port_0(79),
    ar_cb__ar_tid__DSP__HW(_IDcnt, ar_cc__ar_tid_Subframes__DSP__HW, _scc_const_port_0),
    codebook_cn(_IDcnt, code, txdtx_ctrl, L_pn_seed_tx, ar_cc__ar_tid_Codebook_CN__DSP__HW, ar_cc__T0__DSP__HW, T0, ar_cc__cb_ana__HW__DSP, cb_ana, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, exc_i, ar_cc__gain_code__HW__DSP, gain_code, ar_cc__gain_pit__DSP__HW, gain_pit, ar_cc__h1__DSP__HW, h1, ar_cc__res2__DSP__HW, res2, ar_cc__xn__DSP__HW, xn, ar_cc__y1__DSP__HW, y1, ar_cc__y2__HW__DSP, y2)
{   
}

AR_WR_Codebook_CN::~AR_WR_Codebook_CN(void)
{   
}

void AR_WR_Codebook_CN::main(void)
{   
    __PRF_globalCounter[13u]++ ;
    ar_cb__ar_tid__DSP__HW.main();
    codebook_cn.main();
}

AR_WR_Codebook_DSP::AR_WR_Codebook_DSP(unsigned int _idcnt, i_int_sender (&ar_cc__ar_tid_Codebook_CN__DSP__HW), i_short_int_sender (&ar_cc__T0__DSP__HW), short int (&T0), i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), short int (&cb_ana)[10], i_short_int__40__sender (&ar_cc__code__DSP__HW), short int (&code)[40], i_short_int__40__receiver (&ar_cc__code__HW__DSP), i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), short int (&exc_i)[40], i_short_int_receiver (&ar_cc__gain_code__HW__DSP), short int (&gain_code), i_short_int_sender (&ar_cc__gain_pit__DSP__HW), short int (&gain_pit), i_short_int__40__sender (&ar_cc__h1__DSP__HW), short int (&h1)[40], i_short_int__40__sender (&ar_cc__res2__DSP__HW), short int (&res2)[40], i_short_int__40__sender (&ar_cc__xn__DSP__HW), short int (&xn)[40], i_short_int__40__sender (&ar_cc__y1__DSP__HW), short int (&y1)[40], i_short_int__40__receiver (&ar_cc__y2__HW__DSP), short int (&y2)[40])
    : _specc::behavior(_idcnt), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), T0(T0), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), cb_ana(cb_ana), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), code(code), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), exc_i(exc_i), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), gain_code(gain_code), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), gain_pit(gain_pit), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), h1(h1), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), res2(res2), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), xn(xn), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), y1(y1), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP), y2(y2),
    _scc_const_port_0(81),
    ar_cb__ar_tid__DSP__HW(_IDcnt, ar_cc__ar_tid_Codebook_CN__DSP__HW, _scc_const_port_0),
    ar_fork_codebook(_IDcnt, ar_cc__T0__DSP__HW, T0, ar_cc__code__DSP__HW, code, ar_cc__exc_i__DSP__HW, exc_i, ar_cc__gain_pit__DSP__HW, gain_pit, ar_cc__h1__DSP__HW, h1, ar_cc__res2__DSP__HW, res2, ar_cc__xn__DSP__HW, xn, ar_cc__y1__DSP__HW, y1),
    ar_join_codebook(_IDcnt, ar_cc__cb_ana__HW__DSP, cb_ana, ar_cc__code__HW__DSP, code, ar_cc__gain_code__HW__DSP, gain_code, ar_cc__y2__HW__DSP, y2)
{   
}

AR_WR_Codebook_DSP::~AR_WR_Codebook_DSP(void)
{   
}

void AR_WR_Codebook_DSP::main(void)
{   
    __PRF_globalCounter[14u]++ ;
    ar_cb__ar_tid__DSP__HW.main();
    ar_fork_codebook.main();
    ar_join_codebook.main();
}

AR_WR_Coder_12k2::AR_WR_Coder_12k2(unsigned int _idcnt, short int (&speech_proc)[160], short int (&ana)[57], short int (&synth)[160], bool (&dtx_mode), short int (&txdtx_ctrl), bool (&reset_flag), i_int_sender (&ar_cc__ar_tid_Coder__DSP__HW), i_int_sender (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_sender (&ar_cc__ar_tid_Codebook_CN__DSP__HW), short int (&T0), short int (&cb_ana)[10], short int (&code)[40], short int (&exc_i)[40], short int (&gain_code), short int (&gain_pit), short int (&h1)[40], short int (&res2)[40], short int (&xn)[40], short int (&y1)[40], short int (&y2)[40], i_short_int_sender (&ar_cc__T0__DSP__HW), i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), i_short_int__40__sender (&ar_cc__code__DSP__HW), i_short_int__40__receiver (&ar_cc__code__HW__DSP), i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), i_short_int_receiver (&ar_cc__gain_code__HW__DSP), i_short_int_sender (&ar_cc__gain_pit__DSP__HW), i_short_int__40__sender (&ar_cc__h1__DSP__HW), i_short_int__40__sender (&ar_cc__res2__DSP__HW), i_short_int__40__sender (&ar_cc__xn__DSP__HW), i_short_int__40__sender (&ar_cc__y1__DSP__HW), i_short_int__40__receiver (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), speech_proc(speech_proc), ana(ana), synth(synth), dtx_mode(dtx_mode), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag), ar_cc__ar_tid_Coder__DSP__HW(ar_cc__ar_tid_Coder__DSP__HW), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), T0(T0), cb_ana(cb_ana), code(code), exc_i(exc_i), gain_code(gain_code), gain_pit(gain_pit), h1(h1), res2(res2), xn(xn), y1(y1), y2(y2), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    _scc_const_port_0(1),
    ar_cb__ar_tid__DSP__HW(_IDcnt, ar_cc__ar_tid_Coder__DSP__HW, _scc_const_port_0),
    coder_12k2(_IDcnt, speech_proc, ana, synth, dtx_mode, txdtx_ctrl, reset_flag, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, T0, cb_ana, code, exc_i, gain_code, gain_pit, h1, res2, xn, y1, y2, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP)
{   
}

AR_WR_Coder_12k2::~AR_WR_Coder_12k2(void)
{   
}

void AR_WR_Coder_12k2::main(void)
{   
    __PRF_globalCounter[15u]++ ;
    ar_cb__ar_tid__DSP__HW.main();
    coder_12k2.main();
}

void AR_WR_Coder_12k2::reset(void)
{   
    __PRF_globalCounter[16u]++ ;
    coder_12k2.reset();
}

AR_WR_Post_Process::AR_WR_Post_Process(unsigned int _idcnt, short int (&prm)[57], short int (&txdtx_ctrl), bool (&reset_flag), i_sender (&serial), i_sender (&txdtx_ctrl_out), i_int_sender (&ar_cc__ar_tid_Coder__DSP__HW))
    : _specc::behavior(_idcnt), prm(prm), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag), serial(serial), txdtx_ctrl_out(txdtx_ctrl_out), ar_cc__ar_tid_Coder__DSP__HW(ar_cc__ar_tid_Coder__DSP__HW),
    _scc_const_port_0(110),
    ar_cb__ar_tid__DSP__HW(_IDcnt, ar_cc__ar_tid_Coder__DSP__HW, _scc_const_port_0),
    post_process(_IDcnt, prm, txdtx_ctrl, reset_flag, serial, txdtx_ctrl_out)
{   
}

AR_WR_Post_Process::~AR_WR_Post_Process(void)
{   
}

void AR_WR_Post_Process::main(void)
{   
    __PRF_globalCounter[17u]++ ;
    ar_cb__ar_tid__DSP__HW.main();
    post_process.main();
}

void AR_WR_Post_Process::reset(void)
{   
    __PRF_globalCounter[18u]++ ;
    post_process.reset();
}

AR_WR_Post_Process_HW::AR_WR_Post_Process_HW(unsigned int _idcnt)
    : _specc::behavior(_idcnt)
{   
}

AR_WR_Post_Process_HW::~AR_WR_Post_Process_HW(void)
{   
}

void AR_WR_Post_Process_HW::main(void)
{   
    __PRF_globalCounter[19u]++ ;
}

AR_WR_Pre_Process::AR_WR_Pre_Process(unsigned int _idcnt, i_receiver (&speech_frame), i_receiver (&dtx_mode), short int (&speech_frame_filtered)[160], bool (&reset_flag_1), bool (&reset_flag_2), bool (&local_dtx_mode), i_int_sender (&ar_cc__ar_tid_Coder__DSP__HW))
    : _specc::behavior(_idcnt), speech_frame(speech_frame), dtx_mode(dtx_mode), speech_frame_filtered(speech_frame_filtered), reset_flag_1(reset_flag_1), reset_flag_2(reset_flag_2), local_dtx_mode(local_dtx_mode), ar_cc__ar_tid_Coder__DSP__HW(ar_cc__ar_tid_Coder__DSP__HW),
    _scc_const_port_0(115),
    ar_cb__ar_tid__DSP__HW(_IDcnt, ar_cc__ar_tid_Coder__DSP__HW, _scc_const_port_0),
    pre_process(_IDcnt, speech_frame, dtx_mode, speech_frame_filtered, reset_flag_1, reset_flag_2, local_dtx_mode)
{   
}

AR_WR_Pre_Process::~AR_WR_Pre_Process(void)
{   
}

void AR_WR_Pre_Process::main(void)
{   
    __PRF_globalCounter[20u]++ ;
    ar_cb__ar_tid__DSP__HW.main();
    pre_process.main();
}

void AR_WR_Pre_Process::reset(void)
{   
    __PRF_globalCounter[21u]++ ;
    pre_process.reset();
}

AR_WR_Pre_Process_HW::AR_WR_Pre_Process_HW(unsigned int _idcnt)
    : _specc::behavior(_idcnt)
{   
}

AR_WR_Pre_Process_HW::~AR_WR_Pre_Process_HW(void)
{   
}

void AR_WR_Pre_Process_HW::main(void)
{   
    __PRF_globalCounter[22u]++ ;
}

Autocorr_0::Autocorr_0(unsigned int _idcnt, short int *(&x), short int (&wind)[240], short int (&r_h)[11], short int (&r_l)[11], short int (&norm))
    : _specc::behavior(_idcnt), x(x), wind(wind), r_h(r_h), r_l(r_l), norm(norm)
{   
}

Autocorr_0::~Autocorr_0(void)
{   
}

void Autocorr_0::main(void)
{   
    int i;
    int j;
    short int n;
    bool overfl;
    short int overfl_shft;
    int sum;
    short int y[240];

    __PRF_globalCounter[42u]++ ;
    _specc::waitfor(220816666ull);
    __PRF_globalCounter[43u]++ ;
    for(i = 0; i < 240; i++ )
    {   
	__PRF_globalCounter[44u]++ ;
	y[i] = mult_r(x[i], wind[i]);
    }
    __PRF_globalCounter[45u]++ ;
    overfl_shft = 0;
    __PRF_globalCounter[46u]++ ;
    do 
    {   
	__PRF_globalCounter[47u]++ ;
	overfl = 0;
	sum = 0l;
	__PRF_globalCounter[48u]++ ;
	for(i = 0; i < 240; i++ )
	{   
	    __PRF_globalCounter[49u]++ ;
	    sum = L_mac(sum, y[i], y[i]);
	}
	__PRF_globalCounter[50u]++ ;
	if (L_sub(sum, (int)2147483647l) == 0l)
	{   
	    __PRF_globalCounter[51u]++ ;
	    overfl_shft = add(overfl_shft, 4);
	    overfl = 1;
	    __PRF_globalCounter[52u]++ ;
	    for(i = 0; i < 240; i++ )
	    {   
		__PRF_globalCounter[53u]++ ;
		y[i] = shr(y[i], 2);
	    }
	}
    }
    while(overfl != 0);
    __PRF_globalCounter[54u]++ ;
    sum = L_add(sum, 1l);
    n = norm_l(sum);
    sum = L_shl(sum, n);
    L_Extract(sum,  &r_h[0],  &r_l[0]);
    __PRF_globalCounter[55u]++ ;
    for(i = 1; i <= 10; i++ )
    {   
	__PRF_globalCounter[56u]++ ;
	sum = 0;
	__PRF_globalCounter[57u]++ ;
	for(j = 0; j < 240 - i; j++ )
	{   
	    __PRF_globalCounter[58u]++ ;
	    sum = L_mac(sum, y[j], y[j + i]);
	}
	__PRF_globalCounter[59u]++ ;
	sum = L_shl(sum, n);
	L_Extract(sum,  &r_h[i],  &r_l[i]);
    }
    __PRF_globalCounter[60u]++ ;
    norm = sub(n, overfl_shft);
}

Az_Lsp_0::Az_Lsp_0(unsigned int _idcnt, short int (&a)[11], short int (&lsp)[10], short int (&old_lsp)[10])
    : _specc::behavior(_idcnt), a(a), lsp(lsp), old_lsp(old_lsp)
{   
}

Az_Lsp_0::~Az_Lsp_0(void)
{   
}

short int Az_Lsp_0::Chebps(short int x, short int f[], short int n)
{   
    short int b0_h;
    short int b0_l;
    short int b1_h;
    short int b1_l;
    short int b2_h;
    short int b2_l;
    short int cheb;
    int i;
    int t0;

    __PRF_globalCounter[92u]++ ;
    b2_h = 256;
    b2_l = 0;
    t0 = L_mult(x, 512);
    t0 = L_mac(t0, f[1], 8192);
    L_Extract(t0,  &b1_h,  &b1_l);
    __PRF_globalCounter[93u]++ ;
    for(i = 2; i < n; i++ )
    {   
	__PRF_globalCounter[94u]++ ;
	t0 = Mpy_32_16(b1_h, b1_l, x);
	t0 = L_shl(t0, 1);
	t0 = L_mac(t0, b2_h, (short int)32768);
	t0 = L_msu(t0, b2_l, 1);
	t0 = L_mac(t0, f[i], 8192);
	L_Extract(t0,  &b0_h,  &b0_l);
	b2_l = b1_l;
	b2_h = b1_h;
	b1_l = b0_l;
	b1_h = b0_h;
    }
    __PRF_globalCounter[95u]++ ;
    t0 = Mpy_32_16(b1_h, b1_l, x);
    t0 = L_mac(t0, b2_h, (short int)32768);
    t0 = L_msu(t0, b2_l, 1);
    t0 = L_mac(t0, f[i], 4096);
    t0 = L_shl(t0, 6);
    cheb = extract_h(t0);
    return (cheb);
}

void Az_Lsp_0::main(void)
{   
    short int *coef;
    short int exp;
    short int f1[6];
    short int f2[6];
    const short int grid[61] = 
    { ((short int)32760),((short int)32723),((short int)32588),((short int)32364),
      ((short int)32051),((short int)31651),((short int)31164),((short int)30591),
      ((short int)29935),((short int)29196),((short int)28377),((short int)27481),
      ((short int)26509),((short int)25465),((short int)24351),((short int)23170),
      ((short int)21926),((short int)20621),((short int)19260),((short int)17846),
      ((short int)16384),((short int)14876),((short int)13327),((short int)11743),
      ((short int)10125),((short int)8480),((short int)6812),((short int)5126),
      ((short int)3425),((short int)1714),((short int)0),((short int)-1714),
      ((short int)-3425),((short int)-5126),((short int)-6812),((short int)-8480),
      ((short int)-10125),((short int)-11743),((short int)-13327),((short int)-14876),
      ((short int)-16384),((short int)-17846),((short int)-19260),((short int)-20621),
      ((short int)-21926),((short int)-23170),((short int)-24351),((short int)-25465),
      ((short int)-26509),((short int)-27481),((short int)-28377),((short int)-29196),
      ((short int)-29935),((short int)-30591),((short int)-31164),((short int)-31651),
      ((short int)-32051),((short int)-32364),((short int)-32588),((short int)-32723),
      ((short int)-32760) };
    int i;
    int ip;
    int j;
    int nf;
    short int sign;
    int t0;
    short int x;
    short int xhigh;
    short int xint;
    short int xlow;
    short int xmid;
    short int y;
    short int yhigh;
    short int ylow;
    short int ymid;

    __PRF_globalCounter[96u]++ ;
    _specc::waitfor(154333333ull);
    f1[0] = 1024;
    f2[0] = 1024;
    __PRF_globalCounter[97u]++ ;
    for(i = 0; i < 10 / 2; i++ )
    {   
	__PRF_globalCounter[98u]++ ;
	t0 = L_mult(a[i + 1], 8192);
	t0 = L_mac(t0, a[10 - i], 8192);
	x = extract_h(t0);
	f1[i + 1] = sub(x, f1[i]);
	t0 = L_mult(a[i + 1], 8192);
	t0 = L_msu(t0, a[10 - i], 8192);
	x = extract_h(t0);
	f2[i + 1] = add(x, f2[i]);
    }
    __PRF_globalCounter[99u]++ ;
    nf = 0;
    ip = 0;
    coef = f1;
    xlow = grid[0];
    ylow = Chebps(xlow, coef, 10 / 2);
    j = 0;
    __PRF_globalCounter[100u]++ ;
    while((sub(nf, 10) < 0) && (sub(j, 60) < 0))
    {   
	__PRF_globalCounter[101u]++ ;
	j++ ;
	xhigh = xlow;
	yhigh = ylow;
	xlow = grid[j];
	ylow = Chebps(xlow, coef, 10 / 2);
	__PRF_globalCounter[102u]++ ;
	if (L_mult(ylow, yhigh) <= (int)0l)
	{   
	    __PRF_globalCounter[103u]++ ;
	    for(i = 0; i < 4; i++ )
	    {   
		__PRF_globalCounter[104u]++ ;
		xmid = add(shr(xlow, 1), shr(xhigh, 1));
		ymid = Chebps(xmid, coef, 10 / 2);
		__PRF_globalCounter[105u]++ ;
		if (L_mult(ylow, ymid) <= (int)0l)
		{   
		    __PRF_globalCounter[106u]++ ;
		    yhigh = ymid;
		    xhigh = xmid;
		}
		else 
		{   
		    __PRF_globalCounter[107u]++ ;
		    ylow = ymid;
		    xlow = xmid;
		}
	    }
	    __PRF_globalCounter[108u]++ ;
	    x = sub(xhigh, xlow);
	    y = sub(yhigh, ylow);
	    __PRF_globalCounter[109u]++ ;
	    if (y == 0)
	    {   
		__PRF_globalCounter[110u]++ ;
		xint = xlow;
	    }
	    else 
	    {   
		__PRF_globalCounter[111u]++ ;
		sign = y;
		y = abs_s(y);
		exp = norm_s(y);
		y = shl(y, exp);
		y = div_s((short int)16383, y);
		t0 = L_mult(x, y);
		t0 = L_shr(t0, sub(20, exp));
		y = extract_l(t0);
		__PRF_globalCounter[112u]++ ;
		if (sign < 0)
		{   
		    __PRF_globalCounter[113u]++ ;
		    y = negate(y);
		}
		__PRF_globalCounter[114u]++ ;
		t0 = L_mult(ylow, y);
		t0 = L_shr(t0, 11);
		xint = sub(xlow, extract_l(t0));
	    }
	    __PRF_globalCounter[115u]++ ;
	    lsp[nf] = xint;
	    xlow = xint;
	    nf++ ;
	    __PRF_globalCounter[116u]++ ;
	    if (ip == 0)
	    {   
		__PRF_globalCounter[117u]++ ;
		ip = 1;
		coef = f2;
	    }
	    else 
	    {   
		__PRF_globalCounter[118u]++ ;
		ip = 0;
		coef = f1;
	    }
	    __PRF_globalCounter[119u]++ ;
	    ylow = Chebps(xlow, coef, 10 / 2);
	}
    }
    __PRF_globalCounter[120u]++ ;
    if (sub(nf, 10) < 0)
    {   
	__PRF_globalCounter[121u]++ ;
	for(i = 0; i < 10; i++ )
	{   
	    __PRF_globalCounter[122u]++ ;
	    lsp[i] = old_lsp[i];
	}
    }
}

Build_CN_Code::Build_CN_Code(unsigned int _idcnt, short int (&cod)[40], int (&seed))
    : _specc::behavior(_idcnt), cod(cod), seed(seed)
{   
}

Build_CN_Code::~Build_CN_Code(void)
{   
}

void Build_CN_Code::main(void)
{   
    short int i;
    short int j;
    int k;

    __PRF_globalCounter[123u]++ ;
    _specc::waitfor(0ull);
    __PRF_globalCounter[124u]++ ;
    for(k = 0; k < 40; k++ )
    {   
	__PRF_globalCounter[125u]++ ;
	cod[k] = 0;
    }
    __PRF_globalCounter[126u]++ ;
    for(k = 0; k < 10; k++ )
    {   
	__PRF_globalCounter[127u]++ ;
	i = pseudonoise(2);
	i = shr(extract_l(L_mult(i, 10)), 1);
	i = add(i, k);
	j = pseudonoise(1);
	__PRF_globalCounter[128u]++ ;
	if (j > 0)
	{   
	    __PRF_globalCounter[129u]++ ;
	    cod[i] = 4096;
	}
	else 
	{   
	    __PRF_globalCounter[130u]++ ;
	    cod[i] =  -4096;
	}
    }
}

short int Build_CN_Code::pseudonoise(short int no_bits)
{   
    short int Sn;
    int i;
    short int noise_bits;

    __PRF_globalCounter[131u]++ ;
    noise_bits = 0;
    __PRF_globalCounter[132u]++ ;
    for(i = 0; i < no_bits; i++ )
    {   
	__PRF_globalCounter[133u]++ ;
	if ((seed & 1l) != 0)
	{   
	    __PRF_globalCounter[134u]++ ;
	    Sn = 1;
	}
	else 
	{   
	    __PRF_globalCounter[135u]++ ;
	    Sn = 0;
	}
	__PRF_globalCounter[136u]++ ;
	if ((seed & 268435456l) != 0)
	{   
	    __PRF_globalCounter[137u]++ ;
	    Sn = Sn ^ 1;
	}
	else 
	{   
	    __PRF_globalCounter[138u]++ ;
	    Sn = Sn ^ 0;
	}
	__PRF_globalCounter[139u]++ ;
	noise_bits = shl(noise_bits, 1);
	noise_bits = noise_bits | (extract_l(seed) & 1);
	seed = L_shr(seed, 1);
	__PRF_globalCounter[140u]++ ;
	if (Sn & 1)
	{   
	    __PRF_globalCounter[141u]++ ;
	    seed = seed | 1073741824l;
	}
    }
    __PRF_globalCounter[142u]++ ;
    return noise_bits;
}

Closed_Loop::Closed_Loop(unsigned int _idcnt, int (&i_subfr), short int (&A)[11], short int (&Aq)[11], short int (&h1)[40], short int *(&p_speech), short int (&res2)[40], short int *(&p_exc), short int (&xn)[40], short int (&mem_err)[10], short int (&mem_w0)[10], short int (&t0_min), short int (&t0_max), short int (&t0_min_2), short int (&t0_max_2), short int (&t0), short int (&ana)[2], short int (&y1)[40], short int (&gain_pit), short int (&txdtx_ctrl), short int (&CN_excitation_gain), bool (&reset_flag))
    : _specc::behavior(_idcnt), i_subfr(i_subfr), A(A), Aq(Aq), h1(h1), p_speech(p_speech), res2(res2), p_exc(p_exc), xn(xn), mem_err(mem_err), mem_w0(mem_w0), t0_min(t0_min), t0_max(t0_max), t0_min_2(t0_min_2), t0_max_2(t0_max_2), t0(t0), ana(ana), y1(y1), gain_pit(gain_pit), txdtx_ctrl(txdtx_ctrl), CN_excitation_gain(CN_excitation_gain), reset_flag(reset_flag),
    convolve(_IDcnt, p_exc_i, p_h1, y1b),
    enc_lag6(_IDcnt, T0, T0_frac, t0_min, t0_max, pit_flag, ana_content),
    g_pitch(_IDcnt, xn, y1b, gain_pitch),
    par_weight(_IDcnt, A, Ap1, Ap2),
    pitch_fr6(_IDcnt, p_exc_i, xn, p_h1, t0_min, t0_max, pit_flag, T0_frac, T0),
    pred_lt_6(_IDcnt, p_exc_i, T0, T0_frac),
    q_gain_pitch(_IDcnt, gain_pitch, ana_content),
    seq1(_IDcnt, i_subfr, h1, p_exc, p_speech, p_h1, p_exc_i, p_speech_i),
    seq2(_IDcnt, Aq, Ap1, Ap2, h1, p_speech_i, res2, p_exc_i, xn, mem_err, mem_w0, txdtx_ctrl, CN_excitation_gain, reset_flag),
    seq3(_IDcnt, i_subfr, pit_flag, t0_min_2, t0_max_2, t0_min, t0_max),
    seq4(_IDcnt, ana_content, T0, ana, t0),
    seq5(_IDcnt, gain_pit),
    seq6(_IDcnt, y1b, gain_pitch, ana_content, y1, gain_pit, ana)
{   
}

Closed_Loop::~Closed_Loop(void)
{   
}

void Closed_Loop::main(void)
{   
    __PRF_globalCounter[190u]++ ;
    { enum { _scc_state_0, _scc_state_seq1, _scc_state_par_weight, _scc_state_seq2, _scc_state_seq3, _scc_state_pitch_fr6, _scc_state_enc_lag6, _scc_state_seq4, _scc_state_seq5, _scc_state_pred_lt_6, _scc_state_convolve, _scc_state_g_pitch, _scc_state_q_gain_pitch, _scc_state_seq6 } _scc_next_state = _scc_state_seq1; do switch(_scc_next_state) {
	    case _scc_state_seq1: { seq1.main(); if (__PRF_globalCounter[191u]++  , 
		    (txdtx_ctrl & 1) != 0) { _scc_next_state = _scc_state_par_weight; break; }
		{ _scc_next_state = _scc_state_seq2; break; } }
	    case _scc_state_par_weight: { par_weight.main(); if (__PRF_globalCounter[192u]++  , 
		    1u) { _scc_next_state = _scc_state_seq2; break; } }
	    case _scc_state_seq2: { seq2.main(); if (__PRF_globalCounter[193u]++  , 
		    (txdtx_ctrl & 1) != 0) { _scc_next_state = _scc_state_seq3; break; }
		{ _scc_next_state = _scc_state_seq5; break; } }
	    case _scc_state_seq3: { seq3.main(); if (__PRF_globalCounter[194u]++  , 
		    1u) { _scc_next_state = _scc_state_pitch_fr6; break; } }
	    case _scc_state_pitch_fr6: { pitch_fr6.main(); if (__PRF_globalCounter[195u]++  , 
		    1u) { _scc_next_state = _scc_state_enc_lag6; break; } }
	    case _scc_state_enc_lag6: { enc_lag6.main(); if (__PRF_globalCounter[196u]++  , 
		    1u) { _scc_next_state = _scc_state_seq4; break; } }
	    case _scc_state_seq4: { seq4.main(); if (__PRF_globalCounter[197u]++  , 
		    (txdtx_ctrl & 1) != 0) { _scc_next_state = _scc_state_pred_lt_6; break; }
		{ _scc_next_state = _scc_state_0; break; } }
	    case _scc_state_seq5: { seq5.main(); if (__PRF_globalCounter[198u]++  , 
		    (txdtx_ctrl & 1) != 0) { _scc_next_state = _scc_state_pred_lt_6; break; }
		{ _scc_next_state = _scc_state_0; break; } }
	    case _scc_state_pred_lt_6: { pred_lt_6.main(); if (__PRF_globalCounter[199u]++  , 
		    1u) { _scc_next_state = _scc_state_convolve; break; } }
	    case _scc_state_convolve: { convolve.main(); if (__PRF_globalCounter[200u]++  , 
		    1u) { _scc_next_state = _scc_state_g_pitch; break; } }
	    case _scc_state_g_pitch: { g_pitch.main(); if (__PRF_globalCounter[201u]++  , 
		    1u) { _scc_next_state = _scc_state_q_gain_pitch; break; } }
	    case _scc_state_q_gain_pitch: { q_gain_pitch.main(); if (__PRF_globalCounter[202u]++  , 
		    1u) { _scc_next_state = _scc_state_seq6; break; } }
	    case _scc_state_seq6: { seq6.main(); if (__PRF_globalCounter[203u]++  , 
		    1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

void Closed_Loop::reset(void)
{   
    __PRF_globalCounter[204u]++ ;
    seq2.reset();
}

Codebook::Codebook(unsigned int _idcnt, short int (&xn)[40], short int (&y1)[40], short int (&gain_pit), short int (&exc)[40], short int (&h1)[40], short int (&T0), short int (&res2)[40], short int (&code)[40], short int (&y2)[40], short int (&gain_code), short int (&ana)[10])
    : _specc::behavior(_idcnt), xn(xn), y1(y1), gain_pit(gain_pit), exc(exc), h1(h1), T0(T0), res2(res2), code(code), y2(y2), gain_code(gain_code), ana(ana),
    code_10i40(_IDcnt, xn2, res2b, h1b, codeb, y2, ana),
    seq1(_IDcnt, xn, y1, gain_pit, exc, h1, T0, res2, xn2, h1b, res2b),
    seq2(_IDcnt, T0, gain_pit, codeb, xn2, y2, code, gain_code)
{   
}

Codebook::~Codebook(void)
{   
}

void Codebook::main(void)
{   
    __PRF_globalCounter[233u]++ ;
    seq1.main();
    code_10i40.main();
    seq2.main();
}

Codebook_CN::Codebook_CN(unsigned int _idcnt, short int (&code)[40], short int (&txdtx_ctrl), int (&L_pn_seed_tx), i_int_sender (&ar_cc__ar_tid_Codebook_CN__DSP__HW), i_short_int_sender (&ar_cc__T0__DSP__HW), short int (&T0), i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), short int (&cb_ana)[10], i_short_int__40__sender (&ar_cc__code__DSP__HW), i_short_int__40__receiver (&ar_cc__code__HW__DSP), i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), short int (&exc_i)[40], i_short_int_receiver (&ar_cc__gain_code__HW__DSP), short int (&gain_code), i_short_int_sender (&ar_cc__gain_pit__DSP__HW), short int (&gain_pit), i_short_int__40__sender (&ar_cc__h1__DSP__HW), short int (&h1)[40], i_short_int__40__sender (&ar_cc__res2__DSP__HW), short int (&res2)[40], i_short_int__40__sender (&ar_cc__xn__DSP__HW), short int (&xn)[40], i_short_int__40__sender (&ar_cc__y1__DSP__HW), short int (&y1)[40], i_short_int__40__receiver (&ar_cc__y2__HW__DSP), short int (&y2)[40])
    : _specc::behavior(_idcnt), code(code), txdtx_ctrl(txdtx_ctrl), L_pn_seed_tx(L_pn_seed_tx), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), T0(T0), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), cb_ana(cb_ana), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), exc_i(exc_i), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), gain_code(gain_code), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), gain_pit(gain_pit), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), h1(h1), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), res2(res2), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), xn(xn), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), y1(y1), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP), y2(y2),
    _scc_const_port_0(-1),
    ar_cb__ar_stop__DSP__HW(_IDcnt, ar_cc__ar_tid_Codebook_CN__DSP__HW, _scc_const_port_0),
    build_cn_code(_IDcnt, code, L_pn_seed_tx),
    nop(_IDcnt),
    wr_codebook(_IDcnt, ar_cc__ar_tid_Codebook_CN__DSP__HW, ar_cc__T0__DSP__HW, T0, ar_cc__cb_ana__HW__DSP, cb_ana, ar_cc__code__DSP__HW, code, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, exc_i, ar_cc__gain_code__HW__DSP, gain_code, ar_cc__gain_pit__DSP__HW, gain_pit, ar_cc__h1__DSP__HW, h1, ar_cc__res2__DSP__HW, res2, ar_cc__xn__DSP__HW, xn, ar_cc__y1__DSP__HW, y1, ar_cc__y2__HW__DSP, y2)
{   
}

Codebook_CN::~Codebook_CN(void)
{   
}

void Codebook_CN::main(void)
{   
    __PRF_globalCounter[234u]++ ;
    { enum { _scc_state_0, _scc_state_nop, _scc_state_wr_codebook, _scc_state_build_cn_code, _scc_state_ar_cb__ar_stop__DSP__HW } _scc_next_state = _scc_state_nop; do switch(_scc_next_state) {
	    case _scc_state_nop: { nop.main(); if (__PRF_globalCounter[235u]++  , 
		    (txdtx_ctrl & 1) != 0) { _scc_next_state = _scc_state_wr_codebook; break; }
		{ _scc_next_state = _scc_state_build_cn_code; break; } }
	    case _scc_state_wr_codebook: { wr_codebook.main(); if (__PRF_globalCounter[236u]++  , 
		    1u) { _scc_next_state = _scc_state_ar_cb__ar_stop__DSP__HW; break; } }
	    case _scc_state_build_cn_code: { build_cn_code.main(); if (__PRF_globalCounter[237u]++  , 
		    1u) { _scc_next_state = _scc_state_ar_cb__ar_stop__DSP__HW; break; } }
	    case _scc_state_ar_cb__ar_stop__DSP__HW: { ar_cb__ar_stop__DSP__HW.main(); if (__PRF_globalCounter[238u]++  , 
		    1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

Codebook_CN_HW::Codebook_CN_HW(unsigned int _idcnt, short int (&xn)[40], short int (&y1)[40], short int (&gain_pit), short int (&exc)[40], short int (&h1)[40], short int (&T0), short int (&res2)[40], short int (&code)[40], short int (&y2)[40], short int (&gain_code), short int (&ana)[10], i_int_receiver (&ar_cc__ar_tid_Codebook_CN__DSP__HW), i_short_int_receiver (&ar_cc__T0__DSP__HW), i_short_int__10__sender (&ar_cc__cb_ana__HW__DSP), i_short_int__40__receiver (&ar_cc__code__DSP__HW), i_short_int__40__sender (&ar_cc__code__HW__DSP), i_short_int__40__receiver (&ar_cc__exc_i__DSP__HW), i_short_int_sender (&ar_cc__gain_code__HW__DSP), i_short_int_receiver (&ar_cc__gain_pit__DSP__HW), i_short_int__40__receiver (&ar_cc__h1__DSP__HW), i_short_int__40__receiver (&ar_cc__res2__DSP__HW), i_short_int__40__receiver (&ar_cc__xn__DSP__HW), i_short_int__40__receiver (&ar_cc__y1__DSP__HW), i_short_int__40__sender (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), xn(xn), y1(y1), gain_pit(gain_pit), exc(exc), h1(h1), T0(T0), res2(res2), code(code), y2(y2), gain_code(gain_code), ana(ana), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    ar_cb__ar_tid__DSP__HW(_IDcnt, ar_cc__ar_tid_Codebook_CN__DSP__HW, ar_tid),
    wr_codebook(_IDcnt, xn, y1, gain_pit, exc, h1, T0, res2, code, y2, gain_code, ana, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP)
{   
}

Codebook_CN_HW::~Codebook_CN_HW(void)
{   
}

void Codebook_CN_HW::main(void)
{   
    __PRF_globalCounter[239u]++ ;
    { enum { _scc_state_0, _scc_state_ar_cb__ar_tid__DSP__HW, _scc_state_wr_codebook } _scc_next_state = _scc_state_ar_cb__ar_tid__DSP__HW; do switch(_scc_next_state) {
	    case _scc_state_ar_cb__ar_tid__DSP__HW: { ar_cb__ar_tid__DSP__HW.main(); if (__PRF_globalCounter[240u]++  , 
		    ar_tid == 81) { _scc_next_state = _scc_state_wr_codebook; break; }
		{ _scc_next_state = _scc_state_0; break; } }
	    case _scc_state_wr_codebook: { wr_codebook.main(); if (__PRF_globalCounter[241u]++  , 
		    1u) { _scc_next_state = _scc_state_ar_cb__ar_tid__DSP__HW; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

Coder::Coder(unsigned int _idcnt, i_receiver (&speech_samples), i_receiver (&dtx_mode), i_sender (&serial), i_sender (&txdtx_ctrl))
    : _specc::behavior(_idcnt), speech_samples(speech_samples), dtx_mode(dtx_mode), serial(serial), txdtx_ctrl(txdtx_ctrl),
    _scc_const_port_0(1ul),
    _scc_const_port_1(1ul),
    _scc_const_port_2(1ul),
    _scc_const_port_3(1ul),
    _scc_const_port_4(1ul),
    _scc_const_port_5(1ul),
    _scc_const_port_6(1ul),
    _scc_const_port_7(1ul),
    _scc_const_port_8(1ul),
    _scc_const_port_9(1ul),
    _scc_const_port_10(1ul),
    _scc_const_port_11(1ul),
    _scc_const_port_12(1ul),
    _scc_const_port_13(1ul),
    _scc_const_port_14(1ul),
    DSP(++_IDcnt, speech_samples, dtx_mode, serial, txdtx_ctrl, ar_cc__ar_tid_Coder__DSP__HW, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP),
    HW(++_IDcnt, ar_cc__ar_tid_Coder__DSP__HW, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP),
    ar_cc__T0__DSP__HW(_scc_const_port_0),
    ar_cc__ar_tid_Codebook_CN__DSP__HW(_scc_const_port_1),
    ar_cc__ar_tid_Coder__DSP__HW(_scc_const_port_2),
    ar_cc__ar_tid_Subframes__DSP__HW(_scc_const_port_3),
    ar_cc__cb_ana__HW__DSP(_scc_const_port_4),
    ar_cc__code__DSP__HW(_scc_const_port_5),
    ar_cc__code__HW__DSP(_scc_const_port_6),
    ar_cc__exc_i__DSP__HW(_scc_const_port_7),
    ar_cc__gain_code__HW__DSP(_scc_const_port_8),
    ar_cc__gain_pit__DSP__HW(_scc_const_port_9),
    ar_cc__h1__DSP__HW(_scc_const_port_10),
    ar_cc__res2__DSP__HW(_scc_const_port_11),
    ar_cc__xn__DSP__HW(_scc_const_port_12),
    ar_cc__y1__DSP__HW(_scc_const_port_13),
    ar_cc__y2__HW__DSP(_scc_const_port_14)
{   
}

Coder::~Coder(void)
{   
}

void Coder::main(void)
{   
    { _specc::fork _scc_fork_0(&DSP), _scc_fork_1(&HW); _specc::par(&_scc_fork_0, &_scc_fork_1, ((_specc::fork*)0));
    }
}

void Coder::reset(void)
{   
    __PRF_globalCounter[245u]++ ;
    DSP.reset();
}

Coder_12k2::Coder_12k2(unsigned int _idcnt, short int (&speech_proc)[160], short int (&ana)[57], short int (&synth)[160], bool (&dtx_mode), short int (&txdtx_ctrl), bool (&reset_flag), i_int_sender (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_sender (&ar_cc__ar_tid_Codebook_CN__DSP__HW), short int (&T0), short int (&cb_ana)[10], short int (&code)[40], short int (&exc_i)[40], short int (&gain_code), short int (&gain_pit), short int (&h1)[40], short int (&res2)[40], short int (&xn)[40], short int (&y1)[40], short int (&y2)[40], i_short_int_sender (&ar_cc__T0__DSP__HW), i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), i_short_int__40__sender (&ar_cc__code__DSP__HW), i_short_int__40__receiver (&ar_cc__code__HW__DSP), i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), i_short_int_receiver (&ar_cc__gain_code__HW__DSP), i_short_int_sender (&ar_cc__gain_pit__DSP__HW), i_short_int__40__sender (&ar_cc__h1__DSP__HW), i_short_int__40__sender (&ar_cc__res2__DSP__HW), i_short_int__40__sender (&ar_cc__xn__DSP__HW), i_short_int__40__sender (&ar_cc__y1__DSP__HW), i_short_int__40__receiver (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), speech_proc(speech_proc), ana(ana), synth(synth), dtx_mode(dtx_mode), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), T0(T0), cb_ana(cb_ana), code(code), exc_i(exc_i), gain_code(gain_code), gain_pit(gain_pit), h1(h1), res2(res2), xn(xn), y1(y1), y2(y2), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    lp_analysis(_IDcnt, p_window, A_t, Aq_t, ana, ptch, txdtx_ctrl_cur, dtx_mode, L_pn_seed_tx, reset_flag),
    open_loop(_IDcnt, speech, A_t, wsp, T0_min_1, T0_max_1, T0_min_2, T0_max_2, ptch, txdtx_ctrl_cur, dtx_mode, reset_flag),
    seq1(_IDcnt, speech_proc, old_speech, speech, p_window, old_wsp, wsp, old_exc, exc, ptch, txdtx_ctrl_cur, reset_flag),
    shift_signals(_IDcnt, old_speech, old_wsp, old_exc, txdtx_ctrl_cur, txdtx_ctrl),
    subframes(_IDcnt, A_t, Aq_t, speech, exc, T0_min_1, T0_max_1, T0_min_2, T0_max_2, ana, synth, txdtx_ctrl_cur, L_pn_seed_tx, reset_flag, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, T0, cb_ana, code, exc_i, gain_code, gain_pit, h1, res2, xn, y1, y2, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP)
{   
}

Coder_12k2::~Coder_12k2(void)
{   
}

void Coder_12k2::main(void)
{   
    __PRF_globalCounter[246u]++ ;
    seq1.main();
    lp_analysis.main();
    open_loop.main();
    subframes.main();
    shift_signals.main();
}

void Coder_12k2::reset(void)
{   
    __PRF_globalCounter[247u]++ ;
    seq1.reset();
    lp_analysis.reset();
    open_loop.reset();
    subframes.reset();
}

Coder_12k2_Seq1::Coder_12k2_Seq1(unsigned int _idcnt, short int (&speech_proc)[160], short int (&old_speech)[320], short int *(&speech), short int *(&p_window), short int (&old_wsp)[303], short int *(&wsp), short int (&old_exc)[314], short int *(&exc), bool (&ptch), short int (&txdtx_ctrl), bool (&reset_flag))
    : _specc::behavior(_idcnt), speech_proc(speech_proc), old_speech(old_speech), speech(speech), p_window(p_window), old_wsp(old_wsp), wsp(wsp), old_exc(old_exc), exc(exc), ptch(ptch), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag)
{   
}

Coder_12k2_Seq1::~Coder_12k2_Seq1(void)
{   
}

void Coder_12k2_Seq1::init(void)
{   
    __PRF_globalCounter[248u]++ ;
    speech = old_speech + 320 - 160;
    p_window = old_speech + 320 - 240;
    wsp = old_wsp + 143;
    exc = old_exc + 143 + (10 + 1);
    Set_zero(old_speech, 320);
    Set_zero(old_exc, 143 + (10 + 1));
    Set_zero(old_wsp, 143);
    txdtx_ctrl = 1 | 2;
    ptch = 1;
}

void Coder_12k2_Seq1::main(void)
{   
    __PRF_globalCounter[249u]++ ;
    _specc::waitfor(1083333ull);
    __PRF_globalCounter[250u]++ ;
    if (reset_flag == true)
    {   
	__PRF_globalCounter[251u]++ ;
	init();
    }
    __PRF_globalCounter[252u]++ ;
    Copy(speech_proc, speech, 160);
}

void Coder_12k2_Seq1::reset(void)
{   
    __PRF_globalCounter[253u]++ ;
    init();
}

Find_Az_0::Find_Az_0(unsigned int _idcnt, short int *(&x), short int (&wind)[240], short int (&r_h)[11], short int (&r_l)[11], short int (&old_A)[11], short int (&scal_fac), short int (&A_t)[11], short int (&rc)[4])
    : _specc::behavior(_idcnt), x(x), wind(wind), r_h(r_h), r_l(r_l), old_A(old_A), scal_fac(scal_fac), A_t(A_t), rc(rc),
    autocorr(_IDcnt, x, wind, r_h, r_l, scal_fac),
    lag_window(_IDcnt, r_h, r_l),
    levinson(_IDcnt, r_h, r_l, old_A, A_t, rc)
{   
}

Find_Az_0::~Find_Az_0(void)
{   
}

void Find_Az_0::main(void)
{   
    __PRF_globalCounter[360u]++ ;
    autocorr.main();
    lag_window.main();
    levinson.main();
}

Get_Minmax_0::Get_Minmax_0(unsigned int _idcnt, short int (&T_op), short int (&T0_min), short int (&T0_max), short int (&txdtx_ctrl))
    : _specc::behavior(_idcnt), T_op(T_op), T0_min(T0_min), T0_max(T0_max), txdtx_ctrl(txdtx_ctrl)
{   
}

Get_Minmax_0::~Get_Minmax_0(void)
{   
}

void Get_Minmax_0::main(void)
{   
    short int max;
    short int min;

    __PRF_globalCounter[427u]++ ;
    _specc::waitfor(66666ull);
    __PRF_globalCounter[428u]++ ;
    if ((txdtx_ctrl & 1) != 0)
    {   
	__PRF_globalCounter[429u]++ ;
	min = sub(T_op, 3);
	__PRF_globalCounter[430u]++ ;
	if (sub(min, 18) < 0)
	{   
	    __PRF_globalCounter[431u]++ ;
	    min = 18;
	}
	__PRF_globalCounter[432u]++ ;
	max = add(min, 6);
	__PRF_globalCounter[433u]++ ;
	if (sub(max, 143) > 0)
	{   
	    __PRF_globalCounter[434u]++ ;
	    max = 143;
	    min = sub(max, 6);
	}
	__PRF_globalCounter[435u]++ ;
	T0_min = min;
	T0_max = max;
    }
}

HW_Standard_HW::HW_Standard_HW(unsigned int _idcnt, i_int_receiver (&ar_cc__ar_tid_Coder__DSP__HW), i_int_receiver (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_receiver (&ar_cc__ar_tid_Codebook_CN__DSP__HW), i_short_int_receiver (&ar_cc__T0__DSP__HW), i_short_int__10__sender (&ar_cc__cb_ana__HW__DSP), i_short_int__40__receiver (&ar_cc__code__DSP__HW), i_short_int__40__sender (&ar_cc__code__HW__DSP), i_short_int__40__receiver (&ar_cc__exc_i__DSP__HW), i_short_int_sender (&ar_cc__gain_code__HW__DSP), i_short_int_receiver (&ar_cc__gain_pit__DSP__HW), i_short_int__40__receiver (&ar_cc__h1__DSP__HW), i_short_int__40__receiver (&ar_cc__res2__DSP__HW), i_short_int__40__receiver (&ar_cc__xn__DSP__HW), i_short_int__40__receiver (&ar_cc__y1__DSP__HW), i_short_int__40__sender (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), ar_cc__ar_tid_Coder__DSP__HW(ar_cc__ar_tid_Coder__DSP__HW), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    _app(_IDcnt, ar_cc__ar_tid_Coder__DSP__HW, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, T0, cb_ana, code, exc_i, gain_code, gain_pit, h1, res2, xn, y1, y2, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP)
{   
}

HW_Standard_HW::~HW_Standard_HW(void)
{   
}

void HW_Standard_HW::main(void)
{   
    __PRF_globalCounter[436u]++ ;
    while(1)
    {   
	__PRF_globalCounter[437u]++ ;
	_app.main();
    }
}

HW_Standard_HW_App::HW_Standard_HW_App(unsigned int _idcnt, i_int_receiver (&ar_cc__ar_tid_Coder__DSP__HW), i_int_receiver (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_receiver (&ar_cc__ar_tid_Codebook_CN__DSP__HW), short int (&T0), short int (&cb_ana)[10], short int (&code)[40], short int (&exc_i)[40], short int (&gain_code), short int (&gain_pit), short int (&h1)[40], short int (&res2)[40], short int (&xn)[40], short int (&y1)[40], short int (&y2)[40], i_short_int_receiver (&ar_cc__T0__DSP__HW), i_short_int__10__sender (&ar_cc__cb_ana__HW__DSP), i_short_int__40__receiver (&ar_cc__code__DSP__HW), i_short_int__40__sender (&ar_cc__code__HW__DSP), i_short_int__40__receiver (&ar_cc__exc_i__DSP__HW), i_short_int_sender (&ar_cc__gain_code__HW__DSP), i_short_int_receiver (&ar_cc__gain_pit__DSP__HW), i_short_int__40__receiver (&ar_cc__h1__DSP__HW), i_short_int__40__receiver (&ar_cc__res2__DSP__HW), i_short_int__40__receiver (&ar_cc__xn__DSP__HW), i_short_int__40__receiver (&ar_cc__y1__DSP__HW), i_short_int__40__sender (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), ar_cc__ar_tid_Coder__DSP__HW(ar_cc__ar_tid_Coder__DSP__HW), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), T0(T0), cb_ana(cb_ana), code(code), exc_i(exc_i), gain_code(gain_code), gain_pit(gain_pit), h1(h1), res2(res2), xn(xn), y1(y1), y2(y2), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    ar_cb__ar_tid__DSP__HW(_IDcnt, ar_cc__ar_tid_Coder__DSP__HW, ar_tid),
    subframes(_IDcnt, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, T0, cb_ana, code, exc_i, gain_code, gain_pit, h1, res2, xn, y1, y2, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP),
    wr_post_process(_IDcnt),
    wr_pre_process(_IDcnt)
{   
}

HW_Standard_HW_App::~HW_Standard_HW_App(void)
{   
}

void HW_Standard_HW_App::main(void)
{   
    __PRF_globalCounter[438u]++ ;
    { enum { _scc_state_0, _scc_state_ar_cb__ar_tid__DSP__HW, _scc_state_wr_post_process, _scc_state_wr_pre_process, _scc_state_subframes } _scc_next_state = _scc_state_ar_cb__ar_tid__DSP__HW; do switch(_scc_next_state) {
	    case _scc_state_ar_cb__ar_tid__DSP__HW: { ar_cb__ar_tid__DSP__HW.main(); if (__PRF_globalCounter[439u]++  , 
		    ar_tid == 110) { _scc_next_state = _scc_state_wr_post_process; break; }
		if (ar_tid == 115) { _scc_next_state = _scc_state_wr_pre_process; break; }
		if (ar_tid == 1) { _scc_next_state = _scc_state_subframes; break; }
		{ _scc_next_state = _scc_state_0; break; } }
	    case _scc_state_wr_post_process: { wr_post_process.main(); if (__PRF_globalCounter[440u]++  , 
		    1u) { _scc_next_state = _scc_state_ar_cb__ar_tid__DSP__HW; break; } }
	    case _scc_state_wr_pre_process: { wr_pre_process.main(); if (__PRF_globalCounter[441u]++  , 
		    1u) { _scc_next_state = _scc_state_ar_cb__ar_tid__DSP__HW; break; } }
	    case _scc_state_subframes: { subframes.main(); if (__PRF_globalCounter[442u]++  , 
		    1u) { _scc_next_state = _scc_state_ar_cb__ar_tid__DSP__HW; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

LP_Analysis::LP_Analysis(unsigned int _idcnt, short int *(&p_window), short int (&A_t)[4][11], short int (&Aq_t)[4][11], short int (&ana)[57], bool (&ptch), short int (&txdtx_ctrl), bool (&dtx_mode), int (&L_pn_seed_tx), bool (&reset_flag))
    : _specc::behavior(_idcnt), p_window(p_window), A_t(A_t), Aq_t(Aq_t), ana(ana), ptch(ptch), txdtx_ctrl(txdtx_ctrl), dtx_mode(dtx_mode), L_pn_seed_tx(L_pn_seed_tx), reset_flag(reset_flag),
    az_lsp_1(_IDcnt, A_t_1, lsp_mid, lsp_old),
    az_lsp_2(_IDcnt, A_t_3, lsp_new, lsp_mid),
    copy1(_IDcnt, A_t_1, A_t_3, A),
    copy2(_IDcnt, A, A_t),
    init(_IDcnt, reset_flag, old_A, lsp_old, lsp_old_q),
    no_speech_upd(_IDcnt, A, Aq_t, lsp_old, lsp_new, lsp_old_q, lsp_new_q, txdtx_ctrl),
    seq1(_IDcnt, p_window, r_h, r_l, old_A, scal_fac, A_t_1, A_t_3, rc),
    seq2(_IDcnt, lsp_old, lsp_mid, lsp_new, lsp_old_q, lsp_mid_q, lsp_new_q, A, Aq_t, ana, txdtx_ctrl, reset_flag),
    vad_lp(_IDcnt, r_l, r_h, scal_fac, rc, ptch, txdtx_ctrl, dtx_mode, L_pn_seed_tx, reset_flag)
{   
}

LP_Analysis::~LP_Analysis(void)
{   
}

void LP_Analysis::main(void)
{   
    __PRF_globalCounter[469u]++ ;
    init.main();
    seq1.main();
    az_lsp_1.main();
    az_lsp_2.main();
    copy1.main();
    vad_lp.main();
    seq2.main();
    no_speech_upd.main();
    copy2.main();
}

void LP_Analysis::reset(void)
{   
    __PRF_globalCounter[470u]++ ;
    init.reset();
    seq2.reset();
    vad_lp.reset();
}

Lag_Window_0::Lag_Window_0(unsigned int _idcnt, short int (&r_h)[11], short int (&r_l)[11])
    : _specc::behavior(_idcnt), r_h(r_h), r_l(r_l)
{   
}

Lag_Window_0::~Lag_Window_0(void)
{   
}

void Lag_Window_0::main(void)
{   
    int i;
    const short int lag_h[10] = 
    { ((short int)32728),((short int)32619),((short int)32438),((short int)32187),
      ((short int)31867),((short int)31480),((short int)31029),((short int)30517),
      ((short int)29946),((short int)29321) };
    const short int lag_l[10] = 
    { ((short int)11904),((short int)17280),((short int)30720),((short int)25856),
      ((short int)24192),((short int)28992),((short int)24384),((short int)7360),
      ((short int)19520),((short int)14784) };
    int x;

    __PRF_globalCounter[487u]++ ;
    _specc::waitfor(866666ull);
    __PRF_globalCounter[488u]++ ;
    for(i = 1; i <= 10; i++ )
    {   
	__PRF_globalCounter[489u]++ ;
	x = Mpy_32(r_h[i], r_l[i], lag_h[i - 1], lag_l[i - 1]);
	L_Extract(x,  &r_h[i],  &r_l[i]);
    }
}

Levinson_0::Levinson_0(unsigned int _idcnt, short int (&Rh)[11], short int (&Rl)[11], short int (&old_A)[11], short int (&A)[11], short int (&rc)[4])
    : _specc::behavior(_idcnt), Rh(Rh), Rl(Rl), old_A(old_A), A(A), rc(rc)
{   
}

Levinson_0::~Levinson_0(void)
{   
}

void Levinson_0::main(void)
{   
    short int Ah[11];
    short int Al[11];
    short int Anh[11];
    short int Anl[11];
    short int Kh;
    short int Kl;
    short int alp_exp;
    short int alp_h;
    short int alp_l;
    short int hi;
    int i;
    int j;
    short int lo;
    int t0;
    short int t0_n;
    int t1;
    int t2;

    __PRF_globalCounter[518u]++ ;
    _specc::waitfor(17333333ull);
    t1 = L_Comp(Rh[1], Rl[1]);
    t2 = L_abs(t1);
    t0 = Div_32(t2, Rh[0], Rl[0]);
    __PRF_globalCounter[519u]++ ;
    if (t1 > 0)
    {   
	__PRF_globalCounter[520u]++ ;
	t0 = L_negate(t0);
    }
    __PRF_globalCounter[521u]++ ;
    L_Extract(t0,  &Kh,  &Kl);
    rc[0] = round(t0);
    t0 = L_shr(t0, 4);
    L_Extract(t0,  &Ah[1],  &Al[1]);
    t0 = Mpy_32(Kh, Kl, Kh, Kl);
    t0 = L_abs(t0);
    t0 = L_sub((int)2147483647l, t0);
    L_Extract(t0,  &hi,  &lo);
    t0 = Mpy_32(Rh[0], Rl[0], hi, lo);
    alp_exp = norm_l(t0);
    t0 = L_shl(t0, alp_exp);
    L_Extract(t0,  &alp_h,  &alp_l);
    __PRF_globalCounter[522u]++ ;
    for(i = 2; i <= 10; i++ )
    {   
	__PRF_globalCounter[523u]++ ;
	t0 = 0;
	__PRF_globalCounter[524u]++ ;
	for(j = 1; j < i; j++ )
	{   
	    __PRF_globalCounter[525u]++ ;
	    t0 = L_add(t0, Mpy_32(Rh[j], Rl[j], Ah[i - j], Al[i - j]));
	}
	__PRF_globalCounter[526u]++ ;
	t0 = L_shl(t0, 4);
	t1 = L_Comp(Rh[i], Rl[i]);
	t0 = L_add(t0, t1);
	t1 = L_abs(t0);
	t2 = Div_32(t1, alp_h, alp_l);
	__PRF_globalCounter[527u]++ ;
	if (t0 > 0)
	{   
	    __PRF_globalCounter[528u]++ ;
	    t2 = L_negate(t2);
	}
	__PRF_globalCounter[529u]++ ;
	t2 = L_shl(t2, alp_exp);
	L_Extract(t2,  &Kh,  &Kl);
	__PRF_globalCounter[530u]++ ;
	if (sub(i, 5) < 0)
	{   
	    __PRF_globalCounter[531u]++ ;
	    rc[i - 1] = round(t2);
	}
	__PRF_globalCounter[532u]++ ;
	if (sub(abs_s(Kh), 32750) > 0)
	{   
	    __PRF_globalCounter[533u]++ ;
	    for(j = 0; j <= 10; j++ )
	    {   
		__PRF_globalCounter[534u]++ ;
		A[j] = old_A[j];
	    }
	    __PRF_globalCounter[535u]++ ;
	    for(j = 0; j < 4; j++ )
	    {   
		__PRF_globalCounter[536u]++ ;
		rc[j] = 0;
	    }
	    __PRF_globalCounter[537u]++ ;
	    return ;
	}
	__PRF_globalCounter[538u]++ ;
	for(j = 1; j < i; j++ )
	{   
	    __PRF_globalCounter[539u]++ ;
	    t0 = Mpy_32(Kh, Kl, Ah[i - j], Al[i - j]);
	    t0 = L_mac(t0, Ah[j], 32767);
	    t0 = L_mac(t0, Al[j], 1);
	    L_Extract(t0,  &Anh[j],  &Anl[j]);
	}
	__PRF_globalCounter[540u]++ ;
	t2 = L_shr(t2, 4);
	L_Extract(t2,  &Anh[i],  &Anl[i]);
	t0 = Mpy_32(Kh, Kl, Kh, Kl);
	t0 = L_abs(t0);
	t0 = L_sub((int)2147483647l, t0);
	L_Extract(t0,  &hi,  &lo);
	t0 = Mpy_32(alp_h, alp_l, hi, lo);
	t0_n = norm_l(t0);
	t0 = L_shl(t0, t0_n);
	L_Extract(t0,  &alp_h,  &alp_l);
	alp_exp = add(alp_exp, t0_n);
	__PRF_globalCounter[541u]++ ;
	for(j = 1; j <= i; j++ )
	{   
	    __PRF_globalCounter[542u]++ ;
	    Ah[j] = Anh[j];
	    Al[j] = Anl[j];
	}
    }
    __PRF_globalCounter[543u]++ ;
    A[0] = 4096;
    __PRF_globalCounter[544u]++ ;
    for(i = 1; i <= 10; i++ )
    {   
	__PRF_globalCounter[545u]++ ;
	t0 = L_Comp(Ah[i], Al[i]);
	A[i] = old_A[i] = round(L_shl(t0, 1));
    }
}

Motorola_DSP56600_0_DSP::Motorola_DSP56600_0_DSP(unsigned int _idcnt, i_receiver (&speech_samples), i_receiver (&dtx_mode), i_sender (&serial), i_sender (&txdtx_ctrl), i_int_sender (&ar_cc__ar_tid_Coder__DSP__HW), i_int_sender (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_sender (&ar_cc__ar_tid_Codebook_CN__DSP__HW), i_short_int_sender (&ar_cc__T0__DSP__HW), i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), i_short_int__40__sender (&ar_cc__code__DSP__HW), i_short_int__40__receiver (&ar_cc__code__HW__DSP), i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), i_short_int_receiver (&ar_cc__gain_code__HW__DSP), i_short_int_sender (&ar_cc__gain_pit__DSP__HW), i_short_int__40__sender (&ar_cc__h1__DSP__HW), i_short_int__40__sender (&ar_cc__res2__DSP__HW), i_short_int__40__sender (&ar_cc__xn__DSP__HW), i_short_int__40__sender (&ar_cc__y1__DSP__HW), i_short_int__40__receiver (&ar_cc__y2__HW__DSP), OSAPI (&os))
    : _specc::behavior(_idcnt), speech_samples(speech_samples), dtx_mode(dtx_mode), serial(serial), txdtx_ctrl(txdtx_ctrl), ar_cc__ar_tid_Coder__DSP__HW(ar_cc__ar_tid_Coder__DSP__HW), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP), os(os),
    wr_coder_12k2(_IDcnt, speech_frame, prm, syn, local_dtx_mode, txdtx_ctrl_val, reset_flag_1, ar_cc__ar_tid_Coder__DSP__HW, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, T0, cb_ana, code, exc_i, gain_code, gain_pit, h1, res2, xn, y1, y2, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP),
    wr_post_process(_IDcnt, prm, txdtx_ctrl_val, reset_flag_2, serial, txdtx_ctrl, ar_cc__ar_tid_Coder__DSP__HW),
    wr_pre_process(_IDcnt, speech_samples, dtx_mode, speech_frame, reset_flag_1, reset_flag_2, local_dtx_mode, ar_cc__ar_tid_Coder__DSP__HW)
{   
}

Motorola_DSP56600_0_DSP::~Motorola_DSP56600_0_DSP(void)
{   
}

void Motorola_DSP56600_0_DSP::main(void)
{   
    __PRF_globalCounter[552u]++ ;
    { enum { _scc_state_0, _scc_state_wr_pre_process, _scc_state_wr_coder_12k2, _scc_state_wr_post_process } _scc_next_state = _scc_state_wr_pre_process; do switch(_scc_next_state) {
	    case _scc_state_wr_pre_process: { wr_pre_process.main(); if (__PRF_globalCounter[553u]++  , 
		    1u) { _scc_next_state = _scc_state_wr_coder_12k2; break; } }
	    case _scc_state_wr_coder_12k2: { wr_coder_12k2.main(); if (__PRF_globalCounter[554u]++  , 
		    1u) { _scc_next_state = _scc_state_wr_post_process; break; } }
	    case _scc_state_wr_post_process: { wr_post_process.main(); if (__PRF_globalCounter[555u]++  , 
		    1u) { _scc_next_state = _scc_state_wr_pre_process; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

void Motorola_DSP56600_0_DSP::reset(void)
{   
    __PRF_globalCounter[556u]++ ;
    wr_pre_process.reset();
    wr_coder_12k2.reset();
    wr_post_process.reset();
}

Motorola_DSP56600_IntTask_0_DSP::Motorola_DSP56600_IntTask_0_DSP(unsigned int _idcnt, const char *(&name), OSAPI (&os))
    : _specc::behavior(_idcnt), name(name), os(os)
{   
}

Motorola_DSP56600_IntTask_0_DSP::~Motorola_DSP56600_IntTask_0_DSP(void)
{   
}

void Motorola_DSP56600_IntTask_0_DSP::body(void)
{   
    __PRF_globalCounter[557u]++ ;
}

void Motorola_DSP56600_IntTask_0_DSP::main(void)
{   
    __PRF_globalCounter[558u]++ ;
}

void Motorola_DSP56600_IntTask_0_DSP::start(void)
{   
    __PRF_globalCounter[559u]++ ;
    body();
}

Motorola_DSP56600_OSNONE_0_DSP::Motorola_DSP56600_OSNONE_0_DSP(unsigned int _idcnt, i_receiver (&speech_samples), i_receiver (&dtx_mode), i_sender (&serial), i_sender (&txdtx_ctrl), i_int_sender (&ar_cc__ar_tid_Coder__DSP__HW), i_int_sender (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_sender (&ar_cc__ar_tid_Codebook_CN__DSP__HW), i_short_int_sender (&ar_cc__T0__DSP__HW), i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), i_short_int__40__sender (&ar_cc__code__DSP__HW), i_short_int__40__receiver (&ar_cc__code__HW__DSP), i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), i_short_int_receiver (&ar_cc__gain_code__HW__DSP), i_short_int_sender (&ar_cc__gain_pit__DSP__HW), i_short_int__40__sender (&ar_cc__h1__DSP__HW), i_short_int__40__sender (&ar_cc__res2__DSP__HW), i_short_int__40__sender (&ar_cc__xn__DSP__HW), i_short_int__40__sender (&ar_cc__y1__DSP__HW), i_short_int__40__receiver (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), speech_samples(speech_samples), dtx_mode(dtx_mode), serial(serial), txdtx_ctrl(txdtx_ctrl), ar_cc__ar_tid_Coder__DSP__HW(ar_cc__ar_tid_Coder__DSP__HW), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    DSP(++_IDcnt, speech_samples, dtx_mode, serial, txdtx_ctrl, ar_cc__ar_tid_Coder__DSP__HW, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP, os),
    os()
{   
}

Motorola_DSP56600_OSNONE_0_DSP::~Motorola_DSP56600_OSNONE_0_DSP(void)
{   
}

void Motorola_DSP56600_OSNONE_0_DSP::intHandler(void)
{   
    __PRF_globalCounter[560u]++ ;
    os.ienter();
    body:{ 
	__PRF_globalCounter[561u]++ ;
	;
    }
    __PRF_globalCounter[562u]++ ;
    os.ireturn();
}

void Motorola_DSP56600_OSNONE_0_DSP::main(void)
{   
    { _specc::fork _scc_fork_0(&DSP); _specc::par(&_scc_fork_0, ((_specc::fork*)0));
    }
}

void Motorola_DSP56600_OSNONE_0_DSP::reset(void)
{   
    __PRF_globalCounter[564u]++ ;
    DSP.reset();
}

Motorola_DSP56600_OS_FCFS_0::Motorola_DSP56600_OS_FCFS_0(unsigned int _idcnt)
    : _specc::behavior(_idcnt)
{   
}

Motorola_DSP56600_OS_FCFS_0::~Motorola_DSP56600_OS_FCFS_0(void)
{   
}

void Motorola_DSP56600_OS_FCFS_0::intHandler(void)

#line 84 "/ECEnet/Apps1/sce/latest/share/sce/db/processors/dsp/dsp56600os.sc"
{   
    body:{ 

#line 13578 "vocoderSched.cc"
	__PRF_globalCounter[565u]++ ;

#line 85 "/ECEnet/Apps1/sce/latest/share/sce/db/processors/dsp/dsp56600os.sc"
	;
    }
}

#line 13586 "vocoderSched.cc"
void Motorola_DSP56600_OS_FCFS_0::main(void)

#line 89 "/ECEnet/Apps1/sce/latest/share/sce/db/processors/dsp/dsp56600os.sc"
{   
    { _specc::par(((_specc::fork*)0));
    }
}

#line 13595 "vocoderSched.cc"
Motorola_DSP56600_OS_PRIORITY_0::Motorola_DSP56600_OS_PRIORITY_0(unsigned int _idcnt)
    : _specc::behavior(_idcnt)
{   
}

Motorola_DSP56600_OS_PRIORITY_0::~Motorola_DSP56600_OS_PRIORITY_0(void)
{   
}

void Motorola_DSP56600_OS_PRIORITY_0::intHandler(void)

#line 108 "/ECEnet/Apps1/sce/latest/share/sce/db/processors/dsp/dsp56600os.sc"
{   
    body:{ 

#line 13611 "vocoderSched.cc"
	__PRF_globalCounter[567u]++ ;

#line 109 "/ECEnet/Apps1/sce/latest/share/sce/db/processors/dsp/dsp56600os.sc"
	;
    }
}

#line 13619 "vocoderSched.cc"
void Motorola_DSP56600_OS_PRIORITY_0::main(void)

#line 113 "/ECEnet/Apps1/sce/latest/share/sce/db/processors/dsp/dsp56600os.sc"
{   
    { _specc::par(((_specc::fork*)0));
    }
}

#line 13628 "vocoderSched.cc"
Nop_0::Nop_0(unsigned int _idcnt)
    : _specc::behavior(_idcnt)
{   
}

Nop_0::~Nop_0(void)
{   
}

void Nop_0::main(void)
{   
    __PRF_globalCounter[580u]++ ;
    _specc::waitfor(0ull);
}

Open_Loop::Open_Loop(unsigned int _idcnt, short int *(&p_speech), short int (&A_t)[4][11], short int *(&wsp), short int (&T0_min_1), short int (&T0_max_1), short int (&T0_min_2), short int (&T0_max_2), bool (&ptch), short int (&txdtx_ctrl), bool (&dtx_mode), bool (&reset_flag))
    : _specc::behavior(_idcnt), p_speech(p_speech), A_t(A_t), wsp(wsp), T0_min_1(T0_min_1), T0_max_1(T0_max_1), T0_min_2(T0_min_2), T0_max_2(T0_max_2), ptch(ptch), txdtx_ctrl(txdtx_ctrl), dtx_mode(dtx_mode), reset_flag(reset_flag),
    _scc_const_port_0(true),
    for_body1(_IDcnt, A_t, p_speech, i, A_t_i, p_speech_i),
    for_body2(_IDcnt, A_t_i, ap1, ap2),
    for_end(_IDcnt, i, wsp_i, wsp),
    for_init(_IDcnt, i, reset_flag, mem_w),
    ol_lag_estimate(_IDcnt, wsp, T0_min_1, T0_max_1, T0_min_2, T0_max_2, ptch, txdtx_ctrl, dtx_mode, reset_flag),
    residual(_IDcnt, ap1, p_speech_i, wsp_i),
    syn_filter(_IDcnt, ap2, wsp_i, wsp_i, mem_w, _scc_const_port_0)
{   
}

Open_Loop::~Open_Loop(void)
{   
}

void Open_Loop::main(void)
{   
    __PRF_globalCounter[594u]++ ;
    { enum { _scc_state_0, _scc_state_for_init, _scc_state_for_body1, _scc_state_for_body2, _scc_state_residual, _scc_state_syn_filter, _scc_state_for_end, _scc_state_ol_lag_estimate } _scc_next_state = _scc_state_for_init; do switch(_scc_next_state) {
	    case _scc_state_for_init: { for_init.main(); if (__PRF_globalCounter[595u]++  , 
		    i <= 3) { _scc_next_state = _scc_state_for_body1; break; }
		{ _scc_next_state = _scc_state_ol_lag_estimate; break; } }
	    case _scc_state_for_body1: { for_body1.main(); if (__PRF_globalCounter[596u]++  , 
		    1u) { _scc_next_state = _scc_state_for_body2; break; } }
	    case _scc_state_for_body2: { for_body2.main(); if (__PRF_globalCounter[597u]++  , 
		    1u) { _scc_next_state = _scc_state_residual; break; } }
	    case _scc_state_residual: { residual.main(); if (__PRF_globalCounter[598u]++  , 
		    1u) { _scc_next_state = _scc_state_syn_filter; break; } }
	    case _scc_state_syn_filter: { syn_filter.main(); if (__PRF_globalCounter[599u]++  , 
		    1u) { _scc_next_state = _scc_state_for_end; break; } }
	    case _scc_state_for_end: { for_end.main(); if (__PRF_globalCounter[600u]++  , 
		    i <= 3) { _scc_next_state = _scc_state_for_body1; break; }
		{ _scc_next_state = _scc_state_ol_lag_estimate; break; } }
	    case _scc_state_ol_lag_estimate: { ol_lag_estimate.main(); if (__PRF_globalCounter[601u]++  , 
		    1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

void Open_Loop::reset(void)
{   
    __PRF_globalCounter[602u]++ ;
    for_init.reset();
    ol_lag_estimate.reset();
}

Pitch_Ol_0::Pitch_Ol_0(unsigned int _idcnt, short int *(&sig), short int (&p_max1))
    : _specc::behavior(_idcnt), sig(sig), p_max1(p_max1)
{   
}

Pitch_Ol_0::~Pitch_Ol_0(void)
{   
}

short int Pitch_Ol_0::Lag_max(short int scal_sig[], short int scal_fac, 
    short int L_frame, short int lag_max, short int lag_min, short int *cor_max)
{   
    short int ener_h;
    short int ener_l;
    int i;
    int j;
    int max;
    short int max_h;
    short int max_l;
    short int *p;
    short int *p1;
    short int p_max;
    int t0;

    __PRF_globalCounter[709u]++ ;
    max = (int)2147483648ul;
    __PRF_globalCounter[710u]++ ;
    for(i = lag_max; i >= lag_min; i-- )
    {   
	__PRF_globalCounter[711u]++ ;
	p = scal_sig;
	p1 =  &scal_sig[ -i];
	t0 = 0;
	__PRF_globalCounter[712u]++ ;
	for(j = 0; j < L_frame; j++  , p++  , p1++ )
	{   
	    __PRF_globalCounter[713u]++ ;
	    t0 = L_mac(t0,  *p,  *p1);
	}
	__PRF_globalCounter[714u]++ ;
	if (L_sub(t0, max) >= 0)
	{   
	    __PRF_globalCounter[715u]++ ;
	    max = t0;
	    p_max = i;
	}
    }
    __PRF_globalCounter[716u]++ ;
    t0 = 0;
    p =  &scal_sig[ -p_max];
    __PRF_globalCounter[717u]++ ;
    for(i = 0; i < L_frame; i++  , p++ )
    {   
	__PRF_globalCounter[718u]++ ;
	t0 = L_mac(t0,  *p,  *p);
    }
    __PRF_globalCounter[719u]++ ;
    t0 = Inv_sqrt(t0);
    t0 = L_shl(t0, 1);
    L_Extract(max,  &max_h,  &max_l);
    L_Extract(t0,  &ener_h,  &ener_l);
    t0 = Mpy_32(max_h, max_l, ener_h, ener_l);
    t0 = L_shr(t0, scal_fac);
     *cor_max = extract_h(L_shl(t0, 15));
    return (p_max);
}

void Pitch_Ol_0::main(void)
{   
    int i;
    int j;
    short int max1;
    short int max2;
    short int max3;
    short int p_max2;
    short int p_max3;
    short int scal_fac;
    short int *scal_sig;
    short int scaled_signal[512];
    int t0;

    __PRF_globalCounter[720u]++ ;
    _specc::waitfor(552633333ull);
    scal_sig =  &scaled_signal[143];
    t0 = 0l;
    __PRF_globalCounter[721u]++ ;
    for(i =  -143; i < 80; i++ )
    {   
	__PRF_globalCounter[722u]++ ;
	t0 = L_mac(t0, sig[i], sig[i]);
    }
    __PRF_globalCounter[723u]++ ;
    if (L_sub(t0, (int)2147483647l) == 0l)
    {   
	__PRF_globalCounter[724u]++ ;
	for(i =  -143; i < 80; i++ )
	{   
	    __PRF_globalCounter[725u]++ ;
	    scal_sig[i] = shr(sig[i], 3);
	}
	__PRF_globalCounter[726u]++ ;
	scal_fac = 3;
    }
    else 
    {   
	__PRF_globalCounter[727u]++ ;
	if (L_sub(t0, (int)1048576l) < (int)0)
	{   
	    __PRF_globalCounter[728u]++ ;
	    for(i =  -143; i < 80; i++ )
	    {   
		__PRF_globalCounter[729u]++ ;
		scal_sig[i] = shl(sig[i], 3);
	    }
	    __PRF_globalCounter[730u]++ ;
	    scal_fac =  -3;
	}
	else 
	{   
	    __PRF_globalCounter[731u]++ ;
	    for(i =  -143; i < 80; i++ )
	    {   
		__PRF_globalCounter[732u]++ ;
		scal_sig[i] = sig[i];
	    }
	    __PRF_globalCounter[733u]++ ;
	    scal_fac = 0;
	}
    }
    __PRF_globalCounter[734u]++ ;
    j = shl(18, 2);
    p_max1 = Lag_max(scal_sig, scal_fac, 80, 143, j,  &max1);
    i = sub(j, 1);
    j = shl(18, 1);
    p_max2 = Lag_max(scal_sig, scal_fac, 80, i, j,  &max2);
    i = sub(j, 1);
    p_max3 = Lag_max(scal_sig, scal_fac, 80, i, 18,  &max3);
    __PRF_globalCounter[735u]++ ;
    if (sub(mult(max1, 27853), max2) < 0)
    {   
	__PRF_globalCounter[736u]++ ;
	max1 = max2;
	p_max1 = p_max2;
    }
    __PRF_globalCounter[737u]++ ;
    if (sub(mult(max1, 27853), max3) < 0)
    {   
	__PRF_globalCounter[738u]++ ;
	p_max1 = p_max3;
    }
}

Post_Process::Post_Process(unsigned int _idcnt, short int (&prm)[57], short int (&txdtx_ctrl), bool (&reset_flag), i_sender (&serial), i_sender (&txdtx_ctrl_out))
    : _specc::behavior(_idcnt), prm(prm), txdtx_ctrl(txdtx_ctrl), reset_flag(reset_flag), serial(serial), txdtx_ctrl_out(txdtx_ctrl_out),
    cn_encoder(_IDcnt, prm, params, txdtx_ctrl, reset_flag),
    prm2bits_12k2(_IDcnt, params, ser),
    seq1(_IDcnt, txdtx_ctrl, local_serial, txdtx_ctrl_out, serial),
    sid_codeword_encoder(_IDcnt, ser, txdtx_ctrl, local_serial)
{   
}

Post_Process::~Post_Process(void)
{   
}

void Post_Process::main(void)
{   
    __PRF_globalCounter[739u]++ ;
    cn_encoder.main();
    prm2bits_12k2.main();
    sid_codeword_encoder.main();
    seq1.main();
}

void Post_Process::reset(void)
{   
    __PRF_globalCounter[740u]++ ;
    cn_encoder.reset();
}

Pre_Process::Pre_Process(unsigned int _idcnt, i_receiver (&speech_frame), i_receiver (&dtx_mode), short int (&speech_frame_filtered)[160], bool (&reset_flag_1), bool (&reset_flag_2), bool (&local_dtx_mode))
    : _specc::behavior(_idcnt), speech_frame(speech_frame), dtx_mode(dtx_mode), speech_frame_filtered(speech_frame_filtered), reset_flag_1(reset_flag_1), reset_flag_2(reset_flag_2), local_dtx_mode(local_dtx_mode),
    reset_next(false),
    encoder_homingframe_test(_IDcnt, local_speech_frame, reset_next),
    filter_and_scale(_IDcnt, reset_flag, local_speech_frame, speech_frame_filtered),
    seq1(_IDcnt, reset_next, reset_flag, reset_flag_1, reset_flag_2, speech_frame, dtx_mode, local_speech_frame, local_dtx_mode)
{   
}

Pre_Process::~Pre_Process(void)
{   
}

void Pre_Process::main(void)
{   
    __PRF_globalCounter[742u]++ ;
    seq1.main();
    encoder_homingframe_test.main();
    filter_and_scale.main();
}

void Pre_Process::reset(void)
{   
    __PRF_globalCounter[743u]++ ;
    filter_and_scale.reset();
}

Residu_0::Residu_0(unsigned int _idcnt, short int (&a)[11], short int *(&x), short int (&y)[40])
    : _specc::behavior(_idcnt), a(a), x(x), y(y)
{   
}

Residu_0::~Residu_0(void)
{   
}

void Residu_0::main(void)
{   
    int i;
    int j;
    int s;

    __PRF_globalCounter[951u]++ ;
    _specc::waitfor(31366666ull);
    __PRF_globalCounter[952u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[953u]++ ;
	s = L_mult(x[i], a[0]);
	__PRF_globalCounter[954u]++ ;
	for(j = 1; j <= 10; j++ )
	{   
	    __PRF_globalCounter[955u]++ ;
	    s = L_mac(s, a[j], x[i - j]);
	}
	__PRF_globalCounter[956u]++ ;
	s = L_shl(s, 3);
	y[i] = round(s);
    }
}

Residu_1::Residu_1(unsigned int _idcnt, short int (&a)[11], short int *(&x), short int (&y)[40])
    : _specc::behavior(_idcnt), a(a), x(x), y(y)
{   
}

Residu_1::~Residu_1(void)
{   
}

void Residu_1::main(void)
{   
    int i;
    int j;
    int s;

    __PRF_globalCounter[957u]++ ;
    _specc::waitfor(31366666ull);
    __PRF_globalCounter[958u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[959u]++ ;
	s = L_mult(x[i], a[0]);
	__PRF_globalCounter[960u]++ ;
	for(j = 1; j <= 10; j++ )
	{   
	    __PRF_globalCounter[961u]++ ;
	    s = L_mac(s, a[j], x[i - j]);
	}
	__PRF_globalCounter[962u]++ ;
	s = L_shl(s, 3);
	y[i] = round(s);
    }
}

Shift_Signals::Shift_Signals(unsigned int _idcnt, short int (&old_speech)[320], short int (&old_wsp)[303], short int (&old_exc)[314], short int (&txdtx_ctrl_cur), short int (&txdtx_ctrl))
    : _specc::behavior(_idcnt), old_speech(old_speech), old_wsp(old_wsp), old_exc(old_exc), txdtx_ctrl_cur(txdtx_ctrl_cur), txdtx_ctrl(txdtx_ctrl)
{   
}

Shift_Signals::~Shift_Signals(void)
{   
}

void Shift_Signals::main(void)
{   
    __PRF_globalCounter[1048u]++ ;
    _specc::waitfor(66666ull);
    Copy( &old_speech[160],  &old_speech[0], 320 - 160);
    Copy( &old_wsp[160],  &old_wsp[0], 143);
    Copy( &old_exc[160],  &old_exc[0], 143 + (10 + 1));
    txdtx_ctrl = txdtx_ctrl_cur;
}

Subframes::Subframes(unsigned int _idcnt, short int (&A_t)[4][11], short int (&Aq_t)[4][11], short int *(&p_speech), short int *(&p_exc), short int (&T0_min_1), short int (&T0_max_1), short int (&T0_min_2), short int (&T0_max_2), short int (&ana)[57], short int (&synth)[160], short int (&txdtx_ctrl), int (&L_pn_seed_tx), bool (&reset_flag), i_int_sender (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_sender (&ar_cc__ar_tid_Codebook_CN__DSP__HW), short int (&T0), short int (&cb_ana)[10], short int (&code)[40], short int (&exc_i)[40], short int (&gain_code), short int (&gain_pit), short int (&h1)[40], short int (&res2)[40], short int (&xn)[40], short int (&y1)[40], short int (&y2)[40], i_short_int_sender (&ar_cc__T0__DSP__HW), i_short_int__10__receiver (&ar_cc__cb_ana__HW__DSP), i_short_int__40__sender (&ar_cc__code__DSP__HW), i_short_int__40__receiver (&ar_cc__code__HW__DSP), i_short_int__40__sender (&ar_cc__exc_i__DSP__HW), i_short_int_receiver (&ar_cc__gain_code__HW__DSP), i_short_int_sender (&ar_cc__gain_pit__DSP__HW), i_short_int__40__sender (&ar_cc__h1__DSP__HW), i_short_int__40__sender (&ar_cc__res2__DSP__HW), i_short_int__40__sender (&ar_cc__xn__DSP__HW), i_short_int__40__sender (&ar_cc__y1__DSP__HW), i_short_int__40__receiver (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), A_t(A_t), Aq_t(Aq_t), p_speech(p_speech), p_exc(p_exc), T0_min_1(T0_min_1), T0_max_1(T0_max_1), T0_min_2(T0_min_2), T0_max_2(T0_max_2), ana(ana), synth(synth), txdtx_ctrl(txdtx_ctrl), L_pn_seed_tx(L_pn_seed_tx), reset_flag(reset_flag), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), T0(T0), cb_ana(cb_ana), code(code), exc_i(exc_i), gain_code(gain_code), gain_pit(gain_pit), h1(h1), res2(res2), xn(xn), y1(y1), y2(y2), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    _scc_const_port_0(-1),
    ar_cb__ar_stop__DSP__HW(_IDcnt, ar_cc__ar_tid_Subframes__DSP__HW, _scc_const_port_0),
    closed_loop(_IDcnt, i_subfr, A_t_i, Aq_t_i, h1, p_speech, res2, p_exc, xn, mem_err, mem_w0, T0_min, T0_max, T0_min_2, T0_max_2, T0, cl_ana, y1, gain_pit, txdtx_ctrl, CN_excitation_gain, reset_flag_1),
    for_body1(_IDcnt, i, A_t, Aq_t, A_t_i, Aq_t_i),
    for_body2(_IDcnt, i_subfr, p_exc, p_speech, exc_i, speech_i),
    for_end(_IDcnt, cl_ana, cb_ana, up_ana, exc_i, synth_i, p_ana, i, i_subfr, p_exc, synth, reset_flag_1),
    for_init(_IDcnt, T0_min_1, T0_max_1, ana, T0_min, T0_max, p_ana, i, i_subfr, mem_w0, mem_err, reset_flag, reset_flag_1),
    update(_IDcnt, i_subfr, Aq_t_i, gain_pit, gain_code, exc_i, speech_i, synth_i, up_ana, xn, y1, y2, code, mem_err, mem_w0, txdtx_ctrl, CN_excitation_gain, reset_flag_1),
    wr_codebook_cn(_IDcnt, code, txdtx_ctrl, L_pn_seed_tx, ar_cc__ar_tid_Subframes__DSP__HW, ar_cc__ar_tid_Codebook_CN__DSP__HW, ar_cc__T0__DSP__HW, T0, ar_cc__cb_ana__HW__DSP, cb_ana, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, exc_i, ar_cc__gain_code__HW__DSP, gain_code, ar_cc__gain_pit__DSP__HW, gain_pit, ar_cc__h1__DSP__HW, h1, ar_cc__res2__DSP__HW, res2, ar_cc__xn__DSP__HW, xn, ar_cc__y1__DSP__HW, y1, ar_cc__y2__HW__DSP, y2)
{   
}

Subframes::~Subframes(void)
{   
}

void Subframes::main(void)
{   
    __PRF_globalCounter[1058u]++ ;
    { enum { _scc_state_0, _scc_state_for_init, _scc_state_for_body1, _scc_state_closed_loop, _scc_state_for_body2, _scc_state_wr_codebook_cn, _scc_state_update, _scc_state_for_end, _scc_state_ar_cb__ar_stop__DSP__HW } _scc_next_state = _scc_state_for_init; do switch(_scc_next_state) {
	    case _scc_state_for_init: { for_init.main(); if (__PRF_globalCounter[1059u]++  , 
		    i_subfr < 160) { _scc_next_state = _scc_state_for_body1; break; }
		{ _scc_next_state = _scc_state_ar_cb__ar_stop__DSP__HW; break; } }
	    case _scc_state_for_body1: { for_body1.main(); if (__PRF_globalCounter[1060u]++  , 
		    1u) { _scc_next_state = _scc_state_closed_loop; break; } }
	    case _scc_state_closed_loop: { closed_loop.main(); if (__PRF_globalCounter[1061u]++  , 
		    1u) { _scc_next_state = _scc_state_for_body2; break; } }
	    case _scc_state_for_body2: { for_body2.main(); if (__PRF_globalCounter[1062u]++  , 
		    1u) { _scc_next_state = _scc_state_wr_codebook_cn; break; } }
	    case _scc_state_wr_codebook_cn: { wr_codebook_cn.main(); if (__PRF_globalCounter[1063u]++  , 
		    1u) { _scc_next_state = _scc_state_update; break; } }
	    case _scc_state_update: { update.main(); if (__PRF_globalCounter[1064u]++  , 
		    1u) { _scc_next_state = _scc_state_for_end; break; } }
	    case _scc_state_for_end: { for_end.main(); if (__PRF_globalCounter[1065u]++  , 
		    i_subfr < 160) { _scc_next_state = _scc_state_for_body1; break; }
		{ _scc_next_state = _scc_state_ar_cb__ar_stop__DSP__HW; break; } }
	    case _scc_state_ar_cb__ar_stop__DSP__HW: { ar_cb__ar_stop__DSP__HW.main(); if (__PRF_globalCounter[1066u]++  , 
		    1u) { _scc_next_state = _scc_state_0; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

void Subframes::reset(void)
{   
    __PRF_globalCounter[1067u]++ ;
    for_init.reset();
    closed_loop.reset();
    update.reset();
}

Subframes_Body1::Subframes_Body1(unsigned int _idcnt, int (&i), short int (&A_t)[4][11], short int (&Aq_t)[4][11], short int (&A_t_i)[11], short int (&Aq_t_i)[11])
    : _specc::behavior(_idcnt), i(i), A_t(A_t), Aq_t(Aq_t), A_t_i(A_t_i), Aq_t_i(Aq_t_i)
{   
}

Subframes_Body1::~Subframes_Body1(void)
{   
}

void Subframes_Body1::main(void)
{   
    __PRF_globalCounter[1068u]++ ;
    _specc::waitfor(0ull);
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<11;_scc_index_0++) (A_t_i)[_scc_index_0] = (A_t[i])[_scc_index_0]; }
    { unsigned int _scc_index_0; for(_scc_index_0=0;_scc_index_0<11;_scc_index_0++) (Aq_t_i)[_scc_index_0] = (Aq_t[i])[_scc_index_0]; }
}

Subframes_Body2::Subframes_Body2(unsigned int _idcnt, int (&i_subfr), short int *(&p_exc), short int *(&p_speech), short int (&exc_i)[40], short int (&speech_i)[40])
    : _specc::behavior(_idcnt), i_subfr(i_subfr), p_exc(p_exc), p_speech(p_speech), exc_i(exc_i), speech_i(speech_i)
{   
}

Subframes_Body2::~Subframes_Body2(void)
{   
}

void Subframes_Body2::main(void)
{   
    __PRF_globalCounter[1069u]++ ;
    _specc::waitfor(0ull);
    Copy(p_exc + i_subfr, (short int *)exc_i, 40);
    Copy(p_speech + i_subfr, (short int *)speech_i, 40);
}

Subframes_End::Subframes_End(unsigned int _idcnt, short int (&cl_ana)[2], short int (&cb_ana)[10], short int (&up_ana), short int (&exc_i)[40], short int (&synth_i)[40], short int *(&p_ana), int (&i), int (&i_subfr), short int *(&p_exc), short int (&synth)[160], bool (&reset_flag_1))
    : _specc::behavior(_idcnt), cl_ana(cl_ana), cb_ana(cb_ana), up_ana(up_ana), exc_i(exc_i), synth_i(synth_i), p_ana(p_ana), i(i), i_subfr(i_subfr), p_exc(p_exc), synth(synth), reset_flag_1(reset_flag_1)
{   
}

Subframes_End::~Subframes_End(void)
{   
}

void Subframes_End::main(void)
{   
    __PRF_globalCounter[1070u]++ ;
    _specc::waitfor(100000ull);
     *p_ana++  = cl_ana[0];
     *p_ana++  = cl_ana[1];
    Copy(cb_ana, p_ana, 10);
    p_ana += 10;
     *p_ana++  = up_ana;
    Copy(exc_i, p_exc + i_subfr, 40);
    Copy(synth_i, (short int *)synth + i_subfr, 40);
    reset_flag_1 = false;
    i_subfr += 40;
    i++ ;
}

Subframes_HW::Subframes_HW(unsigned int _idcnt, i_int_receiver (&ar_cc__ar_tid_Subframes__DSP__HW), i_int_receiver (&ar_cc__ar_tid_Codebook_CN__DSP__HW), short int (&T0), short int (&cb_ana)[10], short int (&code)[40], short int (&exc_i)[40], short int (&gain_code), short int (&gain_pit), short int (&h1)[40], short int (&res2)[40], short int (&xn)[40], short int (&y1)[40], short int (&y2)[40], i_short_int_receiver (&ar_cc__T0__DSP__HW), i_short_int__10__sender (&ar_cc__cb_ana__HW__DSP), i_short_int__40__receiver (&ar_cc__code__DSP__HW), i_short_int__40__sender (&ar_cc__code__HW__DSP), i_short_int__40__receiver (&ar_cc__exc_i__DSP__HW), i_short_int_sender (&ar_cc__gain_code__HW__DSP), i_short_int_receiver (&ar_cc__gain_pit__DSP__HW), i_short_int__40__receiver (&ar_cc__h1__DSP__HW), i_short_int__40__receiver (&ar_cc__res2__DSP__HW), i_short_int__40__receiver (&ar_cc__xn__DSP__HW), i_short_int__40__receiver (&ar_cc__y1__DSP__HW), i_short_int__40__sender (&ar_cc__y2__HW__DSP))
    : _specc::behavior(_idcnt), ar_cc__ar_tid_Subframes__DSP__HW(ar_cc__ar_tid_Subframes__DSP__HW), ar_cc__ar_tid_Codebook_CN__DSP__HW(ar_cc__ar_tid_Codebook_CN__DSP__HW), T0(T0), cb_ana(cb_ana), code(code), exc_i(exc_i), gain_code(gain_code), gain_pit(gain_pit), h1(h1), res2(res2), xn(xn), y1(y1), y2(y2), ar_cc__T0__DSP__HW(ar_cc__T0__DSP__HW), ar_cc__cb_ana__HW__DSP(ar_cc__cb_ana__HW__DSP), ar_cc__code__DSP__HW(ar_cc__code__DSP__HW), ar_cc__code__HW__DSP(ar_cc__code__HW__DSP), ar_cc__exc_i__DSP__HW(ar_cc__exc_i__DSP__HW), ar_cc__gain_code__HW__DSP(ar_cc__gain_code__HW__DSP), ar_cc__gain_pit__DSP__HW(ar_cc__gain_pit__DSP__HW), ar_cc__h1__DSP__HW(ar_cc__h1__DSP__HW), ar_cc__res2__DSP__HW(ar_cc__res2__DSP__HW), ar_cc__xn__DSP__HW(ar_cc__xn__DSP__HW), ar_cc__y1__DSP__HW(ar_cc__y1__DSP__HW), ar_cc__y2__HW__DSP(ar_cc__y2__HW__DSP),
    ar_cb__ar_tid__DSP__HW(_IDcnt, ar_cc__ar_tid_Subframes__DSP__HW, ar_tid),
    codebook_cn(_IDcnt, xn, y1, gain_pit, exc_i, h1, T0, res2, code, y2, gain_code, cb_ana, ar_cc__ar_tid_Codebook_CN__DSP__HW, ar_cc__T0__DSP__HW, ar_cc__cb_ana__HW__DSP, ar_cc__code__DSP__HW, ar_cc__code__HW__DSP, ar_cc__exc_i__DSP__HW, ar_cc__gain_code__HW__DSP, ar_cc__gain_pit__DSP__HW, ar_cc__h1__DSP__HW, ar_cc__res2__DSP__HW, ar_cc__xn__DSP__HW, ar_cc__y1__DSP__HW, ar_cc__y2__HW__DSP)
{   
}

Subframes_HW::~Subframes_HW(void)
{   
}

void Subframes_HW::main(void)
{   
    __PRF_globalCounter[1071u]++ ;
    { enum { _scc_state_0, _scc_state_ar_cb__ar_tid__DSP__HW, _scc_state_codebook_cn } _scc_next_state = _scc_state_ar_cb__ar_tid__DSP__HW; do switch(_scc_next_state) {
	    case _scc_state_ar_cb__ar_tid__DSP__HW: { ar_cb__ar_tid__DSP__HW.main(); if (__PRF_globalCounter[1072u]++  , 
		    ar_tid == 79) { _scc_next_state = _scc_state_codebook_cn; break; }
		{ _scc_next_state = _scc_state_0; break; } }
	    case _scc_state_codebook_cn: { codebook_cn.main(); if (__PRF_globalCounter[1073u]++  , 
		    1u) { _scc_next_state = _scc_state_ar_cb__ar_tid__DSP__HW; break; } } case _scc_state_0: { _scc_next_state = _scc_state_0; break; } } while(_scc_next_state != _scc_state_0);
    }
}

Subframes_Init::Subframes_Init(unsigned int _idcnt, short int (&T0_min_1), short int (&T0_max_1), short int (&ana)[57], short int (&T0_min), short int (&T0_max), short int *(&p_ana), int (&i), int (&i_subfr), short int (&mem_w0)[10], short int (&mem_err)[10], bool (&reset_flag), bool (&reset_flag_1))
    : _specc::behavior(_idcnt), T0_min_1(T0_min_1), T0_max_1(T0_max_1), ana(ana), T0_min(T0_min), T0_max(T0_max), p_ana(p_ana), i(i), i_subfr(i_subfr), mem_w0(mem_w0), mem_err(mem_err), reset_flag(reset_flag), reset_flag_1(reset_flag_1)
{   
}

Subframes_Init::~Subframes_Init(void)
{   
}

void Subframes_Init::init(void)
{   
    __PRF_globalCounter[1074u]++ ;
    Set_zero((short int *)mem_w0, 10);
    Set_zero((short int *)mem_err, 10);
}

void Subframes_Init::main(void)
{   
    __PRF_globalCounter[1075u]++ ;
    _specc::waitfor(800000ull);
    reset_flag_1 = reset_flag;
    __PRF_globalCounter[1076u]++ ;
    if (reset_flag)
    {   
	__PRF_globalCounter[1077u]++ ;
	init();
    }
    __PRF_globalCounter[1078u]++ ;
    p_ana =  &ana[5];
    T0_min = T0_min_1;
    T0_max = T0_max_1;
    i = 0;
    i_subfr = 0;
}

void Subframes_Init::reset(void)
{   
    __PRF_globalCounter[1079u]++ ;
    init();
}

Syn_Filt_0::Syn_Filt_0(unsigned int _idcnt, short int (&a)[11], short int (&x)[40], short int (&y)[40], short int (&mem)[10], bool (&update))
    : _specc::behavior(_idcnt), a(a), x(x), y(y), mem(mem), update(update)
{   
}

Syn_Filt_0::~Syn_Filt_0(void)
{   
}

void Syn_Filt_0::main(void)
{   
    int i;
    int j;
    int s;
    short int tmp[80];
    short int *yy;

    __PRF_globalCounter[1093u]++ ;
    _specc::waitfor(34833333ull);
    yy = tmp;
    __PRF_globalCounter[1094u]++ ;
    for(i = 0; i < 10; i++ )
    {   
	__PRF_globalCounter[1095u]++ ;
	 *yy++  = mem[i];
    }
    __PRF_globalCounter[1096u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1097u]++ ;
	s = L_mult(x[i], a[0]);
	__PRF_globalCounter[1098u]++ ;
	for(j = 1; j <= 10; j++ )
	{   
	    __PRF_globalCounter[1099u]++ ;
	    s = L_msu(s, a[j], yy[ -j]);
	}
	__PRF_globalCounter[1100u]++ ;
	s = L_shl(s, 3);
	 *yy++  = round(s);
    }
    __PRF_globalCounter[1101u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1102u]++ ;
	y[i] = tmp[i + 10];
    }
    __PRF_globalCounter[1103u]++ ;
    if (update != 0)
    {   
	__PRF_globalCounter[1104u]++ ;
	for(i = 0; i < 10; i++ )
	{   
	    __PRF_globalCounter[1105u]++ ;
	    mem[i] = tmp[40 + i];
	}
    }
}

Syn_Filt_1::Syn_Filt_1(unsigned int _idcnt, short int (&a)[11], short int (&x)[40], short int (&y)[40], short int (&mem)[10], bool (&update))
    : _specc::behavior(_idcnt), a(a), x(x), y(y), mem(mem), update(update)
{   
}

Syn_Filt_1::~Syn_Filt_1(void)
{   
}

void Syn_Filt_1::main(void)
{   
    int i;
    int j;
    int s;
    short int tmp[80];
    short int *yy;

    __PRF_globalCounter[1106u]++ ;
    _specc::waitfor(34833333ull);
    yy = tmp;
    __PRF_globalCounter[1107u]++ ;
    for(i = 0; i < 10; i++ )
    {   
	__PRF_globalCounter[1108u]++ ;
	 *yy++  = mem[i];
    }
    __PRF_globalCounter[1109u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1110u]++ ;
	s = L_mult(x[i], a[0]);
	__PRF_globalCounter[1111u]++ ;
	for(j = 1; j <= 10; j++ )
	{   
	    __PRF_globalCounter[1112u]++ ;
	    s = L_msu(s, a[j], yy[ -j]);
	}
	__PRF_globalCounter[1113u]++ ;
	s = L_shl(s, 3);
	 *yy++  = round(s);
    }
    __PRF_globalCounter[1114u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1115u]++ ;
	y[i] = tmp[i + 10];
    }
    __PRF_globalCounter[1116u]++ ;
    if (update != 0)
    {   
	__PRF_globalCounter[1117u]++ ;
	for(i = 0; i < 10; i++ )
	{   
	    __PRF_globalCounter[1118u]++ ;
	    mem[i] = tmp[40 + i];
	}
    }
}

Syn_Filt_2::Syn_Filt_2(unsigned int _idcnt, short int (&a)[11], short int (&x)[40], short int (&y)[40], short int (&mem)[10], bool (&update))
    : _specc::behavior(_idcnt), a(a), x(x), y(y), mem(mem), update(update)
{   
}

Syn_Filt_2::~Syn_Filt_2(void)
{   
}

void Syn_Filt_2::main(void)
{   
    int i;
    int j;
    int s;
    short int tmp[80];
    short int *yy;

    __PRF_globalCounter[1119u]++ ;
    _specc::waitfor(34833333ull);
    yy = tmp;
    __PRF_globalCounter[1120u]++ ;
    for(i = 0; i < 10; i++ )
    {   
	__PRF_globalCounter[1121u]++ ;
	 *yy++  = mem[i];
    }
    __PRF_globalCounter[1122u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1123u]++ ;
	s = L_mult(x[i], a[0]);
	__PRF_globalCounter[1124u]++ ;
	for(j = 1; j <= 10; j++ )
	{   
	    __PRF_globalCounter[1125u]++ ;
	    s = L_msu(s, a[j], yy[ -j]);
	}
	__PRF_globalCounter[1126u]++ ;
	s = L_shl(s, 3);
	 *yy++  = round(s);
    }
    __PRF_globalCounter[1127u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1128u]++ ;
	y[i] = tmp[i + 10];
    }
    __PRF_globalCounter[1129u]++ ;
    if (update != 0)
    {   
	__PRF_globalCounter[1130u]++ ;
	for(i = 0; i < 10; i++ )
	{   
	    __PRF_globalCounter[1131u]++ ;
	    mem[i] = tmp[40 + i];
	}
    }
}

Syn_Filt_3::Syn_Filt_3(unsigned int _idcnt, short int (&a)[11], short int (&x)[40], short int (&y)[40], short int (&mem)[10], bool (&update))
    : _specc::behavior(_idcnt), a(a), x(x), y(y), mem(mem), update(update)
{   
}

Syn_Filt_3::~Syn_Filt_3(void)
{   
}

void Syn_Filt_3::main(void)
{   
    int i;
    int j;
    int s;
    short int tmp[80];
    short int *yy;

    __PRF_globalCounter[1132u]++ ;
    _specc::waitfor(34833333ull);
    yy = tmp;
    __PRF_globalCounter[1133u]++ ;
    for(i = 0; i < 10; i++ )
    {   
	__PRF_globalCounter[1134u]++ ;
	 *yy++  = mem[i];
    }
    __PRF_globalCounter[1135u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1136u]++ ;
	s = L_mult(x[i], a[0]);
	__PRF_globalCounter[1137u]++ ;
	for(j = 1; j <= 10; j++ )
	{   
	    __PRF_globalCounter[1138u]++ ;
	    s = L_msu(s, a[j], yy[ -j]);
	}
	__PRF_globalCounter[1139u]++ ;
	s = L_shl(s, 3);
	 *yy++  = round(s);
    }
    __PRF_globalCounter[1140u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1141u]++ ;
	y[i] = tmp[i + 10];
    }
    __PRF_globalCounter[1142u]++ ;
    if (update != 0)
    {   
	__PRF_globalCounter[1143u]++ ;
	for(i = 0; i < 10; i++ )
	{   
	    __PRF_globalCounter[1144u]++ ;
	    mem[i] = tmp[40 + i];
	}
    }
}

Syn_Filt_4::Syn_Filt_4(unsigned int _idcnt, short int (&a)[11], short int (&x)[40], short int (&y)[40], short int (&mem)[10], bool (&update))
    : _specc::behavior(_idcnt), a(a), x(x), y(y), mem(mem), update(update)
{   
}

Syn_Filt_4::~Syn_Filt_4(void)
{   
}

void Syn_Filt_4::main(void)
{   
    int i;
    int j;
    int s;
    short int tmp[80];
    short int *yy;

    __PRF_globalCounter[1145u]++ ;
    _specc::waitfor(34833333ull);
    yy = tmp;
    __PRF_globalCounter[1146u]++ ;
    for(i = 0; i < 10; i++ )
    {   
	__PRF_globalCounter[1147u]++ ;
	 *yy++  = mem[i];
    }
    __PRF_globalCounter[1148u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1149u]++ ;
	s = L_mult(x[i], a[0]);
	__PRF_globalCounter[1150u]++ ;
	for(j = 1; j <= 10; j++ )
	{   
	    __PRF_globalCounter[1151u]++ ;
	    s = L_msu(s, a[j], yy[ -j]);
	}
	__PRF_globalCounter[1152u]++ ;
	s = L_shl(s, 3);
	 *yy++  = round(s);
    }
    __PRF_globalCounter[1153u]++ ;
    for(i = 0; i < 40; i++ )
    {   
	__PRF_globalCounter[1154u]++ ;
	y[i] = tmp[i + 10];
    }
    __PRF_globalCounter[1155u]++ ;
    if (update != 0)
    {   
	__PRF_globalCounter[1156u]++ ;
	for(i = 0; i < 10; i++ )
	{   
	    __PRF_globalCounter[1157u]++ ;
	    mem[i] = tmp[40 + i];
	}
    }
}

Update::Update(unsigned int _idcnt, int (&i_subfr), short int (&Aq)[11], short int (&gain_pit), short int (&gain_code), short int (&exc_i)[40], short int (&speech_i)[40], short int (&synth_i)[40], short int (&ana), short int (&xn)[40], short int (&y1)[40], short int (&y2)[40], short int (&code)[40], short int (&mem_err)[10], short int (&mem_w0)[10], short int (&txdtx_ctrl), short int (&CN_excitation_gain), bool (&reset_flag))
    : _specc::behavior(_idcnt), i_subfr(i_subfr), Aq(Aq), gain_pit(gain_pit), gain_code(gain_code), exc_i(exc_i), speech_i(speech_i), synth_i(synth_i), ana(ana), xn(xn), y1(y1), y2(y2), code(code), mem_err(mem_err), mem_w0(mem_w0), txdtx_ctrl(txdtx_ctrl), CN_excitation_gain(CN_excitation_gain), reset_flag(reset_flag),
    ex_syn_upd_sh(_IDcnt, gain_pit, qgain_code, exc_i, speech_i, synth_i, xn, y1, y2, code, Aq, mem_err, mem_w0, txdtx_ctrl, reset_flag),
    q_gain_code(_IDcnt, i_subfr, code, gain_code, qgain_code, txdtx_ctrl, CN_excitation_gain, ana, reset_flag)
{   
}

Update::~Update(void)
{   
}

void Update::main(void)
{   
    __PRF_globalCounter[1183u]++ ;
    q_gain_code.main();
    ex_syn_upd_sh.main();
}

void Update::reset(void)
{   
    __PRF_globalCounter[1184u]++ ;
    q_gain_code.reset();
    ex_syn_upd_sh.reset();
}

Weight_Ai_0::Weight_Ai_0(unsigned int _idcnt, short int (&a)[11], short int (&fac)[10], short int (&a_exp)[11])
    : _specc::behavior(_idcnt), a(a), fac(fac), a_exp(a_exp)
{   
}

Weight_Ai_0::~Weight_Ai_0(void)
{   
}

void Weight_Ai_0::main(void)
{   
    int i;

    __PRF_globalCounter[1396u]++ ;
    _specc::waitfor(716666ull);
    a_exp[0] = a[0];
    __PRF_globalCounter[1397u]++ ;
    for(i = 1; i <= 10; i++ )
    {   
	__PRF_globalCounter[1398u]++ ;
	a_exp[i] = round(L_mult(a[i], fac[i - 1]));
    }
}

Weight_Ai_1::Weight_Ai_1(unsigned int _idcnt, short int (&a)[11], short int (&fac)[10], short int (&a_exp)[11])
    : _specc::behavior(_idcnt), a(a), fac(fac), a_exp(a_exp)
{   
}

Weight_Ai_1::~Weight_Ai_1(void)
{   
}

void Weight_Ai_1::main(void)
{   
    int i;

    __PRF_globalCounter[1399u]++ ;
    _specc::waitfor(716666ull);
    a_exp[0] = a[0];
    __PRF_globalCounter[1400u]++ ;
    for(i = 1; i <= 10; i++ )
    {   
	__PRF_globalCounter[1401u]++ ;
	a_exp[i] = round(L_mult(a[i], fac[i - 1]));
    }
}

Weight_Ai_2::Weight_Ai_2(unsigned int _idcnt, short int (&a)[11], short int (&fac)[10], short int (&a_exp)[11])
    : _specc::behavior(_idcnt), a(a), fac(fac), a_exp(a_exp)
{   
}

Weight_Ai_2::~Weight_Ai_2(void)
{   
}

void Weight_Ai_2::main(void)
{   
    int i;

    __PRF_globalCounter[1402u]++ ;
    _specc::waitfor(716666ull);
    a_exp[0] = a[0];
    __PRF_globalCounter[1403u]++ ;
    for(i = 1; i <= 10; i++ )
    {   
	__PRF_globalCounter[1404u]++ ;
	a_exp[i] = round(L_mult(a[i], fac[i - 1]));
    }
}

// function definitions //////////////////////////////////////////////

void Convolut(

#line 30 "src/common/basic_func.sc"
    short int x[], 
    short int h[], 
    short int y[], 
    short int L)

{   
    int i; int n;
    int s;

#line 14565 "vocoderSched.cc"
    __PRF_globalCounter[1405u]++ ;

#line 39 "src/common/basic_func.sc"
    for(n = 0; n < L; n++ )
    {   

#line 14572 "vocoderSched.cc"
	__PRF_globalCounter[1406u]++ ;

#line 41 "src/common/basic_func.sc"
	s = 0;

#line 14578 "vocoderSched.cc"
	__PRF_globalCounter[1407u]++ ;

#line 42 "src/common/basic_func.sc"
	for(i = 0; i <= n; i++ )
	{   

#line 14585 "vocoderSched.cc"
	    __PRF_globalCounter[1408u]++ ;

#line 44 "src/common/basic_func.sc"
	    s = L_mac(s, x[i], h[n - i]);
	}

#line 14592 "vocoderSched.cc"
	__PRF_globalCounter[1409u]++ ;

#line 46 "src/common/basic_func.sc"
	s = L_shl(s, 3);
	y[n] = extract_h(s);
    }

#line 14600 "vocoderSched.cc"
    __PRF_globalCounter[1410u]++ ;

#line 50 "src/common/basic_func.sc"
    return ;
}

#line 14607 "vocoderSched.cc"
void Copy(

#line 41 "src/common/array_op.sc"
    short int x[], 
    short int y[], 
    short int L)

{   
    int i;

#line 14618 "vocoderSched.cc"
    __PRF_globalCounter[1411u]++ ;

#line 48 "src/common/array_op.sc"
    for(i = 0; i < L; i++ )
    {   

#line 14625 "vocoderSched.cc"
	__PRF_globalCounter[1412u]++ ;

#line 50 "src/common/array_op.sc"
	y[i] = x[i];
    }

#line 14632 "vocoderSched.cc"
    __PRF_globalCounter[1413u]++ ;

#line 53 "src/common/array_op.sc"
    return ;
}

#line 14639 "vocoderSched.cc"
int Div_32(

#line 2222 "src/common/basic_op.sc"
    int L_num, short int denom_hi, short int denom_lo)
{   
    short int approx; short int hi; short int lo; short int n_hi; short int n_lo;
    int L_32;

#line 14648 "vocoderSched.cc"
    __PRF_globalCounter[1414u]++ ;

#line 2229 "src/common/basic_op.sc"
    approx = div_s((short int)16383, denom_hi);



    L_32 = Mpy_32_16(denom_hi, denom_lo, approx);

    L_32 = L_sub((int)2147483647l, L_32);

    L_Extract(L_32,  &hi,  &lo);

    L_32 = Mpy_32_16(hi, lo, approx);



    L_Extract(L_32,  &hi,  &lo);
    L_Extract(L_num,  &n_hi,  &n_lo);
    L_32 = Mpy_32(n_hi, n_lo, hi, lo);
    L_32 = L_shl(L_32, 2);

    return (L_32);
}

#line 14674 "vocoderSched.cc"
static void Get_lsp_pol(

#line 88 "src/common/lsp_az.sc"
    short int *lsp, int *f)
{   
    int i; int j;
    short int hi; short int lo;
    int t0;

#line 14684 "vocoderSched.cc"
    __PRF_globalCounter[1415u]++ ;

#line 95 "src/common/lsp_az.sc"
     *f = L_mult(4096, 2048);
    f++ ;
     *f = L_msu((int)0,  *lsp, 512);
    f++ ;
    lsp += 2;

#line 14694 "vocoderSched.cc"
    __PRF_globalCounter[1416u]++ ;

#line 101 "src/common/lsp_az.sc"
    for(i = 2; i <= 5; i++ )
    {   

#line 14701 "vocoderSched.cc"
	__PRF_globalCounter[1417u]++ ;

#line 103 "src/common/lsp_az.sc"
	 *f = f[ -2];

#line 14707 "vocoderSched.cc"
	__PRF_globalCounter[1418u]++ ;

#line 105 "src/common/lsp_az.sc"
	for(j = 1; j < i; j++  , f-- )
	{   

#line 14714 "vocoderSched.cc"
	    __PRF_globalCounter[1419u]++ ;

#line 107 "src/common/lsp_az.sc"
	    L_Extract(f[ -1],  &hi,  &lo);
	    t0 = Mpy_32_16(hi, lo,  *lsp);
	    t0 = L_shl(t0, 1);
	     *f = L_add( *f, f[ -2]);
	     *f = L_sub( *f, t0);
	}

#line 14725 "vocoderSched.cc"
	__PRF_globalCounter[1420u]++ ;

#line 113 "src/common/lsp_az.sc"
	 *f = L_msu( *f,  *lsp, 512);
	f += i;
	lsp += 2;
    }

#line 14734 "vocoderSched.cc"
    __PRF_globalCounter[1421u]++ ;

#line 118 "src/common/lsp_az.sc"
    return ;
}

#line 14741 "vocoderSched.cc"
int Inv_sqrt(

#line 230 "src/common/basic_func.sc"
    int L_x)

{   
    int i = 0;
    short int a; short int exp; short int tmp;
    int L_y;

#line 14752 "vocoderSched.cc"
    __PRF_globalCounter[1422u]++ ;

#line 238 "src/common/basic_func.sc"
    if (L_x <= (int)0)

#line 14758 "vocoderSched.cc"
    {   
	__PRF_globalCounter[1423u]++ ;

#line 239 "src/common/basic_func.sc"
	return ((int)1073741823l);
    }

#line 14766 "vocoderSched.cc"
    __PRF_globalCounter[1424u]++ ;

#line 241 "src/common/basic_func.sc"
    exp = norm_l(L_x);
    L_x = L_shl(L_x, exp);

    exp = sub(30, exp);

#line 14775 "vocoderSched.cc"
    __PRF_globalCounter[1425u]++ ;

#line 246 "src/common/basic_func.sc"
    if ((exp & 1) == 0)
    {   

#line 14782 "vocoderSched.cc"
	__PRF_globalCounter[1426u]++ ;

#line 248 "src/common/basic_func.sc"
	L_x = L_shr(L_x, 1);
    }

#line 14789 "vocoderSched.cc"
    __PRF_globalCounter[1427u]++ ;

#line 250 "src/common/basic_func.sc"
    exp = shr(exp, 1);
    exp = add(exp, 1);


    L_x = L_shr(L_x, 9);
    i = extract_h(L_x);

#line 261 "src/common/basic_func.sc"
    L_x = L_shr(L_x, 1);
    a = extract_l(L_x);
    a = a & (short int)32767;

#line 269 "src/common/basic_func.sc"
    i = sub(i, 16);

    L_y = L_deposit_h(sqrt_table[i]);
    tmp = sub(sqrt_table[i], sqrt_table[i + 1]);
    L_y = L_msu(L_y, tmp, a);

    L_y = L_shr(L_y, exp);

    return (L_y);
}

#line 14817 "vocoderSched.cc"
int L_Comp(

#line 2113 "src/common/basic_op.sc"
    short int hi, short int lo)
{   
    int L_32;

#line 14825 "vocoderSched.cc"
    __PRF_globalCounter[1428u]++ ;

#line 2117 "src/common/basic_op.sc"
    L_32 = L_deposit_h(hi);
    return (L_mac(L_32, lo, 1));
}

#line 14833 "vocoderSched.cc"
void L_Extract(

#line 2085 "src/common/basic_op.sc"
    int L_32, short int *hi, short int *lo)
{   

#line 14840 "vocoderSched.cc"
    __PRF_globalCounter[1429u]++ ;

#line 2087 "src/common/basic_op.sc"
     *hi = extract_h(L_32);
     *lo = extract_l(L_msu(L_shr(L_32, 1),  *hi, 16384));
    return ;
}

#line 14849 "vocoderSched.cc"
int L_abs(

#line 1739 "src/common/basic_op.sc"
    int L_var1)
{   
    int L_var_out;

#line 14857 "vocoderSched.cc"
    __PRF_globalCounter[1430u]++ ;

#line 1743 "src/common/basic_op.sc"
    if (L_var1 == (int)2147483648ul)
    {   

#line 14864 "vocoderSched.cc"
	__PRF_globalCounter[1431u]++ ;

#line 1745 "src/common/basic_op.sc"
	L_var_out = (int)2147483647l;
    }
    else 
    {   

#line 14873 "vocoderSched.cc"
	__PRF_globalCounter[1432u]++ ;

#line 1749 "src/common/basic_op.sc"
	if (L_var1 < 0)
	{   

#line 14880 "vocoderSched.cc"
	    __PRF_globalCounter[1433u]++ ;

#line 1751 "src/common/basic_op.sc"
	    L_var_out =  -L_var1;
	}
	else 
	{   

#line 14889 "vocoderSched.cc"
	    __PRF_globalCounter[1434u]++ ;

#line 1755 "src/common/basic_op.sc"
	    L_var_out = L_var1;
	}
    }

#line 14897 "vocoderSched.cc"
    __PRF_globalCounter[1435u]++ ;

#line 1760 "src/common/basic_op.sc"
    return (L_var_out);
}

#line 14904 "vocoderSched.cc"
int L_add(

#line 903 "src/common/basic_op.sc"
    int L_var1, int L_var2)
{   
    int L_var_out;

#line 14912 "vocoderSched.cc"
    __PRF_globalCounter[1436u]++ ;

#line 907 "src/common/basic_op.sc"
    L_var_out = L_var1 + L_var2;

#line 14918 "vocoderSched.cc"
    __PRF_globalCounter[1437u]++ ;

#line 909 "src/common/basic_op.sc"
    if (((L_var1 ^ L_var2) & (int)2147483648ul) == 0)
    {   

#line 14925 "vocoderSched.cc"
	__PRF_globalCounter[1438u]++ ;

#line 911 "src/common/basic_op.sc"
	if ((L_var_out ^ L_var1) & (int)2147483648ul)
	{   

#line 14932 "vocoderSched.cc"
	    __PRF_globalCounter[1439u]++ ;

#line 913 "src/common/basic_op.sc"
	    L_var_out = ((L_var1 < 0) ? (int)2147483648ul : (int)2147483647l);
	    Overflow = 1;
	}
    }

#line 14941 "vocoderSched.cc"
    __PRF_globalCounter[1440u]++ ;

#line 918 "src/common/basic_op.sc"
    return (L_var_out);
}

#line 14948 "vocoderSched.cc"
int L_add_c(

#line 1008 "src/common/basic_op.sc"
    int L_var1, int L_var2)
{   
    int L_var_out;
    int L_test;
    bool carry_int = false;

#line 14958 "vocoderSched.cc"
    __PRF_globalCounter[1441u]++ ;

#line 1014 "src/common/basic_op.sc"
    L_var_out = L_var1 + L_var2 + Carry;

    L_test = L_var1 + L_var2;

#line 14966 "vocoderSched.cc"
    __PRF_globalCounter[1442u]++ ;

#line 1018 "src/common/basic_op.sc"
    if ((L_var1 > 0) && (L_var2 > 0) && (L_test < 0))
    {   

#line 14973 "vocoderSched.cc"
	__PRF_globalCounter[1443u]++ ;

#line 1020 "src/common/basic_op.sc"
	Overflow = 1;
	carry_int = 0;
    }
    else 
    {   

#line 14983 "vocoderSched.cc"
	__PRF_globalCounter[1444u]++ ;

#line 1025 "src/common/basic_op.sc"
	if ((L_var1 < 0) && (L_var2 < 0))
	{   

#line 14990 "vocoderSched.cc"
	    __PRF_globalCounter[1445u]++ ;

#line 1027 "src/common/basic_op.sc"
	    if (L_test >= 0)
	    {   

#line 14997 "vocoderSched.cc"
		__PRF_globalCounter[1446u]++ ;

#line 1029 "src/common/basic_op.sc"
		Overflow = 1;
		carry_int = 1;
	    }
	    else 
	    {   

#line 15007 "vocoderSched.cc"
		__PRF_globalCounter[1447u]++ ;

#line 1034 "src/common/basic_op.sc"
		Overflow = 0;
		carry_int = 1;
	    }
	}
	else 
	{   

#line 15018 "vocoderSched.cc"
	    __PRF_globalCounter[1448u]++ ;

#line 1040 "src/common/basic_op.sc"
	    if (((L_var1 ^ L_var2) < 0) && (L_test >= 0))
	    {   

#line 15025 "vocoderSched.cc"
		__PRF_globalCounter[1449u]++ ;

#line 1042 "src/common/basic_op.sc"
		Overflow = 0;
		carry_int = 1;
	    }
	    else 
	    {   

#line 15035 "vocoderSched.cc"
		__PRF_globalCounter[1450u]++ ;

#line 1047 "src/common/basic_op.sc"
		Overflow = 0;
		carry_int = 0;
	    }
	}
    }

#line 15045 "vocoderSched.cc"
    __PRF_globalCounter[1451u]++ ;

#line 1053 "src/common/basic_op.sc"
    if (Carry)
    {   

#line 15052 "vocoderSched.cc"
	__PRF_globalCounter[1452u]++ ;

#line 1055 "src/common/basic_op.sc"
	if (L_test == (int)2147483647l)
	{   

#line 15059 "vocoderSched.cc"
	    __PRF_globalCounter[1453u]++ ;

#line 1057 "src/common/basic_op.sc"
	    Overflow = 1;
	    Carry = carry_int;
	}
	else 
	{   

#line 15069 "vocoderSched.cc"
	    __PRF_globalCounter[1454u]++ ;

#line 1062 "src/common/basic_op.sc"
	    if (L_test == (int)4294967295ul)
	    {   

#line 15076 "vocoderSched.cc"
		__PRF_globalCounter[1455u]++ ;

#line 1064 "src/common/basic_op.sc"
		Carry = 1;
	    }
	    else 
	    {   

#line 15085 "vocoderSched.cc"
		__PRF_globalCounter[1456u]++ ;

#line 1068 "src/common/basic_op.sc"
		Carry = carry_int;
	    }
	}
    }
    else 
    {   

#line 15096 "vocoderSched.cc"
	__PRF_globalCounter[1457u]++ ;

#line 1074 "src/common/basic_op.sc"
	Carry = carry_int;
    }

#line 15103 "vocoderSched.cc"
    __PRF_globalCounter[1458u]++ ;

#line 1077 "src/common/basic_op.sc"
    return (L_var_out);
}

#line 15110 "vocoderSched.cc"
int L_deposit_h(

#line 1597 "src/common/basic_op.sc"
    short int var1)
{   
    int L_var_out;

#line 15118 "vocoderSched.cc"
    __PRF_globalCounter[1459u]++ ;

#line 1601 "src/common/basic_op.sc"
    L_var_out = (int)var1 << 16;

    return (L_var_out);
}

#line 15127 "vocoderSched.cc"
int L_deposit_l(

#line 1635 "src/common/basic_op.sc"
    short int var1)
{   
    int L_var_out;

#line 15135 "vocoderSched.cc"
    __PRF_globalCounter[1460u]++ ;

#line 1639 "src/common/basic_op.sc"
    L_var_out = (int)var1;

    return (L_var_out);
}

#line 15144 "vocoderSched.cc"
int L_mac(

#line 704 "src/common/basic_op.sc"
    int L_var3, short int var1, short int var2)
{   
    int L_var_out;
    int L_product;

#line 15153 "vocoderSched.cc"
    __PRF_globalCounter[1461u]++ ;

#line 709 "src/common/basic_op.sc"
    L_product = L_mult(var1, var2);
    L_var_out = L_add(L_var3, L_product);

    return (L_var_out);
}

#line 15163 "vocoderSched.cc"
int L_macNs(

#line 806 "src/common/basic_op.sc"
    int L_var3, short int var1, short int var2)
{   
    int L_var_out;

#line 15171 "vocoderSched.cc"
    __PRF_globalCounter[1462u]++ ;

#line 810 "src/common/basic_op.sc"
    L_var_out = L_mult(var1, var2);
    L_var_out = L_add_c(L_var3, L_var_out);

    return (L_var_out);
}

#line 15181 "vocoderSched.cc"
int L_msu(

#line 752 "src/common/basic_op.sc"
    int L_var3, short int var1, short int var2)
{   
    int L_var_out;
    int L_product;

#line 15190 "vocoderSched.cc"
    __PRF_globalCounter[1463u]++ ;

#line 757 "src/common/basic_op.sc"
    L_product = L_mult(var1, var2);
    L_var_out = L_sub(L_var3, L_product);

    return (L_var_out);
}

#line 15200 "vocoderSched.cc"
int L_mult(

#line 494 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    int L_var_out;

#line 15208 "vocoderSched.cc"
    __PRF_globalCounter[1464u]++ ;

#line 498 "src/common/basic_op.sc"
    L_var_out = (int)var1 * (int)var2;

#line 15214 "vocoderSched.cc"
    __PRF_globalCounter[1465u]++ ;

#line 500 "src/common/basic_op.sc"
    if (L_var_out != (int)1073741824l)
    {   

#line 15221 "vocoderSched.cc"
	__PRF_globalCounter[1466u]++ ;

#line 502 "src/common/basic_op.sc"
	L_var_out *= 2;
    }
    else 
    {   

#line 15230 "vocoderSched.cc"
	__PRF_globalCounter[1467u]++ ;

#line 506 "src/common/basic_op.sc"
	Overflow = 1;
	L_var_out = (int)2147483647l;
    }

#line 15238 "vocoderSched.cc"
    __PRF_globalCounter[1468u]++ ;

#line 510 "src/common/basic_op.sc"
    return (L_var_out);
}

#line 15245 "vocoderSched.cc"
int L_negate(

#line 1206 "src/common/basic_op.sc"
    int L_var1)
{   
    int L_var_out;

#line 15253 "vocoderSched.cc"
    __PRF_globalCounter[1469u]++ ;

#line 1210 "src/common/basic_op.sc"
    L_var_out = ((L_var1 == (int)2147483648ul) ? (int)2147483647l :  -L_var1);

    return (L_var_out);
}

#line 15262 "vocoderSched.cc"
int L_shl(

#line 1302 "src/common/basic_op.sc"
    int L_var1, short int var2)
{   
    int L_var_out;

#line 15270 "vocoderSched.cc"
    __PRF_globalCounter[1470u]++ ;

#line 1306 "src/common/basic_op.sc"
    if (var2 <= 0)
    {   

#line 15277 "vocoderSched.cc"
	__PRF_globalCounter[1471u]++ ;

#line 1308 "src/common/basic_op.sc"
	L_var_out = L_shr(L_var1,  -var2);
    }
    else 

    {   

#line 15287 "vocoderSched.cc"
	__PRF_globalCounter[1472u]++ ;

#line 1313 "src/common/basic_op.sc"
	for(; var2 > 0; var2-- )
	{   

#line 15294 "vocoderSched.cc"
	    __PRF_globalCounter[1473u]++ ;

#line 1315 "src/common/basic_op.sc"
	    if (L_var1 > (int)1073741823l)
	    {   

#line 15301 "vocoderSched.cc"
		__PRF_globalCounter[1474u]++ ;

#line 1317 "src/common/basic_op.sc"
		Overflow = 1;
		L_var_out = (int)2147483647l;
		break;
	    }
	    else 
	    {   

#line 15312 "vocoderSched.cc"
		__PRF_globalCounter[1475u]++ ;

#line 1323 "src/common/basic_op.sc"
		if (L_var1 < (int)3221225472ul)
		{   

#line 15319 "vocoderSched.cc"
		    __PRF_globalCounter[1476u]++ ;

#line 1325 "src/common/basic_op.sc"
		    Overflow = 1;
		    L_var_out = (int)2147483648ul;
		    break;
		}
	    }

#line 15329 "vocoderSched.cc"
	    __PRF_globalCounter[1477u]++ ;

#line 1330 "src/common/basic_op.sc"
	    L_var1 *= 2;
	    L_var_out = L_var1;
	}
    }

#line 15338 "vocoderSched.cc"
    __PRF_globalCounter[1478u]++ ;

#line 1335 "src/common/basic_op.sc"
    return (L_var_out);
}

#line 15345 "vocoderSched.cc"
int L_shr(

#line 1372 "src/common/basic_op.sc"
    int L_var1, short int var2)
{   
    int L_var_out;

#line 15353 "vocoderSched.cc"
    __PRF_globalCounter[1479u]++ ;

#line 1376 "src/common/basic_op.sc"
    if (var2 < 0)
    {   

#line 15360 "vocoderSched.cc"
	__PRF_globalCounter[1480u]++ ;

#line 1378 "src/common/basic_op.sc"
	L_var_out = L_shl(L_var1,  -var2);
    }
    else 
    {   

#line 15369 "vocoderSched.cc"
	__PRF_globalCounter[1481u]++ ;

#line 1382 "src/common/basic_op.sc"
	if (var2 >= 31)
	{   

#line 15376 "vocoderSched.cc"
	    __PRF_globalCounter[1482u]++ ;

#line 1384 "src/common/basic_op.sc"
	    L_var_out = ((L_var1 < 0l) ?  -1 : 0);
	}
	else 
	{   

#line 15385 "vocoderSched.cc"
	    __PRF_globalCounter[1483u]++ ;

#line 1388 "src/common/basic_op.sc"
	    if (L_var1 < 0)
	    {   

#line 15392 "vocoderSched.cc"
		__PRF_globalCounter[1484u]++ ;

#line 1390 "src/common/basic_op.sc"
		L_var_out =  ~(( ~L_var1) >> var2);
	    }
	    else 
	    {   

#line 15401 "vocoderSched.cc"
		__PRF_globalCounter[1485u]++ ;

#line 1394 "src/common/basic_op.sc"
		L_var_out = L_var1 >> var2;
	    }
	}
    }

#line 15410 "vocoderSched.cc"
    __PRF_globalCounter[1486u]++ ;

#line 1399 "src/common/basic_op.sc"
    return (L_var_out);
}

#line 15417 "vocoderSched.cc"
int L_shr_r(

#line 1686 "src/common/basic_op.sc"
    int L_var1, short int var2)
{   
    int L_var_out;

#line 15425 "vocoderSched.cc"
    __PRF_globalCounter[1487u]++ ;

#line 1690 "src/common/basic_op.sc"
    if (var2 > 31)
    {   

#line 15432 "vocoderSched.cc"
	__PRF_globalCounter[1488u]++ ;

#line 1692 "src/common/basic_op.sc"
	L_var_out = 0;
    }
    else 
    {   

#line 15441 "vocoderSched.cc"
	__PRF_globalCounter[1489u]++ ;

#line 1696 "src/common/basic_op.sc"
	L_var_out = L_shr(L_var1, var2);

#line 15447 "vocoderSched.cc"
	__PRF_globalCounter[1490u]++ ;

#line 1698 "src/common/basic_op.sc"
	if (var2 > 0)
	{   

#line 15454 "vocoderSched.cc"
	    __PRF_globalCounter[1491u]++ ;

#line 1700 "src/common/basic_op.sc"
	    if ((L_var1 & ((int)1 << (var2 - 1))) != 0)
	    {   

#line 15461 "vocoderSched.cc"
		__PRF_globalCounter[1492u]++ ;

#line 1702 "src/common/basic_op.sc"
		L_var_out++ ;
	    }
	}
    }

#line 15470 "vocoderSched.cc"
    __PRF_globalCounter[1493u]++ ;

#line 1707 "src/common/basic_op.sc"
    return (L_var_out);
}

#line 15477 "vocoderSched.cc"
int L_sub(

#line 953 "src/common/basic_op.sc"
    int L_var1, int L_var2)
{   
    int L_var_out;

#line 15485 "vocoderSched.cc"
    __PRF_globalCounter[1494u]++ ;

#line 957 "src/common/basic_op.sc"
    L_var_out = L_var1 - L_var2;

#line 15491 "vocoderSched.cc"
    __PRF_globalCounter[1495u]++ ;

#line 959 "src/common/basic_op.sc"
    if (((L_var1 ^ L_var2) & (int)2147483648ul) != 0)
    {   

#line 15498 "vocoderSched.cc"
	__PRF_globalCounter[1496u]++ ;

#line 961 "src/common/basic_op.sc"
	if ((L_var_out ^ L_var1) & (int)2147483648ul)
	{   

#line 15505 "vocoderSched.cc"
	    __PRF_globalCounter[1497u]++ ;

#line 963 "src/common/basic_op.sc"
	    L_var_out = ((L_var1 < 0l) ? (int)2147483648ul : (int)2147483647l);
	    Overflow = 1;
	}
    }

#line 15514 "vocoderSched.cc"
    __PRF_globalCounter[1498u]++ ;

#line 968 "src/common/basic_op.sc"
    return (L_var_out);
}

#line 15521 "vocoderSched.cc"
void Log2(

#line 84 "src/common/basic_func.sc"
    int L_x, 
    short int *exponent, 
    short int *fraction)

{   
    int i = 0;
    short int a; short int exp; short int tmp;
    int L_y;

#line 15534 "vocoderSched.cc"
    __PRF_globalCounter[1499u]++ ;

#line 94 "src/common/basic_func.sc"
    if (L_x <= (int)0)
    {   

#line 15541 "vocoderSched.cc"
	__PRF_globalCounter[1500u]++ ;

#line 96 "src/common/basic_func.sc"
	 *exponent = 0;
	 *fraction = 0;
	return ;
    }

#line 15550 "vocoderSched.cc"
    __PRF_globalCounter[1501u]++ ;

#line 101 "src/common/basic_func.sc"
    exp = norm_l(L_x);
    L_x = L_shl(L_x, exp);

     *exponent = sub(30, exp);


    L_x = L_shr(L_x, 9);
    i = extract_h(L_x);

#line 114 "src/common/basic_func.sc"
    L_x = L_shr(L_x, 1);
    a = extract_l(L_x);
    a = a & (short int)32767;

#line 122 "src/common/basic_func.sc"
    i = sub(i, 32);

    L_y = L_deposit_h(log_table[i]);
    tmp = sub(log_table[i], log_table[i + 1]);
    L_y = L_msu(L_y, tmp, a);

     *fraction = extract_h(L_y);

    return ;
}

#line 15580 "vocoderSched.cc"
void Lsp_Az(

#line 26 "src/common/lsp_az.sc"
    short int lsp[], 
    short int a[])

{   
    int i; int j;

    int f1[6]; int f2[6];
    int t0;

#line 15593 "vocoderSched.cc"
    __PRF_globalCounter[1502u]++ ;

#line 35 "src/common/lsp_az.sc"
    Get_lsp_pol( &lsp[0], f1);
    Get_lsp_pol( &lsp[1], f2);

#line 15600 "vocoderSched.cc"
    __PRF_globalCounter[1503u]++ ;

#line 38 "src/common/lsp_az.sc"
    for(i = 5; i > 0; i-- )
    {   

#line 15607 "vocoderSched.cc"
	__PRF_globalCounter[1504u]++ ;

#line 40 "src/common/lsp_az.sc"
	f1[i] = L_add(f1[i], f1[i - 1]);
	f2[i] = L_sub(f2[i], f2[i - 1]);
    }

#line 15615 "vocoderSched.cc"
    __PRF_globalCounter[1505u]++ ;

#line 44 "src/common/lsp_az.sc"
    a[0] = 4096;

#line 15621 "vocoderSched.cc"
    __PRF_globalCounter[1506u]++ ;

#line 45 "src/common/lsp_az.sc"
    for(i = 1 , j = 10; i <= 5; i++  , j-- )
    {   

#line 15628 "vocoderSched.cc"
	__PRF_globalCounter[1507u]++ ;

#line 47 "src/common/lsp_az.sc"
	t0 = L_add(f1[i], f2[i]);
	a[i] = extract_l(L_shr_r(t0, 13));
	t0 = L_sub(f1[i], f2[i]);
	a[j] = extract_l(L_shr_r(t0, 13));
    }

#line 15638 "vocoderSched.cc"
    __PRF_globalCounter[1508u]++ ;

#line 53 "src/common/lsp_az.sc"
    return ;
}

#line 15645 "vocoderSched.cc"
int Mpy_32(

#line 2141 "src/common/basic_op.sc"
    short int hi1, short int lo1, short int hi2, short int lo2)
{   
    int L_32;

#line 15653 "vocoderSched.cc"
    __PRF_globalCounter[1509u]++ ;

#line 2145 "src/common/basic_op.sc"
    L_32 = L_mult(hi1, hi2);
    L_32 = L_mac(L_32, mult(hi1, lo2), 1);
    L_32 = L_mac(L_32, mult(lo1, hi2), 1);

    return (L_32);
}

#line 15664 "vocoderSched.cc"
int Mpy_32_16(

#line 2170 "src/common/basic_op.sc"
    short int hi, short int lo, short int n)
{   
    int L_32;

#line 15672 "vocoderSched.cc"
    __PRF_globalCounter[1510u]++ ;

#line 2174 "src/common/basic_op.sc"
    L_32 = L_mult(hi, n);
    L_32 = L_mac(L_32, mult(lo, n), 1);

    return (L_32);
}

#line 15682 "vocoderSched.cc"
int Pow2(

#line 160 "src/common/basic_func.sc"
    short int exponent, 
    short int fraction)

{   
    int i = 0;
    short int a; short int exp; short int tmp;
    int L_x;

#line 15694 "vocoderSched.cc"
    __PRF_globalCounter[1511u]++ ;

#line 169 "src/common/basic_func.sc"
    L_x = L_mult(fraction, 32);
    i = extract_h(L_x);

#line 177 "src/common/basic_func.sc"
    L_x = L_shr(L_x, 1);
    a = extract_l(L_x);
    a = a & (short int)32767;

#line 185 "src/common/basic_func.sc"
    L_x = L_deposit_h(pow_table[i]);
    tmp = sub(pow_table[i], pow_table[i + 1]);
    L_x = L_msu(L_x, tmp, a);

    exp = sub(30, exponent);
    L_x = L_shr_r(L_x, exp);

    return (L_x);
}

#line 15717 "vocoderSched.cc"
void Set_zero(

#line 14 "src/common/array_op.sc"
    short int x[], 
    short int L)

{   
    int i;

#line 15727 "vocoderSched.cc"
    __PRF_globalCounter[1512u]++ ;

#line 20 "src/common/array_op.sc"
    for(i = 0; i < L; i++ )
    {   

#line 15734 "vocoderSched.cc"
	__PRF_globalCounter[1513u]++ ;

#line 22 "src/common/array_op.sc"
	x[i] = 0;
    }

#line 15741 "vocoderSched.cc"
    __PRF_globalCounter[1514u]++ ;

#line 25 "src/common/array_op.sc"
    return ;
}

#line 15748 "vocoderSched.cc"
short int abs_s(

#line 257 "src/common/basic_op.sc"
    short int var1)
{   
    short int var_out;

#line 15756 "vocoderSched.cc"
    __PRF_globalCounter[1515u]++ ;

#line 261 "src/common/basic_op.sc"
    if (var1 == (short int)32768)
    {   

#line 15763 "vocoderSched.cc"
	__PRF_globalCounter[1516u]++ ;

#line 263 "src/common/basic_op.sc"
	var_out = (short int)32767;
    }
    else 
    {   

#line 15772 "vocoderSched.cc"
	__PRF_globalCounter[1517u]++ ;

#line 267 "src/common/basic_op.sc"
	if (var1 < 0)
	{   

#line 15779 "vocoderSched.cc"
	    __PRF_globalCounter[1518u]++ ;

#line 269 "src/common/basic_op.sc"
	    var_out =  -var1;
	}
	else 
	{   

#line 15788 "vocoderSched.cc"
	    __PRF_globalCounter[1519u]++ ;

#line 273 "src/common/basic_op.sc"
	    var_out = var1;
	}
    }

#line 15796 "vocoderSched.cc"
    __PRF_globalCounter[1520u]++ ;

#line 277 "src/common/basic_op.sc"
    return (var_out);
}

#line 15803 "vocoderSched.cc"
short int add(

#line 173 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    short int var_out;
    int L_sum;

#line 15812 "vocoderSched.cc"
    __PRF_globalCounter[1521u]++ ;

#line 178 "src/common/basic_op.sc"
    L_sum = (int)var1 + var2;
    var_out = saturate(L_sum);

    return (var_out);
}

#line 15822 "vocoderSched.cc"
bool arg_handler(

#line 71 "src/arg_handler.sc"
    int argc, char **argv, 
    char **speechfile_name, char **serialfile_name)
{   
    bool dtx_mode = false;

#line 15831 "vocoderSched.cc"
    __PRF_globalCounter[1522u]++ ;

#line 77 "src/arg_handler.sc"
    printf("\n/*******************************************************\n\n");
    printf("     European digital cellular telecommunications system\n");
    printf("                12200 bits/s speech codec for\n");
    printf("          enhanced full rate speech traffic channels\n\n");
    printf("     Bit-Exact SpecC Simulation Code - encoder\n");
    printf("     Version 1.0\n");
    printf("     March 13, 1999\n\n");
    printf("*********************************************************/\n\n");

#line 15844 "vocoderSched.cc"
    __PRF_globalCounter[1523u]++ ;

#line 88 "src/arg_handler.sc"
    if ((argc < 3) || (argc > 4))
    {   

#line 15851 "vocoderSched.cc"
	__PRF_globalCounter[1524u]++ ;

#line 90 "src/arg_handler.sc"
	printf("   Usage:\n\n");
	printf("      coder  speech_file  bitstream_file  <dtx|nodtx>\n");
	printf("\n");
	exit(1);
    }

#line 15861 "vocoderSched.cc"
    __PRF_globalCounter[1525u]++ ;

#line 96 "src/arg_handler.sc"
    if (argc == 4)
    {   

#line 15868 "vocoderSched.cc"
	__PRF_globalCounter[1526u]++ ;

#line 98 "src/arg_handler.sc"
	if (strcmp(argv[3], "nodtx") == 0)
	{   

#line 15875 "vocoderSched.cc"
	    __PRF_globalCounter[1527u]++ ;

#line 100 "src/arg_handler.sc"
	    dtx_mode = 0;
	}
	else 

#line 15883 "vocoderSched.cc"
	{   
	    __PRF_globalCounter[1528u]++ ;

#line 102 "src/arg_handler.sc"
	    if (strcmp(argv[3], "dtx") == 0)
	    {   

#line 15891 "vocoderSched.cc"
		__PRF_globalCounter[1529u]++ ;

#line 104 "src/arg_handler.sc"
		dtx_mode = 1;
	    }
	    else 
	    {   

#line 15900 "vocoderSched.cc"
		__PRF_globalCounter[1530u]++ ;

#line 108 "src/arg_handler.sc"
		printf("\nWrong DTX switch:  %s !!\n", argv[3]);
		exit(1);
	    }
	}
    }

#line 15910 "vocoderSched.cc"
    __PRF_globalCounter[1531u]++ ;

#line 112 "src/arg_handler.sc"
    if (dtx_mode == 1)
    {   

#line 15917 "vocoderSched.cc"
	__PRF_globalCounter[1532u]++ ;

#line 114 "src/arg_handler.sc"
	printf(" DTX:  enabled\n");
    }
    else 
    {   

#line 15926 "vocoderSched.cc"
	__PRF_globalCounter[1533u]++ ;

#line 118 "src/arg_handler.sc"
	printf(" DTX:  disabled\n");
    }

#line 15933 "vocoderSched.cc"
    __PRF_globalCounter[1534u]++ ;

#line 121 "src/arg_handler.sc"
     *speechfile_name = argv[1];
     *serialfile_name = argv[2];

    return dtx_mode;
}

#line 15943 "vocoderSched.cc"
void clear_carry(void)

#line 71 "src/common/basic_op.sc"
{   

#line 15949 "vocoderSched.cc"
    __PRF_globalCounter[1535u]++ ;

#line 72 "src/common/basic_op.sc"
    Carry = 0;
}

#line 15956 "vocoderSched.cc"
void clear_overflow(void)

#line 61 "src/common/basic_op.sc"
{   

#line 15962 "vocoderSched.cc"
    __PRF_globalCounter[1536u]++ ;

#line 62 "src/common/basic_op.sc"
    Overflow = 0;
}

#line 15969 "vocoderSched.cc"
short int div_s(

#line 1922 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    short int var_out = ((short int)0);
    int iteration;
    int L_num;
    int L_denom;

#line 15980 "vocoderSched.cc"
    __PRF_globalCounter[1537u]++ ;

#line 1944 "src/common/basic_op.sc"
    if (var1 == 0)
    {   

#line 15987 "vocoderSched.cc"
	__PRF_globalCounter[1538u]++ ;

#line 1946 "src/common/basic_op.sc"
	var_out = 0;
    }
    else 
    {   

#line 15996 "vocoderSched.cc"
	__PRF_globalCounter[1539u]++ ;

#line 1950 "src/common/basic_op.sc"
	if (var1 == var2)
	{   

#line 16003 "vocoderSched.cc"
	    __PRF_globalCounter[1540u]++ ;

#line 1952 "src/common/basic_op.sc"
	    var_out = (short int)32767;
	}
	else 
	{   

#line 16012 "vocoderSched.cc"
	    __PRF_globalCounter[1541u]++ ;

#line 1956 "src/common/basic_op.sc"
	    L_num = L_deposit_l(var1);
	    L_denom = L_deposit_l(var2);

#line 16019 "vocoderSched.cc"
	    __PRF_globalCounter[1542u]++ ;

#line 1959 "src/common/basic_op.sc"
	    for(iteration = 0; iteration < 15; iteration++ )
	    {   

#line 16026 "vocoderSched.cc"
		__PRF_globalCounter[1543u]++ ;

#line 1961 "src/common/basic_op.sc"
		var_out <<= 1;
		L_num <<= 1;

#line 16033 "vocoderSched.cc"
		__PRF_globalCounter[1544u]++ ;

#line 1964 "src/common/basic_op.sc"
		if (L_num >= L_denom)
		{   

#line 16040 "vocoderSched.cc"
		    __PRF_globalCounter[1545u]++ ;

#line 1966 "src/common/basic_op.sc"
		    L_num = L_sub(L_num, L_denom);
		    var_out = add(var_out, 1);
		}
	    }
	}
    }

#line 16051 "vocoderSched.cc"
    __PRF_globalCounter[1546u]++ ;

#line 1974 "src/common/basic_op.sc"
    return (var_out);
}

#line 16058 "vocoderSched.cc"
short int extract_h(

#line 579 "src/common/basic_op.sc"
    int L_var1)
{   
    short int var_out;

#line 16066 "vocoderSched.cc"
    __PRF_globalCounter[1547u]++ ;

#line 583 "src/common/basic_op.sc"
    var_out = (short int)(L_var1 >> 16);

    return (var_out);
}

#line 16075 "vocoderSched.cc"
short int extract_l(

#line 616 "src/common/basic_op.sc"
    int L_var1)
{   
    short int var_out;

#line 16083 "vocoderSched.cc"
    __PRF_globalCounter[1548u]++ ;

#line 620 "src/common/basic_op.sc"
    var_out = (short int)L_var1;

    return (var_out);
}

#line 16092 "vocoderSched.cc"
short int gmed5(

#line 292 "src/common/basic_func.sc"
    short int ind[])

{   
    short int i; short int ix = ((short int)0); short int j; short int tmp[5];
    short int max; short int tmp2[5];

#line 16102 "vocoderSched.cc"
    __PRF_globalCounter[1549u]++ ;

#line 298 "src/common/basic_func.sc"
    for(i = 0; i < 5; i++ )
    {   

#line 16109 "vocoderSched.cc"
	__PRF_globalCounter[1550u]++ ;

#line 300 "src/common/basic_func.sc"
	tmp2[i] = ind[i];
    }

#line 16116 "vocoderSched.cc"
    __PRF_globalCounter[1551u]++ ;

#line 303 "src/common/basic_func.sc"
    for(i = 0; i < 5; i++ )
    {   

#line 16123 "vocoderSched.cc"
	__PRF_globalCounter[1552u]++ ;

#line 305 "src/common/basic_func.sc"
	max =  -8192;

#line 16129 "vocoderSched.cc"
	__PRF_globalCounter[1553u]++ ;

#line 306 "src/common/basic_func.sc"
	for(j = 0; j < 5; j++ )
	{   

#line 16136 "vocoderSched.cc"
	    __PRF_globalCounter[1554u]++ ;

#line 308 "src/common/basic_func.sc"
	    if (sub(tmp2[j], max) >= 0)
	    {   

#line 16143 "vocoderSched.cc"
		__PRF_globalCounter[1555u]++ ;

#line 310 "src/common/basic_func.sc"
		max = tmp2[j];
		ix = j;
	    }
	}

#line 16152 "vocoderSched.cc"
	__PRF_globalCounter[1556u]++ ;

#line 314 "src/common/basic_func.sc"
	tmp2[ix] =  -16384;
	tmp[i] = ix;
    }

#line 16160 "vocoderSched.cc"
    __PRF_globalCounter[1557u]++ ;

#line 318 "src/common/basic_func.sc"
    return (ind[tmp[2]]);
}

#line 16167 "vocoderSched.cc"
short int interpolate_CN_param(

#line 342 "src/common/basic_func.sc"
    short int old_param, 
    short int new_param, 
    short int rx_dtx_state)

{   
    static const short int interp_factor[24] = { 

      ((short int)1365),((short int)2730),((short int)4096),((short int)5461),((short int)6826),((short int)8192),
      ((short int)9557),((short int)10922),((short int)12288),((short int)13653),((short int)15018),((short int)16384),
      ((short int)17749),((short int)19114),((short int)20480),((short int)21845),((short int)23210),((short int)24576),
      ((short int)25941),((short int)27306),((short int)28672),((short int)30037),((short int)31402),((short int)32767) };
    short int temp;
    int L_temp;

#line 16185 "vocoderSched.cc"
    __PRF_globalCounter[1558u]++ ;

#line 356 "src/common/basic_func.sc"
    L_temp = L_mult(interp_factor[rx_dtx_state], new_param);
    temp = sub(32767, interp_factor[rx_dtx_state]);
    temp = add(temp, 1);
    L_temp = L_mac(L_temp, temp, old_param);
    temp = round(L_temp);

    return temp;
}

#line 16198 "vocoderSched.cc"
short int mac_r(

#line 1507 "src/common/basic_op.sc"
    int L_var3, short int var1, short int var2)
{   
    short int var_out;

#line 16206 "vocoderSched.cc"
    __PRF_globalCounter[1559u]++ ;

#line 1511 "src/common/basic_op.sc"
    L_var3 = L_mac(L_var3, var1, var2);
    L_var3 = L_add(L_var3, (int)32768l);
    var_out = extract_h(L_var3);

    return (var_out);
}

#line 16217 "vocoderSched.cc"
short int msu_r(

#line 1557 "src/common/basic_op.sc"
    int L_var3, short int var1, short int var2)
{   
    short int var_out;

#line 16225 "vocoderSched.cc"
    __PRF_globalCounter[1560u]++ ;

#line 1561 "src/common/basic_op.sc"
    L_var3 = L_msu(L_var3, var1, var2);
    L_var3 = L_add(L_var3, (int)32768l);
    var_out = extract_h(L_var3);

    return (var_out);
}

#line 16236 "vocoderSched.cc"
short int mult(

#line 442 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    short int var_out;
    int L_product;

#line 16245 "vocoderSched.cc"
    __PRF_globalCounter[1561u]++ ;

#line 447 "src/common/basic_op.sc"
    L_product = (int)var1 * (int)var2;

    L_product = (L_product & (int)4294934528ul) >> 15;

#line 16253 "vocoderSched.cc"
    __PRF_globalCounter[1562u]++ ;

#line 451 "src/common/basic_op.sc"
    if (L_product & (int)65536l)

#line 16259 "vocoderSched.cc"
    {   
	__PRF_globalCounter[1563u]++ ;

#line 452 "src/common/basic_op.sc"
	L_product = L_product | (int)4294901760ul;
    }

#line 16267 "vocoderSched.cc"
    __PRF_globalCounter[1564u]++ ;

#line 454 "src/common/basic_op.sc"
    var_out = saturate(L_product);

    return (var_out);
}

#line 16276 "vocoderSched.cc"
short int mult_r(

#line 1249 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    short int var_out;
    int L_product_arr;

#line 16285 "vocoderSched.cc"
    __PRF_globalCounter[1565u]++ ;

#line 1254 "src/common/basic_op.sc"
    L_product_arr = (int)var1 * (int)var2;
    L_product_arr += (int)16384ul;
    L_product_arr &= (int)4294934528ul;
    L_product_arr >>= 15;

#line 16294 "vocoderSched.cc"
    __PRF_globalCounter[1566u]++ ;

#line 1259 "src/common/basic_op.sc"
    if (L_product_arr & (int)65536ul)
    {   

#line 16301 "vocoderSched.cc"
	__PRF_globalCounter[1567u]++ ;

#line 1261 "src/common/basic_op.sc"
	L_product_arr |= (int)4294901760ul;
    }

#line 16308 "vocoderSched.cc"
    __PRF_globalCounter[1568u]++ ;

#line 1263 "src/common/basic_op.sc"
    var_out = saturate(L_product_arr);

    return (var_out);
}

#line 16317 "vocoderSched.cc"
short int negate(

#line 542 "src/common/basic_op.sc"
    short int var1)
{   
    short int var_out;

#line 16325 "vocoderSched.cc"
    __PRF_globalCounter[1569u]++ ;

#line 546 "src/common/basic_op.sc"
    var_out = ((var1 == (short int)32768) ? (short int)32767 :  -var1);

    return (var_out);
}

#line 16334 "vocoderSched.cc"
short int norm_l(

#line 2010 "src/common/basic_op.sc"
    int L_var1)
{   
    short int var_out;

#line 16342 "vocoderSched.cc"
    __PRF_globalCounter[1570u]++ ;

#line 2014 "src/common/basic_op.sc"
    if (L_var1 == 0)
    {   

#line 16349 "vocoderSched.cc"
	__PRF_globalCounter[1571u]++ ;

#line 2016 "src/common/basic_op.sc"
	var_out = 0;
    }
    else 
    {   

#line 16358 "vocoderSched.cc"
	__PRF_globalCounter[1572u]++ ;

#line 2020 "src/common/basic_op.sc"
	if (L_var1 == (int)4294967295ul)
	{   

#line 16365 "vocoderSched.cc"
	    __PRF_globalCounter[1573u]++ ;

#line 2022 "src/common/basic_op.sc"
	    var_out = 31;
	}
	else 
	{   

#line 16374 "vocoderSched.cc"
	    __PRF_globalCounter[1574u]++ ;

#line 2026 "src/common/basic_op.sc"
	    if (L_var1 < 0)
	    {   

#line 16381 "vocoderSched.cc"
		__PRF_globalCounter[1575u]++ ;

#line 2028 "src/common/basic_op.sc"
		L_var1 =  ~L_var1;
	    }

#line 16388 "vocoderSched.cc"
	    __PRF_globalCounter[1576u]++ ;

#line 2030 "src/common/basic_op.sc"
	    for(var_out = 0; L_var1 < (int)1073741824l; var_out++ )
	    {   

#line 16395 "vocoderSched.cc"
		__PRF_globalCounter[1577u]++ ;

#line 2032 "src/common/basic_op.sc"
		L_var1 <<= 1;
	    }
	}
    }

#line 16404 "vocoderSched.cc"
    __PRF_globalCounter[1578u]++ ;

#line 2037 "src/common/basic_op.sc"
    return (var_out);
}

#line 16411 "vocoderSched.cc"
short int norm_s(

#line 1854 "src/common/basic_op.sc"
    short int var1)
{   
    short int var_out;

#line 16419 "vocoderSched.cc"
    __PRF_globalCounter[1579u]++ ;

#line 1858 "src/common/basic_op.sc"
    if (var1 == 0)
    {   

#line 16426 "vocoderSched.cc"
	__PRF_globalCounter[1580u]++ ;

#line 1860 "src/common/basic_op.sc"
	var_out = 0;
    }
    else 
    {   

#line 16435 "vocoderSched.cc"
	__PRF_globalCounter[1581u]++ ;

#line 1864 "src/common/basic_op.sc"
	if (var1 == (short int)65535)
	{   

#line 16442 "vocoderSched.cc"
	    __PRF_globalCounter[1582u]++ ;

#line 1866 "src/common/basic_op.sc"
	    var_out = 15;
	}
	else 
	{   

#line 16451 "vocoderSched.cc"
	    __PRF_globalCounter[1583u]++ ;

#line 1870 "src/common/basic_op.sc"
	    if (var1 < 0)
	    {   

#line 16458 "vocoderSched.cc"
		__PRF_globalCounter[1584u]++ ;

#line 1872 "src/common/basic_op.sc"
		var1 =  ~var1;
	    }

#line 16465 "vocoderSched.cc"
	    __PRF_globalCounter[1585u]++ ;

#line 1874 "src/common/basic_op.sc"
	    for(var_out = 0; var1 < 16384; var_out++ )
	    {   

#line 16472 "vocoderSched.cc"
		__PRF_globalCounter[1586u]++ ;

#line 1876 "src/common/basic_op.sc"
		var1 <<= 1;
	    }
	}
    }

#line 16481 "vocoderSched.cc"
    __PRF_globalCounter[1587u]++ ;

#line 1882 "src/common/basic_op.sc"
    return (var_out);
}

#line 16488 "vocoderSched.cc"
bool overflow(void)

#line 81 "src/common/basic_op.sc"
{   

#line 16494 "vocoderSched.cc"
    __PRF_globalCounter[1588u]++ ;

#line 82 "src/common/basic_op.sc"
    return (Overflow);
}

#line 16501 "vocoderSched.cc"
short int round(

#line 656 "src/common/basic_op.sc"
    int L_var1)
{   
    short int var_out;
    int L_rounded;

#line 16510 "vocoderSched.cc"
    __PRF_globalCounter[1589u]++ ;

#line 661 "src/common/basic_op.sc"
    L_rounded = L_add(L_var1, (int)32768l);
    var_out = extract_h(L_rounded);

    return (var_out);
}

#line 16520 "vocoderSched.cc"
short int saturate(

#line 116 "src/common/basic_op.sc"
    int L_var1)
{   
    short int var_out;

#line 16528 "vocoderSched.cc"
    __PRF_globalCounter[1590u]++ ;

#line 120 "src/common/basic_op.sc"
    if (L_var1 > 32767l)
    {   

#line 16535 "vocoderSched.cc"
	__PRF_globalCounter[1591u]++ ;

#line 122 "src/common/basic_op.sc"
	Overflow = 1;
	var_out = (short int)32767;
    }
    else 

#line 16544 "vocoderSched.cc"
    {   
	__PRF_globalCounter[1592u]++ ;

#line 125 "src/common/basic_op.sc"
	if (L_var1 < (int)4294934528ul)
	{   

#line 16552 "vocoderSched.cc"
	    __PRF_globalCounter[1593u]++ ;

#line 127 "src/common/basic_op.sc"
	    Overflow = 1;
	    var_out = (short int)32768;
	}
	else 
	{   

#line 16562 "vocoderSched.cc"
	    __PRF_globalCounter[1594u]++ ;

#line 132 "src/common/basic_op.sc"
	    Overflow = 0;
	    var_out = extract_l(L_var1);
	}
    }

#line 16571 "vocoderSched.cc"
    __PRF_globalCounter[1595u]++ ;

#line 136 "src/common/basic_op.sc"
    return (var_out);
}

#line 16578 "vocoderSched.cc"
short int shl(

#line 315 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    short int var_out;
    int result;

#line 16587 "vocoderSched.cc"
    __PRF_globalCounter[1596u]++ ;

#line 320 "src/common/basic_op.sc"
    if (var2 < 0)
    {   

#line 16594 "vocoderSched.cc"
	__PRF_globalCounter[1597u]++ ;

#line 322 "src/common/basic_op.sc"
	var_out = shr(var1,  -var2);
    }
    else 
    {   

#line 16603 "vocoderSched.cc"
	__PRF_globalCounter[1598u]++ ;

#line 326 "src/common/basic_op.sc"
	result = (int)var1 * ((int)1 << var2);

#line 16609 "vocoderSched.cc"
	__PRF_globalCounter[1599u]++ ;

#line 328 "src/common/basic_op.sc"
	if ((var2 > 15 && var1 != 0) || (result != (int)((short int)result)))
	{   

#line 16616 "vocoderSched.cc"
	    __PRF_globalCounter[1600u]++ ;

#line 330 "src/common/basic_op.sc"
	    Overflow = 1;
	    var_out = ((var1 > 0) ? (short int)32767 : (short int)32768);
	}
	else 
	{   

#line 16626 "vocoderSched.cc"
	    __PRF_globalCounter[1601u]++ ;

#line 335 "src/common/basic_op.sc"
	    var_out = extract_l(result);
	}
    }

#line 16634 "vocoderSched.cc"
    __PRF_globalCounter[1602u]++ ;

#line 339 "src/common/basic_op.sc"
    return (var_out);
}

#line 16641 "vocoderSched.cc"
short int shr(

#line 377 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    short int var_out;

#line 16649 "vocoderSched.cc"
    __PRF_globalCounter[1603u]++ ;

#line 381 "src/common/basic_op.sc"
    if (var2 < 0)
    {   

#line 16656 "vocoderSched.cc"
	__PRF_globalCounter[1604u]++ ;

#line 383 "src/common/basic_op.sc"
	var_out = shl(var1,  -var2);
    }
    else 
    {   

#line 16665 "vocoderSched.cc"
	__PRF_globalCounter[1605u]++ ;

#line 387 "src/common/basic_op.sc"
	if (var2 >= 15)
	{   

#line 16672 "vocoderSched.cc"
	    __PRF_globalCounter[1606u]++ ;

#line 389 "src/common/basic_op.sc"
	    var_out = ((var1 < 0) ?  -1 : 0);
	}
	else 
	{   

#line 16681 "vocoderSched.cc"
	    __PRF_globalCounter[1607u]++ ;

#line 393 "src/common/basic_op.sc"
	    if (var1 < 0)
	    {   

#line 16688 "vocoderSched.cc"
		__PRF_globalCounter[1608u]++ ;

#line 395 "src/common/basic_op.sc"
		var_out =  ~(( ~var1) >> var2);
	    }
	    else 
	    {   

#line 16697 "vocoderSched.cc"
		__PRF_globalCounter[1609u]++ ;

#line 399 "src/common/basic_op.sc"
		var_out = var1 >> var2;
	    }
	}
    }

#line 16706 "vocoderSched.cc"
    __PRF_globalCounter[1610u]++ ;

#line 404 "src/common/basic_op.sc"
    return (var_out);
}

#line 16713 "vocoderSched.cc"
short int shr_r(

#line 1444 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    short int var_out;

#line 16721 "vocoderSched.cc"
    __PRF_globalCounter[1611u]++ ;

#line 1448 "src/common/basic_op.sc"
    if (var2 > 15)
    {   

#line 16728 "vocoderSched.cc"
	__PRF_globalCounter[1612u]++ ;

#line 1450 "src/common/basic_op.sc"
	var_out = 0;
    }
    else 
    {   

#line 16737 "vocoderSched.cc"
	__PRF_globalCounter[1613u]++ ;

#line 1454 "src/common/basic_op.sc"
	var_out = shr(var1, var2);

#line 16743 "vocoderSched.cc"
	__PRF_globalCounter[1614u]++ ;

#line 1456 "src/common/basic_op.sc"
	if (var2 > 0)
	{   

#line 16750 "vocoderSched.cc"
	    __PRF_globalCounter[1615u]++ ;

#line 1458 "src/common/basic_op.sc"
	    if ((var1 & ((short int)1 << (var2 - 1))) != 0)
	    {   

#line 16757 "vocoderSched.cc"
		__PRF_globalCounter[1616u]++ ;

#line 1460 "src/common/basic_op.sc"
		var_out++ ;
	    }
	}
    }

#line 16766 "vocoderSched.cc"
    __PRF_globalCounter[1617u]++ ;

#line 1465 "src/common/basic_op.sc"
    return (var_out);
}

#line 16773 "vocoderSched.cc"
short int sub(

#line 218 "src/common/basic_op.sc"
    short int var1, short int var2)
{   
    short int var_out;
    int L_diff;

#line 16782 "vocoderSched.cc"
    __PRF_globalCounter[1618u]++ ;

#line 223 "src/common/basic_op.sc"
    L_diff = (int)var1 - var2;
    var_out = saturate(L_diff);

    return (var_out);
}

#line 16792 "vocoderSched.cc"
void _scc_bit4_err_handle(
    const _bit4& bit4vec)
{   
    char temp_bits[1024], *p;
    p=bit2str(2,&temp_bits[1023], bit4vec);
    _specc::abort(
	"ERROR:\t Casting a bit4 vector failed \n"
	"Bit4 vector contains X/Z values %s\n"
	"Simulation aborted.\n", p);
	
}

//////////////////////////////////////////////////////////////////////
// End of file vocoderSched.cc
//////////////////////////////////////////////////////////////////////
