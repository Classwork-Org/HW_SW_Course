...
#line 1602 "vocoderTlm.cc"
DSP56600Protocol_2_0MasterSlaveTLM_Wrap::DSP56600Protocol_2_0MasterSlaveTLM_Wrap(IDSP56600Protocol_2_0MasterSlave_Wrap (&tlm))
    : _specc::channel(), tlm(tlm),
    mutex()
{   
}

DSP56600Protocol_2_0MasterSlaveTLM_Wrap::~DSP56600Protocol_2_0MasterSlaveTLM_Wrap(void)
{   
}

bit<_BITLEN(1,0),true> DSP56600Protocol_2_0MasterSlaveTLM_Wrap::listen(bit<_BITLEN(15,0),true> *addr, 
    bit<_BITLEN(15,0),true> mask)
{   
    bit<_BITLEN(1,0),true> rvar;

    rvar = tlm.listen(addr, mask);
    return rvar;
}

void DSP56600Protocol_2_0MasterSlaveTLM_Wrap::masterRead(bit<_BITLEN(15,0),true> addr, 
    bit<_BITLEN(23,0),true> *data)
{   
    mutex.acquire();
    tlm.masterRead(addr, data);
    mutex.release();
}

void DSP56600Protocol_2_0MasterSlaveTLM_Wrap::masterWrite(bit<_BITLEN(15,0),true> addr, 
    bit<_BITLEN(23,0),true> data)
{   
    mutex.acquire();
    tlm.masterWrite(addr, data);
    mutex.release();
}

bit<_BITLEN(23,0),true> DSP56600Protocol_2_0MasterSlaveTLM_Wrap::serveRead(bit<_BITLEN(15,0),true> addr)
{   
    bit<_BITLEN(23,0),true> rvar;

    rvar = tlm.serveRead(addr);
    return rvar;
}

void DSP56600Protocol_2_0MasterSlaveTLM_Wrap::serveWrite(bit<_BITLEN(15,0),true> addr, 
    bit<_BITLEN(23,0),true> data)
{   
    tlm.serveWrite(addr, data);
}

void DSP56600Protocol_2_0MasterSlaveTLM_Wrap::slaveRead(bit<_BITLEN(15,0),true> addr, 
    bit<_BITLEN(23,0),true> *data)
{   
    tlm.slaveRead(addr, data);
}

void DSP56600Protocol_2_0MasterSlaveTLM_Wrap::slaveWrite(bit<_BITLEN(15,0),true> addr, 
    bit<_BITLEN(23,0),true> data)
{   
    tlm.slaveWrite(addr, data);
}
...
Coder::Coder(unsigned int _idcnt, i_receiver (&speech_samples), i_receiver (&dtx_mode), i_sender (&serial), i_sender (&txdtx_ctrl))
    ...
    Bus0_intC(),
    Bus0_intD(),
    DSP(++_IDcnt, Bus0, Bus0_intA, Bus0_intB, Bus0_intC, Bus0_intD, speech_samples, dtx_mode, serial, txdtx_ctrl),
    HW(++_IDcnt, Bus0, Bus0_intA)
{   
}
...
HW_Standard_HW_TLM::HW_Standard_HW_TLM(unsigned int _idcnt, IDSP56600SlaveProtocol_2_0 (&Port0_tlm), i_send (&Bus0_intA))
    : _specc::behavior(_idcnt), Port0_tlm(Port0_tlm), Bus0_intA(Bus0_intA),
    _scc_const_port_0(((unsigned short int)32768)),
    HW(_IDcnt, linkDSP2HW__Bus0, Bus0_intA),
    Port0_mac_s(Port0_tlm),
    linkDSP2HW__Bus0(Port0_mac_s, _scc_const_port_0)
{   
}

HW_Standard_HW_TLM::~HW_Standard_HW_TLM(void)
{   
}

void HW_Standard_HW_TLM::main(void)
{   
    HW.main();
}

HW_Standard_HW_opt::HW_Standard_HW_opt(unsigned int _idcnt, i_tranceiver (&c_stream_0), i_send (&Bus0_intA))
    : _specc::behavior(_idcnt), c_stream_0(c_stream_0), Bus0_intA(Bus0_intA),
    _app(_IDcnt, pre3, pre5, pre1, T0, cb_ana, code, exc_i, gain_code, gain_pit, h1, res2, xn, y1, y2, pre9, pre6),
    pre1(c_stream_0, Bus0_intA),
    pre3(c_stream_0, Bus0_intA),
    pre5(c_stream_0, Bus0_intA),
    pre6(c_stream_0, Bus0_intA),
    pre9(c_stream_0, Bus0_intA)
{   
}
...
Motorola_DSP56600_0_DSP_TLM::Motorola_DSP56600_0_DSP_TLM(unsigned int _idcnt, IDSP56600Protocol_2_0MasterSlave_Wrap (&Bus0_ms_tlm), i_receive (&intA_tlm), i_receive (&intB_tlm), i_receive (&intC_tlm), i_receive (&intD_tlm))
    : _specc::behavior(_idcnt), Bus0_ms_tlm(Bus0_ms_tlm), intA_tlm(intA_tlm), intB_tlm(intB_tlm), intC_tlm(intC_tlm), intD_tlm(intD_tlm),
    core_TLM(++_IDcnt, Bus0_ms_tlm, intA_tlm, intB_tlm, intC_tlm, intD_tlm)
{   
}

Motorola_DSP56600_0_DSP_TLM::~Motorola_DSP56600_0_DSP_TLM(void)
{   
}

void Motorola_DSP56600_0_DSP_TLM::main(void)
{   
    { _specc::fork _scc_fork_0(&core_TLM); _specc::par(&_scc_fork_0, ((_specc::fork*)0));
    }
}

Motorola_DSP56600_0_DSP_TLM_DSP::Motorola_DSP56600_0_DSP_TLM_DSP(unsigned int _idcnt, IDSP56600Protocol_2_0MasterSlave_Wrap (&Bus0_ms_tlm), i_receive (&intA_tlm), i_receive (&intB_tlm), i_receive (&intC_tlm), i_receive (&intD_tlm), i_receiver (&speech_samples), i_receiver (&dtx_mode), i_sender (&serial), i_sender (&txdtx_ctrl))
    : _specc::behavior(_idcnt), Bus0_ms_tlm(Bus0_ms_tlm), intA_tlm(intA_tlm), intB_tlm(intB_tlm), intC_tlm(intC_tlm), intD_tlm(intD_tlm), speech_samples(speech_samples), dtx_mode(dtx_mode), serial(serial), txdtx_ctrl(txdtx_ctrl),
    core_TLM(++_IDcnt, Bus0_ms_tlm, intA_tlm, intB_tlm, intC_tlm, intD_tlm, speech_samples, dtx_mode, serial, txdtx_ctrl)
{   
}

Motorola_DSP56600_0_DSP_TLM_DSP::~Motorola_DSP56600_0_DSP_TLM_DSP(void)
{   
}

void Motorola_DSP56600_0_DSP_TLM_DSP::main(void)
{   
    { _specc::fork _scc_fork_0(&core_TLM); _specc::par(&_scc_fork_0, ((_specc::fork*)0));
    }
}

void Motorola_DSP56600_0_DSP_TLM_DSP::reset(void)
{   
    core_TLM.reset();
}
...
Motorola_DSP56600_Core_0_DSP_TLM::Motorola_DSP56600_Core_0_DSP_TLM(unsigned int _idcnt, IDSP56600Protocol_2_0MasterSlave_Wrap (&Bus0_ms_tlm), i_receive (&intA_tlm), i_receive (&intB_tlm), i_receive (&intC_tlm), i_receive (&intD_tlm))
    : _specc::behavior(_idcnt), Bus0_ms_tlm(Bus0_ms_tlm), intA_tlm(intA_tlm), intB_tlm(intB_tlm), intC_tlm(intC_tlm), intD_tlm(intD_tlm),
    INTR(bit<1,true>("0")),
    hw_TLM(++_IDcnt, tlm_ms_A_wrapper, INTR),
    pic_TLM(++_IDcnt, intD_tlm, intC_tlm, intB_tlm, intA_tlm, tlm_ms_A_wrapper, INTR),
    tlm_ms_A_wrapper(Bus0_ms_tlm)
{   
}

Motorola_DSP56600_Core_0_DSP_TLM::~Motorola_DSP56600_Core_0_DSP_TLM(void)
{   
}

void Motorola_DSP56600_Core_0_DSP_TLM::main(void)
{   
    { _specc::fork _scc_fork_0(&hw_TLM), _scc_fork_1(&pic_TLM); _specc::par(&_scc_fork_0, &_scc_fork_1, ((_specc::fork*)0));
    }
}

Motorola_DSP56600_Core_0_DSP_TLM_DSP::Motorola_DSP56600_Core_0_DSP_TLM_DSP(unsigned int _idcnt, IDSP56600Protocol_2_0MasterSlave_Wrap (&Bus0_ms_tlm), i_receive (&intA_tlm), i_receive (&intB_tlm), i_receive (&intC_tlm), i_receive (&intD_tlm), i_receiver (&speech_samples), i_receiver (&dtx_mode), i_sender (&serial), i_sender (&txdtx_ctrl))
    : _specc::behavior(_idcnt), Bus0_ms_tlm(Bus0_ms_tlm), intA_tlm(intA_tlm), intB_tlm(intB_tlm), intC_tlm(intC_tlm), intD_tlm(intD_tlm), speech_samples(speech_samples), dtx_mode(dtx_mode), serial(serial), txdtx_ctrl(txdtx_ctrl),
    INTR(bit<1,true>("0")),
    hw_TLM(++_IDcnt, tlm_ms_A_wrapper, INTR, speech_samples, dtx_mode, serial, txdtx_ctrl),
    pic_TLM(++_IDcnt, intD_tlm, intC_tlm, intB_tlm, intA_tlm, tlm_ms_A_wrapper, INTR),
    tlm_ms_A_wrapper(Bus0_ms_tlm)
{   
}

Motorola_DSP56600_Core_0_DSP_TLM_DSP::~Motorola_DSP56600_Core_0_DSP_TLM_DSP(void)
{   
}

void Motorola_DSP56600_Core_0_DSP_TLM_DSP::main(void)
{   
    { _specc::fork _scc_fork_0(&hw_TLM), _scc_fork_1(&pic_TLM); _specc::par(&_scc_fork_0, &_scc_fork_1, ((_specc::fork*)0));
    }
}

void Motorola_DSP56600_Core_0_DSP_TLM_DSP::reset(void)
{   
    hw_TLM.reset();
}
...
Motorola_DSP56600_HW_0_DSP_TLM::Motorola_DSP56600_HW_0_DSP_TLM(unsigned int _idcnt, IDSP56600MasterProtocol_2_0 (&Bus0_m_tlm), _specc::signal<bit<_BITLEN(0,0),true> > (&INTR))
    : _specc::behavior(_idcnt), Bus0_m_tlm(Bus0_m_tlm), INTR(INTR),
    INTR_MASKED(bit<1,true>("0")),
    bus(Bus0_m_tlm, INTR_MASKED),
    hal(_IDcnt, bus),
    intr(_IDcnt, Bus0_m_tlm, INTR, INTR_MASKED, hal)
{   
}

Motorola_DSP56600_HW_0_DSP_TLM::~Motorola_DSP56600_HW_0_DSP_TLM(void)
{   
}

void Motorola_DSP56600_HW_0_DSP_TLM::main(void)
{   
    { _specc::try_block _scc_try_hal(&hal); _specc::exception_block _scc_exception_0(false, &intr, event(&INTR), event(&INTR_MASKED), ((void*)0)); _specc::tryTrapInterrupt(&_scc_try_hal,
	    &_scc_exception_0, ((_specc::exception_block*)0));
    }
...
Motorola_DSP56600_HW_0_DSP_TLM_DSP::Motorola_DSP56600_HW_0_DSP_TLM_DSP(unsigned int _idcnt, IDSP56600MasterProtocol_2_0 (&Bus0_m_tlm), _specc::signal<bit<_BITLEN(0,0),true> > (&INTR), i_receiver (&speech_samples), i_receiver (&dtx_mode), i_sender (&serial), i_sender (&txdtx_ctrl))
    : _specc::behavior(_idcnt), Bus0_m_tlm(Bus0_m_tlm), INTR(INTR), speech_samples(speech_samples), dtx_mode(dtx_mode), serial(serial), txdtx_ctrl(txdtx_ctrl),
    INTR_MASKED(bit<1,true>("0")),
    bus(Bus0_m_tlm, INTR_MASKED),
...
Motorola_DSP56600_HW_0_DSP_TLM_DSP::~Motorola_DSP56600_HW_0_DSP_TLM_DSP(void)
{   
}

void Motorola_DSP56600_HW_0_DSP_TLM_DSP::main(void)
{   
    { _specc::try_block _scc_try_hal(&hal); _specc::exception_block _scc_exception_0(false, &intr, event(&INTR), event(&INTR_MASKED), ((void*)0)); _specc::tryTrapInterrupt(&_scc_try_hal,
	    &_scc_exception_0, ((_specc::exception_block*)0));
    }
...
void Motorola_DSP56600_HW_0_DSP_TLM_DSP::reset(void)
{   
    hal.reset();
}
...
#line 11271 "vocoderTlm.cc"
Motorola_DSP56600_PIC_0_DSP_TLM::Motorola_DSP56600_PIC_0_DSP_TLM(unsigned int _idcnt, i_receive (&intD_tlm), i_receive (&intC_tlm), i_receive (&intB_tlm), i_receive (&intA_tlm), IDSP56600SlaveProtocol_2_0 (&Bus0_s_tlm), _specc::signal<bit<_BITLEN(0,0),true> > (&INTR))
    : _specc::behavior(_idcnt), intD_tlm(intD_tlm), intC_tlm(intC_tlm), intB_tlm(intB_tlm), intA_tlm(intA_tlm), Bus0_s_tlm(Bus0_s_tlm), INTR(INTR),
    IMR(bit<4,true>("0000")),
    ISR(bit<4,true>("0000")),
    _scc_bus_0(ISR.sigbus_slice(3,0, 0,0)),
    _scc_bus_1(ISR.sigbus_slice(3,0, 1,1)),
    _scc_bus_2(ISR.sigbus_slice(3,0, 2,2)),
    _scc_bus_3(ISR.sigbus_slice(3,0, 3,3)),
    control(++_IDcnt, Bus0_s_tlm, INTR, ISR, IMR),
    intAdetect(++_IDcnt, intA_tlm, _scc_bus_0),
    intBdetect(++_IDcnt, intB_tlm, _scc_bus_1),
    intCdetect(++_IDcnt, intC_tlm, _scc_bus_2),
    intDdetect(++_IDcnt, intD_tlm, _scc_bus_3)
{   
}

Motorola_DSP56600_PIC_0_DSP_TLM::~Motorola_DSP56600_PIC_0_DSP_TLM(void)
{   
}

void Motorola_DSP56600_PIC_0_DSP_TLM::main(void)
{   
    { _specc::fork _scc_fork_0(&control), _scc_fork_1(&intAdetect), _scc_fork_2(&intBdetect), _scc_fork_3(&intCdetect), _scc_fork_4(&intDdetect); _specc::par(&_scc_fork_0, &_scc_fork_1, &_scc_fork_2, &_scc_fork_3, &_scc_fork_4, ((_specc::fork*)0));
    }
}
